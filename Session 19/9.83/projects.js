// FinanceFlow · v9.83 · projects.js · 2026-08-17
//  PROJEKTY
// ══════════════════════════════════════════════════════
            ...
            <div style="display:flex;gap:22px;flex-wrap:wrap;align-items:center">
              ...
              <div style="flex:1;min-width:230px">
                <div style="display:flex;justify-content:space-between;font-size:.78rem;padding:4px 0;border-bottom:1px solid var(--border)">
                  <span>Ohodnoceno výdajů</span><b>${fmt(Math.round(r.sum))} / ${fmtB(Math.round(totalExp))}</b></div>
                <div style="display:flex;justify-content:space-between;font-size:.78rem;padding:4px 0;border-bottom:1px solid var(--border)">
...
function renderRadar() {
  ...
  if(isCurrentMonth && eomLeft < 0) {
    alerts.unshift({level:'danger', icon:'📉', text:`Do konce měsíce ti chybí ${fmtB(Math.abs(eomLeft))}. Příjem ${fmtB(expectedIncMonth)} − výdaje ${fmtB(totalExp)} − známé platby ${fmt(Math.round(budToEOM))}.`});
  }
  if(pred3Total < 0) {
