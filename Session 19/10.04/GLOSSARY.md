...
## Doplněno v Session 19 (2026-08-21)

**`activity`** – uzel `users/{uid}/activity`. Evidence používání aplikace: čas posledního
použití, počet spuštění, značky aktivních dnů, verze appky, PWA vs. prohlížeč, aktivace.
Bez IP, polohy a otisku zařízení (ADR-103).

**`backups`** – uzel `users/{uid}/backups/{YYYY-MM-DD}`. Denní snímek dat jako **jeden JSON
řetězec**, rotace na 5. Klíč `pred-obnovou` = pojistka uložená před obnovou (ADR-102).

**`baseToCzk(v)`** – převod ze základní měny do CZK. Opak `czkToBase()`. Použití při
ukládání peněžních vstupních polí.

**Cyklus (výplatní)** – období od výplaty k výplatě. Kotva = den **největšího jistého nebo
pravděpodobného příjmu** v kalendářním měsíci (FIX-253), ne `radarPaydayInfo()`.

**`detTxs`** – vyčištěný zdroj transakcí pro Detektor úspor: bez vyrovnání, bez rodičů
splitu a bez přesunů (FIX-254).

**EOP (External Offers Program)** – program Google Play, který v EHP dovoluje odvádět
uživatele k platbě mimo aplikaci. Vyžaduje organizační účet (D-U-N-S), nativní integraci
API a hlášení transakcí do 24 hodin. Servisní poplatek 10 %.

**`fxLossOf(t)` / `fxLossSummary()`** – výpočet kurzové ztráty a její rozpad podle
peněženky, typu platby a měny.

**`fxRef` / `fxRefDate`** – referenční kurz ČNB uložený u cizoměnové transakce a datum
kurzovního lístku. Ukládá se **jen živý kurz**, nikdy orientační průměr (ADR-099).

**Kumulovaný tok** – nasčítaný rozdíl příjmů a výdajů od začátku záznamů. Použit v Ose
života **místo čistého jmění**, které historicky spočítat nelze (ADR-106).

**Kurz banky** – dopočítaný jako `amtCZK / amount`. Rozdíl proti kurzu ČNB je marže
a poplatek za směnu.

**`moneyInFill()` / `moneyInRead()`** – převod peněžních vstupních polí mezi CZK
(uložení) a základní měnou (zobrazení). Musí se používat **oba současně** (ADR-108).

**Osa života** – varianta B rozšíření Životní mapy (Deník): vodorovná osa událostí
a etap nad finančními křivkami. Historie se neořezává, mění se hustota.

**Pokrytí hodnocení** – podíl utracené částky, která má vyplněnou prioritu 1–5.
Pod 15 % se u souhrnu zobrazí varování.

**`pristiCfg`** – `S.pristiCfg['YYYY-MM']`: ruční úpravy karty Příští měsíc — přepsané
částky, vyřazené řádky, vlastní zápisy, počáteční zůstatek.

**`PRISTI_ENABLED`** – rollback přepínač modulu `pristi.js`. `false` = položka v menu
zmizí a stránka se nevykreslí.

**SHA-256 fingerprint** – otisk podpisového klíče aplikace v `assetlinks.json`.
⚠️ Musí to být **app signing key** z Play Console, ne upload key — jinak se v TWA
zobrazí adresní řádek prohlížeče.

**Tempo (projekt)** – `utracenoPct / casPct × 100`. Pod 100 = utrácíš pomaleji,
než ubíhá čas projektu.

**Whisker** – svislá čára s nožkami v grafu cyklů: minimum a maximum daného týdne
napříč cykly. Doplňuje medián, který sám neřekne, jak je týden stabilní.

---

## Doplněno po hloubkové analýze (2026-08-24)

**`_claimed` / `_free` / `_claim`** – evidence transakcí v Detektoru. Každý nález si
transakci „zabere", další ji nevidí. Bez toho padla jedna útrata do tří nálezů
a součet dal 110 % z útraty (ADR-114).

**`_splitSeen`** – klíče, které v tomto sezení v databázi opravdu existovaly.
Rozlišuje „nikdy nezapsáno" od „smazáno" (ADR-111).

**`availMax`** – součet maxim složek skóre, které lze dnes změřit. Jmenovatel
finančního skóre. Skóre = dosažené / dosažitelné (ADR-113).

**`ensureBaseData()`** – doplní kategorie, typy plateb a peněženku, pokud chybí.
Idempotentní, běží při každém přihlášení. Nahrazuje jednorázový `seedData()` (ADR-110).

**`receiptCompleteness(r)`** – porovná součet položek se sumou natištěnou na účtence.
Tolerance 1 Kč. Rozdíl se hlásí neutrálně („chybí / přebývá"), ne jako chyba AI (ADR-115).

**Doložitelné vs. odhad (Detektor)** – bankovní poplatky, refinancování a kurzy se
počítají ze skutečných čísel; ostatní nálezy jsou odhad podle koeficientu.
Souhrn se proto zobrazuje jako rozsah, ne jedno číslo.

**Dynamický jmenovatel** – složka skóre, kterou nelze změřit, vypadne z čitatele
i jmenovatele. Nový uživatel dostával 217/310 = 70/100 dřív, než zadal transakci.

**EOP (External Offers Program)** – program Google Play pro odvádění uživatele
k platbě mimo aplikaci v EHP. Vyžaduje organizační účet (D-U-N-S), nativní integraci
API a hlášení transakcí do 24 hodin. Servisní poplatek 10 %.

**Falešná shoda** – sloučení dvou různých položek pod jeden klíč. Horší než rozdělení
téže položky na dvě: rozdělená z výpočtu vypadne, sloučená vyrobí neexistující
zdražení (SKILL 33, ADR-112).
