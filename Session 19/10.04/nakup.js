...
let _goalDeposits  = {};
let _cileTab       = 'aktivni';   // 'aktivni' | 'splneno'
function cileSwitchTab(t){ _cileTab = t; renderNakupCile(); }          // goalId → [{ id, amount, date, note }]
...
async function renderNakup() {
  ...
  // Plány a cíle přesunuty do sekce Narozeniny a přání (S13).
  // Nákupní seznam nyní renderuje rovnou obsah bez záložek.
  el.innerHTML = `<div id="nakupTabContent">
...
async function renderNakupSeznam() {
  ...
  el.innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;flex-wrap:wrap">
      ...
      <!-- S16.11 (Milan): přepínač jednoduchého režimu (jen název, cena, zaškrtávátko) -->
      <button class="btn btn-ghost btn-sm" onclick="nakupToggleSimple()" title="Přepnout mezi kartami a jednoduchým seznamem"
        style="font-size:.74rem;${_nakupSimple?'border-color:rgba(139,124,246,.5);color:#b9aefc':''}">${_nakupSimple?'🗂 Karty':'📝 Jednoduchý'}</button>
...
// S16.11 (Milan): jednoduchý režim seznamu – uloženo lokálně (preference zobrazení, ne data)
let _nakupSimple = (()=>{ try{ return localStorage.getItem('ff_nakupSimple')==='1'; }catch(e){ return false; } })();
function nakupToggleSimple(){
  _nakupSimple = !_nakupSimple;
  try{ localStorage.setItem('ff_nakupSimple', _nakupSimple?'1':'0'); }catch(e){}
  renderNakupSeznam();
}

// ── TODO-229 (S19, Milan): PLATNOST SLEVY ──
//  Sleva se držela, dokud ji nepřepsala novější cena – mohla viset týdny.
//  Milan: „pak už a) nemusí být aktuální b) bude menší sleva, v jiném obchodě".
//  Katalog přitom `latestDate` UŽ NESE, jen se nikde nekontrolovalo.
//
//  Tři stavy podle stáří ceny:
//    do 7 dní   → 🎉 SLEVA        plnohodnotný nález
//    8–30 dní   → ⏳ BYLA SLEVA   cena může být jiná, nález se nepočítá
//    nad 30 dní → nález zaniká úplně
//
//  ⚠️ Ceny se liší i REGIONÁLNĚ – katalog zná obchod, ne kraj. Proto se píše
//  „naposledy viděno v Lidlu", ne „v Lidlu je za X". Nepředstírat přesnost,
//  kterou nemáme.
const NAKUP_SLEVA_CERSTVA = 7;    // dní – plnohodnotný nález
const NAKUP_SLEVA_MAX     = 30;   // dní – nad to nález zaniká

function nakupPriceAge(catItem){
  const d = catItem && catItem.latestDate;
  if(!d) return null;                       // starší záznamy datum nemají
  const t = new Date(String(d).slice(0,10) + 'T12:00:00');
  if(isNaN(t)) return null;
  const dnes = new Date(); dnes.setHours(12,0,0,0);
  return Math.max(0, Math.round((dnes - t) / 86400000));
}

// 'fresh' | 'stale' | 'expired' | null (neznámé stáří → chováme se jako dřív)
function nakupPriceState(catItem){
  const age = nakupPriceAge(catItem);
  if(age === null) return null;
  if(age <= NAKUP_SLEVA_CERSTVA) return 'fresh';
  if(age <= NAKUP_SLEVA_MAX)     return 'stale';
  return 'expired';
}
...
function nakupIsTriggered(item) {
  ...
  // TODO-229: stará cena už není nález. Neznámé stáří (záznamy bez data) se
  //   chová jako dřív, aby se starší položky nepřestaly hlásit ze dne na den.
  const st = nakupPriceState(catItem);
  if (st === 'stale' || st === 'expired') return false;
...
function nakupBuildList() {
  ...
  const cartBar = inCartN ? `<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:10px;background:rgba(74,222,128,.08);border:1px solid rgba(74,222,128,.25);border-radius:10px;padding:7px 12px">
      <span style="font-size:.78rem;color:var(--income);font-weight:600">🛒 V košíku ${inCartN} z ${_nakupItems.length}</span>
      ${!viewingUid?`<button class="btn btn-ghost btn-sm" onclick="nakupClearCart()" style="font-size:.72rem">Vysypat košík</button>`:''}
    </div>` : '';
  // S16.11: jednoduchý režim – jen zaškrtávátko, název a cena
  if(_nakupSimple){
    return `${cartBar}<div style="border:1px solid var(--border);border-radius:12px;overflow:hidden">
      ${items.map(item => nakupBuildSimpleRow(item)).join('')}
    </div>`;
  }
...
// S16.11 (Milan): jednoduchý řádek – název, cena, zaškrtávátko. Bez ikon, slev, referencí, tlačítek.
function nakupBuildSimpleRow(item) {
  const cat = item.catalogKey ? _nakupCatalog.find(c => c.key === item.catalogKey) : null;
  const price = (cat && cat.latestPrice) || item.refPrice || 0;
  const done = !!item.inCart;
  return `<div style="display:flex;align-items:center;gap:10px;padding:11px 12px;border-bottom:1px solid var(--border);background:${done?'rgba(74,222,128,.05)':'transparent'}">
    <div onclick="${viewingUid?'':`nakupToggleCart('${item.id}')`}" style="width:22px;height:22px;flex-shrink:0;border-radius:6px;border:2px solid ${done?'var(--income)':'var(--border2, rgba(255,255,255,.2))'};background:${done?'var(--income)':'transparent'};display:flex;align-items:center;justify-content:center;cursor:${viewingUid?'default':'pointer'}">
      ${done?'<span style="color:#0f172a;font-size:.8rem;font-weight:800">✓</span>':''}
    </div>
    <span style="flex:1;min-width:0;font-size:.88rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;${done?'text-decoration:line-through;color:var(--text3)':'color:var(--text)'}">${item.name}</span>
    <span style="font-size:.86rem;font-weight:700;white-space:nowrap;color:${done?'var(--text3)':'#c9cede'}">${price?fmtB(price):'—'}</span>
    ${viewingUid?'':`<button onclick="nakupDelete('${item.id}')" style="background:none;border:none;color:var(--text3);cursor:pointer;font-size:.9rem;padding:2px 4px;flex-shrink:0" title="Smazat">✕</button>`}
  </div>`;
}
...
function nakupBuildRow(item) {
  ...
  if (latestPrice) {
    priceHtml = `
      <div style="display:flex;align-items:baseline;gap:6px;flex-wrap:wrap;margin-top:2px">
        <strong style="font-size:.95rem;color:${triggered?'var(--income)':'var(--text)'}">${fmtBP(latestPrice)}</strong>
      ...
      ${(catItem.latestStore||catItem.latestDate) ? (()=>{
        // TODO-229: „naposledy viděno" místo „je za" – katalog zná obchod, ne kraj
        const _age = nakupPriceAge(catItem);
        const _st  = nakupPriceState(catItem);
        const _kdy = _age === null ? '' : _age === 0 ? 'dnes' : _age === 1 ? 'včera' : `před ${_age} dny`;
        const _col = _st === 'fresh' ? '#a8aec8' : _st === 'stale' ? 'var(--debt)' : 'var(--text3)';
        return `<div style="font-size:.66rem;color:${_col}">naposledy viděno${catItem.latestStore?' v '+catItem.latestStore:''}${catItem.latestDate?' · '+fmtD(catItem.latestDate):''}${_kdy?' ('+_kdy+')':''}</div>`;
      })() : ''}`;
  ...
  } else if (item.alertPct > 0 && nakupPriceState(catItem) === 'stale' && nakupGetDrop(item) >= item.alertPct) {
    // TODO-229: sleva odpovídala, ale cena je starší než týden – ukázat, nepočítat
    badgeHtml = `<span style="font-size:.66rem;background:rgba(251,191,36,.18);color:var(--debt);padding:2px 7px;border-radius:6px;font-weight:700" title="Cena je starší než ${NAKUP_SLEVA_CERSTVA} dní – akce už nemusí platit a v jiném obchodě může být jinak.">⏳ BYLA SLEVA</span>`;
  ...
  const refHtml = item.refPrice
    ? `<div style="font-size:.64rem;color:#a8aec8;margin-top:auto;padding-top:4px">Ref.: ${fmtBP(item.refPrice)}${item.alertPct?` · alert ${fmtBP(item.refPrice*(1-item.alertPct/100))}`:''}</div>`
    : '';
  ...
  return `<div style="background:var(--surface2);border:1px solid ${inCart?'rgba(74,222,128,.35)':triggered?'rgba(74,222,128,.4)':'var(--border)'};${triggered&&!inCart?'box-shadow:0 0 14px rgba(74,222,128,.12);':''}border-radius:14px;padding:12px;display:flex;flex-direction:column;gap:6px;min-height:150px;position:relative;${inCart?'opacity:.55;':''}">
    ...
    ${item.note ? `<div style="font-size:.68rem;color:#a8aec8">📝 ${item.note}</div>` : ''}
    ${priceHtml}
    ${refHtml}
...
async function renderNakupCile() {
  const el = document.getElementById('cileContent'); if (!el) return;

  // Načti vklady ze Firebase
  await goalLoadAllDeposits();

  const D    = getData();
  const allGoals = (D.wishes || []).filter(w => w.isGoal);
  const wishCount = (D.wishes || []).filter(w => !w.isGoal).length;

  // Rozdělení: splněné (done flag NEBO naspořeno >= cíl) vs aktivní
  const isGoalDone = g => g.done === true || goalGetSaved(g.id) >= (g.targetAmount||1);
  const activeGoals = allGoals.filter(g => !isGoalDone(g));
  const doneGoals   = allGoals.filter(g => isGoalDone(g));

  // Statistiky pro banner
  const totalSaved = allGoals.reduce((s,g) => s + goalGetSaved(g.id), 0);
  const totalTarget = allGoals.reduce((s,g) => s + (g.targetAmount||0), 0);
  const totalRemaining = Math.max(0, totalTarget - totalSaved);
  const totalPct = totalTarget > 0 ? Math.min(100, Math.round(totalSaved/totalTarget*100)) : 0;
  const tab = _cileTab || 'aktivni';
  const shownGoals = tab === 'splneno' ? doneGoals : activeGoals;

  el.innerHTML = `
    <!-- Banner: Aktivních / Pasivních / Splněno -->
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px">
      <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid var(--border)">
        <div style="font-size:1.4rem;font-weight:800;font-family:Syne,sans-serif;color:var(--bank)">${activeGoals.length}</div>
        <div style="font-size:.72rem;color:var(--text3)">Aktivních cílů</div>
      </div>
      <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid var(--border)">
        <div style="font-size:1.4rem;font-weight:800;font-family:Syne,sans-serif;color:var(--text2)">${wishCount}</div>
        <div style="font-size:.72rem;color:var(--text3)">Pasivních (přání)</div>
      </div>
      <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid ${doneGoals.length?'rgba(74,222,128,.4)':'var(--border)'}">
        <div style="font-size:1.4rem;font-weight:800;font-family:Syne,sans-serif;color:${doneGoals.length?'var(--income)':'var(--text3)'}">${doneGoals.length}</div>
        <div style="font-size:.72rem;color:var(--text3)">Splněno</div>
      </div>
    </div>

    <!-- Počítadlo: Naspořeno / Celkový cíl / Zbývá (přesunuto z Peněženek) -->
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(105px,1fr));gap:10px;margin-bottom:12px">
      <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid var(--border);min-width:0">
        <div style="font-size:clamp(.8rem,3.3vw,1rem);font-weight:800;font-family:Syne,sans-serif;color:var(--income);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${fmtBP(totalSaved)}</div>
        <div style="font-size:.7rem;color:var(--text3)">Naspořeno</div>
      </div>
      <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid var(--border);min-width:0">
        <div style="font-size:clamp(.8rem,3.3vw,1rem);font-weight:800;font-family:Syne,sans-serif;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${fmtBP(totalTarget)}</div>
        <div style="font-size:.7rem;color:var(--text3)">Celkový cíl</div>
      </div>
      <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid var(--border);min-width:0">
        <div style="font-size:clamp(.8rem,3.3vw,1rem);font-weight:800;font-family:Syne,sans-serif;color:var(--text2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${fmtBP(totalRemaining)}</div>
    ...
    <div style="margin-bottom:14px">
      ...
      <div style="height:8px;background:var(--surface3);border-radius:5px;overflow:hidden">
        <div style="height:100%;width:${totalPct}%;background:var(--income);border-radius:5px;transition:width .4s"></div>
      </div>
    </div>

    <!-- Virtuální peněženka (v8.09 styl – odkaz do Peněženek) -->
    ...
    <!-- Záložky Aktivní / Splněno + tlačítko Nový cíl -->
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px">
      <div style="display:flex;gap:0;background:var(--surface2);border-radius:10px;padding:3px;border:1px solid var(--border);flex:1">
        <button onclick="cileSwitchTab('aktivni')" style="flex:1;padding:8px 0;border:none;border-radius:8px;font-size:.8rem;font-weight:600;cursor:pointer;transition:all .15s;background:${tab==='aktivni'?'var(--surface)':'transparent'};color:${tab==='aktivni'?'var(--text)':'var(--text3)'}">🎯 Aktivní (${activeGoals.length})</button>
        <button onclick="cileSwitchTab('splneno')" style="flex:1;padding:8px 0;border:none;border-radius:8px;font-size:.8rem;font-weight:600;cursor:pointer;transition:all .15s;background:${tab==='splneno'?'var(--surface)':'transparent'};color:${tab==='splneno'?'var(--text)':'var(--text3)'}">🏆 Splněno (${doneGoals.length})</button>
      </div>
      ${!viewingUid ? `<button class="btn btn-accent btn-sm" onclick="openGoalModal()" style="white-space:nowrap">🎯 + Nový cíl</button>` : ''}
    </div>

    <!-- Seznam cílů -->
    <div id="goalList">
      ${shownGoals.length ? shownGoals.map(g => goalBuildCard(g)).join('') : `
        <div class="empty" style="padding:36px">
          <div class="ei">${tab==='splneno'?'🏆':'🎯'}</div>
          <div class="et">${tab==='splneno'?'Zatím žádné splněné cíle':'Zatím žádné aktivní cíle'}</div>
          ${tab!=='splneno' && !viewingUid ? '<div style="font-size:.78rem;color:var(--text3);margin-top:6px">Přidej první finanční cíl a sleduj, jak se blížíš k jeho splnění</div><div style="margin-top:12px"><button class="btn btn-accent btn-sm" onclick="openGoalModal()">🎯 Přidat první cíl</button></div>' : ''}
        </div>`}
    </div>
  `;
}
...
function goalBuildCard(goal) {
  ...
  <div class="goal-card" style="background:var(--surface2);border:1px solid var(--border);border-radius:14px;padding:16px;margin-bottom:12px;${done?'border-color:rgba(74,222,128,.4);':''}">
    ...
    <div style="margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:5px">
        <span style="font-size:.8rem;color:var(--text2)">
          <strong style="color:${progressColor}">${fmtBP(st.saved)}</strong>
          <span style="color:var(--text3)"> / ${fmtBP(st.target)}</span>
        </span>
        ...
        ${deps.map(d => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;font-size:.78rem">
            ...
            <div style="display:flex;align-items:center;gap:8px">
              <span style="color:var(--income);font-weight:600">+${fmtBP(d.amount)}</span>
    ...
    <!-- CTA: vklad přes peněženku / označení splnění -->
    ${!done && !viewingUid ? `
      <div style="margin-top:10px;text-align:center">
        <button class="btn btn-accent btn-sm" onclick="showPage('penezenky')">💰 Vložit převodem z peněženky</button>
      </div>` : ''}
    ${done && goal.done !== true && !viewingUid ? `
      <div style="margin-top:10px;text-align:center">
        <button class="btn btn-accent btn-sm" onclick="goalMarkDone('${goal.id}')">🏆 Označit jako splněno</button>
      </div>` : ''}
  </div>`;
}
...
function openGoalModal(editId) {
  ...
  // S13: přidání i editace cíle jde přes sjednocený modal (modalWish) v charts.js
  if (typeof openWishModal === 'function' && typeof wishSetType === 'function') {
    if (editId && typeof editWish === 'function') { editWish(editId); return; }
    openWishModal('goal'); return;
  }
  ...
  moneyInFill('goalTargetAmount', goal?.targetAmount);   // TODO-216
  moneyInFill('goalMonthlyTarget', goal?.monthlyTarget);   // TODO-216
...
function goalUpdateEstimate() {
  const target  = moneyInRead('goalTargetAmount');   // TODO-216
  const monthly = moneyInRead('goalMonthlyTarget');   // TODO-216
...
function saveGoal() {
  ...
  const targetAmount = moneyInRead('goalTargetAmount');   // TODO-216
  const monthlyTarget = moneyInRead('goalMonthlyTarget');   // TODO-216
...
// Smaže párový vklad do cíle když se smaže transfer transakce (txOut)
function reverseGoalDepositForTx(txId) {
  let foundGoalId = null, foundDepId = null;
  for (const gid in _goalDeposits) {
    const dep = (_goalDeposits[gid]||[]).find(d => d.txOutId === txId);
    if (dep) { foundGoalId = gid; foundDepId = dep.id; break; }
  }
  if (!foundGoalId) return;
  // Odeber z cache
  _goalDeposits[foundGoalId] = _goalDeposits[foundGoalId].filter(d => d.id !== foundDepId);
  // Odeber z Firebase
  if (window._currentUser && window._db) {
    _set(_ref(_db, `users/${window._currentUser.uid}/goal_deposits/${foundDepId}`), null).catch(()=>{});
  }
}
...
function goalMarkDone(id) {
  if (viewingUid) return;
  const D = getData();
  const goal = (D.wishes||[]).find(w => w.id === id);
  if (!goal) return;
  if (!confirm(`Označit cíl „${goal.name}" jako splněný?\n\nNaspořené peníze se vrátí zpět na peněženky, ze kterých přišly, a cíl se přesune do záložky Splněno.`)) return;

  // REVERZ: vrať peníze na peněženky (smaž párové výdajové transakce)
  const deps = _goalDeposits[id] || [];
  const txOutIds = deps.map(d => d.txOutId).filter(Boolean);
  if (txOutIds.length && S.transactions) {
    S.transactions = S.transactions.filter(t => !txOutIds.includes(t.id));
  }
  // Smaž vklady (cíl je splněn, peníze vráceny)
  if (window._currentUser) {
    const uid2 = window._currentUser.uid;
    _get(_ref(_db, `users/${uid2}/goal_deposits`)).then(snap => {
      if (!snap.exists()) return;
      const updates = {};
      Object.entries(snap.val()).forEach(([k, v]) => {
        if (v.goalId === id) updates[`users/${uid2}/goal_deposits/${k}`] = null;
      });
  ...
  // Označ cíl jako splněný (přesune se do záložky Splněno)
  const gi = (S.wishes||[]).findIndex(w => w.id === id);
  if (gi >= 0) { S.wishes[gi].done = true; S.wishes[gi].doneAt = Date.now(); }
  save();
  if (typeof showToast === 'function') showToast('🏆 Cíl splněn! Peníze vráceny na peněženky.');
  renderNakupCile();
}

function goalDelete(id) {
  if (viewingUid) return;
  if (!confirm('Smazat cíl? Vklady se vrátí zpět na původní peněženky.')) return;
  // REVERZ: smaž párové výdajové transakce (txOut) → peníze se vrátí na peněženky
  const deps = _goalDeposits[id] || [];
  const txOutIds = deps.map(d => d.txOutId).filter(Boolean);
  if (txOutIds.length && S.transactions) {
    S.transactions = S.transactions.filter(t => !txOutIds.includes(t.id));
  ...
  if (el) {
    el.innerHTML = `
      <div style="display:flex;justify-content:space-between;font-size:.78rem;margin-bottom:8px">
        ...
        <strong style="color:var(--income)">${fmtP(st.saved)} / ${fmtBP(st.target)} (${st.pct}%)</strong>
      </div>
      ...
      ${st.remaining > 0 ? `<div style="font-size:.76rem;color:var(--text3)">Zbývá: <strong>${fmtBP(st.remaining)}</strong>${st.monthly ? ` · doporučeno: ${fmtBP(st.monthly)}` : ''}</div>` : '<div style="font-size:.76rem;color:var(--income)">🎉 Cíl byl splněn!</div>'}
    `;
  }
...
async function saveGoalDeposit() {
  ...
  const amount = document.getElementById('depositAmount').value==='' ? NaN : moneyInRead('depositAmount');   // TODO-216
...
async function goalDeleteDeposit(goalId, depositId) {
  if (viewingUid || !window._currentUser) return;
  if (!confirm('Smazat vklad? Peníze se vrátí zpět na původní peněženku.')) return;

  try {
    const userUid = window._currentUser.uid;
    // REVERZ: najdi vklad a smaž jeho párovou výdajovou transakci → peníze zpět na peněženku
    const dep = (_goalDeposits[goalId]||[]).find(d => d.id === depositId);
    if (dep && dep.txOutId && S.transactions) {
      S.transactions = S.transactions.filter(t => t.id !== dep.txOutId);
    }
...
function openNakupModal(editId) {
  ...
  moneyInFill('nakupItemRefPrice', item.refPrice);   // TODO-216
...
function nakupUpdateAlertPreview() {
  const refPrice = moneyInRead('nakupItemRefPrice');   // TODO-216
  ...
  el.innerHTML = `<span style="color:var(--income)">🔔 Upozornění při ceně ≤ <strong>${fmtBP(targetPrice)}</strong> (−${alertPct}% z ${fmtBP(refPrice)})</span>`;
}
...
function nakupSelectCatalogItem(key, name) {
  ...
  if (catItem?.latestPrice && !document.getElementById('nakupItemRefPrice').value) {
    moneyInFill('nakupItemRefPrice', catItem.latestPrice);   // TODO-216
    nakupUpdateAlertPreview();
  }
}
  ...
  el.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px">
      <div style="background:var(--surface2);border-radius:10px;padding:10px;text-align:center">
        <div style="font-size:1.1rem;font-weight:700;color:var(--income)">${fmtBP(minP)}</div>
        <div style="font-size:.7rem;color:var(--text3)">Minimum</div>
      </div>
      <div style="background:var(--surface2);border-radius:10px;padding:10px;text-align:center">
        <div style="font-size:1.1rem;font-weight:700">${fmtBP(Math.round(avgP))}</div>
        <div style="font-size:.7rem;color:var(--text3)">Průměr</div>
      </div>
      <div style="background:var(--surface2);border-radius:10px;padding:10px;text-align:center">
        <div style="font-size:1.1rem;font-weight:700;color:var(--expense)">${fmtBP(maxP)}</div>
    ...
    <div style="max-height:200px;overflow-y:auto">
      ${prices.slice().reverse().map(p => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--border)">
          <div>
            <div style="font-size:.82rem;font-weight:600">${fmtBP(p.price)}</div>
...
// ══════════════════════════════════════════════════════
// Dashboard widget: virtuální peněženka (žluté kolečko) s naspořenou hodnotou
async function renderDashVirtualWallet() {
  const el = document.getElementById('dashVirtualWallet'); if (!el) return;
  if (Object.keys(_goalDeposits).length === 0) { await goalLoadAllDeposits(); }
  const D = getData();
  const goals = (D.wishes || []).filter(w => w.isGoal && w.done !== true);
  if (!goals.length) { el.innerHTML = ''; return; }
  const totalSaved = goals.reduce((s,g) => s + goalGetSaved(g.id), 0);
  const totalTarget = goals.reduce((s,g) => s + (g.targetAmount||0), 0);
  const pct = totalTarget > 0 ? Math.min(100, Math.round(totalSaved/totalTarget*100)) : 0;
  el.innerHTML = `
    <div class="card" style="cursor:pointer" onclick="showPage('narozeniny')">
      <div class="card-body" style="display:flex;align-items:center;gap:14px;padding:14px 16px">
        <div style="width:44px;height:44px;border-radius:50%;background:radial-gradient(circle at 35% 30%,#fde68a,#f59e0b 70%,#b45309);display:flex;align-items:center;justify-content:center;font-size:1.4rem;flex-shrink:0;box-shadow:0 4px 14px -4px rgba(245,158,11,.6)">🟡</div>
        <div style="flex:1;min-width:0">
          <div style="font-size:.84rem;font-weight:700">Virtuální peněženka</div>
          <div style="font-size:.74rem;color:var(--text3)">${goals.length} ${goals.length===1?'aktivní cíl':'aktivních cílů'} · ${pct}% naplněno</div>
        </div>
        <div style="text-align:right">
          <div style="font-size:1.1rem;font-weight:800;font-family:Syne,sans-serif;color:var(--income)">${fmtBP(totalSaved)}</div>
          <div style="font-size:.7rem;color:var(--text3)">z ${fmtBP(totalTarget)}</div>
        </div>
      </div>
    </div>`;
}
  ...
  el.innerHTML = `
    <div class="card" style="margin-top:16px">
      <div class="card-header">
        ...
        <button class="btn btn-accent btn-sm" onclick="showPage('narozeniny')">Spravovat cíle</button>
      </div>
      <div class="card-body">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px">
          <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid var(--border)">
            <div style="font-size:1rem;font-weight:800;font-family:Syne,sans-serif;color:var(--income)">${fmtBP(totalSaved)}</div>
            <div style="font-size:.7rem;color:var(--text3)">Naspořeno</div>
          </div>
          <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid var(--border)">
            <div style="font-size:1rem;font-weight:800;font-family:Syne,sans-serif">${fmtBP(totalTarget)}</div>
            <div style="font-size:.7rem;color:var(--text3)">Celkový cíl</div>
          </div>
          <div style="background:var(--surface2);border-radius:12px;padding:12px;text-align:center;border:1px solid var(--border)">
            <div style="font-size:1rem;font-weight:800;font-family:Syne,sans-serif;color:${totalFree>0?'var(--income)':'var(--text3)'}">${fmtBP(totalTarget - totalSaved > 0 ? totalTarget - totalSaved : 0)}</div>
            <div style="font-size:.7rem;color:var(--text3)">Zbývá celkem</div>
          </div>
        </div>
            ...
            return `
              <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--surface3);border-radius:10px;cursor:pointer"
                ...
                <div style="flex:1;min-width:0">
                  ...
                  <div style="font-size:.7rem;color:var(--text3);margin-top:3px">${fmtP(saved)} / ${fmtBP(target)}</div>
                </div>
  ...
  alertEl.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;padding:12px 14px;background:rgba(74,222,128,.1);border:1px solid rgba(74,222,128,.3);border-radius:12px;margin-bottom:14px;cursor:pointer"
      ...
      <div style="flex:1">
        ...
        <div style="font-size:.76rem;color:var(--text2)">Aktuální cena ${fmtBP(currentPrice)} · pokles o <strong>${dropPct}%</strong></div>
      </div>
...
// === Explicitní zpřístupnění funkcí na window (volané z onclick napříč moduly) ===
// Oprava: nakupSwitchTab a další volané z budouci.js/onclick musí být globální i při odlišném scope.
if(typeof cileSwitchTab==='function') window.cileSwitchTab = cileSwitchTab;
if(typeof goalDelete==='function') window.goalDelete = goalDelete;
if(typeof goalDeleteDeposit==='function') window.goalDeleteDeposit = goalDeleteDeposit;
if(typeof goalMarkDone==='function') window.goalMarkDone = goalMarkDone;
if(typeof nakupDelete==='function') window.nakupDelete = nakupDelete;
if(typeof nakupSelectCatalogItem==='function') window.nakupSelectCatalogItem = nakupSelectCatalogItem;
if(typeof nakupSetFilter==='function') window.nakupSetFilter = nakupSetFilter;
if(typeof openGoalModal==='function') window.openGoalModal = openGoalModal;
if(typeof openNakupEdit==='function') window.openNakupEdit = openNakupEdit;
if(typeof openNakupModal==='function') window.openNakupModal = openNakupModal;
if(typeof openNakupPriceHistory==='function') window.openNakupPriceHistory = openNakupPriceHistory;
if(typeof saveGoal==='function') window.saveGoal = saveGoal;
if(typeof saveGoalDeposit==='function') window.saveGoalDeposit = saveGoalDeposit;
if(typeof saveNakupItem==='function') window.saveNakupItem = saveNakupItem;
if(typeof renderNakup==='function') window.renderNakup = renderNakup;
