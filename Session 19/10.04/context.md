...
## Stav po Session 19 (2026-08-21)

**Verze:** v9.98 · **38 JS modulů** (nový `pristi.js`) · 12 nástrojů v `tools/`

### Co Session 19 přinesla
Nová karta **Příští měsíc**, **automatické zálohy dat**, **kurzové ztráty**,
**evidence aktivity** v adminu, **měsíční review fáze 2+3**, **Osa života**,
přepracovaná **karta Projektu** a dotažená **základní měna** (143 míst + 20 vstupních polí).

**Deset oprav FIX-252 až FIX-261.** Nejzávažnější FIX-252 — `getHistAvg()` sčítal
přes `t.amt` a nefiltroval splity, což zasáhlo celý predikční engine včetně
`computeBaseIncome()`, tedy i finanční skóre.

### Pracovní vztah — co se v S19 potvrdilo
Milan chytil ze screenshotů a vlastní úvahou **šest** věcí, které testy nezachytily:
posun výplaty ve výplatním cyklu, rozpor hlášky s grafem, dva koše se stejným názvem,
chybějící tagy v Projektu, díru v ochraně kurzu a nevhodnost 6letého ořezání osy života.
U kurzu byla jeho námitka **přesnější než moje původní oprava** — vedla na ADR-101.

Milan také zamítl dvě věci, které jsem postavil: sběr času zápisu (v9.92 → zrušeno v9.93)
a ořezání osy života. Obojí správně.

### Kde to teď stojí
**Nasazeno:** vše z v9.98 kromě `worker.js` (Cloudflare, zvlášť).

**Rozdělané:** zbytek základní měny v 6 modulech (TODO-219), přesuny v kategoriích
typu `both` (TODO-220).

**Google Play:** čeká na D-U-N-S (ověřit, jestli ho OSVČ dostane), SHA-256 z Play Console
a čisté mobilní screenshoty. Rozhodnutí: EOP (10 % + nativní integrace) vs. Premium
v TWA vůbec nenabízet.

**Navrženo zavřít jako zamítnuté:** TODO-200 (diff-read 2b) a TODO-198 fáze 4
(varování při zadávání — přichází až po nákupu).

**Na horizontu:** rešerše ~20 funkčních celků (TODO-221), PSD2 bankovní propojení.

---

## Stav po druhé vlně Session 19 (v10.03, 2026-08-24)

Session pokračovala **hloubkovou analýzou karet**, ze které vypadlo deset dalších oprav.
Tři analytické dokumenty (`ANALYZA-obraz-detektor-report.md`,
`ANALYZA-komunita-uctenky.md`, `ANALYZA-radar-pristi-inflace.md`) pokrývají 8 ze 14 karet.

**Co se ukázalo jako nejcennější:** čtení kódu s pamětí na dřívější opravy.
FIX-267 (11 míst v Radaru) vznikl tím, že jsem si při analýze vzpomněl na FIX-252
a FIX-266 a napadlo mě ten vzor prohledat znovu jinde. O to nikdo nežádal.

**Dvě kritické chyby nahlásil Milan při testu cizího účtu:** nový uživatel neměl
kategorie ani peněženku (FIX-264) a aplikace mu je mazala i po ruční obnově (FIX-265).

**Bezpečnost:** GitHub Secret Scanning zablokoval push kvůli realisticky vypadajícímu
Resend klíči v `SECURITY.md`. Opraveno; klíč je nutné revokovat, je v historii commitů.
