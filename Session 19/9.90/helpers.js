// FinanceFlow · v9.90 · helpers.js · 2026-08-19
//  HELPERS
// ══════════════════════════════════════════════════════
...
function fxLossSummary(txs, D){
  ...
  (txs||[]).forEach(t=>{
    ...
    add(out.byWallet, t.wallet||'_none', w?((w.icon?w.icon+' ':'')+w.name):'Bez peněženky');
    const pt=(D.payTypes||[]).find(x=>x.id===t.payType);
    add(out.byPay, t.payType||'_none', pt?((pt.icon?pt.icon+' ':'')+pt.name):'Neuvedeno');
