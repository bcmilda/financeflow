// FinanceFlow · v9.89 · projects.js · 2026-08-19
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function _osaZivotaData(D){
  ...
  const MAX = 72;                                     // 6 let dozadu stačí
  const win = months.slice(-MAX);
  const idx = {}; win.forEach((x,i)=>{ idx[x.key]=i; });
  txs.forEach(t=>{
    const i = idx[String(t.date).slice(0,7)];
    if(i===undefined) return;
    const a = (typeof txCZK==='function') ? txCZK(t, D) : (t.amount||0);
    if(t.type==='income') win[i].inc += a;
    else if(t.type==='expense') win[i].exp += a;
  });
  let run = 0;
  win.forEach(x=>{ run += (x.inc - x.exp); x.cum = run; });

  const inRange = k => idx[String(k||'').slice(0,7)] !== undefined;
  const ms = (S.milestones||[]).filter(x=>!x.hidden);
  const events = ms.filter(x=>x.kind!=='era' && inRange(x.date))
    .map(x=>({ ...x, i: idx[String(x.date).slice(0,7)] }))
    .sort((a,b)=>a.i-b.i);
  const eras = ms.filter(x=>x.kind==='era' && x.date).map(x=>{
    const fromI = idx[String(x.date).slice(0,7)];
    const toI = x.dateTo ? idx[String(x.dateTo).slice(0,7)] : win.length-1;
    return { ...x, from: (fromI===undefined?0:fromI), to: (toI===undefined?win.length-1:toI) };
  }).filter(x=>x.to>=0 && x.from<win.length);

  return { win, events, eras };
}
...
function _osaZivotaHTML(){
  ...
  const vals = W.flatMap(p=>[p.inc, p.exp]).concat(W.map(p=>p.cum));
  ...
  // ── popisky let ──
  const yearTicks = W.map((p,i)=> (p.m===0 || i===0) ? `<g>
    ...
    <div class="denik-book" style="margin-top:14px">
      <div style="padding:16px 18px">
        ...
        <div style="font-size:.72rem;color:#b09f82;line-height:1.6;margin-top:9px;padding-top:8px;border-top:1px solid rgba(138,106,62,.35)">
          ...
          ${W.length>=60?'<br>Zobrazeno posledních 6 let.':''}
        </div>
      </div>
    </div>`;
}
