...
## Session 16–17 (v8.74 → v9.42) **(souhrnný dopočet, 2026-08-03)**

> Tato mezera (S16–17) nebyla v architecture.md průběžně zapsána. Stručný dopočet, detail v `Summary_s17.md` a příslušných patchích.

### Nové moduly
- `report.js` (S17, v9.09) — matice roků, Premium/Pro
- `inflace.js` (S17, v9.11) — osobní inflace z účtenek, shrinkflace
- `review.js` (S17, v9.34) — hodnocení útrat 1–5 (TODO-198 fáze 1)

### Klíčová rozhodnutí S16–17
- Finanční obraz — sekce „Kam směřuju", přepínač okna 6M/12M/Celkově, čtyři podmetriky (S16, TODO-166/173)
- Menu reorganizováno do 11 sekcí (S17, v9.00)
- Stripe live platby end-to-end vč. bezpečnostních FIX (S17)

---

## Session 18 (v9.42 → v9.78) **(2026-08-03)**

### Nový datový uzel: `milestones`
```
users/{uid}/data/milestones[] = {
  id, kind: 'point'|'era', date, dateTo (jen era),
  icon, label, note, auto ('trackStart' u automatického), hidden
}
```
Registrován na **4 místech** v `app.js`: `_DW_META`, `_dwMetaVals()`, 2× lokální snapshot. Firebase pravidla: **netřeba** — je uvnitř `users/{uid}`, kaskáda `.write` ho kryje; jen `.validate` na délku `label`/`note`/`icon`.

### Nový top-level uzel: `reviews`
```
reviews/{uid} = { stars (1–5), text (≤600 zn.), at, ver, name }
```
**Mimo** `users/{uid}` → vyžaduje **explicitní pravidla** (write jen vlastník, read přihlášení pro výpočet souhrnu na klientovi). Admin panel čte vše přes novou záložku ⭐ Recenze.

### Diff-read — stav k v9.78
- **Fáze 2** (v9.46): transakce přes `onChildAdded/Changed/Removed`, meta po klíčích. **Aktivní.**
- **Fáze 2b** (okno 12M + `stats/{YYYY}`): postavena ve v9.55, **odstraněna v9.57** (ADR-099). V kódu už není.

### Kontrolní nástroj: `tools/check_tdz.js`
Mimo hash chain aplikace, mimo nasazení (`firebase.json` ignore). Vyžaduje `acorn`+`acorn-walk` (`npm install --save-dev`, jednorázově). Detaily použití: `CLAUDE_SKILLS.md` SKILL 23.

### CRLF/LF soubory — ověřený stav k S18
`charts.js` je **CRLF**, `ui.js` je **LF** — při editaci obou vždy ověřit `newline=''` a počet `\r\n` před/po.
...
## Session 19 — změny architektury (v9.79–v9.98) **(2026-08-21)**

### Nový modul: `js/pristi.js` (38. modul)
Karta „Příští měsíc". Struktura podle vzoru `advisorBuildData` / `advisorRenderHTML` —
výpočet oddělený od renderu, render **nikdy nemutuje** globální `S`.
Rollback přes `PRISTI_ENABLED` na prvním řádku.

Zapojení: `app.html` (nav `navPristi`, `page-pristi`, script tag),
`ui.js` (dispatch v `renderPage`), `app.js` (`PAGE_TITLES.pristi` + `pristiCfg`).

### Nová pole datového modelu

**Transakce (`S.transactions[]`):**
| Pole | Kdy vzniká | Poznámka |
|---|---|---|
| `currency` | při zápisu, **jen když ≠ CZK** | chybí-li, odvodí se z peněženky → **žádná migrace** |
| `fxRef` | u cizoměnové transakce | kurz ČNB **k datu transakce**; jen živý, nikdy fallback |
| `fxRefDate` | tamtéž | datum kurzovního lístku — prozradí spolehlivost srovnání |

⚠️ `amtCZK` + `fxRef` tvoří **nedělitelný pár** (ADR-101).

**Nastavení (`S`):**
| Klíč | Obsah |
|---|---|
| `pristiCfg` | `{'YYYY-MM': {start, rows:{key:{amt?,off?}}, custom:[…]}}` — ruční úpravy karty Příští měsíc |

Registrováno na **všech 4 místech** v `app.js` (`_DW_META`, `_dwMetaVals`, oba snapshoty).

### Nové uzly Firebase (mimo `users/{uid}/data`)

```
users/{uid}/activity          čas použití, počet spuštění, d/{YYYY-MM-DD},
                              verze, PWA, firstSeen, firstTx
users/{uid}/backups/{datum}   {at, bytes, ver, json}  — snímek jako JEDEN řetězec
users/{uid}/backups/pred-obnovou   pojistka před obnovou
```

**Obojí leží mimo `data` schválně** — `data` má rozšířené `.read` pro partnera.
Zápis kaskáduje z `users/$uid` → **Firebase pravidla se nemění**.

### Worker: `/cnb` přijímá `?date=`
```
GET /cnb                      → dnešní kurzovní lístek (beze změny)
GET /cnb?date=DD.MM.RRRR      → historický lístek, cache 7 dní
```
Klient (`kurzy.js` → `fxRatesForDate()`) drží výsledky v paměti do konce sezení.
⚠️ **`worker.js` se nasazuje zvlášť** do Cloudflare — dokud se nenahraje, klient
dostane dnešní kurzy jako dosud a nic se nerozbije.

### Nové sdílené helpery (`helpers.js`)
| Funkce | K čemu |
|---|---|
| `fxLossOf(t)` | kurzová ztráta jedné transakce |
| `fxLossSummary(txs, D)` | souhrn + rozpad podle peněženky, typu platby a měny |
| `baseToCzk(v)` | opak `czkToBase()` |
| `moneyInFill(id, czk)` / `moneyInRead(id)` | peněžní vstupní pole (ADR-108) |
| `refreshMoneyLabels()` | přepis popisků `(Kč)` symbolem základní měny |

### Nové funkce v `app.js`
`trackActivity()` · `trackFirstTx()` · `backupRun()` / `backupRotate()` /
`backupList()` / `backupRestore()` / `backupMaybeDaily()`

Všechny běží **bez `await`** a ve vlastním `try/catch` — telemetrie ani záloha
nesmí shodit start aplikace.

### `tools/` — 12 nástrojů
`check_tdz.js` (Milanův, acorn) + 11 smoke testů. `firebase.json` má `tools/**`
v `ignore`, takže se **nikdy nenasadí**. Spouští je Claude před dodávkou.
