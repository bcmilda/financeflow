...
check('FIX-258: smazaný typ platby nevyrobí dva koše se stejným názvem',()=>{
  const t2=[
    {id:'a',currency:'EUR',amount:100,amtCZK:2465,fxRef:24.16,payType:'smazany1'},
    {id:'b',currency:'EUR',amount:100,amtCZK:2465,fxRef:24.16,payType:'smazany2'},
    {id:'c',currency:'EUR',amount:100,amtCZK:2687,fxRef:24.16},
  ];
  const r=sum(t2,DATA);
  const nazvy=r.pays.map(x=>x.label);
  assert(new Set(nazvy).size===nazvy.length,'dva koše se stejným popiskem: '+nazvy.join(' | '));
  assert(r.pays.length===1,'nezařaditelné se mají slít do jednoho koše, je jich '+r.pays.length);
});
