  ...
  el.innerHTML = projects.map(p => {
    ...
    return `<div class="project-card ${p.closed?'closed':''}" onclick="openProjectDetail('${p.id}')">
      ...
      <div style="display:flex;justify-content:space-between;font-size:.82rem;margin-bottom:6px">
        <span style="color:var(--expense)">Výdaje: <strong>${fmtB(spent)}</strong></span>
        <span style="color:var(--income)">Příjmy: <strong>${fmtB(income)}</strong></span>
        ${budget>0?`<span style="color:var(--text3)">Limit: ${fmtB(budget)}</span>`:''}
      </div>
      ${(()=>{const cl=projectCurLabel(txs,D,'expense');return cl?`<div style="font-size:.72rem;color:#a8aec8;margin-bottom:6px">z toho v cizí měně: <strong style="color:#c9cede">${cl}</strong></div>`:''})()}
...
// S19 (Milan): projekt může být z podstaty v cizí měně (dovolená v zahraničí).
//  Součty vedeme VŽDY v CZK přes txCZK() – jinak by se sčítala jablka s hruškami –
//  ale pokud v projektu figuruje jiná měna než základní, ukážeme navíc rozpad
//  „kolik to bylo v té měně". Vrací pole {cur, sum} seřazené od největší částky.
function projectCurBreakdown(txs, D, type){
  const by={};
  (txs||[]).filter(t=>t && (type ? (type==='expense' ? (t.type==='expense'||t.type==='debt') : t.type===type) : true)).forEach(t=>{
    const w=(D.wallets||[]).find(x=>x.id===t.wallet);
    const cur=(w&&w.currency)?w.currency:'CZK';
    by[cur]=(by[cur]||0)+((t.amount||t.amt)||0);
  });
  return Object.keys(by).map(cur=>({cur,sum:by[cur]})).sort((a,b)=>b.sum-a.sum);
}
// Popisek „1 000 € · 25 000 Kč" – cizí měna jen když se liší od základní.
function projectCurLabel(txs, D, type){
  const base=(typeof baseCur==='function')?baseCur():'CZK';
  const parts=projectCurBreakdown(txs,D,type).filter(x=>x.cur!==base && x.sum>0);
  if(!parts.length) return '';
  return parts.map(x=>`${fmt(x.sum)} ${(typeof curSym==='function')?curSym(x.cur):x.cur}`).join(' · ');
}
...
// ── S19 (TODO-217): kumulativní vývoj útraty vs. rozpočet ──
//  Ukazuje se AŽ OD 4 TRANSAKCÍ – ze dvou bodů je to úsečka, ne graf.
function _projGraphHTML(txs, D, budget, casPct){
  const exp = txs.filter(t=>t.type==='expense'||t.type==='debt')
                 .sort((a,b)=>new Date(a.date)-new Date(b.date));
  if(exp.length < 4) return '';
  let run = 0;
  const pts = exp.map(t=>{ run += txCZK(t,D); return { d:new Date(t.date+'T12:00:00'), v:run, name:t.name||'' }; });
  const t0 = pts[0].d.getTime(), t1 = pts[pts.length-1].d.getTime();
  const span = Math.max(1, t1-t0);
  const maxV = Math.max(run, budget>0?budget:0) * 1.06;
  const W=680, H=170, padL=52, padR=14, padT=14, padB=26;
  const x = d => padL + ((d.getTime()-t0)/span)*(W-padL-padR);
  const y = v => padT + (1 - v/maxV)*(H-padT-padB);
  const line = pts.map((p,i)=>`${i?'L':'M'} ${x(p.d).toFixed(1)} ${y(p.v).toFixed(1)}`).join(' ');
  const plocha = `${line} L ${x(pts[pts.length-1].d).toFixed(1)} ${y(0).toFixed(1)} L ${x(pts[0].d).toFixed(1)} ${y(0).toFixed(1)} Z`;
  const bud = budget>0 ? `<line x1="${padL}" y1="${y(budget).toFixed(1)}" x2="${W-padR}" y2="${y(budget).toFixed(1)}" stroke="#fbbf24" stroke-width="1.5" stroke-dasharray="5 4"/>
      <text x="${W-padR}" y="${(y(budget)-5).toFixed(1)}" text-anchor="end" font-size="9" fill="#fbbf24">rozpočet ${fmtB(budget)}</text>` : '';
  const osaY = [0, maxV/2, maxV].map(v=>`<line x1="${padL}" y1="${y(v).toFixed(1)}" x2="${W-padR}" y2="${y(v).toFixed(1)}" stroke="rgba(168,174,200,.14)"/>
      <text x="${padL-7}" y="${(y(v)+3).toFixed(1)}" text-anchor="end" font-size="9" fill="#a8aec8">${Math.round(czkToBase(v)/1000)}k</text>`).join('');
  const den = d=>`${d.getDate()}. ${d.getMonth()+1}.`;
  return `<div class="card" style="margin-bottom:12px"><div class="card-header"><span class="card-title">📈 Jak útrata narůstala</span></div>
    <div class="card-body" style="padding:10px 12px">
      <div style="overflow-x:auto"><svg viewBox="0 0 ${W} ${H}" style="width:100%;max-width:${W}px;display:block" preserveAspectRatio="xMidYMid meet">
        ${osaY}${bud}
        <path d="${plocha}" fill="rgba(6,182,212,.16)"/>
        <path d="${line}" fill="none" stroke="#06b6d4" stroke-width="2.2" stroke-linejoin="round"/>
        ${pts.map(p=>`<circle cx="${x(p.d).toFixed(1)}" cy="${y(p.v).toFixed(1)}" r="3" fill="#06b6d4" stroke="#0f111c" stroke-width="1"><title>${den(p.d)} · ${escHtml(p.name)} → celkem ${fmtB(p.v)}</title></circle>`).join('')}
        <text x="${padL}" y="${H-8}" font-size="9" fill="#a8aec8">${den(pts[0].d)}</text>
        <text x="${W-padR}" y="${H-8}" text-anchor="end" font-size="9" fill="#a8aec8">${den(pts[pts.length-1].d)}</text>
      </svg></div>
      <div style="font-size:.73rem;color:#a8aec8;line-height:1.6;margin-top:7px">
        Kumulativní součet výdajů. Strmý úsek = několik plateb za sebou.
        ${budget>0?'Čárkovaná čára je rozpočet.':''}
      </div>
    </div></div>`;
}

// ── S19 (TODO-217): srovnání s ostatními projekty téhož typu ──
//  Vyžaduje aspoň 2 UKONČENÉ srovnatelné projekty – z jednoho se průměr dělat nedá.
function _projCompareHTML(p, D, spent){
  const all = getProjects(D).filter(x=>x.id!==p.id && x.type===p.type);
  const dnes = new Date(); dnes.setHours(12,0,0,0);
  const hotove = all.filter(x=>x.end && new Date(x.end+'T12:00:00') < dnes);
  if(hotove.length < 2) return '';
  const data = hotove.map(x=>{
    const tx = getProjectTxs(x.id, D);
    return { name:x.name, sum: tx.filter(t=>t.type==='expense'||t.type==='debt').reduce((a,t)=>a+txCZK(t,D),0) };
  }).filter(x=>x.sum>0).sort((a,b)=>b.sum-a.sum);
  if(data.length < 2) return '';
  const prum = data.reduce((a,x)=>a+x.sum,0)/data.length;
  const rozdil = spent - prum;
  const maxS = Math.max(spent, data[0].sum) || 1;
  const radek = (nazev, sum, tento) => `<div style="display:flex;align-items:center;gap:9px;padding:5px 0">
      <div style="width:34%;min-width:0;font-size:.78rem;color:${tento?'#e8eaf2':'#a8aec8'};font-weight:${tento?'700':'400'};overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(nazev)}</div>
      <div style="flex:1;height:14px;background:var(--surface2);border-radius:4px;overflow:hidden">
        <div style="width:${Math.round(sum/maxS*100)}%;height:100%;background:${tento?'#06b6d4':'rgba(168,174,200,.4)'}"></div>
      </div>
      <div style="font-size:.76rem;color:${tento?'#e8eaf2':'#a8aec8'};white-space:nowrap">${fmtB(sum)}</div>
    </div>`;
  return `<div class="card" style="margin-bottom:12px"><div class="card-header">
      <span class="card-title">⚖️ Srovnání s podobnými</span>
      <span style="font-size:.72rem;color:var(--text3)">${PROJECT_TYPES[p.type]||p.type}</span></div>
    <div class="card-body" style="padding:10px 12px">
      ${radek(p.name + ' (tento)', spent, true)}
      ${data.slice(0,5).map(x=>radek(x.name, x.sum, false)).join('')}
      <div style="font-size:.76rem;color:#c9cede;line-height:1.6;margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">
        Průměr ${data.length} dokončených: <strong>${fmtB(prum)}</strong>.
        Tenhle projekt je ${Math.abs(rozdil)<prum*0.05 ? 'zhruba na průměru'
          : `o <strong style="color:${rozdil>0?'var(--debt)':'var(--income)'}">${fmtB(Math.abs(rozdil))}</strong> ${rozdil>0?'dražší':'levnější'}`}.
        <span style="color:#a8aec8">Srovnávají se jen ukončené projekty stejného typu.</span>
      </div>
    </div></div>`;
}
...
function renderProjectDetail(id) {
  ...
  // S19 (TODO-217): od NEJSTARŠÍ – u projektu je zajímavý příběh (letenka → ubytování → útrata na místě)
  const txs = getProjectTxs(id, D).sort((a,b) => new Date(a.date)-new Date(b.date));
  ...
  // ── S19 (TODO-217): ČAS vedle PENĚZ ─────────────────────────────────────
  //  Dosud karta říkala „29 % vyčerpáno", ale neřekla, jestli je to po týdnu,
  //  nebo tři dny před koncem. Bez časové osy je procento rozpočtu k ničemu.
  const _dnes = new Date(); _dnes.setHours(12,0,0,0);
  const _pStart = p.start ? new Date(p.start+'T12:00:00') : null;
  const _pEnd   = p.end   ? new Date(p.end+'T12:00:00')   : null;
  let casPct = null, dnyZbyva = null, tempo = null, predikce = null;
  if(_pStart && _pEnd && _pEnd > _pStart){
    const celkem = Math.round((_pEnd-_pStart)/86400000);
    const ubehlo = Math.round((_dnes-_pStart)/86400000);
    casPct = Math.max(0, Math.min(100, Math.round(ubehlo/celkem*100)));
    dnyZbyva = Math.max(0, Math.round((_pEnd-_dnes)/86400000));
    if(casPct > 5 && spent > 0){                 // pod 5 % času je extrapolace nesmysl
      tempo = Math.round(pct/casPct*100);
      predikce = Math.round(spent/casPct*100);
    }
  }
  // Rozpad podle kategorií – slouží i jako barevný pruh rozpočtu
  const _catAgg = {};
  txs.filter(t=>t.type==='expense'||t.type==='debt').forEach(t=>{
    const c = getCat(t.catId||t.category, D.categories);
    const k = c.name || 'Ostatní';
    if(!_catAgg[k]) _catAgg[k] = { ico:c.icon||'📦', sum:0, n:0 };
    _catAgg[k].sum += txCZK(t,D); _catAgg[k].n++;
  });
  const catList = Object.keys(_catAgg).map(k=>({name:k,..._catAgg[k]})).sort((a,b)=>b.sum-a.sum);
  const PROJ_COLS = ['#06b6d4','#a78bfa','#4ade80','#fbbf24','#f87171','#f472b6','#60a5fa'];
  const nejvetsi = catList[0] || null;
  ...
  el.innerHTML = `
    ...
    ${(() => {                          // S19 (TODO-217): pruh rozpočtu = zároveň rozpad podle kategorií
      if(!(budget>0) && !catList.length) return '';
      const zaklad = budget>0 ? budget : (spent||1);
      const segs = catList.map((c,k)=>`<div title="${escHtml(c.name)}: ${fmtB(c.sum)} (${c.n}×)" style="width:${Math.min(100,c.sum/zaklad*100)}%;background:${PROJ_COLS[k%PROJ_COLS.length]};opacity:.85"></div>`).join('');
      return `
      <div style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;font-size:.74rem;color:#a8aec8;margin-bottom:5px;flex-wrap:wrap;gap:6px">
          <span>Utraceno <strong style="color:#e8eaf2">${budget>0?pct+' %':fmtB(spent)}</strong>${budget>0?' rozpočtu':''}</span>
          ${casPct!==null?`<span>Uplynulo <strong style="color:#e8eaf2">${casPct} %</strong> času${dnyZbyva!==null?` · zbývá ${dnyZbyva} dní`:''}</span>`:''}
        </div>
        <div style="position:relative">
          <div style="display:flex;height:24px;border-radius:8px;overflow:hidden;border:1px solid var(--border)">
            ${segs}<div style="flex:1;background:var(--surface2)"></div>
          </div>
          ${casPct!==null?`<div style="position:absolute;top:-3px;bottom:-3px;left:${casPct}%;width:2px;background:#e8eaf2;opacity:.9" title="dnes"></div>`:''}
        </div>
        ${catList.length?`<div style="display:flex;gap:11px;flex-wrap:wrap;margin-top:7px;font-size:.72rem">
          ${catList.slice(0,7).map((c,k)=>`<span style="display:flex;align-items:center;gap:5px;color:#a8aec8"><span style="width:9px;height:9px;border-radius:2px;background:${PROJ_COLS[k%PROJ_COLS.length]}"></span>${c.ico} ${escHtml(c.name)} ${fmtB(c.sum)}</span>`).join('')}
        </div>`:''}
      </div>`;
    })()}

    ${(() => {                          // hodnocení tempa – jen když je z čeho počítat
      if(tempo===null || !(budget>0)) return '';
      const pod = predikce <= budget;
      return `<div style="margin-bottom:14px;padding:10px 12px;border-radius:9px;background:${pod?'rgba(74,222,128,.12)':'rgba(251,191,36,.12)'};font-size:.8rem;color:#c9cede;line-height:1.6">
        ${pod?'✅':'⚠️'} <strong style="color:${pod?'var(--income)':'var(--debt)'}">Utrácíš ${pod?'pomaleji':'rychleji'}, než ubíhá čas.</strong>
        Za ${casPct} % doby projektu jsi utratil ${pct} % rozpočtu. Při tomhle tempu skončíš na
        <strong style="color:${pod?'var(--income)':'var(--debt)'}">${fmtB(predikce)}</strong>,
        tedy ${fmtB(Math.abs(budget-predikce))} ${pod?'pod':'nad'} rozpočtem.
        <span style="color:#a8aec8">Lineární odhad z dosavadního tempa, ne předpověď.</span>
      </div>`;
    })()}

    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(148px,1fr));gap:10px;margin-bottom:12px">
      <div class="stat-card expense"><div class="stat-label">Výdaje</div><div class="stat-value down">${fmtB(spent)}</div>${budget>0?`<div class="stat-sub">z ${fmtB(budget)}</div>`:''}</div>
      ${/* S19 (Milan): příjmy a bilance JEN když projekt příjem opravdu má –
            u dotace na rekonstrukci nebo vrácené půjčky dávají smysl, u dovolené
            je „0 Kč" a „bilance = záporná útrata" totéž číslo dvakrát. */''}
      ${income>0?`<div class="stat-card income"><div class="stat-label">Příjmy / Dotace</div><div class="stat-value up">${fmtB(income)}</div><div class="stat-sub">${Math.round(income/(spent||1)*100)} % výdajů pokryto</div></div>
      <div class="stat-card balance"><div class="stat-label">Bilance</div><div class="stat-value ${balance>=0?'up':'down'}">${fmtB(balance)}</div><div class="stat-sub">příjmy − výdaje</div></div>`:''}
      ${budget>0?`<div class="stat-card debt"><div class="stat-label">Zbývá z rozpočtu</div><div class="stat-value ${budget-spent>=0?'bankc':'down'}">${fmtB(budget-spent)}</div>${dnyZbyva?`<div class="stat-sub">${fmtB((budget-spent)/Math.max(1,dnyZbyva))} na den</div>`:`<div class="stat-sub">${pct} % vyčerpáno</div>`}</div>`:''}
      ${nejvetsi?`<div class="stat-card"><div class="stat-label">Největší položka</div><div class="stat-value" style="font-size:1.05rem">${nejvetsi.ico} ${escHtml(nejvetsi.name)}</div><div class="stat-sub">${fmtB(nejvetsi.sum)} · ${Math.round(nejvetsi.sum/(spent||1)*100)} %</div></div>`:''}
    </div>

    ${_projGraphHTML(txs, D, budget, casPct)}
    ${_projCompareHTML(p, D, spent)}

    <div class="card">
      <div class="card-header">
        <span class="card-title">Transakce projektu</span>
        <span style="font-size:.74rem;color:var(--text3)">${txs.length} záznamů · od nejstarší</span>
      </div>
        ...
        ${txs.length ? txs.map(t => {
          ...
          return `<div class="tx-row-v2" style="padding:10px 12px">
            ...
            <div class="tx-body">
              ...
              ${t.note?`<div class="tx-sub-name">📝 ${escHtml(t.note)}</div>`:''}
              ${/* FIX-260 (S19, nahlásil Milan): tagy se v projektu nevykreslovaly,
                    přestože v seznamu Transakcí ano – uživatel je zapsal a nikde neviděl. */''}
              ${(t.tags||[]).length?`<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:3px">${
                t.tags.slice(0,4).map(tg=>`<span style="font-size:.66rem;color:#8fb3d9;background:rgba(96,165,250,.14);border-radius:5px;padding:1px 6px">#${escHtml(tg)}</span>`).join('')
              }${t.tags.length>4?`<span style="font-size:.66rem;color:#a8aec8">+${t.tags.length-4}</span>`:''}</div>`:''}
            </div>
...
function editProject(id) {
  ...
  moneyInFill('projectBudget', p.budget);   // TODO-216
...
function saveProject() {
  ...
  const p={
    ...
    budget:moneyInRead('projectBudget'),   // TODO-216
  ...
  ov.innerHTML=`<div style="background:var(--surface);border:1px solid var(--border);border-radius:16px;max-width:520px;width:100%;max-height:86vh;display:flex;flex-direction:column">
    ...
    <div style="padding:12px 16px;overflow-y:auto;flex:1">
      <div style="font-size:.72rem;color:#a8aec8;line-height:1.5;margin-bottom:10px">
        ${hasHistory
          ? `Návrh podle tvých skutečných výdajů za poslední 3 měsíce, vztaženo k základu příjmu <strong style="color:var(--text)">${fmtB(base)}</strong>. Uprav dle potřeby a ulož.`
      ...
      <div style="display:flex;align-items:center;gap:9px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.05)">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:.66rem;color:#a8aec8">${hasHistory?`Ø ${fmtB(Math.round(r.avg))}/měs`:''}${r.cur?` · nyní ${r.cur} %`:''}</div>
        </div>
    ...
    if (nMonths === 1) {
      ...
      if (limit > 0 && cur.v > limit)
        out.push({ lvl: explained ? 'info' : 'high', cat: c,
          t: `${c.name}: překročen limit o ${fmtB(Math.round(cur.v - limit))}`,
          d: `${fmtB(Math.round(cur.v))} z limitu ${fmtB(Math.round(limit))}.` });
      else if (prev > 0 && d > 500 && d / prev > 0.25)
        out.push({ lvl: explained ? 'info' : 'mid', cat: c,
          t: `${c.name}: +${fmtB(Math.round(d))} vs. minulý měsíc`,
      ...
      if (rise >= 2 && cur.v > 500)
        out.push({ lvl: 'mid', cat: c,
          ...
          d: vals.map(x => fmtB(Math.round(x.v))).join(' → ') + ' Kč.' });
      ...
      if (nz.length >= 3) {
        ...
        if (med > 0 && peak.v > med * 2 && peak.v - med > 1000)
          out.push({ lvl: 'mid', cat: c,
            t: `${c.name}: ${CZ_M[peak.m]} ${fmtB(Math.round(peak.v))}, jinak kolem ${fmtB(Math.round(med))}`,
...
function renderReport() {
  ...
  const catRows = expCats.map(cat => {
    ...
    const limitPct = (cat.isSaving||cat.isInvest)
      ? (cat.healthPct ? `min ${cat.healthPct} %` : (cat.healthAmt ? `min ${fmtB(cat.healthAmt)}` : '–'))
      : (_eff===Infinity ? '–' : (_la < _lp ? `strop ${fmtB(cat.healthAmt)}` : `${cat.healthPct} % základu`));
    const limitAmt = cat.healthAmt ? fmtB(cat.healthAmt) : (cat.isSaving ? 'min' : '–');
    const pctOfInc = scores.baseIncome > 0 ? Math.round(spent/scores.baseIncome*100) : (totalInc > 0 ? Math.round(spent/totalInc*100) : 0);
    const planAmt = (_eff>0 && _eff!==Infinity) ? fmtB(Math.round(_eff)) : '–';
    ...
    const trend = (() => {
      ...
      const tip = ` title="vs. předchozí období: ${fmtB(prev)}"`;
    ...
    return `<div class="cat-tile" title="${cat.name}: ${fmtB(spent)}${noLimit?'':' / plán '+planAmt+' ('+limitPct+')'}">
      ...
      <div class="cat-tile-bar">
        ...
        <div class="cat-tile-txt">${fmtB(spent)}${noLimit?'':' / '+planAmt}</div>
      </div>
  ...
  el.innerHTML = tabBar + `
    ...
    <div class="report-stat-grid">
      ...
      ${(()=>{ let pm=S.curMonth-1, py=S.curYear; if(pm<0){pm=11;py--;}
        ...
        return `
      <div class="stat-card income"><div class="stat-label">Příjmy</div><div class="stat-value up">${fmtB(totalInc)}</div>
        <div class="stat-sub" style="font-size:.68rem">${incDiff!==null?arrow(incDiff,false):(prevInc?'minule '+fmtB(prevInc):'bez srovnání')}</div></div>
      <div class="stat-card expense"><div class="stat-label">Výdaje</div><div class="stat-value down">${fmtB(totalExp)}</div>
        <div class="stat-sub" style="font-size:.68rem">${expDiff!==null?arrow(expDiff,true):'bez srovnání'}</div></div>
      <div class="stat-card balance"><div class="stat-label">Saldo</div><div class="stat-value ${saldo>=0?'up':'down'}">${fmtB(saldo)}</div>
        ...
        ${mv.map(r=>`
          <div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--border)">
            ...
            <span style="margin-left:auto;font-family:Syne,sans-serif;font-weight:800;font-size:.92rem;color:${r.d>0?'var(--expense)':'var(--income)'};min-width:110px;text-align:right">
              ${r.d>0?'+':'−'}${fmtB(Math.abs(Math.round(r.d)))}</span>
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-body">
        <div style="background:var(--surface2);border-radius:9px;padding:9px 12px;margin-bottom:12px;font-size:.7rem;color:#a8aec8;line-height:1.6">
          ...
          <div style="color:#a8aec8">ℹ️ Plán i % se počítají ze <strong style="color:var(--text)">Základu příjmu ${fmtB(scores.baseIncome)}</strong> (vážený průměr stabilních příjmů za 3 měsíce), ne z příjmu aktuálního měsíce.</div>
        </div>
        ${catRows?`
          ...
          <div style="flex:1;min-width:210px">
            ...
            <div style="font-size:.76rem;color:#c9cede;line-height:1.6">
              ${(()=>{ const base=scores.baseIncome||0;
                ...
                return base>0
                  ? `Ze Základu příjmu <b>${fmtB(Math.round(base))}</b> jsi odložil <b>${pct} %</b> do 📈 investic a spořicích kategorií.`
        ...
        if(nw) nwHtml = `
          ...
          <div class="report-stat-grid" style="margin-bottom:14px">
            <div class="stat-card ${nw.netWorth>=0?'income':'expense'}">
              ...
              <div class="stat-value ${nw.netWorth>=0?'up':'down'}">${fmtB(Math.round(nw.netWorth))}</div>
              <div class="stat-sub" style="font-size:.68rem">majetek ${fmtB(Math.round(nw.totalAssets+nw.totalWallets))} − dluhy ${fmtB(Math.round(nw.totalDebts))}</div></div>
            <div class="stat-card ${nw.totalDebts>0?'debt':'income'}">
              <div class="stat-label">Dluhy</div>
              <div class="stat-value" style="color:${nw.totalDebts>0?'var(--debt)':'var(--income)'}">${fmtB(Math.round(nw.totalDebts))}</div>
            ...
            <div class="stat-card bank">
              ...
              <div class="stat-sub" style="font-size:.68rem">${fmtB(Math.round(liqNow))} při výdajích ${fmtB(Math.round(mExp))}</div></div>
          </div>`;
      }catch(e){}
      ...
      try{
        ...
        if(!recs.length){
          ...
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:6px 14px 12px">
            ...
            ${row('Utraceno na účtenkách', fmtB(Math.round(sum)))}
            ${row('💚 Ušetřeno na slevách', saved>0?fmtB(Math.round(saved)):'–',
                  saved>0&&sum>0?Math.round(saved/(sum+saved)*100)+' % z nákupu':'', saved>0?'var(--income)':'#a8aec8')}
            ${top?row('Nejvíc utraceno', top[0], fmtB(Math.round(top[1]))):''}
            ${cheap&&top&&cheap[0]!==top[0]?row('Nejmenší útrata', cheap[0], fmtB(Math.round(cheap[1]))):''}
        ...
        if(plan!==null || pred!==null) fwHtml = `
          ...
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:6px 14px 12px">
            ${plan!==null?`<div style="display:flex;justify-content:space-between;align-items:center;padding:9px 0;border-bottom:1px solid var(--border)">
              ...
              <b style="font-family:Syne,sans-serif;font-size:1rem">${fmtB(Math.round(plan))}</b></div>`:''}
            ${pred!==null?`<div style="display:flex;justify-content:space-between;align-items:center;padding:9px 0;border-bottom:1px solid var(--border)">
              ...
              <b style="font-family:Syne,sans-serif;font-size:1rem">${fmtB(Math.round(pred))}</b></div>`:''}
            ${(()=>{ const exp = (pred!==null?pred:0);
              ...
              return `<div style="display:flex;justify-content:space-between;align-items:center;padding:9px 0;border-bottom:1px solid var(--border)">
                <span style="font-size:.84rem">Očekávané saldo <span style="color:#a8aec8;font-size:.72rem">(základ příjmu ${fmtB(Math.round(inc))})</span></span>
                <b style="font-family:Syne,sans-serif;font-size:1.05rem;color:${sal>=0?'var(--income)':'var(--expense)'}">${sal>=0?'+':''}${fmtB(Math.round(sal))}</b></div>
              <div style="display:flex;justify-content:space-between;align-items:center;padding:9px 0">
                ...
                <b style="font-family:Syne,sans-serif;font-size:1.05rem;color:${realSal>=0?'var(--income)':'var(--expense)'}">${realSal>=0?'+':''}${fmtB(Math.round(realSal))}</b></div>`;
            })()}
          ...
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:12px 14px">
            <div style="display:flex;gap:22px;flex-wrap:wrap;align-items:center">
              ...
              <div style="flex:1;min-width:230px">
                <div style="display:flex;justify-content:space-between;font-size:.78rem;padding:4px 0;border-bottom:1px solid var(--border)">
                  <span>Ohodnoceno výdajů</span><b>${fmtB(Math.round(r.sum))} / ${fmtB(Math.round(totalExp))}</b></div>
                <div style="display:flex;justify-content:space-between;font-size:.78rem;padding:4px 0;border-bottom:1px solid var(--border)">
                  <span>Hodnoceno 4–5 ⭐</span><b style="color:var(--income)">${fmtB(Math.round(r.good))}</b></div>
                <div style="display:flex;justify-content:space-between;font-size:.78rem;padding:4px 0">
                  <span>Hodnoceno 1–2 ⭐</span><b style="color:${r.bad>0?'var(--debt)':'#a8aec8'}">${fmtB(Math.round(r.bad))}</b></div>
              </div>
            </div>
            ${r.worst.length?`<div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--border)">
              ...
              ${r.worst.map(w=>`<div style="display:flex;align-items:center;font-size:.78rem;padding:5px 0;border-bottom:1px solid var(--border)">
                ...
                <b style="min-width:80px;text-align:right">${fmtB(Math.round(w.a))}</b>
              ...
              <div style="font-size:.74rem;color:#c9cede;margin-top:9px;line-height:1.55">
                Kdybys polovinu z toho příště nasměroval jinam, je to <b style="color:var(--income)">${fmtB(Math.round(r.bad*6))} za rok</b>.
...
function renderRadar() {
  ...
  if(isCurrentMonth && eomLeft < 0) {
    alerts.unshift({level:'danger', icon:'📉', text:`Do konce měsíce ti chybí ${fmtB(Math.abs(eomLeft))}. Příjem ${fmtB(expectedIncMonth)} − výdaje ${fmtB(totalExp)} − známé platby ${fmtB(Math.round(budToEOM))}.`});
  }
  if(pred3Total < 0) {
    alerts.push({level:'warn', icon:'🔮', text:`Predikce: další 3 měsíce směřují k zápornému saldu ${fmtB(pred3Total)} (průměrný příjem ${fmtB(avgInc)} vs výdaje ${fmtB(avgExp)} + plánované platby).`});
  }
  // Session 10: kvartální alert – nikdo dosud nehlídal kvartál.
  if(qRemainingMonths > 0 && qProjectedSaldo < 0) {
    alerts.push({level:'warn', icon:'📅', text:`${qLabel} směřuje k zápornému saldu ${fmtB(qProjectedSaldo)} (dosud ${fmtB(Math.round(qActualSaldo))} + odhad ${qRemainingMonths} zbývajících měs.).`});
  }
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="card-body">
        ...
        <div style="background:${freeToSpend>=0?'rgba(74,222,128,.08)':'var(--expense-bg)'};border:1px solid ${freeToSpend>=0?'rgba(74,222,128,.25)':'rgba(248,113,113,.3)'};border-radius:10px;padding:11px 14px;margin-bottom:12px">
          ...
          <div style="font-size:.66rem;color:#a8aec8;margin-top:4px">Příjem ${fmtB(Math.round(incForFree))} − utraceno ${fmtB(totalExp)} − budoucí platby ${fmtB(Math.round(budRestMonth))}${freeToSpend<0?' → na plánované platby ti nestačí příjem':''}</div>
        </div>` : ''}
      ...
      <div class="card-body">
        ...
        ${(() => {
          ...
          <div style="position:relative">
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;align-items:end;min-height:${chartH+52}px">
              ${bars.map(b=>{
                ...
                return `<div style="display:flex;flex-direction:column;align-items:center;gap:5px">
                  <div style="position:relative;z-index:3;font-family:Syne;font-size:.82rem;font-weight:800;color:${b.color};background:rgba(15,17,28,.9);border-radius:5px;padding:1px 6px;white-space:nowrap">${b.val>=0?'':'−'}${fmtB(Math.abs(b.val))}</div>
                  ...
                  <div style="display:inline-flex;align-items:baseline;gap:4px;padding:2px 7px;border-radius:6px;background:rgba(15,17,28,.85);border:1px solid ${b.color}44;white-space:nowrap">
                    ...
                    <span style="font-size:.72rem;font-weight:800;color:${_dCol}">${b.real>=0?'':'−'}${fmtB(Math.abs(b.real))}</span>
            ...
            <svg viewBox="0 0 100 ${baseY+4}" preserveAspectRatio="none" style="position:absolute;top:18px;left:0;width:100%;height:${chartH+8}px;pointer-events:none;z-index:1;overflow:visible">
        ...
        <div style="margin-top:6px;padding:10px 12px;border-radius:8px;background:var(--surface2);border:1px solid var(--border);font-size:.78rem;color:var(--text2)">
          ${pred3Total>=0
            ? `📈 Při zachování průměru (příjem ${fmtB(avgInc)} / výdaje ${fmtB(avgExp)} měsíčně) bys za 3 měsíce <strong style="color:var(--income)">našetřil ${fmtB(pred3Total)}</strong>.`
            : `📉 Při zachování průměru (příjem ${fmtB(avgInc)} / výdaje ${fmtB(avgExp)} měsíčně) by ti za 3 měsíce <strong style="color:var(--expense)">chybělo ${fmtB(Math.abs(pred3Total))}</strong>. Zváž úpravu výdajů nebo plánovaných plateb.`}
        </div>
            ...
            return `<div style="flex:1;min-width:210px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;overflow:hidden">
              ...
              <table style="width:100%;border-collapse:collapse;font-size:.74rem">
                ...
                ${items.map(b=>`<tr style="border-top:1px solid var(--border)">
                  ...
                  <td style="padding:4px 8px;text-align:right;font-weight:600;color:${b.color||'var(--expense)'};white-space:nowrap">${fmtB(b.amount)}</td>
                </tr>`).join('')}
      ...
      <div class="card-body">
        ...
        ${(() => {
          ...
          cfSeries.forEach((s,i)=>{
            ...
            bars += `<text x="${cx-barW/2-3}" y="${iy-3}" text-anchor="middle" font-size="7.5" fill="var(--income)">${fmt(Math.round(czkToBase(s.inc)/1000))}k</text>`;
            bars += `<text x="${cx+barW/2+3}" y="${ey-3}" text-anchor="middle" font-size="7.5" fill="var(--expense)">${fmt(Math.round(czkToBase(s.exp)/1000))}k</text>`;
            const cfy = yOf(s.cf);
            cfPts.push(`${cx},${cfy}`);
            ...
            cfDots += `<text x="${cx}" y="${H-9}" text-anchor="middle" font-size="9" fill="${s.cf>=0?'var(--income)':'var(--expense)'}" font-weight="700">${s.cf>=0?'+':''}${fmtB(s.cf)}</text>`;
          });
          const zeroY = yOf(0);
          // osa Y popisky (max a 0)
          const axisY = `<text x="2" y="${yOf(maxPos)+3}" font-size="8" fill="var(--text3)">${fmt(Math.round(czkToBase(maxPos)/1000))}k</text>
      ...
      <div class="card-body" style="padding:0">
        ...
        ${visibleQ.map((q,i)=>{
          ...
          return `<div style="display:grid;grid-template-columns:48px 1fr 1fr 1fr;padding:9px 12px;border-bottom:1px solid var(--border);font-size:.78rem;${isCurQ?'font-weight:600;background:var(--surface2)':''}">
            ...
            <span style="text-align:right;color:${q.inc>0?'var(--income)':'var(--text3)'}">${q.inc>0?fmtB(q.inc):'–'}</span>
            <span style="text-align:right;color:${q.exp>0?'var(--expense)':'var(--text3)'}">${q.exp>0?fmtB(q.exp):'–'}</span>
            <span style="text-align:right;color:${q.saldo>=0?'var(--income)':'var(--expense)'}">${q.inc>0||q.exp>0?(q.saldo>=0?'+':'')+fmtB(q.saldo):'–'}</span>
          </div>`;
        }).join('')}
        ${visibleQ.length>0?`
        <div style="display:grid;grid-template-columns:48px 1fr 1fr 1fr;padding:9px 12px;font-size:.78rem;font-weight:700">
          ...
          <span style="text-align:right;color:var(--income)">${fmtB(visibleQ.reduce((a,q)=>a+q.inc,0))}</span>
          <span style="text-align:right;color:var(--expense)">${fmtB(visibleQ.reduce((a,q)=>a+q.exp,0))}</span>
          <span style="text-align:right;color:${visibleQ.reduce((a,q)=>a+q.saldo,0)>=0?'var(--income)':'var(--expense)'}">${visibleQ.reduce((a,q)=>a+q.saldo,0)>=0?'+':''}${fmtB(visibleQ.reduce((a,q)=>a+q.saldo,0))}</span>
        </div>`:``}
      </div>
    </div>`;
...
function drawRadarDaily(canvas, hoverDay){
  ...
  if(isCurrent && cumolEnd<days){
    ...
    ctx.textAlign='right';ctx.font='9px Instrument Sans';ctx.fillText('odhad '+fmt(Math.round(czkToBase(predEnd)))+' '+curSym(),W-right,yOf(predEnd)-6);  // S19: symbol základní měny (převod už proběhl – NEpoužívat fmtB, byl by dvojí)
  }
  // tečka dne příjmu
  if(incomeDay>0){
...
function radarPaydayInfo(D){
  ...
  if(cand.length){
    let best=cand[0]; cand.forEach(t=>{ if(txCZK(t,D)>txCZK(best,D)) best=t; });
...
function renderRadarPayday(el, D){
  ...
  const varAgg={};
  cycAllExp.forEach(t=>{ if(grpOf(t)!=='variable')return; varAgg[t.catId]=(varAgg[t.catId]||0)+txCZK(t,D); });
  ...
  { // přichycení na reálnou výplatu ±6 dní (stejně jako u aktuálního cyklu)
    ...
    if(cand.length){ let b=cand[0]; cand.forEach(t=>{ if(txCZK(t,D)>txCZK(b,D)) b=t; }); prevStart=new Date(b.date); prevStart.setHours(0,0,0,0); }
  }
  ...
  let weSum=0,wdSum=0;
  cycSoFar.forEach(t=>{ if(t.type!=='expense'||t.isBalancing||t.splitParent||isTransferTx(t)) return; const wd=new Date(t.date).getDay(); const a=txCZK(t,D); if(wd===0||wd===6) weSum+=a; else wdSum+=a; });
  ...
  const weekBars=weeks.map(w=>{
    ...
    return `<div style="flex:1;min-width:0;display:flex;flex-direction:column;align-items:center;gap:5px">
      <div style="font-size:.66rem;font-weight:700;color:#c2c7da;font-family:Syne">${w.total>0?fmtB(w.total):(w.future?'·':'0')}</div>
  ...
  const weekRows=weeks.map(w=>`<tr style="border-top:1px solid var(--border);${w.future?'opacity:.45':''}">
    ...
    <td style="padding:5px 6px;text-align:right;font-weight:700;color:#e8eaf2">${w.lived>0?fmtB(w.perDay):'–'}</td>
  </tr>`).join('');
      ...
      <div class="card-body">
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:10px">
          <div class="stat-card-h" style="background:var(--surface2);border-radius:10px;padding:11px;text-align:center;border:1px solid var(--border);min-width:0">
            <div class="stat-value-h" style="color:#a8aec8">${fmtB(prevSame)}</div>
          ...
          <div class="stat-card-h" style="background:var(--surface2);border-radius:10px;padding:11px;text-align:center;border:1px solid var(--border);min-width:0">
            <div class="stat-value-h" style="color:#a8aec8">${fmtB(prevTotal)}</div>
...
function _obrazProjChart(proj){
  ...
  all.forEach((r,i)=>{
    ...
    bars.forEach(([v,col,lbl],bi)=>{
      ...
      g+=`<rect x="${x.toFixed(1)}" y="${rY.toFixed(1)}" width="${bw.toFixed(1)}" height="${rH.toFixed(1)}" rx="2.5" fill="${col}" opacity="${r.isNow?'.95':'.8'}"><title>${lbl}: ${fmtB(Math.round(v))}</title></rect>`;
  ...
  resPts.forEach(p=>g+=`<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="2.6" fill="#fbbf24"><title>Rezerva: ${fmtB(p.v)}</title></circle>`);
  ...
  cfPts.forEach(p=>{
    g+=`<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3.4" fill="#60a5fa"><title>Cashflow: ${p.v>=0?'+':''}${fmtB(p.v)}</title></circle>`;
...
function _obrazProjVerdict(proj){
  ...
  if(avg>0 && res6>0){
    ...
    // FIX-257 (S19, nahlásil Milan): dřív tu stálo „do plusu se takhle nedostaneš"
    //   vždy, když rezerva po 6 měsících ještě nebyla kladná. Jenže avg>0 znamená,
    //   že rezerva ROSTE – uživatel se do plusu dostane, jen o něco později.
    //   Text tvrdil pravý opak toho, co ukazoval graf vedle něj. Nyní se dopočítá,
    //   ZA KOLIK MĚSÍCŮ se nula překročí, což je i mnohem užitečnější informace.
    const doNuly = Math.ceil(Math.abs(resNow)/avg);
    const kdy = new Date(); kdy.setDate(1); kdy.setMonth(kdy.getMonth()+doNuly);
    tone='var(--debt)'; icon='⚠️'; head='Jdeš správným směrem, jen to potrvá';
    body=`Měsíčně ti zbývá <strong>${fmtB(avg)}</strong> a rezerva roste, jenže startuješ z ${fmtB(resNow)}. `
       + `Za 6 měsíců budeš na <strong>${fmtB(res6)}</strong> – pořád v mínusu, ale o ${fmtB(Math.abs(diff))} líp. `
       + `Při tomhle tempu se přes nulu dostaneš <strong>za ${doNuly} ${doNuly===1?'měsíc':doNuly<5?'měsíce':'měsíců'}</strong> (${CZ_M[kdy.getMonth()].toLowerCase()} ${kdy.getFullYear()}). `
       + `Rychleji to půjde jen vyšším příjmem nebo nižšími výdaji.`;
  } else {
...
function radarPastCycles(D, nWant){
  ...
  const snap=(expected)=>{ // přichycení očekávané výplaty na reálný příjem ±6 dní
    ...
    let b=cand[0]; cand.forEach(t=>{ if(txCZK(t,D)>txCZK(b,D)) b=t; });
...
function _obrazCyclesChart(cycles){
  ...
  // S19 (Milan): k mediánu i ROZPTYL – medián sám neřekne, jestli je týden stabilní,
  //   nebo jestli jednou utratíš 500 a podruhé 15 000. Whisker (svislá čára s nožkami)
  //   ukazuje minimum a maximum napříč cykly.
  const weekRange = Array.from({length:nW},(_,i)=>{
    const v=cycles.map(c=>(c.weeks||[])[i]).filter(x=>isFinite(x)&&x>0);
    return v.length>=2 ? {min:Math.min(...v), max:Math.max(...v), n:v.length} : null;
  });
  ...
  const allVals=[...weekMed,...weekRange.filter(Boolean).map(r=>r.max),...(showDays?dayMed:[]),...green].filter(v=>isFinite(v));
  ...
  weekMed.forEach((v,i)=>{
    ...
    g+=`<rect x="${(cx-wbw/2).toFixed(1)}" y="${yy.toFixed(1)}" width="${wbw.toFixed(1)}" height="${hh.toFixed(1)}" rx="4"
      fill="#60a5fa" opacity="${showDays?'.24':'.8'}"><title>${i+1}. týden – medián: ${fmtB(v)}</title></rect>`;
    const rg=weekRange[i];
    if(rg && !showDays){
      const yTop=y(rg.max), yBot=y(rg.min), wx=cx, cap=Math.min(9,wbw*0.28);
      g+=`<g opacity=".85"><title>${i+1}. týden – nejméně ${fmtB(rg.min)}, nejvíc ${fmtB(rg.max)} (z ${rg.n} cyklů)</title>`
       + `<line x1="${wx.toFixed(1)}" y1="${yTop.toFixed(1)}" x2="${wx.toFixed(1)}" y2="${yBot.toFixed(1)}" stroke="#c9dcf5" stroke-width="1.4"/>`
       + `<line x1="${(wx-cap).toFixed(1)}" y1="${yTop.toFixed(1)}" x2="${(wx+cap).toFixed(1)}" y2="${yTop.toFixed(1)}" stroke="#c9dcf5" stroke-width="1.6"/>`
       + `<line x1="${(wx-cap).toFixed(1)}" y1="${yBot.toFixed(1)}" x2="${(wx+cap).toFixed(1)}" y2="${yBot.toFixed(1)}" stroke="#c9dcf5" stroke-width="1.6"/></g>`;
    }
  ...
  if(showDays){
    ...
    dayMed.forEach((v,i)=>{
      ...
      g+=`<rect x="${(cx-dbw/2).toFixed(1)}" y="${yy.toFixed(1)}" width="${dbw.toFixed(1)}" height="${hh.toFixed(1)}" rx="1.5"
        fill="#f87171" opacity=".9"><title>${i+1}. den – medián: ${fmtB(v)}</title></rect>`;
    });
  }
  ...
  if(green.length){
    ...
    pts.forEach((p,i)=>{
      ...
      g+=`<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3.6" fill="#4ade80" stroke="#0c101c" stroke-width="1.3">
        <title>Zbývá po ${showDays?(i+1)+'. dni':(i+1)+'. týdnu'}: ${fmtB(p.v)}</title></circle>`;
    });
  ...
  if(!showDays && weekRange.some(Boolean)){
    g+=`<text x="${(W-pad.r).toFixed(1)}" y="${H-pad.b+36}" text-anchor="end" font-size="9" fill="#a8aec8">svislá čára = nejméně a nejvíc napříč cykly</text>`;
  }
...
function renderObraz() {
  ...
  const _pctTxt=vals=>{ const {a,b}=_half(vals);
    if(Math.abs(a)<1000){ const d=Math.round(b-a); return {txt:`${d>0?'↑ ':d<0?'↓ ':'↔ '}${d>=0?'+':''}${fmtB(d)}`, dir:d}; }
  ...
  const _lsTable = (series.length<2 || (!_lsB.inc && !_lsB.exp)) ? '' : `
    ...
    <table style="width:100%;border-collapse:collapse;font-size:.75rem;min-width:360px">
      ...
      <div style="font-size:.72rem;color:#a8aec8;line-height:1.65;margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">
        <strong style="color:#c9cede">Dvě různé věci vedle sebe.</strong>
        <b>Expense Ratio</b> měří <b>úroveň</b> — kolik z příjmu právě teď spotřebuješ.
        Funguje od prvního měsíce.
        Verdikt nahoře měří <b>tempo</b> — jak rychle výdaje rostou proti dřívějšku,
        a k tomu potřebuje <b>aspoň 6 měsíců záznamů</b>. Do té doby zůstává prázdný,
        i když Expense Ratio už číslo ukazuje.
        <br><span style="color:#8b90a8">Pozn.: Expense Ratio a míra úspor jsou dvě strany téhož —
        součet dá vždy 100 %, proto se nikdy nezobrazují vedle sebe.</span>
      </div>`;
      ...
      <div class="card-body" style="padding:14px">
        ...
        ${(()=>{ // S16 (TODO-172): tabulka 1.–5. týden od výplaty + trend mezi cykly
          ...
          const cell=(v,prev)=>{
            ...
            return `<td style="padding:5px 6px;text-align:right;color:${col}">${arr}${fmtB(v)}</td>`;
          };
          const rows=cycles.map((c,ci)=>{
            ...
            return `<tr style="border-top:1px solid var(--border)">
              ...
              <td style="padding:5px 6px;text-align:right;font-weight:700;color:var(--expense)">${fmtB(c.exp)}</td>
              ...
              <td style="padding:5px 6px;text-align:right;font-weight:700;color:${saldo>=0?'var(--income)':'var(--expense)'}">${saldo>=0?'+':''}${fmtB(saldo)}</td>
            </tr>`;
          }).join('');
            ...
            <table style="width:100%;border-collapse:collapse;font-size:.72rem;min-width:600px">
              ...
              <tbody>${rows}
                <tr style="border-top:2px solid var(--border2);background:var(--surface2)">
                  ...
                  ${wAvg.map(v=>`<td style="padding:5px 6px;text-align:right;font-weight:600;color:#a8aec8">${fmtB(v)}</td>`).join('')}
                  ...
                  <td style="padding:5px 6px;text-align:right;font-weight:700;color:var(--expense)">${fmtB(Math.round(cycles.reduce((a,c)=>a+c.exp,0)/cycles.length))}</td>
                  ...
                  ${(()=>{ const sal=cycles.map(c=>(c.inc||0)-(c.exp||0));
                    ...
                    return `<td style="padding:5px 6px;text-align:right;font-weight:700;color:${avg>=0?'var(--income)':'var(--expense)'}">${avg>=0?'+':''}${fmtB(avg)}</td>`; })()}
                </tr>
                <tr style="background:var(--surface2)">
                  ...
                  ${wAvg.map((v,i)=>{ const sum=cycles.reduce((a,c)=>a+((c.weeks&&c.weeks[i])||0),0);
                    return `<td style="padding:5px 6px;text-align:right;color:#c9cede">${sum?fmtB(Math.round(sum)):'–'}</td>`; }).join('')}
                  <td style="padding:5px 6px;text-align:right;font-weight:800;color:var(--expense)">${fmtB(Math.round(cycles.reduce((a,c)=>a+c.exp,0)))}</td>
                  <td style="padding:5px 6px;text-align:right;color:#a8aec8">·</td>
                  ${(()=>{ const tot=cycles.reduce((a,c)=>a+((c.inc||0)-(c.exp||0)),0);
                    return `<td style="padding:5px 6px;text-align:right;font-weight:800;color:${tot>=0?'var(--income)':'var(--expense)'}">${tot>=0?'+':''}${fmtB(Math.round(tot))}</td>`; })()}
                </tr>
              </tbody>
            </table></div>
      ...
      <div class="card-body" style="padding:0">
        ...
        ${series.map((s,i)=>`
          <div style="display:grid;grid-template-columns:46px repeat(6,minmax(58px,1fr));padding:8px 10px;border-bottom:1px solid var(--border);font-size:.73rem;${i===series.length-1?'font-weight:600':''}">
            ...
            <span style="text-align:right;color:${s.savings>=0?'var(--income)':'var(--expense)'}">${s.savings>=0?'+':''}${fmtB(s.savings)}</span>
            ...
            <span style="text-align:right;color:${s.debt>0?'var(--debt)':'var(--text3)'}">${s.debt>0?fmtB(Math.round(s.debt)):'·'}</span>
          </div>
        `).join('')}
        ${(()=>{ // v8.74: sumář období – součty + průměry
          ...
          return `<div style="display:grid;grid-template-columns:46px repeat(6,minmax(58px,1fr));padding:9px 10px;border-top:2px solid var(--border2);font-size:.72rem;font-weight:700;background:var(--surface2)">
            ...
            <span style="text-align:right;color:${sSav>=0?'var(--income)':'var(--expense)'}">${sSav>=0?'+':''}${fmtB(sSav)}</span>
          ...
          <div style="display:grid;grid-template-columns:46px repeat(6,minmax(58px,1fr));padding:6px 10px;font-size:.66rem;color:#a8aec8">
            ...
            <span style="text-align:right">${sSav>=0?'+':''}${fmtB(Math.round(sSav/n))}</span>
      ...
      <div class="card-body">
        <div style="display:flex;gap:14px;flex-wrap:wrap;align-items:end;margin-bottom:12px">
          ...
          <div style="flex:1 1 100%;min-width:0;padding:9px 12px;background:var(--surface2);border-left:3px solid ${sumCol};border-radius:0 10px 10px 0">
            ...
            <div style="font-family:Syne,sans-serif;font-size:clamp(1.1rem,4.5vw,1.35rem);font-weight:800;color:${sumCol};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${lostYear>0?'−':''}${fmtB(lostYear)}</div>
            <div style="font-size:.7rem;color:#a8aec8">≈ ${fmtB(lostMonth)}/měsíc</div>
          </div>
        </div>
        <div style="overflow-x:auto"><table class="stat-table" style="width:100%;min-width:460px;font-size:.76rem">
          ...
          ${rows.map(r=>`<tr>
            ...
            <td style="text-align:right;color:#c9cede">${fmtB(r.bal)}</td>
            ...
            <td style="text-align:right;font-weight:700;color:${r.lost>0?'var(--expense)':'var(--income)'}">${r.lost>0?'−'+fmtB(r.lost):'0'}</td>
          </tr>`).join('')}
          </tbody>
        </table></div>
...
function renderDetektor() {
  ...
  // FIX-254 (S19, fáze C): JEDEN vyčištěný zdroj pro celý Detektor.
  //   Dřív se filtrovalo jen na type==='expense', takže do doporúčení úspor padaly:
  //   • ROZDĚLENÉ transakce – rodič i jeho děti → nákup započítán DVAKRÁT
  //   • VYROVNÁVACÍ transakce – umělá korekce zůstatku vydávaná za útratu
  //   • PŘESUNY – vlastní peníze poslané na spořicí účet vydávané za výdaj
  //   Detektor z těchto čísel počítá „ušetříš X Kč/měs", takže chyba nešla jen
  //   do zobrazení, ale přímo do doporučení, která uživatel má následovat.
  //   Vyloučení splitParent je zde PLOŠNÉ (ne jen rodič s dětmi jako v getActual):
  //   Detektor hledá konkrétní útraty podle názvu a rodič nese jméno celého nákupu,
  //   zatímco děti nesou jeho rozpad – započítat obojí by nález vždy zdvojilo.
  const detTxs = txs.filter(t=>t&&!t.isBalancing&&!t.splitParent&&!isTransferTx(t));

  // ── FIX-270 (S19, Milan): DETEKTORY SE NAVZÁJEM NEVYLUČOVALY ──
  //  Devětkrát se procházel tentýž seznam a nikde nebyla evidence, co už bylo
  //  započítané. Jedna útrata padla do tří nálezů zároveň:
  //     McDonald 180 Kč × 5 = 900 Kč/měs
  //       Jídlo venku       ×0,30 → 270
  //       Zbytečné utrácení ×0,50 → 450
  //       Častý nákup       ×0,30 → 270
  //       ────────────────────────────
  //       Detektor sečetl        990 Kč   ← 110 % z útraty, kterou vůbec máš
  //  Nyní si každý nález transakce „zabere" a další už ji nevidí. Pořadí je
  //  dané pořadím detektorů v kódu – od nejjistějších (poplatky, předplatné)
  //  po nejspekulativnější (zbytečné utrácení).
  const _claimed = new Set();
  const _free = arr => (arr||[]).filter(t => t && !_claimed.has(t.id));
  const _claim = arr => { (arr||[]).forEach(t => { if(t && t.id) _claimed.add(t.id); }); return arr; };

  const subTxs = detTxs.filter(t=>t.type==='expense');
  ...
  // 2. Bankovní poplatky
  const bankTxs = _claim(_free(subTxs).filter(t=>{
  ...
  // Zdroj: názvy transakcí + položky z účtenek (COICOP oddíl 2).
  {
    ...
    const alcTx = _claim(_free(subTxs).filter(t => alcKw.some(kw => (t.name||'').toLowerCase().includes(kw))));
  ...
  // 3. Pojištění
  // S19 (Milan): rozpad souhrnu – kdy, jaký kurz dala banka, jaký měla ČNB a kolik
//  to stálo navíc. Souhrnné číslo je k ničemu, když si ho uživatel nemůže ověřit.
function fxLossDetailHTML(fx, txs, D){
  try{
    const rows=(txs||[]).map(t=>({t, r:(typeof fxLossOf==='function')?fxLossOf(t):null}))
      .filter(x=>x.r).sort((a,b)=>b.r.loss-a.r.loss).slice(0,12);
    if(!rows.length) return '';
    const n1=v=>v.toFixed(2).replace('.',',');
    const den=d=>{const x=new Date(d); return isNaN(x)?'–':`${x.getDate()}. ${x.getMonth()+1}.`;};
    const tr=rows.map(({t,r})=>`<tr style="border-bottom:1px solid var(--border)">
        <td style="padding:5px 6px 5px 0;white-space:nowrap;color:#c9cede">${den(t.date)}</td>
        <td style="padding:5px 6px;color:#e8eaf2">${escHtml(t.name||'–')}</td>
        <td style="padding:5px 6px;text-align:right;white-space:nowrap;color:#c9cede">${Math.round(Math.abs(t.amount||0))} ${r.cur}</td>
        <td style="padding:5px 6px;text-align:right;white-space:nowrap;color:#c9cede">${n1(r.bankRate)}</td>
        <td style="padding:5px 6px;text-align:right;white-space:nowrap;color:#a8aec8">${n1(r.refRate)}</td>
        <td style="padding:5px 0 5px 6px;text-align:right;white-space:nowrap;font-weight:700;color:${r.loss>0?'var(--debt)':'var(--income)'}">${r.loss>0?'+':'−'}${fmtB(Math.abs(r.loss))}</td>
      </tr>`).join('');
    const th=t=>`<th style="text-align:${t[1]};padding:0 6px 5px;font-size:.66rem;color:#a8aec8;font-weight:600;text-transform:uppercase;letter-spacing:.04em">${t[0]}</th>`;
    return `<details style="margin-top:8px">
      <summary style="cursor:pointer;font-size:.74rem;color:#60a5fa;padding:4px 0">Rozpad po transakcích (${rows.length})</summary>
      <div style="overflow-x:auto;margin-top:6px">
      <table style="width:100%;border-collapse:collapse;font-size:.74rem">
        <thead><tr style="border-bottom:1px solid var(--border2)">
          ${[['Datum','left'],['Platba','left'],['Částka','right'],['Kurz banky','right'],['ČNB','right'],['Rozdíl','right']].map(th).join('')}
        </tr></thead><tbody>${tr}</tbody>
      </table></div>
      <div style="font-size:.7rem;color:#a8aec8;line-height:1.5;margin-top:6px">
        Kurz banky = skutečně stržená částka ÷ zaplacená částka v cizí měně.
        ČNB = kurz z kurzovního lístku v době zápisu. Rozdíl je marže a poplatek za směnu.
      </div>
    </details>`;
  }catch(e){ return ''; }
}

// ── TODO-215 fáze 3: kurzové ztráty ──
  //  Jediný nález v Detektoru, který nevychází z odhadu procent, ale ze SKUTEČNÉHO
  //  rozdílu dvou uložených čísel. Proto se ukazuje celá zjištěná ztráta, ne odhad úspory.
  //  Okno je 12 měsíců – zahraniční platby jsou sezónní (dovolená), měsíční vzorek
  //  by u většiny lidí byl prázdný.
  if(typeof fxLossSummary==='function'){
    try{
      const yearAgo=new Date(); yearAgo.setFullYear(yearAgo.getFullYear()-1);
      const fxTxs=(D.transactions||[]).filter(t=>t&&t.date&&new Date(t.date)>=yearAgo);
      const fx=fxLossSummary(fxTxs,D);
      if(fx.n>=3 && fx.loss>200){
        // S19 (Milan): popisek byl přeplácaný. Hlavní číslo drží karta, detaily
        //   patří do rozpadu níž – jedna věta místo odstavce.
        const pct1=x=>`${x.pct>=0?'+':''}${x.pct.toFixed(1).replace('.',',')} %`;
        const kandidati=fx.pays.filter(x=>x.n>=2);
        const best=kandidati.slice().sort((a,b)=>a.pct-b.pct)[0];
        const worst=kandidati.slice().sort((a,b)=>b.pct-a.pct)[0];
        let tip=`Z ${fmtB(Math.round(fx.spentCZK))} zaplacených v cizí měně (${fx.n} plateb).`;
        if(best&&worst&&worst.key!==best.key&&(worst.pct-best.pct)>1.5){
          tip+=` Nejdráž vychází „${worst.label}" (${pct1(worst)}), nejlevněji „${best.label}" (${pct1(best)}).`;
        }
        if(fx.missing) tip+=` ${fx.missing} starších plateb nemá uložený referenční kurz.`;
        suggestions.push({
          category:'💱 Kurzy a poplatky',
          item:'Platby v cizí měně',
          current:`${pct1(fx)} nad kurz ČNB`,
          saving: Math.round(fx.loss*0.5),
          tip, severity: fx.pct>5?'high':'mid',
          detail: fxLossDetailHTML(fx, fxTxs, D)
        });
        totalSavable += Math.round(fx.loss*0.5);
      }
    }catch(e){ /* nález navíc, nesmí shodit celý Detektor */ }
  }

  const pojKeywords = ['pojištění','pojistné','allianz','kooperativa','generali','čsob pojišt','česká pojišt','uniqa','direct pojišt'];
  const pojTxs = _claim(_free(subTxs).filter(t=>pojKeywords.some(kw=>(t.name||'').toLowerCase().includes(kw))));
  ...
  const telTxs = _claim(_free(subTxs).filter(t=>telKeywords.some(kw=>(t.name||'').toLowerCase().includes(kw))));
  ...
  expCats.forEach(cat=>{
    const spent = subTxs.filter(t=>(t.catId||t.category)===cat.id).reduce((a,t)=>a+txCZK(t,D),0);  // FIX-252/A + FIX-254
  ...
  const smallExpMap = {};
  _free(subTxs).forEach(t => {
    // FIX-267: práh 300 Kč se testoval proti NOMINÁLU – nákup za 20 € (506 Kč)
    //   se tvářil jako „malá platba" a padal do Zbytečného utrácení.
    const amt = txCZK(t, D);
    if(amt <= 0 || amt > 300) return; // max 300 Kč = "malá platba"
    const key = (t.name||'').toLowerCase().trim().slice(0,25);
    if(!key || key.length < 3) return;
    if(!smallExpMap[key]) smallExpMap[key] = {name:t.name, count:0, total:0, txs:[]};
    smallExpMap[key].count++;
    smallExpMap[key].total += amt;
    smallExpMap[key].txs.push(t);
  });
    // FIX-270: zaber jen transakce, které do nálezu opravdu vstoupily
    Object.values(smallExpMap).filter(v=>v.count>=4).forEach(v=>_claim(v.txs||[]));
  ...
  const incTxsAll = detTxs.filter(t=>t.type==='income');   // FIX-254: bez přesunů a vyrovnání
  // FIX-252/A: porovnání přes txCZK – jinak by výplata 1 200 EUR prohrála s bonusem 3 000 Kč
  ...
  if(paydayTx?.date) {
    ...
    const expAll = subTxs.filter(t=>txCZK(t,D)>0);  // FIX-252/A
  ...
  const foodOutTxs = _claim(_free(subTxs).filter(t => {
    const n=(t.name||'').toLowerCase();
  ...
  if(priceAlerts.length > 0) {
    priceAlerts.sort((a,b)=>b.changePct-a.changePct).slice(0,3).forEach(pa=>{
      suggestions.push({
        ...
        item: pa.name,
        current: `${fmtB(pa.oldPrice)} → ${fmtB(pa.newPrice)} (+${pa.changePct}%)`,
  ...
  el.innerHTML=tabIntro('detektor','💡','Detektor úspor',
    ...
    <div style="background:linear-gradient(135deg,rgba(74,222,128,.08),rgba(96,165,250,.05));border:1px solid rgba(74,222,128,.2);border-radius:var(--radius);padding:16px;margin-bottom:14px;text-align:center">
      ${(() => {
        // ── FIX-271 (S19, Milan): ROZSAH MÍSTO JEDNOHO ČÍSLA ──
        //  „1 876 Kč/měs · Ročně 22 512 Kč" vypadalo jako výsledek výpočtu.
        //  Byl to součet dvanácti ODHADŮ, které navíc nemají stejnou povahu:
        //    • bankovní poplatky ×0,80 – přechod k bance bez poplatků je jistá úspora
        //    • refinancování a kurzy   – počítá se ze SKUTEČNÝCH čísel, ne z koeficientu
        //    • pojištění ×0,20         – srovnávač možná nic lepšího nenajde
        //    • zbytečné utrácení ×0,50 – appka NEVÍ, jestli ti ta káva za to stála
        //  Sečíst je znamená tvrdit, že jsou stejně spolehlivé. Nejsou.
        const JISTE = ['Bankovní','Refinancování','Kurzy'];
        const jisteSug = suggestions.filter(x => JISTE.some(k => (x.category||'').includes(k)));
        const odhadSug = suggestions.filter(x => !JISTE.some(k => (x.category||'').includes(k)));
        const jisteKc  = jisteSug.reduce((a,x)=>a+(x.saving||0),0);
        const odhadKc  = odhadSug.reduce((a,x)=>a+(x.saving||0),0);
        const dolni = jisteKc;                      // co lze doložit
        const horni = jisteKc + odhadKc;            // kdyby vyšlo všechno
        return `
        ...
        <div style="font-family:Syne,sans-serif;font-size:2rem;font-weight:800;color:var(--income)">${
          dolni > 0 && horni > dolni ? `${fmtB(dolni)} – ${fmtB(horni)}` : `až ${fmtB(horni)}`
        }<span style="font-size:1rem;font-weight:600">/měs</span></div>
        <div style="font-size:.8rem;color:var(--text2);margin-top:4px">${suggestions.length} doporučení</div>
        <div style="font-size:.74rem;color:var(--text2);margin-top:8px;line-height:1.7;text-align:left;max-width:520px;margin-left:auto;margin-right:auto">
          ${jisteKc > 0 ? `<div>✅ <strong style="color:var(--income)">${fmtB(jisteKc)}</strong> doložitelných – poplatky, úroky a kurzy se počítají ze skutečných čísel.</div>` : ''}
          ${odhadKc > 0 ? `<div>🔎 <strong style="color:var(--debt)">${fmtB(odhadKc)}</strong> odhadem – kolik z toho ušetříš, záleží na tom, čeho se vzdáš.</div>` : ''}
        </div>
        <div style="font-size:.72rem;color:var(--text3);margin-top:8px;line-height:1.6;max-width:520px;margin-left:auto;margin-right:auto">
          Každá útrata se počítá jen v jednom nálezu, takže se čísla nesčítají dvakrát.
          Roční dopad ${dolni>0?`${fmtB(dolni*12)} – ${fmtB(horni*12)}`:`až ${fmtB(horni*12)}`}.
        </div>`;
      })()}
    </div>
...
function runSimulace() {
  ...
  const bestLabel = best === scenC ? 'C – Splacení dluhu'
                  : best === scenB ? 'B – Investování'
                  : 'A – Stejné tempo';
  const bestRenta = best === scenC ? monthlyC : best === scenB ? monthlyB : monthlyA;
  const winA = best === scenA, winB = best === scenB, winC = best === scenC;
  ...
  rEl.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:16px">
      ...
      ${karta('💳','Scénář C<br>Splatím dluh dřív', scenC,
              `dluh splacen ${simDoba(sim.C.freeMonth)}`, monthlyC, winC, winC?'var(--income)':fc(scenC))}
    </div>
      ...
      <div class="card-body">
        ...
        <div class="insight-item good"><div class="insight-icon">✅</div><div class="insight-text">
           Dnes zbývá na investice <strong>${fmtB(Math.round(monthlyInvest))}/měs</strong>${debt > 0
             ? `. Po splacení dluhu se uvolní splátka ${fmtB(debtPayment)} a scénáře B i C počítají s <strong>${fmtB(Math.round(sim.investPoSplaceni))}/měs</strong> — peníze, které dnes odcházejí na dluh, zůstanou vyčleněné.`
             : '.'}</div></div>

        ${debt > 0 ? `<div class="insight-item ${debtRate > investReturn ? 'warn' : 'good'}"><div class="insight-icon">💳</div><div class="insight-text">
        ...
        ${debt === 0 && scenB !== scenC ? `<div class="insight-item good"><div class="insight-icon">🎉</div><div class="insight-text">
           Bez dluhu jsou scénáře B a C totéž — není co splácet dřív.</div></div>` : ''}
...
function drawSimulaceChart(age, retireAge, sim) {
  ...
  setTimeout(()=>{
    ...
    const vzorek = (nw) => {
      const out=[];
      for(let i=0;i<=pts;i++) out.push(Math.round(nw[Math.min(nw.length-1, Math.round(i/pts*(nw.length-1)))]));
      return out;
    };
    const serA=vzorek(sim.A.nw), serB=vzorek(sim.B.nw), serC=vzorek(sim.C.nw);
    ...
    let lx=pad.l;
    [{c:COLS.A,l:'A: Neinvestuji'},{c:COLS.B,l:'B: Investuji'},{c:COLS.C,l:'C: Splatím dluh dřív'}].forEach(it=>{
    ...
    canvas.onmousemove = canvas.ontouchstart = function(ev){
      ...
      tt.innerHTML=`<b>${age+Math.round(i*years/pts)} let</b>`
        ...
        +`<br><span style="color:${COLS.C}">●</span> Splatím dluh dřív: ${fmtB(serC[i])}`;
        ...
        ${(()=>{ // S16.8 (Deník v2.1): řádek cyklu od výplaty přímo na listu
          ...
          try{
            ...
            return `<div style="border-top:1px solid rgba(90,60,30,.35);margin-top:12px;padding-top:10px;font-size:.8rem;color:#5b4636;line-height:1.6">
              ...
              týdny: ${weeks2.map(w=>fmtB(w)).join(' / ')} ·
...
// ══════════════════════════════════════════════════════
//  OSA ŽIVOTA (TODO-207, varianta B – Milanova volba, v9.89)
//  Vodorovná osa nahoře: události jako body, etapy jako pruhy. Pod nimi finanční
//  křivky, aby bylo vidět, JAK se život promítl do peněz.
//
//  ⚠️ Křivky jsou tři, ne čtyři. „Čisté jmění" v návrhu vypadalo hezky, ale
//  historicky ho spočítat nelze – aplikace nezná stav aktiv a dluhů zpětně po
//  měsících, jen dnešní. Místo vymyšlené čáry je tu KUMULOVANÝ TOK
//  (nasčítané příjmy − výdaje), který se z transakcí odvodit dá a je poctivý.
//
...
function _osaZivotaData(D){
  ...
  const keys = txs.map(t=>String(t.date).slice(0,7)).sort();
  const first = keys[0];
  const endY = S.curYear, endM = S.curMonth;
  let y = parseInt(first.slice(0,4)), m = parseInt(first.slice(5,7))-1;
  const months = [];
  let guard = 0;
  while(guard++ < 1200){                              // strop 100 let – jen pojistka proti nekonečné smyčce
    months.push({ y, m, key:`${y}-${String(m+1).padStart(2,'0')}`, inc:0, exp:0 });
    if(y===endY && m===endM) break;
    if(++m>11){ m=0; y++; }
  }
  if(months.length < 3) return null;                  // pod čtvrt rokem osa nedává smysl

  const idxM = {}; months.forEach((x,i)=>{ idxM[x.key]=i; });
  txs.forEach(t=>{
    const i = idxM[String(t.date).slice(0,7)];
    if(i===undefined) return;
    const a = (typeof txCZK==='function') ? txCZK(t, D) : (t.amount||0);
    if(t.type==='income') months[i].inc += a;
  ...
  //  Proto se dlouhá historie slučuje do čtvrtletí a nakonec do let. Součty
        ...
        return eras.map(e=>`
          <div style="margin-bottom:7px">
            <div style="display:flex;justify-content:space-between;gap:8px;font-size:.76rem">
              ...
              <b>${typeof fmt==='function'?fmtB(Math.round(e.avg)):Math.round(e.avg)}/měs</b>
            </div>
