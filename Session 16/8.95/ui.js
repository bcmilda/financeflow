// FinanceFlow · v8.95 · ui.js · 2026-07-15
//  RENDER ROUTER
// ══════════════════════════════════════════════════════
...
function renderSouhrnInto(targetId){
  const el=document.getElementById(targetId); if(!el) return;
  const prevTarget=window._suhrnTarget; window._suhrnTarget=targetId;
  try{ renderSouhrn(true); } finally { window._suhrnTarget=prevTarget; }
}
...
function renderSouhrn(tableOnly){
  ...
  let totalPred=0;
  let html=`<table class="stat-table" style="width:100%"><thead><tr><th>Kategorie</th><th>Tento měsíc</th><th>Minulý měsíc</th><th>🔮 Predikce</th><th>Změna</th><th>Podíl</th></tr></thead><tbody>`;
  expCats.forEach(cat=>{
    ...
    html+=`<tr><td><span style="margin-right:5px">${cat.icon}</span>${cat.name}</td><td>${cur?fmt(cur):'–'}</td><td style="color:#a8aec8">${prev?fmt(prev):'–'}</td>${predCell(pc,cur)}<td>${pct!==null?`<span class="pct-pill ${pct>5?'pct-up':pct<-5?'pct-dn':'pct-neu'}">${pct>0?'+':''}${pct}%</span>`:'–'}</td><td>${pctCelku}%</td></tr>`;
    (cat.subs||[]).forEach(sub=>{
      ...
      html+=`<tr style="font-size:.78rem;color:var(--text2)"><td style="padding-left:20px">↳ ${sub}</td><td>${sc?fmt(sc):'–'}</td><td style="color:#a8aec8">${sp?fmt(sp):'–'}</td>${predCell(sPc,sc)}<td>${spct!==null?`<span class="pct-pill ${spct>5?'pct-up':spct<-5?'pct-dn':'pct-neu'}">${spct>0?'+':''}${spct}%</span>`:'–'}</td><td></td></tr>`;
    });
  });
  const totPredDiff = totalPred>0 ? Math.round((totalCur-totalPred)/totalPred*100) : null;
  html+=`<tr style="font-weight:700;border-top:2px solid var(--border2)"><td>💰 CELKEM VÝDAJE</td><td style="color:var(--expense)">${fmt(totalCur)}</td><td style="color:#a8aec8">${fmt(totalPrev)}</td><td style="color:#a8aec8">${totalPred?fmt(totalPred):'–'}${totPredDiff!==null?`<br><span style="font-size:.68rem;color:${totPredDiff>10?'var(--expense)':totPredDiff<-10?'var(--income)':'#a8aec8'}">${totPredDiff>0?'+':''}${totPredDiff} %</span>`:''}</td><td>${totalPrev>0?`<span class="pct-pill ${totalCur>totalPrev?'pct-up':'pct-dn'}">${totalCur>=totalPrev?'+':''}${Math.round((totalCur-totalPrev)/totalPrev*100)}%</span>`:'–'}</td><td>100%</td></tr>`;
  html+=`</tbody></table>`;
  ...
  el.innerHTML=html;
  if(!tableOnly) renderSuhrnReport(expCats,totalCur,totalPrev,pm,py,D);  // insighty jen na vlastní stránce
}
...
function renderTx(){
  ...
  } else if (searchAllMonths && (searchFilter || tagFilter)) {
