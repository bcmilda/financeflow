// FinanceFlow · v8.96 · projects.js · 2026-07-15
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderReport() {
  ...
  const expCats = (D.categories||[]).filter(c => c.type==='expense'||c.type==='both');
  // S16.14 (Milan): agregáty pro banner (sledováno / splněno / Ø skóre)
  let _hlTracked=0, _hlMet=0, _hlScoreSum=0, _hlScored=0, _hlShown=0;
  const catRows = expCats.map(cat => {
    ...
    if(!noLimit){
      _hlTracked++;
      _hlScoreSum+=sc; _hlScored++;
      const _met=(cat.isSaving||cat.isInvest) ? (spent>=_eff) : (spent<=_eff);
      if(_met) _hlMet++;
    }
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-body">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px 22px;align-items:center;background:linear-gradient(135deg,rgba(139,124,246,.08),rgba(74,222,128,.06));border:1px solid var(--border);border-radius:10px;padding:10px 14px;margin-bottom:12px">
          <span style="font-size:.78rem;color:#c9cede">📊 Sledovaných kategorií: <strong style="color:var(--text);font-size:.9rem">${_hlTracked}</strong><span style="color:#a8aec8"> z ${_hlShown} s výdaji</span></span>
          ...
          <span style="font-size:.78rem;color:#c9cede">⭐ Skóre: <strong style="color:${healthColor(_hlScored?Math.round(_hlScoreSum/_hlScored):0)};font-size:.9rem">${_hlScored?Math.round(_hlScoreSum/_hlScored):'–'}</strong><span style="color:#a8aec8"> (průměr)</span></span>
        </div>
      ...
      <div class="card-body">
        ...
        ${(() => {
          ...
          <div style="position:relative">
            ...
            <!-- S16.11 (Milan): ČÍSELNÉ hodnoty skutečného stavu u bodů tečkované čáry (dřív jen vizuál) -->
            <div style="position:absolute;top:18px;left:0;width:100%;height:${chartH+8}px;pointer-events:none">
              ${pts.map((p,i)=>{
                const b=bars[i];
                const done = b.val!==0 ? Math.round(Math.abs(b.real)/Math.abs(b.val)*100) : 0;
                return `<div style="position:absolute;left:${p.cx}%;top:${p.cy}px;transform:translate(-50%,-140%);white-space:nowrap;text-align:center">
                  <div style="font-size:.68rem;font-weight:800;color:#e8eaf2;background:rgba(15,17,28,.88);border:1px solid rgba(232,234,242,.25);border-radius:6px;padding:1px 5px;line-height:1.35">
                    ${b.real>=0?'':'−'}${fmt(Math.abs(b.real))}
                    <span style="font-weight:600;color:#a8aec8">· ${done} %</span>
                  </div>
                </div>`;
              }).join('')}
            </div>
          ...
          <div style="display:flex;align-items:center;gap:6px;font-size:.68rem;color:#a8aec8;margin-top:8px;justify-content:center">
            ...
            <strong style="color:#e8eaf2">tečkovaná čára = skutečný stav teď</strong> (${daysElapsed}. den, čísla v rámečcích + % z odhadu) · sloupce = odhad konce měsíce
          </div>
          <div style="font-size:.66rem;color:#a8aec8;margin-top:8px;padding:8px 10px;background:var(--surface3);border-radius:8px;line-height:1.5">
  ...
  const smerujCard = proj.hasData ? `
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:14px">
        ...
        <div style="font-size:.68rem;color:#a8aec8;margin-top:8px;padding:7px 9px;background:var(--surface3);border-radius:7px">ℹ️ Orientační predikce: příjem = 12M klouzavý průměr; výdaje = engine karty Predikce (historie kategorií + sezónnost + narozeniny); cashflow = příjem − predikce výdajů; rezerva = dnešní hotovost + kumulovaný cashflow (v tooltipu). Budoucí platby = ZNÁMÉ šablony a splátky (informativní – opakované už jsou v predikci, proto se neodečítají dvakrát). Dluh = rovnoměrné umořování dle splátek. Najeď na měsíc pro všechny hodnoty.</div>
      </div>
    </div>` : '';
