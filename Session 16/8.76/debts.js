//  FinanceFlow · v8.75 · debts.js · 2026-07-07
//  ADD / EDIT TX
// ══════════════════════════════════════════════════════
function openAddTx(){
...
function renderDebtStressWidget(D) {
  ...
  // S16 (TODO-160): DTI = 12M průměr, DSTI = 3–12M adaptivní (baseIncome = DSTI příjem)
  const incDTI  = computeEffectiveIncome(D, 12);
  const baseIncome = computeEffectiveIncome(D, 12);
...
function _payoffSim(strategy, extra, capMonths){
  ...
  const focusCurve=[];                                       // v8.74: na kterou půjčku šly extra peníze
...
function renderAvalanche(){
  ...
  el.innerHTML=`
    ...
    <div style="font-size:.72rem;color:#c9cede;font-weight:700;margin-bottom:2px">📉 Zaplacené úroky v čase</div>
    <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:${W}px;height:auto;display:block;margin-bottom:12px">
      ...
      <text x="11" y="${pad.t+ch/2}" font-size="9" fill="#a8aec8" transform="rotate(-90 11 ${pad.t+ch/2})" text-anchor="middle">Úroky (${curSym()})</text>
    ...
    <div style="font-size:.72rem;color:#c9cede;font-weight:700;margin-bottom:2px">🔢 Počet aktivních půjček (Avalanche)</div>
    <svg viewBox="0 0 ${W} ${H2}" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:${W}px;height:auto;display:block;margin-bottom:12px">
