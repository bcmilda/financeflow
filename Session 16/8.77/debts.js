//  FinanceFlow · v8.77 · debts.js · 2026-07-07
//  ADD / EDIT TX
// ══════════════════════════════════════════════════════
function openAddTx(){
...
function renderDebtStressWidget(D) {
  ...
  const emergencyCash = (liq.wallets || 0);                     // Emergency Fund = hotovost / běžné účty
  const liquidAssets = (liq.reserve || 0) + (liq.mid || 0);     // Likvidita = rezerva + investice/spoření
        ...
        <div style="display:flex;align-items:center;gap:8px">
          <div style="flex:1;min-width:0">
            ...
            <div style="height:6px;background:var(--surface3);border-radius:3px;overflow:hidden">
              <div style="height:100%;width:${Math.round(f.score/f.max*100)}%;background:${(f.score/f.max)<0.34?'var(--income)':(f.score/f.max)<0.67?'var(--debt)':'var(--expense)'};border-radius:3px;transition:width .6s"></div>
            </div>
          </div>
          <div style="font-size:.7rem;font-weight:700;color:${(f.score/f.max)<0.34?'var(--income)':(f.score/f.max)<0.67?'var(--debt)':'var(--expense)'};min-width:28px;text-align:right">${f.score}/${f.max}</div>
        </div>
      `).join('')}
    </div>
