// FinanceFlow · v8.82 · projects.js · 2026-07-08
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderObraz() {
  ...
  const _tt=sl=>`${sl>0?'↑':sl<0?'↓':'↔'} ${fmtB(Math.abs(sl))}/měs`;
  // S16 (TODO-171): Úspory (saldo aktuálního měsíce) nahrazeny Wealth Momentum (Ø saldo 6 měs.)
  const metrics = [
    {label:'💰 Příjmy',   val:fmtB(last.inc),  trendTxt:_tt(slInc),  good:slInc>=0},
    {label:'💸 Výdaje',   val:fmtB(last.exp),  trendTxt:_tt(slExp),  good:slExp<=0},
    {label:'🚀 Momentum', val:`${momentum.perMonth>=0?'+':''}${fmtB(momentum.perMonth)}/měs`, trendTxt:_tt(slMom), good:slMom>=0},
    {label:'🏦 Dluhy',    val:fmtB(last.debt), trendTxt:_tt(slDebt), good:slDebt<=0},
  ];
  ...
  const smerujCard = proj.hasData ? `
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:14px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:.74rem;min-width:520px">
          <thead><tr style="color:#a8aec8;text-transform:uppercase;font-size:.62rem;letter-spacing:.04em">
            <th style="text-align:left;padding:5px 6px">Měsíc</th>
            <th style="text-align:right;padding:5px 6px">Příjem Ø</th>
            <th style="text-align:right;padding:5px 6px">Výdaje Ø</th>
            <th style="text-align:right;padding:5px 6px">Budoucí platby</th>
            <th style="text-align:right;padding:5px 6px">Cashflow</th>
            <th style="text-align:right;padding:5px 6px">Rezerva</th>
          </tr></thead>
          <tbody>
          ${proj.months.map(mo=>`<tr style="border-top:1px solid var(--border)">
            <td style="padding:6px;color:#c9cede;font-weight:600;white-space:nowrap">${mo.label} ${String(mo.y).slice(2)}</td>
            <td style="padding:6px;text-align:right;color:var(--income)">${fmt(proj.avgInc)}</td>
            <td style="padding:6px;text-align:right;color:var(--expense)">${fmt(proj.avgExp)}</td>
            <td style="padding:6px;text-align:right;color:#a78bfa">${mo.bud?fmt(mo.bud):'–'}</td>
            <td style="padding:6px;text-align:right;font-weight:700;color:${proj.saldo>=0?'var(--income)':'var(--expense)'}">${proj.saldo>=0?'+':''}${fmt(proj.saldo)}</td>
            <td style="padding:6px;text-align:right;font-weight:700;color:${mo.reserve>=0?'#4ade80':'#f87171'}">${fmt(mo.reserve)}</td>
          </tr>`).join('')}
          </tbody>
        </table></div>
        ${_obrazProjDebtChart(proj)}
        <div style="font-size:.68rem;color:var(--text3);margin-top:8px;padding:7px 9px;background:var(--surface3);border-radius:7px">ℹ️ Orientační predikce: příjem = 12M klouzavý průměr, výdaje = Ø posledních 3 měsíců, cashflow = příjem − výdaje, rezerva = dnešní hotovost + kumulovaný cashflow. Budoucí platby = ZNÁMÉ šablony a splátky (informativní – opakované platby už jsou v Ø výdajích, proto se neodečítají dvakrát). Dluh = rovnoměrné umořování dle splátek.</div>
      </div>
    </div>` : '';
