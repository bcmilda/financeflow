...
function saveCat(){
  ...
  // Váha stability – jen pro příjmové kategorie
  const sliderEl=document.getElementById('catStabilitySlider');
  const stabilityWeight = (type==='income'||type==='both') && sliderEl
    ? parseFloat(sliderEl.value)/100 : null;
  // stable = true pokud váha > 0 a je příjmová kategorie
  const stable = stabilityWeight !== null ? stabilityWeight > 0 : undefined;
  if(!name){alert('Zadej název');return;}
  const obj={name,icon,color,type,subs:[..._catSubsList],healthPct,healthAmt,isSaving,incomeChar,expenseChar,stabilityWeight,stable};
