...
|---|---|---|
| 🔴 Kritické (P1) | 2 | **AI Rate Limiting** (TODO-075) – Worker otevřený pro zneužití, **Bubble chart přepracování** (TODO-076) |
| 🟡 Střední (P2) | ~18 | Error handler, Sentry (ověřit), JSON validace, Box plot, Landing page, Playwright, AI mapování, **Měsíční report přepočet** (S7.1), **Bubliny pod lištu** (S7.1), **Gradient bez dat** (S7.1), **Aktualizace grafů po podkategoriích** (TODO-080) |
...
| 🔵 Nice-to-have (P4) | ~10 | Exporty, Push notif, Vlastní doména, Motivy, **Tooltip bubliny** (S7.1), **AI rozřazení importu** (TODO-078), **Adopce kategorií analytika** (TODO-079) |
...
### TODO-078 · AI rozřazení transakcí při importu – keyword matching + Claude API **(Session 9, 🔵 P4 nice-to-have)**
- **Popis:** Při importu z banky mají transakce jen název obchodníka, žádné kategorie. Uživatel dostane vše jako „Ostatní". Řešení ve třech fázích:
  1. **Fáze 1:** Lokální keyword tabulka (~150 obchodníků CZ) → rychlé, bezplatné, offline
  2. **Fáze 2:** Uživatelovo učení – aplikace si pamatuje `obchodník → kategorie` do Firebase
...
- **Stav:** Navrženo, neimplementováno. Odkládáme na fázi reálných uživatelů.
- **Poznámka:** Řešit až při testování s reálnými uživateli – bez jejich zpětné vazby nelze dobře kalibrovat.

### TODO-079 · Uživatelská adopce kategorií – analytika využití **(Session 9, 🔵 P4 nice-to-have)**
- **Popis:** Sledovat která kategorie/podkategorie se reálně používá a která ne. Metriky:
...
- **Stav:** Navrženo, neimplementováno. Data pro analýzu existují ve Firebase, chybí agregační vrstva.

### TODO-081 · Admin rozhraní – přiřazení COICOP čísla vlastním kategoriím uživatelů **(Session 9, 🟡 P2)**
- **Popis:** Uživatel může přidat vlastní kategorii (např. „Kryptoměny") – ta nemá `coicop` číslo a nevstupuje do COICOP statistik. Admin (Milan) uvidí v Admin panelu seznam kategorií bez `coicop` a může jim číslo přiřadit.
- **Rozsah:** `admin.js` – sekce „Kategorie bez COICOP", select 1–13 + tlačítko uložit → zapíše do `DEFAULT_CATEGORIES` (kód), ne do Firebase jednotlivých uživatelů.
- **Poznámka:** Uživatel COICOP sám nevolí – byl by zmatený. Admin přiřazuje globálně přes kód.
- **Stav:** Navrženo, neimplementováno.

### TODO-082 · COICOP agregáty – výpočet a komunitní přehled **(Session 9, 🟡 P2)**
- **Popis:** Funkce `computeCoicopAggregates(D)` která projde transakce uživatele, přiřadí každé `coicop` číslo (z kategorie nebo `coicopOverrides` podkategorie) a vrátí `{1: sum, 2: sum, ...13: sum}`. Výsledek se zobrazí v novém UI „Komunitní přehled" – sloupcový graf uživatel vs. ČSÚ průměr (`COICOP_GROUPS_DEF.avg_osoba`).
- **Duplicity:** Každá transakce se započítá právě jednou. Pokud kategorie nemá `coicop`, transakce se nezapočítá (zobrazí se jako „nezařazeno").
- **Komunitní data:** Anonymní agregát (`/community/{uid}/coicop/{číslo}`) → průměr komunity FinanceFlow.
- **Závislost:** Vyžaduje TODO-081 pro pokrytí vlastních kategorií.
- **Stav:** Navrženo, neimplementováno.


- **Popis:** Po tom co Milan dodělá podkategorie transakcí, aktualizovat všechny grafy a tabulky aby zobrazovaly podkategorii jako drill-down vrstvu:
...
- **Stav:** Navrženo, neimplementováno. Blokováno – čeká na TODO-080 prerekvizitu (podkategorie v transakcích).
...
| v6.66+ | 🔄 Plánované | AI Rate Limiting (TODO-075), Bubble chart (TODO-076), Stripe (TODO-073) |
...
*Konsolidováno: 2026-04-23 | Doplněno S6: 2026-05-15 | Session 7.0+7.1 patch: 2026-05-15 | Sessions: 1 → 8 | Poslední update: Session 8, 2026-05-24 | Autor: Milan Migdal*
