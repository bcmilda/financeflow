...
> **Zdrojový soubor (základ):** Claude consolidated merge Sessions 1–3 (`formulas_s4.md`)
> **Porovnávací soubor (Milanova verze):** `formulas_m1-3_2026-05-08.md` (Milan merge Sessions 1–3)
> **Procedura:** Porovnání Claude vs Milan merge → doplnění OECD průměru ČR, DSTI/DTI tabulky limitů a Completeness Score z Milanova merge, označených jako `(Merge Session 1-3)`. Žádné patche ze S6 ani S7 (formulas.md nebyl v combined patchi).
...
| 2 | **ČNB limity (DSTI/DTI)** | S1 (DSTI <40%, DTI <800%) vs S2/S3 (DSTI <35%, DTI <700%) | ✅ S2/S3 jsou aktuální – viz `decisions.md` tabulka historických limitů (ADR sekce). S1 hodnoty zastaralé. 🔗 `decisions.md` sekce 1 |
...
### Historické limity napříč sessions **(Merge Session 1-3)**

| Session | DSTI bezpečné | DSTI rizikové | DSTI kritické | DTI bezpečné | DTI rizikové | DTI kritické |
|---|---|---|---|---|---|---|
| **S1** | < 40 % | 40–50 % | > 50 % | < 800 % | 800–1000 % | > 1000 % |
| **S2** | < 35 % | 35–45 % | > 45 % | < 700 % | 700–900 % | > 900 % |
| **S3** | < 35 % | 35–50 % | > 50 % | < 700 % | 700–900 % | > 900 % |

**Poznámka S3:** DSTI ČNB limit je MAX 50 % (od 2023, dříve 45 %). Vysvětluje rozdíl S2 vs S3 u DSTI horní hranice.
...
Zkrácený zápis **(Merge Session 1-3):**
  equiv = 1.0 + (adults − 1) × 0.5 + ch14 × 0.5 + ch013 × 0.3
...
## Scoring v2 – 4 nezávislé složky **(Session 8, ADR-043)**

Implementace v `premium.js` → `computeFinancialScore()`. Nasazeno v6.64 (FIX-078).

```
S1 Cash Flow (0–25):
  expRatio = totalExp / totalInc
  Lookup tabulka 26 řádků: expRatio ≤ 0.50 → 25b, ..., expRatio > 1.60 → 0b

S2 Zadluženost (0–25):
  DTI_score  = DTI lookup (0–13b):  DTI ≤ 100% → 13, ..., DTI > 900% → 0
  DSTI_score = DSTI lookup (0–12b): DSTI ≤ 10% → 12, ..., DSTI > 50% → 0
  S2 = DTI_score + DSTI_score

S3 Rezerva (0–25):
  monthsReserve = savBalance / baseIncome
  monthsReserve ≥ 6  → 25b
  monthsReserve ≥ 3  → 20b
  monthsReserve ≥ 1  → 12b
  monthsReserve ≥ 0.5 → 6b
  jinak              → 2b
  (Eliminuje dvojitý postih se S1 – starý kód trestal záporné saldo v obou S1 i S3)

S4 Spoření (0–25):
  activeSavingRate = sum(výdaje v isSaving=true kategoriích) / baseIncome × 100
  activeSavingRate ≥ 20% → 25b
  activeSavingRate ≥ 10% → 18b
  activeSavingRate ≥ 5%  → 10b
  activeSavingRate ≥ 1%  → 4b
  jinak                  → 0b

Konzistenční bonus (cap 100):
  Počet měsíců nepřetržitého zlepšení skóre:
  2 měsíce → +2b
  3 měsíce → +5b
  4 měsíce → +9b
  5 měsíce → +13b
...
Celkové skóre = S1 + S2 + S3 + S4 + bonus (cap 100)
```

**FIX oproti v1:** Odstraněn dvojitý postih za záporné saldo. V1 trestalo S1 (expRatio > 1) i S3 (savingRate záporná). V2 S3 měří pouze rezervu (monthsReserve), ne míru úspor.

**🔗 Cross-reference:** `decisions.md` ADR-043, FIX-078

---

## isBalancing – KB Multiměnové transakce **(Session 8)**

```
isBalancing: boolean (default: false)

Komerční banka vytváří pro EUR platby 3 záznamy:
  1. Původní EUR výdaj  → isBalancing: false  (počítá se do expSum)
  2. Vyrovnávací příjem → isBalancing: true   (vyloučen z incSum/expSum)
  3. Vyrovnávací výdaj  → isBalancing: true   (vyloučen z incSum/expSum)

Pravidlo v helpers.js (FIX-073):
  incSum(txs) = sum(t.amount || t.amt || 0 for t where type='income' AND !t.isBalancing)
  expSum(txs) = sum(t.amount || t.amt || 0 for t where type='expense' AND !t.isBalancing)

getActual(catId, sub, m, y, D):
  Filtruje !t.isBalancing při výpočtu skutečných výdajů.
```
...
*Konsolidováno: 2026-04-16 | Doplněno z Milan merge S1-3: 2026-05-15 | Sessions: 1 → 9 | Poslední update: Session 9, 2026-05-28 | Autor: Milan Migdal*
