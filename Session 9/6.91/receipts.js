...
function buildStatsTab(hasData, receipts, totalSpent, allItems, catStats) {
  ...
  allItems.forEach(it=>{
    ...
    itemFreq[k].count++; itemFreq[k].total+=(it.price||0)*(it.qty||1);
  });
  ...
  receipts.forEach(r=>{
    (r.items||[]).forEach(it=>{
      ...
      catStatsById[cid].total += (it.price||0)*(it.qty||1);
...
function renderItemStatsList(topItems, allItems, D, period) {
  ...
  filtered.forEach(it=>{
    const k=(it.name||'').trim(); if(k.length<2)return;
    if(!freq[k])freq[k]={count:0,total:0,catId:it.itemCatId||'',prices:[]};
    freq[k].count++;
  ...
  return sorted.map(([name,v])=>{
    ...
    const minP = v.prices.length ? Math.min(...v.prices) : 0;
    const maxP = v.prices.length ? Math.max(...v.prices) : 0;
    const priceTrend = v.prices.length>=2
      ? (v.prices[v.prices.length-1] > v.prices[0]
          ? `<span style="color:var(--expense);font-size:.75rem;font-weight:700">↑</span>`
          : v.prices[v.prices.length-1] < v.prices[0]
            ? `<span style="color:var(--income);font-size:.75rem;font-weight:700">↓</span>` : '')
      : '';
    const stores = [...new Set((allItems.filter(i=>(i.name||'').trim()===name&&i.store).map(i=>i.store)))].slice(0,2);
    return `<div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--border)">
      <div style="flex:1;min-width:0">
        <div style="font-size:.88rem;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px">
          ${icon} ${name} ${priceTrend}
          <span style="font-size:.72rem;color:var(--text2);font-weight:400;background:var(--surface3);padding:1px 6px;border-radius:8px">${v.count}×</span>
        </div>
        <div style="display:flex;align-items:center;gap:8px;margin-top:3px;flex-wrap:wrap">
          ${cat?`<span style="font-size:.72rem;font-weight:600;color:${color}">${cat.name}</span>`:''}
          ${stores.length?`<span style="font-size:.68rem;color:var(--text3)">${stores.join(', ')}</span>`:''}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0;margin-left:10px">
        <div style="font-family:Syne,sans-serif;font-size:1.05rem;font-weight:800;color:var(--expense)">${fmt(Math.round(v.total))} Kč</div>
        <div style="font-size:.7rem;color:var(--text3)">ø ${fmt(avgPrice)} Kč${v.prices.length>=2?' · '+fmt(minP)+'–'+fmt(maxP)+' Kč':''}</div>
      </div>
    </div>`;
  }).join('');
}
...
function filterItemStats(period, btn) {
  ...
  const freq = {};
  allItems.forEach(it=>{const k=(it.name||'').trim();if(k.length<2)return;if(!freq[k])freq[k]={count:0,total:0,catId:it.itemCatId||''};freq[k].count++;freq[k].total+=(it.price||0)*(it.qty||1);});
...
function buildStoresTab(storeStats, totalSpent) {
  ...
  html += '<div style="background:var(--surface2);border-radius:10px;padding:10px 14px;margin-bottom:14px;font-size:.76rem;color:var(--text2);border:1px solid var(--border)">🏪 Klikněte na obchod pro detail účtenek a položek.</div>';

  // Seskup receipty dle obchodu
  const storeReceipts = {};
  receipts.forEach((r,i) => {
    const key = (r.store||'Neznámý').trim();
  ...
  Object.entries(storeStats).sort((a,b)=>b[1].total-a[1].total).forEach(([store,stats]) => {
    ...
    html += `<div class="card" style="margin-bottom:10px">
      <div style="padding:12px 14px;cursor:pointer;display:flex;align-items:center;gap:10px" onclick="toggleHistGroup('${storeId}')">
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;font-size:.92rem">${store}</div>
          <div style="font-size:.74rem;color:var(--text2);margin-top:2px">${stats.visits} návštěv · ${stats.count} položek</div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-family:Syne,sans-serif;font-size:1.2rem;font-weight:800;color:var(--expense)">${fmt(Math.round(stats.total))} Kč</div>
          ${avg !== null ? `<div style="font-size:.72rem;color:var(--text3)">ø ${fmt(avg)} Kč/nákup</div>` : ''}
        </div>
        <span id="${storeId}_arrow" style="color:var(--text3);font-size:.8rem;flex-shrink:0;transition:transform .2s">▶</span>
      </div>
      <div style="padding:0 14px 6px;display:none" id="${storeId}_bar">
        <div class="trap-bar"><div class="trap-bar-fill" style="width:${pct}%;background:var(--bank)"></div></div>
        <div style="font-size:.7rem;color:var(--text3);margin-top:3px">${pct}% z celkových výdajů</div>
      </div>
      <!-- Rozkliknuté účtenky -->
      <div id="${storeId}" style="display:none;border-top:1px solid var(--border)">
        ${rcts.map(r => {
          ...
          (r.items||[]).forEach(it=>{
            ...
            catGroups[k].total+=(it.price||0)*(it.qty||1);
          });
          const catSummary = Object.entries(catGroups).slice(0,3).map(([n,v])=>`${v.icon} ${n} ${fmt(Math.round(v.total))} Kč`).join(' · ');
          return `<div style="border-bottom:1px solid var(--border)">
            <div style="padding:10px 14px;display:flex;align-items:center;gap:8px;cursor:pointer"
              ...
              <div style="flex:1;min-width:0">
                <div style="font-size:.8rem;font-weight:600">${r.date||'–'}</div>
                <div style="font-size:.68rem;color:var(--text3);margin-top:1px">${(r.items||[]).length} položek${catSummary?' · '+catSummary.slice(0,60):''}</div>
              </div>
              <div style="font-weight:700;color:var(--expense);font-size:.88rem;flex-shrink:0">${fmtP(r.total||0)} Kč</div>
              <button class="btn btn-edit btn-icon btn-sm" onclick="event.stopPropagation();editReceiptFromHistory(${r._idx})" title="Upravit">✎</button>
              <button class="btn btn-danger btn-icon btn-sm" onclick="event.stopPropagation();deleteReceipt(${r._idx})">✕</button>
              <span id="${rcptId}_arrow" style="color:var(--text3);font-size:.7rem">▶</span>
            </div>
            <div id="${rcptId}" style="display:none;padding:6px 14px 10px 28px;background:var(--surface2)">
              ${(r.items||[]).map(it=>{
                const cat=(D.categories||[]).find(c=>c.id===it.itemCatId);
                return `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);font-size:.76rem">
                  <div style="flex:1;min-width:0">
                    <span style="color:var(--text)">${it.name||'–'}</span>
                    ${cat?`<span style="font-size:.65rem;color:${cat.color||'var(--text3)'};margin-left:5px">${cat.icon} ${cat.name}</span>`:''}
                    ${it.itemSubcat?`<span style="font-size:.63rem;color:var(--text3);margin-left:3px">· ${it.itemSubcat}</span>`:''}
                  </div>
                  <div style="text-align:right;flex-shrink:0;margin-left:8px">
                    <span style="font-weight:600">${fmtP(it.price||0)} Kč</span>
                    ${(it.qty||1)!==1?`<span style="font-size:.65rem;color:var(--text3)"> ×${it.qty} ks</span>`:''}
                  </div>
                </div>`;
              }).join('')||'<div style="color:var(--text3);font-size:.75rem;padding:4px 0">Žádné položky</div>'}
            </div>
          </div>`;
        }).join('')}
      </div>
...
function buildHistoryTab(receipts) {
  ...
  receipts.forEach((r,i) => {
    const key = (r.store||'Neznámý').trim();
    if(!storeGroups[key]) storeGroups[key] = {store:key, receipts:[], total:0, items:0};
    storeGroups[key].receipts.push({...r, _idx:i});
    storeGroups[key].total += r.total||0;
    storeGroups[key].items += (r.items||[]).length;
  });

  const sorted = Object.values(storeGroups).sort((a,b)=>b.total-a.total);

  sorted.forEach(group => {
    const avg = group.receipts.length > 1 ? Math.round(group.total/group.receipts.length) : null;
    const groupId = 'hist_'+group.store.replace(/[^a-z0-9]/gi,'_');

    html += `<div class="card" style="margin-bottom:10px">
      <div style="padding:12px 14px;cursor:pointer;display:flex;align-items:center;gap:10px"
           onclick="toggleHistGroup('${groupId}')">
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;font-size:.9rem">${group.store}</div>
          <div style="font-size:.72rem;color:var(--text2);margin-top:2px">
            ${group.receipts.length} účtenek · ${group.items} položek
          </div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-family:Syne,sans-serif;font-size:1.1rem;font-weight:800;color:var(--expense)">${fmt(Math.round(group.total))} Kč</div>
          ${avg !== null ? `<div style="font-size:.7rem;color:var(--text3)">ø ${fmt(avg)} Kč/nákup</div>` : ''}
        </div>
        <span id="${groupId}_arrow" style="color:var(--text3);font-size:.8rem;flex-shrink:0;transition:transform .2s">▶</span>
      </div>
      <!-- Účtenky tohoto obchodu -->
      <div id="${groupId}" style="display:none;border-top:1px solid var(--border)">
        ${group.receipts.map(r => {
          const rcptId = 'rcpt_'+r._idx;
          const D = getData();
          // Skupiny kategorií z položek
          const catGroups = {};
          (r.items||[]).forEach(it=>{
            const cat = (D.categories||[]).find(c=>c.id===it.itemCatId);
            const k = cat?.name||'Ostatní';
          ...
          const catSummary = Object.entries(catGroups).map(([n,v])=>`${v.icon} ${n} ${fmt(Math.round(v.total))} Kč`).join(' · ');

          return `<div style="border-bottom:1px solid var(--border)">
            <div style="padding:10px 14px;display:flex;align-items:center;gap:8px;cursor:pointer"
                 onclick="toggleHistReceipt('${rcptId}')">
              <div style="flex:1;min-width:0">
                <div style="font-size:.78rem;font-weight:600;color:var(--text)">${r.date||'–'}</div>
                <div style="font-size:.68rem;color:var(--text3);margin-top:1px">${(r.items||[]).length} položek${catSummary?(' · '+catSummary.slice(0,60)+(catSummary.length>60?'…':'')):''}</div>
              </div>
              <div style="font-weight:700;color:var(--expense);font-size:.88rem;flex-shrink:0">${fmtP(r.total||0)} Kč</div>
              <button class="btn btn-edit btn-icon btn-sm" onclick="event.stopPropagation();editReceiptFromHistory(${r._idx})" title="Upravit">✎</button>
              <button class="btn btn-danger btn-icon btn-sm" onclick="event.stopPropagation();deleteReceipt(${r._idx})">✕</button>
            ...
            <!-- Položky účtenky -->
            <div id="${rcptId}" style="display:none;padding:8px 14px 10px 28px;background:var(--surface2)">
              ${(r.items||[]).length ? r.items.map(it=>{
                ...
                return `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);font-size:.76rem">
                  ...
                  <div style="text-align:right;flex-shrink:0;margin-left:8px">
                    <span style="font-weight:600">${fmtP(it.price||0)} Kč</span>
                    ${(it.qty||1)!==1?`<span style="font-size:.65rem;color:var(--text3)"> ×${it.qty}</span>`:''}
                  </div>
                </div>`;
...
function editReceiptFromHistory(index) {
  ...
  if(!r) return;
  // FIX (S9): Nezměnit aktivní záložku – zůstat v Historii
  // Editor se zobrazí jako overlay nad historií
  _lastReceiptResult = {receipt: JSON.parse(JSON.stringify(r)), n: 1, historyIndex: index};
  // Zobraz editor v modal overlay místo přepnutí záložky
  const existing = document.getElementById('receiptEditOverlay');
  if(existing) existing.remove();
  const overlay = document.createElement('div');
  overlay.id = 'receiptEditOverlay';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:8000;overflow-y:auto;padding:16px';
  overlay.innerHTML = `
    <div style="max-width:600px;margin:0 auto;background:var(--surface);border-radius:14px;overflow:hidden">
      <div style="padding:12px 14px;background:var(--surface2);display:flex;align-items:center;justify-content:space-between">
        <span style="font-weight:700">✎ Upravit účtenku</span>
        <button onclick="document.getElementById('receiptEditOverlay').remove()" style="background:none;border:none;color:var(--text3);font-size:1.2rem;cursor:pointer">✕</button>
      </div>
      <div id="receiptEditInner" style="padding:14px"></div>
    </div>`;
  document.body.appendChild(overlay);
  const inner = document.getElementById('receiptEditInner');
  if(inner) {
    inner.innerHTML = buildReceiptPreviewHTML(_lastReceiptResult.receipt, 1);
...
function rpRender() {
  ...
  for(const [key, group] of Object.entries(groups)) {
    ...
    const icon = cat?.icon || '📦';
    const color = cat?.color || '#6b7280';
    const catTotal = group.items.reduce((a,{it})=>a+(parseFloat(it.price)||0)*(parseFloat(it.qty)||1),0);
    html += `
      ...
      html += `
        <div style="display:flex;align-items:center;gap:5px;padding:5px 2px;border-bottom:1px solid var(--border)" id="rp_item_${i}">
          <input id="rp_name_${i}"
            ...
            style="flex:1;background:var(--surface2);border:1px solid var(--border);border-radius:7px;padding:6px 8px;color:var(--text);font-size:.78rem;min-width:0"
  ...
  if(!items.length || groupKeys.length === 0) {
    ...
    groupKeys.forEach(key => {
      ...
      S.transactions.push({
        ...
        note,
        receiptItems: group.items.map(it=>({name:it.name, price:it.price, qty:it.qty})),
      });
