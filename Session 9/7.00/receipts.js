...
function buildHistoryTab(receipts) {
  ...
  sorted.forEach(r => {
    ...
    const catTags = Object.entries(catGroups).map(([n,v])=>
      `<span style="font-size:.66rem;padding:1px 6px;background:${v.color}22;border:1px solid ${v.color}55;border-radius:8px;color:var(--text2);white-space:nowrap">${v.icon} ${n} ${Math.round(v.total)} Kč</span>`
    ).join('');
...
function buildReceiptPreviewHTML(receipt, n) {
  ...
  return `<div id="receiptEditForm" onclick="event.stopPropagation()" style="background:var(--surface);border:2px solid rgba(74,222,128,.3);border-radius:14px;padding:16px;margin-top:12px;box-shadow:0 4px 24px rgba(0,0,0,.3)">
    ...
    <div style="display:flex;gap:8px;margin-top:4px">
      ...
      <button class="btn btn-ghost btn-sm" style="min-width:36px" onclick="(function(){
        const histIdx = window._lastReceiptResult?.historyIndex;
        if(histIdx !== undefined) {
          ...
          if(slot) { slot.style.display='none'; slot.innerHTML=''; return; }
...
async function saveItemTagMapping(itemName, tag) {
  ...
  const key = itemName.toLowerCase().trim()
    .replace(/\d+\s*(g|kg|ml|l|ks)\b/g,'').replace(/\s+/g,' ').trim().slice(0,30)
    .replace(/[.#$/\[\]]/g,'_');
  if(!key || key.length < 2) return;
  // Uložení do community/itemTags/{key}/{tag}
  try {
    const uid = window._currentUser?.uid; if(!uid) return;
    const idToken = await window._currentUser.getIdToken?.();
    await fetch(
      `https://financeflow-a249c-default-rtdb.europe-west1.firebasedatabase.app/community/itemTags/${key}/${tag.replace(/[.#$/\[\]]/g,'_')}.json?auth=${idToken}`,
      {method:'TRANSACTION', body: JSON.stringify(0)} // nevadí pokud selže
    ).catch(()=>{});
    // Jednoduchý increment přes PUT s načtením
    ...
    await fetch(
      `https://financeflow-a249c-default-rtdb.europe-west1.firebasedatabase.app/community/itemTags/${key}/${tag.replace(/[.#$/\[\]]/g,'_')}.json?auth=${idToken}`,
      {method:'PUT', body:JSON.stringify(current+1)}
    );
