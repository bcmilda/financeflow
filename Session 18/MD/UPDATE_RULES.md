...
## 7. Poučení ze Session 18 — proč se aktualizovalo jen 5–6 z 15 living docs

**Zjištění (Milan, 2026-08-03):** Přestože pravidla v tomto souboru platí pro **všechny** dokumenty ve složce `doc/`, v praxi se mezi Session 9 a 18 průběžně aktualizovaly jen `todo.md`, `bugs.md`, `decisions.md`, `features.md` — zbytek (`GLOSSARY.md`, `SECURITY.md`, `architecture.md`, `context.md`, `explanations.md`, `VERSIONING.md`, `formulas.md`, `Resume.md`) zůstával týdny až měsíce pozadu, aniž by si toho někdo všiml v průběhu.

**Příčina:** Na konci session obvykle vzniká `patch-sessionN.md` a aktualizují se dokumenty přímo relevantní k danému úkolu. Dokumenty, které session nezmínila explicitně, se jednoduše přeskočí — a nikdo je nekontroluje zpětně.

### Nové pravidlo: checklist všech living docs na konci session

Před uzavřením session (nebo na explicitní žádost o aktualizaci dokumentace) projít **všech 15** dokumentů, ne jen ty zjevně dotčené:

```
todo.md · bugs.md · decisions.md · features.md · architecture.md ·
context.md · explanations.md · GLOSSARY.md · SECURITY.md · VERSIONING.md ·
formulas.md · Resume.md · CLAUDE.md · CLAUDE_SKILLS.md · UPDATE_RULES.md
```

Pro každý: i když session daný dokument přímo nezmínila, **krátce zvážit**, jestli něco z session patří i tam (nová funkce → GLOSSARY termín, nový Firebase uzel → SECURITY, nový modul → architecture, nové poučení → CLAUDE_SKILLS). Pokud opravdu nic nepatří, dokument se nechává beze změny — ale vědomě, ne proto, že se na něj zapomnělo.

**Rychlá kontrola zastaralosti** (spustit na začátku, ne na konci):
```bash
for f in *.md; do echo "$f: $(grep -oE 'Session [0-9]+' $f | sort -V | tail -1)"; done
```
Ukáže, který dokument je kolik sessions pozadu za aktuální.
