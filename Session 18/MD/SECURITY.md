...
### Sdílení partnerů (ADR-051) – read-only model
- Firebase rules: partner uvedený v `users/$uid/partners` má **read** přístup k `users/$uid/data`. Zápis jen vlastník.
- Žádný společný „owner" – každý zapisuje pod svůj UID. Rodinný souhrn jen čte `partnerData`.
- ⚠️ Ověřit nasazení pravidel v konzoli (database_rules.json) před spoléháním na ně.

### Stripe / Platby (ADR-053) – až bude aktivováno
- **Payment Links** = hosted Stripe stránky → v aplikaci NENÍ žádný tajný klíč.
- `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` a klíč pro zápis do Firebase **POUZE ve Cloudflare Worker Secrets**, NIKDY v klientu ani v repu.
- Webhook MUSÍ ověřovat `Stripe-Signature` (jinak kdokoli podvrhne „zaplaceno").
- `client_reference_id` = Firebase UID – ověřit, že odpovídá přihlášenému uživateli.
- Blokováno: Milan nemá IČO/OSVČ. Pro donate zvážit Ko-fi/QR.

### Leads node (připomínka)
- `leads` uzel má širší přístup – pročistit Firebase Security Rules (TODO).
...
## Session 11 – bezpečnostní poznámky

### Partner/Affiliate dedup (S11)
- Partner bonus (50 bodů) je chráněn dedup záznamem v `partner_bonus/{ownerUid}_{myUid}`.
- Uživatel nemůže přidat sám sebe jako partnera (check `ownerUid !== myUid`).
- Starý `?partnerOf=UID` mechanismus zachován ale nepropagován v UI.

### GA4 a soukromí (S11)
- GA4 nakonfigurován s `anonymize_ip: true` (IP adresy se neukládají úplné).
- App neposílá finanční data do GA4 – jen page views a navigaci.
- GDPR Consent Mode zatím neimplementován (TODO-121). Pro přísný soulad nutné přidat cookie banner.

### Jednoznačné referenční linky (S11)
- Affiliate link `?ref=CODE` resolves přes `referrals/{code}/uid` (Firebase RTDB).
- Kódy jsou generovány jako `UID[:4] + 4 random chars` (ADR-058).
- Kliknutí jsou logována do `referral_clicks/` s timestampem a refCode.
...
### Nový top-level uzel `reviews/{uid}`
- `.write`: pouze `auth.uid === $uid` (nemůže hodnotit za jiného uživatele ani po sobě smazat cizí hodnocení)
- `.read`: `auth != null` — čtou přihlášení, protože souhrn (průměr, počet) se dnes počítá na klientovi
- `.validate`: `stars` číslo 1–5, `text` do 600 znaků, `name`/`ver` do limitů, neznámé klíče odmítnuty
- **Texty vidí jen admin panel** (aplikační logika, ne Firebase pravidlo) — veřejně se zobrazuje pouze agregát
- Uzel je **mimo `users/{uid}`**, takže se na něj nevztahuje kaskáda `.write` zdokumentovaná výše (S17) — musela dostat vlastní pravidla, stejně jako `banned/`, `stripeCustomers/` apod.

### Uzel `users/{uid}/data/milestones`
Uvnitř `users/{uid}`, kaskáda `.write` ho kryje automaticky. Přidána jen `.validate` na délku `label`/`note`/`icon` (ochrana proti zneužití, ne podmínka funkčnosti).
