// FinanceFlow · v9.47 · charts.js · 2026-08-03
//  GRAFY (simplified)
// ══════════════════════════════════════════════════════
...
function renderRocniGraf() {
  ...
  if (heatEl) {
    ...
    allTxs.forEach(t => {
      const d = new Date(t.date);
      if (d.getFullYear() !== y || _txKind(t) === 'income') return;
    ...
    if (!rows.length) {
      heatEl.innerHTML = `<div style="font-size:.78rem;color:#a8aec8;padding:10px 2px">
        Pro rok ${y} a zvolený filtr nejsou žádné výdaje. Zkontrolováno: ${allTxs.length} transakcí ve filtru.</div>`;
    } else {
      ...
      heatEl.innerHTML = `
        <div style="font-size:.78rem;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">
          Kategorie × měsíce · ${y} <span style="text-transform:none;letter-spacing:0;font-weight:400">· ${rows.length} kategorií · sytější = vyšší výdaj</span></div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:.76rem;white-space:nowrap">
          <thead><tr>
            <th style="position:sticky;left:0;background:var(--surface2);text-align:left;padding:6px 8px;font-size:.66rem;color:#a8aec8;letter-spacing:.04em;z-index:2">KATEGORIE</th>
            ${CZ_M.map(mn => `<th style="padding:6px 4px;font-size:.64rem;color:#a8aec8">${mn.slice(0,3).toUpperCase()}</th>`).join('')}
          ...
          <tbody>
            ${rows.map(r => `<tr>
              <td style="position:sticky;left:0;background:var(--surface);text-align:left;padding:5px 8px;font-weight:600;z-index:1;max-width:150px;overflow:hidden;text-overflow:ellipsis">${catName(r.k)}</td>
              ${r.arr.map(v => `<td style="text-align:right;padding:5px 4px;background:${_heatBg(v, r.max)};color:${_heatFg(v, r.max)}">${v > 0 ? fmtB(Math.round(v)) : '<span style="color:#7e84a0">–</span>'}</td>`).join('')}
            ...
            <tr style="border-top:2px solid var(--border);background:var(--surface2)">
              ...
              ${colTot.map(v => `<td style="text-align:right;padding:6px 4px;font-weight:700;background:${_heatBg(v, colMax)};color:${_heatFg(v, colMax)}">${v > 0 ? fmtB(Math.round(v)) : '–'}</td>`).join('')}
...
function renderVsechnyRoky() {
  ...
  const cell = (v)=>{
    ...
    const int = Math.min(v/matMax,1);
    return `<td style="text-align:right;background:rgba(139,124,246,${(0.06+int*0.34).toFixed(3)});color:#e8eaf2;font-weight:${int>0.5?'700':'500'}">${fmt(Math.round(czkToBase(v)))}</td>`;
  };
  let matHtml = mat.length ? `
    <div class="card" style="margin-bottom:16px">
      <div class="card-title">🗂️ Kategorie × roky <span style="font-size:.72rem;font-weight:400;color:#a8aec8">· ${mat.length} kategorií · sytější = vyšší výdaj</span></div>
      ...
      <table class="pred-tbl" style="min-width:${180+years.length*74}px">
        ...
        <tbody>
          ...
          <tr style="border-top:2px solid var(--border);background:rgba(255,255,255,.03)">
            ...
            ${colSums.map(v=>`<td style="text-align:right;font-weight:800;color:#fbbf24">${v?fmt(Math.round(czkToBase(v))):'–'}</td>`).join('')}
  ...
  const _catsWithData = [...new Set(allTxs.filter(t => _txKind(t) !== 'income').map(t => t.catId == null ? '_none' : String(t.catId)))];
  ...
  if (!_optIds.length) {
    ...
    html += `<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;margin-bottom:6px">
      <table style="width:100%;border-collapse:collapse;font-size:.76rem;white-space:nowrap">
        <thead><tr>
          <th style="position:sticky;left:0;background:var(--surface2);padding:6px 8px;font-size:.64rem;color:#a8aec8;z-index:2">MĚS</th>
          ${years.map(y => `<th style="padding:6px 6px;font-size:.64rem;color:#a8aec8">${y}</th>`).join('')}
        </tr></thead><tbody>
        ${mx.map((row, m) => `<tr>
          <td style="position:sticky;left:0;background:var(--surface);padding:5px 8px;font-weight:700;z-index:1">${m + 1}</td>
          ${row.map(v => `<td style="text-align:right;padding:5px 6px;background:${_heatBg(v, mxMax)};color:${_heatFg(v, mxMax)}">${v > 0 ? fmtB(Math.round(v)) : '<span style="color:#7e84a0">0</span>'}</td>`).join('')}
        </tr>`).join('')}
        <tr style="border-top:2px solid var(--border);background:var(--surface2)">
          <td style="position:sticky;left:0;background:var(--surface2);padding:6px 8px;font-weight:800;z-index:1">Ø</td>
          ${colAvgNZ.map(v => `<td style="text-align:right;padding:6px;font-weight:700;color:var(--debt)">${v > 0 ? fmtB(Math.round(v)) : '–'}</td>`).join('')}
        </tr>
        <tr style="background:var(--surface2)">
          <td style="position:sticky;left:0;background:var(--surface2);padding:6px 8px;font-weight:800;z-index:1">Celk</td>
          ${colSum.map(v => `<td style="text-align:right;padding:6px;font-weight:800;color:var(--expense)">${v > 0 ? fmtB(Math.round(v)) : '–'}</td>`).join('')}
        </tr>
      </tbody></table></div>
  ...
  const _vrKindLbl = (window._grafF && window._grafF.kind === 'income') ? 'Příjmy'
                   : (window._grafF && window._grafF.kind === 'transfer') ? 'Přesuny' : 'Výdaje';
...
function drawVrBars(years, totals, kindLbl) {
  const cv = document.getElementById('vrBarsCanvas'); if (!cv) return;
  ...
  ctx.fillText(kindLbl + ' (Kč) · osa X: roky', W - pad.r, pad.t - 4);
  if (typeof attachChartTouch === 'function') attachChartTouch(cv);
}

// ══ v9.47: kumulace – kolik celkem od prvního roku ══
function drawVrCum(years, totals) {
  const cv = document.getElementById('vrCumCanvas'); if (!cv) return;
  ...
  ctx.fillStyle = '#7e84a0'; ctx.font = '9px system-ui'; ctx.textAlign = 'right';
  ctx.fillText('Kumulativně (Kč) · osa X: roky', W - pad.r, pad.t - 4);
  if (typeof attachChartTouch === 'function') attachChartTouch(cv);
}
