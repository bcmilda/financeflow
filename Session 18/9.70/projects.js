// FinanceFlow · v9.70 · projects.js · 2026-08-03
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderObraz() {
  ...
  const _sub = (title,val,desc,color,why) => `
    <div style="margin-top:11px;padding:10px 12px;background:var(--surface2);border-left:3px solid ${color};border-radius:0 10px 10px 0">
      <div style="display:flex;justify-content:space-between;gap:6px;align-items:baseline">
        ...
        <span style="font-size:.62rem;color:var(--text3)">${_winTxt}</span>
      </div>
    ...
    <div class="card" style="margin-bottom:12px">
      <div class="card-body" style="padding:14px">
        ...
        ${(()=>{
          ...
          return `
          <div style="display:flex;align-items:flex-end;gap:5px;height:52px;margin-bottom:4px">
          ...
          <div style="font-size:.66rem;color:#8b93ad;text-align:center;margin-bottom:14px">Vývoj skóre v okně · poslední sloupec = aktuální stav · skóre 0–100</div>
          ...
          <div style="padding:11px 13px;background:var(--surface2);border-left:3px solid #60a5fa;border-radius:0 10px 10px 0;margin-bottom:9px">
            <div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline">
              ...
              <span style="font-size:.62rem;color:var(--text3)">tento měsíc vs baseline</span>
            </div>
          ...
          <div style="padding:11px 13px;background:var(--surface2);border-left:3px solid ${momCol};border-radius:0 10px 10px 0;margin-bottom:14px">
            <div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline">
              ...
              <span style="font-size:.62rem;color:var(--text3)">trend ${nM} měsíců</span>
            </div>
            <div style="font-family:Syne,sans-serif;font-size:1.5rem;font-weight:800;margin:3px 0 5px;color:${momCol}">${_dScore!==null?(_dScore>0?'+':'')+_dScore+' '+(Math.abs(_dScore)===1?'bod':Math.abs(_dScore)<5?'body':'bodů'):'–'}${momLbl}</div>
        ...
        ${_sc.parts.map(p=>`
          ...
          <div style="font-size:.64rem;color:#8b93ad;margin:-4px 0 3px 113px">${p.note}</div>`).join('')}
        </div>
      </div>
    </div>`;
  ...
  const _lsRow = (label, b, a, chg, chgCol, bold) => `
    <tr style="${bold?'font-weight:800;background:var(--surface2)':''}">
      <td style="padding:6px 8px;text-align:left">${label}</td>
      <td style="padding:6px 8px;text-align:right;color:#a8aec8">${b}</td>
      <td style="padding:6px 8px;text-align:right">${a}</td>
      <td style="padding:6px 8px;text-align:right;color:${chgCol||'#a8aec8'}">${chg}</td>
    </tr>`;
  const _pctS = v => (v>0?'+':'')+v.toFixed(1).replace('.',',')+' %';
  const _lsTable = (series.length<2 || (!_lsB.inc && !_lsB.exp)) ? '' : `
    <div style="overflow-x:auto;margin-top:12px">
    <table style="width:100%;border-collapse:collapse;font-size:.75rem;min-width:380px">
      <thead><tr style="font-size:.62rem;color:var(--text3);text-transform:uppercase;letter-spacing:.04em">
      ...
      <div class="card" style="margin-bottom:12px">
        <div class="card-body" style="padding:14px">
          <div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline;margin-bottom:4px">
            ...
            <span style="font-size:.62rem;color:var(--text3)">aktuální stav</span></div>
          ...
          <div style="margin-top:11px;padding:10px 12px;background:var(--surface2);border-left:3px solid ${dM<0?'var(--debt)':'var(--income)'};border-radius:0 10px 10px 0">
            <div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline">
              ...
              <span style="font-size:.62rem;color:var(--text3)">aktuální</span></div>
    ...
    <div class="card" style="margin-bottom:12px;border-color:${_lsVerdict.bd}">
      <div class="card-body" style="padding:14px">
        ...
        ${(()=>{
          ...
          return `
          <div style="margin-top:11px;padding:10px 12px;background:var(--surface2);border-left:3px solid var(--debt);border-radius:0 10px 10px 0">
            <div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline">
              <span style="font-size:.7rem;font-weight:800;color:var(--debt);text-transform:uppercase;letter-spacing:.05em">Kam růst přistál</span>
              <span style="font-size:.62rem;color:var(--text3)">${series.length}M vs baseline</span></div>
            <div style="font-family:Syne,sans-serif;font-size:1.15rem;font-weight:800;margin:3px 0 5px;color:var(--debt)">${fmtB(Math.round(inFixed))} do trvalých závazků</div>
            <div style="font-size:.7rem;color:#a8aec8;line-height:1.5">Z nárůstu výdajů o ${fmtB(Math.round(dExp))} připadá ${fmtB(Math.round(inFixed))} na pravidelné měsíční platby, zbytek byly jednorázové výdaje.</div>
            <div style="margin-top:7px;padding-top:6px;border-top:1px dashed var(--border);font-size:.68rem;color:#a8aec8;line-height:1.55">
        ...
        ${(()=>{
          ...
          <div style="margin-top:9px;padding:10px 12px;background:var(--surface2);border-left:3px solid ${real===null?'#60a5fa':real>=0?'var(--income)':'var(--expense)'};border-radius:0 10px 10px 0">
            <div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline">
              ...
              <span style="font-size:.62rem;color:var(--text3)">očištěno o tvoji inflaci</span></div>
  ...
  el.innerHTML=tabIntro('obraz','🖼️','Finanční obraz',
      ...
      ${metrics.map(m=>`
        <div class="card">
          <div class="card-body" style="padding:14px">
            ...
            ${m.trendNote?`<div style="font-size:.64rem;color:#8b93ad;margin-top:2px;line-height:1.45">${m.trendNote}</div>`:''}
            ${m.subm||''}
          </div>
        </div>
      `).join('')}
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:0">
        <div style="display:grid;grid-template-columns:46px repeat(6,minmax(58px,1fr));font-size:.6rem;font-weight:700;color:var(--text3);text-transform:uppercase;padding:8px 10px;background:var(--surface3)">
