// FinanceFlow · v9.55 · charts.js · 2026-08-03
//  GRAFY (simplified)
// ══════════════════════════════════════════════════════
...
function renderVsechnyRoky() {
  //  v9.55: záložka pracuje se všemi roky – při okně 12M nejdřív dotáhnout historii.
  if (typeof window.ensureFullHistory === 'function' && (window.dwHistoryInfo||(()=>({})))().partial) {
    const _el = document.getElementById('gtab-vsechny-content');
    if (_el) _el.innerHTML = '<div style="padding:22px;text-align:center;color:#a8aec8;font-size:.82rem">Načítám celou historii…</div>';
    window.ensureFullHistory().then(() => renderVsechnyRoky());
    return;
  }
