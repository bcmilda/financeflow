        ...
        <table style="width:100%;border-collapse:collapse">
          ...
          <tbody>
            ${rows.slice(0,10).map(r => `
              <tr style="border-bottom:1px solid var(--border);opacity:${dups.some(d=>d===r)?'.4':r.isBalancing?'.55':'1'}">
                <td style="padding:4px 6px">${r.date}</td>
                <td style="padding:4px 6px;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                ...
                <td style="padding:4px 6px;color:var(--text2)">${r.isBalancing ? '⚖️ vyrov.' : r.type==='income'?'příjem':'výdaj'}</td>
              </tr>`).join('')}
...
function calcDupScore(r, existing) {
  ...
  // FIX-070 (Session 8): Zpřísnění – původní vzorec označoval příliš mnoho transakcí jako žluté.
  // Hlavní pravidla:
  //   • Bez shody názvu = max 40 bodů (nikdy nebude "možný" pokud se jméno úplně liší)
  //   • Datum musí být ±1 den pro plné body (ne ±3)
  //   • Stejná částka ±0.01 Kč = 35 bodů (klíčový identifikátor)
  //   • Shoda alespoň 1 slova = +10, celý název = +30
  //   • Threshold yellow: ≥60 (bylo 25), red: ≥85 (bylo 75)
  //
  // Logika:
  //   Naprosto jistý duplikát (score ≥ 85): stejný název + stejná částka + datum ±1 → červená
  //   Možný duplikát (score ≥ 60): alespoň 2 ze 3 silných kritérií
  //   Nová transakce (score < 60): jiná částka nebo jiný název a datum > 1 den
  ...
  for(let i = 0; i < existing.length; i++) {
    ...
    // Rychlý skip: datum > 7 dní = nemůže být duplikát
    const tTs = t._ts;
    if (tTs != null && !isNaN(rTs)) {
      if (Math.abs((rTs - tTs) / 86400000) > 7) continue;
    }
    ...
    // ČÁSTKA (max 40 bodů) – nejdůležitější identifikátor
    const tAmt = t._amt || 0;
    const diffAmt = Math.abs(rAmt - tAmt);
    if (diffAmt <= 0.01)      score += 40; // přesná shoda
    else if (diffAmt <= 1)    score += 25; // haléřový rozdíl (zaokrouhlení)
    else if (diffAmt <= 10)   score += 10; // malý rozdíl
    // Větší rozdíl = 0 bodů

    // TYP (max 5 bodů)
    if (r.type === t.type) score += 5;

    // NÁZEV (max 35 bodů) – fuzzy match
    const n2 = t._nameLow || '';
    if (rNameLow && n2) {
      if (rNameLow === n2) {
        score += 35; // přesná shoda
      } else if (rNameLow.includes(n2) || n2.includes(rNameLow)) {
        score += 22; // jeden je podřetězcem druhého
      } else if (rWords.length && t._words?.length) {
        // Sdílená slova (min délka 3)
        let shared = 0;
        for (const w of rWords) {
          if (t._words.some(w2 => w2 === w || (w.length >= 4 && w2.includes(w)))) shared++;
        }
        if (shared >= 2) score += 18;
        else if (shared === 1) score += 8;
        // 0 sdílených slov = 0 bodů
      }
    } else {
      // Jeden nebo oba nemají název → název nepomůže, ale není to penalizace
    }

    // GUARD: Pokud název nemá žádnou shodu (0 bodů za název) → cap na 55
    // Brání situaci kde datum + částka dají 60+ bez jakékoli shody v názvu
    const nameScore = score - (tTs != null && !isNaN(rTs) ? Math.min(20, score) : 0) - Math.min(40, score) - 5;
    // (Jednodušší implementace: pokud n2 a rNameLow jsou oba neprázdné a neshodují se v ničem)
    if (rNameLow && n2 && !rNameLow.includes(n2.slice(0,4)) && !n2.includes(rNameLow.slice(0,4))) {
      score = Math.min(score, 55); // cap – nemůže být "možný" duplikát bez shody názvu
    }

    if (score > best) { best = score; bestTx = t; }
    if (best >= 95) break; // jistý duplikát
  }
...
async function openImportEditor(filename) {
  ...
  for (let i = 0; i < _importRows.length; i += CHUNK) {
    ...
    for (const r of slice) {
      ...
      const color = score >= 85 ? 'red' : score >= 60 ? 'yellow' : 'green';
