//  FinanceFlow · v8.34 · assets.js · 2026-06-25
// ══════════════════════════════════════════════════════
...
function syncInvestmentAssets(){
  ...
  const cats = S.categories.filter(c => c.type === 'transfer');
  if (!cats.length) return;
  if (!Array.isArray(S.assets)) S.assets = [];
  const txs = (S.transactions || []).filter(t => !t.splitParent);
  cats.forEach(c => {
    if (c.noAutoAsset) return; // uživatel napojené aktivum smazal → nevytvářet znovu
    let invested = 0, hasTx = false;
    for (const t of txs) {
      if ((t.catId || t.category) === c.id) {
        const amt = (t.amount || t.amt || 0);
        if (t.type === 'expense') { invested += amt; hasTx = true; }
        else if (t.type === 'income') { invested -= amt; hasTx = true; }
      }
    }
    let asset = S.assets.find(a => a.linkedCatId === c.id);
    if (!hasTx && !asset) return;
    if (!asset) {
      const nm = (c.name || '').trim().toLowerCase();
      const grp = _SAVE_ASSET_NAMES.some(s => nm.includes(s)) ? 'savings' : 'investment';
      asset = { id: uid(), name: c.name, icon: c.icon || (grp === 'savings' ? '🏦' : '📈'),
                type: grp, linkedCatId: c.id, auto: true, valueManual: false,
                invested: 0, value: 0, updatedAt: Date.now() };
      S.assets.push(asset);
    }
    const inv = Math.max(0, Math.round(invested));
    asset.invested = inv;
    if (!asset.valueManual) asset.value = inv; // bez ruční tržní hodnoty = vklady
...
function saveAsset() {
  ...
  const obj = {
    ...
    valueManual: true, // S14: uživatel zadal tržní hodnotu ručně → auto-sync ji nepřepíše
    updatedAt: Date.now(),
  };
...
function assetDelete(id) {
  ...
  const _a = (S.assets || []).find(x => x.id === id); // S14: napojené aktivum už znovu nevytvářet
  if (_a && _a.linkedCatId) { const _c = (S.categories||[]).find(x => x.id === _a.linkedCatId); if (_c) _c.noAutoAsset = true; }
