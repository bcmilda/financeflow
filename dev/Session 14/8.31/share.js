//  FinanceFlow · v8.31 · share.js · 2026-06-24
// ══════════════════════════════════════════════════════
  ...
  if (_myRefCode && window._db) {
    try {
      ...
      if (convSnap.exists()) {
        ...
        const ks = Object.keys(conv);
        const totalEarned = ks.reduce((a,k)=>a+((conv[k]&&conv[k].points)||0),0);
        const convCount = ks.length;
    ...
    try {
      ...
      if (ownerUid && ownerUid !== window._currentUser.uid) {
        ...
        try {
          ...
          if (!existing.exists()) {
            await _set(convRef, { points: SHARE_REWARDS.signup, type: 'signup', addedAt: new Date().toISOString() });
          }
...
async function adminCreditReferralSelf(points){
  ...
  try {
    ...
    await _set(_ref(_db, `referrals/${_myRefCode}/conversions/${key}`), { points: pts, type:'manual', addedAt: new Date().toISOString() });
    const convSnap = await _get(_ref(_db, `referrals/${_myRefCode}/conversions`));
    const conv = convSnap.exists() ? (convSnap.val()||{}) : {};
    const ks = Object.keys(conv);
    const totalEarned = ks.reduce((a,k)=>a+((conv[k]&&conv[k].points)||0),0);
    await _update(_ref(_db, `users/${myUid}/referral`), { earned: totalEarned, conversions: ks.length });
    _refStats.earned = totalEarned; _refStats.conversions = ks.length;
