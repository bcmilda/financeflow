//  FinanceFlow · v8.31 · transactions.js · 2026-06-24
//  BANK
// ══════════════════════════════════════════════════════
function renderBank(){
  const D=getData();
...
function drawSeasLines(canvas,real,model){
  ...
  // osa Y po 10 %
  for(let v=minV; v<=maxV+0.001; v+=0.10){
    const y=yOf(v);
    const is100 = Math.abs(v-1.0)<0.001;
...
function switchPredGraph(tab,btn){
  ...
  document.querySelectorAll('#predGraphTab-kumul,#predGraphTab-seas,#predGraphTab-pace').forEach(b=>b.classList.remove('active'));
...
function renderPaceChart(D){
  ...
  getTx(cm,cy,D).filter(t=>t.type==='expense'&&!t.isBalancing).forEach(t=>{const d=new Date(t.date).getDate();curDaily[d]+=Math.abs(t.amount||t.amt||0);});
  ...
  for(let k=1;k<=6;k++){
    ...
    getTx(m,y,D).filter(t=>t.type==='expense'&&!t.isBalancing).forEach(t=>{const d=new Date(t.date).getDate();md[d]+=Math.abs(t.amount||t.amt||0);any=true;});
    if(!any) continue;
    histMonthsCount++;
...
function drawPaceChart(canvas, days, todayDay, curCum, histAvg, histMonthsCount){
  ...
  // mřížka + osa Y
  for(let i=0;i<=4;i++){const y=top+H2*(1-i/4);ctx.strokeStyle='rgba(46,51,71,.5)';ctx.lineWidth=1;ctx.setLineDash([3,3]);ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(W-right,y);ctx.stroke();
    ctx.fillStyle='#a8aec8';ctx.font='9px Instrument Sans';ctx.textAlign='right';ctx.fillText(fmt(Math.round(maxV*i/4)),pad-5,y+3);}
  ctx.setLineDash([]);
...
function renderDebtTxTable(d, paid, D){
  ...
  const actCell = (tx)=> tx
    ? `<span onclick="editTx('${tx.id}')" style="cursor:pointer;color:var(--income);text-decoration:underline;text-decoration-style:dotted" title="Otevřít transakci">${fmt(tx.amount||tx.amt||0)} Kč</span>`
  ...
  if(sched.length){
    ...
    for(let i=0;i<showN;i++){
      ...
      rows+=`<div class="debt-schedule-row ${isPaid?'paid':''}${overdue?' overdue':''}" style="${GRID}"><span>#${s.num} · ${s.date}</span><span style="text-align:right">${fmt(s.payment||0)} Kč</span><span style="text-align:right">${actCell(tx)}</span><span style="text-align:center">${status}</span></div>`;
    }
...
function renderDebts(){
  ...
  el.innerHTML=(D.debts||[]).map(d=>{
    ...
    const _totalToPay = (schedule&&schedule.length) ? schedule.reduce((a,s)=>a+(s.payment||0),0) : (d.total||d.remaining||0);
    const _paidPct = _totalToPay>0 ? Math.min(100, paid.paidSum/_totalToPay*100) : 0;
    ...
    return `<div class="debt-card">
      ...
      <div style="display:flex;justify-content:space-between;font-size:.68rem;color:#a8aec8;margin:-2px 0 6px;gap:8px;flex-wrap:wrap">
        <span>✅ Zaplaceno ${fmt(paid.paidSum)} Kč${_overdueAmt>0?` · <span style="color:#fca5a5">po splatnosti ${fmt(_overdueAmt)} Kč</span>`:''}</span>
        <span>z ${fmt(_totalToPay)} Kč (${Math.round(_paidPct)} %)</span>
      </div>
      ...
      ${paid.paidSum>0?`<div class="debt-stats" style="margin-top:6px">
        <div class="dst"><div class="dst-val" style="color:var(--income)">${fmt(paid.paidSum)} Kč</div><div class="dst-lbl">Zaplaceno (${paid.count}×)</div></div>
        <div class="dst"><div class="dst-val">${fmt(paid.paidPrincipal)} Kč</div><div class="dst-lbl">Na jistině</div></div>
        <div class="dst"><div class="dst-val" style="color:var(--debt)">${fmt(paid.paidInterest)} Kč</div><div class="dst-lbl">Na úrocích</div></div>
      </div>`:''}
