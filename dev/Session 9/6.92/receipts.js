                ...
                ${(r.items||[]).map(it=>{
                  ...
                  return `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
                    <div style="flex:1;min-width:0">
                      ...
                      ${cat?`<span style="font-size:.68rem;color:${cat.color||'var(--text3)'};margin-left:6px">${cat.icon} ${cat.name}</span>`:''}
                    </div>
                    <div style="text-align:right;flex-shrink:0;margin-left:8px">
                      <span style="font-size:.82rem;font-weight:600">${fmtP(it.price||0)} Kč</span>
                      ${(it.qty||1)!==1?`<span style="font-size:.68rem;color:var(--text3)"> ×${it.qty} ks</span>`:''}
                    </div>
                  </div>`;
  ...
  sorted.forEach(group => {
    ...
    html += `<div class="card" style="margin-bottom:10px">
      <div style="padding:12px 14px;cursor:pointer;display:flex;align-items:center;gap:10px"
        ...
        <div style="text-align:right;flex-shrink:0">
          <div style="font-family:Syne,sans-serif;font-size:1.2rem;font-weight:800;color:var(--expense)">${fmt(Math.round(group.total))} Kč</div>
          ${avg !== null ? `<div style="font-size:.75rem;color:var(--text2)">ø ${fmt(avg)} Kč/nákup</div>` : ''}
        </div>
        <span id="${groupId}_arrow" style="color:var(--text3);font-size:.85rem;flex-shrink:0;transition:transform .2s">▶</span>
      </div>
        ...
        ${group.receipts.map(r => {
          ...
          return `<div style="border-bottom:1px solid var(--border)">
            <div style="padding:10px 14px;display:flex;align-items:center;gap:8px;cursor:pointer"
              ...
              <div style="flex:1;min-width:0">
                <div style="font-size:.85rem;font-weight:600;color:var(--text)">${r.date||'–'}</div>
                <div style="font-size:.74rem;color:var(--text2);margin-top:2px">${(r.items||[]).length} položek${catSummary?' · '+catSummary.slice(0,60)+(catSummary.length>60?'…':''):''}</div>
              </div>
              <div style="text-align:right;flex-shrink:0">
                <div style="font-weight:700;color:var(--expense);font-size:.95rem">${fmtP(r.total||0)} Kč</div>
              </div>
              <button class="btn btn-edit btn-icon btn-sm" onclick="event.stopPropagation();editReceiptFromHistory(${r._idx})" title="Upravit">✎</button>
              <button class="btn btn-danger btn-icon btn-sm" onclick="event.stopPropagation();deleteReceipt(${r._idx})">✕</button>
              <span id="${rcptId}_arrow" style="color:var(--text3);font-size:.72rem;flex-shrink:0">▶</span>
