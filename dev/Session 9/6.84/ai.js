...
async function aiCategorizeTxForce(nameOverride){
  ...
  try{
    ...
    el.innerHTML=`<div style="background:var(--surface2);border-radius:10px;padding:12px;border:1px solid var(--border)">
      ...
      <div style="font-size:.76rem;color:var(--text3)">${j.reason}</div>
      <button class="btn btn-accent btn-sm" style="margin-top:8px" onclick="applyAiCat('${j.catId}','${(j.subcat||'').replace(/'/g,"\\'")}','${name.replace(/'/g,"\\'")}')">✓ Použít a zapamatovat</button>
    </div>`;
...
function applyAiCat(catId, subcat, txName){
  ...
  // TODO-014: Ulož mapování do Firebase
