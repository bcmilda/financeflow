...
function buildHistoryTab(receipts) {
  ...
  html += `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
    <div style="font-size:.8rem;color:var(--text2);font-weight:500">${receipts.length} účtenek · ${sorted.reduce((a,r)=>a+(r.total||0),0).toLocaleString('cs-CZ',{maximumFractionDigits:0})} Kč celkem</div>
  ...
  sorted.forEach(r => {
    ...
    const catTags = Object.entries(catGroups).map(([n,v])=>
      `<span style="font-size:.68rem;padding:1px 6px;background:${v.color}22;border:1px solid ${v.color}55;border-radius:8px;color:var(--text2)">${v.icon} ${n} ${(v.total).toLocaleString('cs-CZ',{maximumFractionDigits:0})} Kč</span>`
    ).join('');

    html += `<div class="card" style="margin-bottom:8px;overflow:hidden">
      <!-- Hlavní řádek účtenky -->
      <div style="padding:12px 14px;display:flex;align-items:flex-start;gap:10px">
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:3px">
            <span style="font-size:.9rem;font-weight:700;color:var(--text)">${r.store||'Neznámý obchod'}</span>
            <span style="font-size:.72rem;color:var(--text3)">${r.date||'–'}</span>
            <span style="font-size:.68rem;color:var(--text3)">${(r.items||[]).length} pol.</span>
          </div>
          <div style="display:flex;flex-wrap:wrap;gap:4px">${catTags}</div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-family:Syne,sans-serif;font-size:1.1rem;font-weight:800;color:var(--expense)">${(r.total||0).toLocaleString('cs-CZ',{minimumFractionDigits:2,maximumFractionDigits:2})} Kč</div>
        </div>
...
function validateReceiptJSON(r) {
  ...
  if(!r.total && r.items.length) {
    r.total = Math.round(r.items.reduce((a,it)=>a+it.price*it.qty,0)*100)/100;
  }
