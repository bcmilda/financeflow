...
function renderStats(){
  ...
  // Category breakdown s podkategoriemi
  const catEl=document.getElementById('statCats');
  if(catEl){
    const items=expCats.map(c=>{
      const val=getActual(c.id,null,S.curMonth,S.curYear,D);
      if(!val) return null;
      // Podkategorie s daty
      const activeSubs=(c.subs||[]).map(sub=>({name:sub,val:getActual(c.id,sub,S.curMonth,S.curYear,D)})).filter(s=>s.val>0).sort((a,b)=>b.val-a.val);
    ...
    catEl.innerHTML = items.map(d=>`
      <div style="margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;font-size:.82rem;margin-bottom:3px;align-items:center">
          <span style="font-weight:600">${d.icon} ${d.name}</span>
          <strong>${fmt(d.val)}</strong>
        </div>
      ...
      </div>`).join('') || '<div class="empty"><div class="et">Žádné výdaje</div></div>';
  }
  // Insights
