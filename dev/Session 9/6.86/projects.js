...
function renderReport() {
  ...
  const catRows = expCats.map(cat => {
    ...
    const activeSubs = (cat.subs||[]).map(sub=>({
      name:sub, val: getActual(cat.id, sub, S.curMonth, S.curYear, D)
...
function renderDetektor() {
  ...
  el.innerHTML=`
    ...
    ${(() => {
      ...
      const catColor = (cat) => {
        if (cat.includes('Předplatné')) return '#a78bfa';     // fialová
        if (cat.includes('Pojištění')) return '#60a5fa';      // modrá
        if (cat.includes('Bankovní')) return '#fbbf24';       // žlutá
        if (cat.includes('Telefon')) return '#34d399';        // zelená
        if (cat.includes('Refinancování')) return '#f87171';  // červená
        return '#94a3b8'; // šedá (default – přečerpání kategorie)
      };
