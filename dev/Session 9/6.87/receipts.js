...
function addReceiptAsTx(receipt) {
  ...
  // TODO-014: Nejdřív zkus AI mappings cache
  const cached = lookupCategoryMapping(receipt.store||'');
  let cat = cached ? D.categories?.find(c=>c.id===cached.catId) : null;

  // Fallback: hardcoded catMap dle kategorie z receipt preview
  if(!cat) {
    const catMap = {
      ...
      'Restaurace': D.categories?.find(c=>c.name.includes('Jídlo')||c.name.includes('Pití')||c.name.includes('Zábava')),
      'Benzín': D.categories?.find(c=>c.name.includes('Doprava')||c.name.includes('Auto')),
      'Elektronika': D.categories?.find(c=>c.name.includes('Elektronika')),
      'Oblečení': D.categories?.find(c=>c.name.includes('Oblečení')),
      'Sport': D.categories?.find(c=>c.name.includes('Fitness')||c.name.includes('Sport')),
      'Domácí mazlíčci': D.categories?.find(c=>c.name.includes('Mazlíček')||c.name.includes('Domácí mazlíček')),
      'Dům & Zahrada': D.categories?.find(c=>c.name.includes('Domácí potřeby')||c.name.includes('Rekonstrukce')),
      'Lékárna': D.categories?.find(c=>c.name.includes('Zdraví')),
    };
    cat = catMap[receipt.category] || D.categories?.find(c=>c.name===receipt.category) || D.categories?.[0];
  }
  ...
  const tx = {
    ...
    catId: cat?.id||'',
    category: cat?.id||'',
    subcat: cached?.subcat||'',
    note: `📸 Naskenováno · ${(receipt.items||[]).length} položek`,
  };

  // TODO-014: Ulož mapování pro příští použití
  if(receipt.store && cat?.id) {
    saveCategoryMapping(receipt.store, cat.id, '');
  }
  ...
  const status = document.getElementById('receiptStatus');
  if(preview) preview.style.display='none';
  if(status) { status.style.display='block'; status.innerHTML=`<div class="insight-item good"><div class="insight-icon">✅</div><div class="insight-text">Transakce přidána! Kategorie: <strong>${cat?.name||'Jiné'}</strong>. Uložena do AI paměti.</div></div>`; }
