...
function renderItemStatsList(topItems, allItems, D, period) {
  ...
  return sorted.map(([name,v])=>{
    ...
    const priceTrend = v.prices.length>=2
      ? (v.prices[v.prices.length-1] > v.prices[0] ? `<span style="color:var(--expense);font-size:.65rem">↑</span>` : v.prices[v.prices.length-1] < v.prices[0] ? `<span style="color:var(--income);font-size:.65rem">↓</span>` : '')
      : '';
    return `<div style="display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid var(--border)">
      <div style="flex:1;min-width:0">
        <div style="font-size:.82rem;font-weight:600;display:flex;align-items:center;gap:5px">
          ...
          <span style="font-size:.65rem;color:var(--text3);font-weight:400">${v.count}×</span>
        </div>
        ${cat?`<div style="font-size:.68rem;color:var(--text3)">${cat.name}</div>`:''}
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:.8rem;font-weight:600">ø ${fmt(avgPrice)} Kč</div>
        ${v.prices.length>=2?`<div style="font-size:.66rem;color:var(--text3)">${fmt(minP)}–${fmt(maxP)} Kč</div>`:''}
      </div>
    </div>`;
  }).join('');
}
        ...
        <div style="display:flex;align-items:center;gap:5px;padding:5px 2px;border-bottom:1px solid var(--border)" id="rp_item_${i}">
          ...
          <div style="position:relative;flex-shrink:0;display:flex;gap:3px">
            ...
            ${it.itemCatId ? `<select id="rp_subcat_${i}"
              style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;padding:5px 4px;color:var(--text3);font-size:.65rem;max-width:80px">
