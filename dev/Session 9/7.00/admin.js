...
function switchAdminTab(tab, btn) {
  ['users','keywords','corrections','lowconf','stats','adopce','itemtags','leads','verze'].forEach(t => {
...
async function loadCommunityItemTags() {
  ...
  try {
    ...
    const items = Object.entries(data).map(([itemKey, tags]) => {
      const tagList = Object.entries(tags||{}).map(([tag, cnt]) => ({tag, cnt})).sort((a,b)=>b.cnt-a.cnt);
      ...
      ${items.map(item => `
        <div style="padding:10px 0;border-bottom:1px solid var(--border)">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px">
            ${item.tagList.map(({tag, cnt}) => {
              const fireKey = `${item.itemKey}/${tag.replace(/[.#$/\[\]]/g,'_')}`;
              return `<div style="display:flex;align-items:center;gap:4px;padding:4px 8px;background:var(--surface2);border-radius:8px;border:1px solid var(--border)">
                <span style="font-size:.78rem;color:var(--income);font-weight:600">${tag}</span>
                <span style="font-size:.64rem;color:var(--text3)">(${cnt}×)</span>
                <button onclick="validateItemTag('${item.itemKey}','${tag}','approved')" class="btn btn-ghost btn-sm" style="padding:1px 5px;font-size:.7rem;color:var(--income)" title="Schválit">✓</button>
                <button onclick="validateItemTag('${item.itemKey}','${tag}','rejected')" class="btn btn-ghost btn-sm" style="padding:1px 5px;font-size:.7rem;color:var(--expense)" title="Odmítnout">✕</button>
              </div>`;
            }).join('')}
          </div>
        </div>`).join('')}
...
async function renderKomunita() {
  ...
  el.innerHTML = `
    ...
    <!-- Tabs -->
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap">
      <button class="tx-filt-btn active" id="ktab-coicop" onclick="switchKomunitaTab('coicop',this)">🔢 COICOP přehled</button>
      <button class="tx-filt-btn" id="ktab-csu" onclick="switchKomunitaTab('csu',this)">📊 ČSÚ průměry</button>
