...
function switchAdminTab(tab, btn) {
  ['users','keywords','corrections','lowconf','stats','adopce','leads','verze'].forEach(t => {
...
async function loadCategoryAdoption() {
  ...
  try {
    ...
    // Agreguj využití kategorií a podkategorií napříč uživateli
    const catUsage = {};    // {catId: {name, txCount, userCount, subs: {subName: txCount}}}
    const customCats = {}; // {name: {userCount, txCount, coicop}} – kategorie mimo DEFAULT
    ...
    Object.values(data).forEach(udata => {
      const txs = Array.isArray(udata?.data?.transactions) ? udata.data.transactions : Object.values(udata?.data?.transactions||{});
      const cats = Array.isArray(udata?.data?.categories) ? udata.data.categories : Object.values(udata?.data?.categories||{});

      txs.forEach(tx => {
        if(tx.type !== 'expense') return;
        totalTx++;
        const catId = tx.catId || tx.category || '';
        const sub = tx.subCategory || tx.sub || '';

        if(!catId) { uncategorized++; return; }

        // Najdi kategorii
        const cat = cats.find(c=>c.id===catId) || DEFAULT_CATEGORIES.find(c=>c.id===catId);
        const catName = cat?.name || catId;
        if(catName === 'Jiné' || catId === 'cat21') inJine++;

        if(!catUsage[catId]) catUsage[catId] = {name:catName, icon:cat?.icon||'📦', color:cat?.color||'#6b7280', txCount:0, userCount:0, isDefault:defIds.has(catId), coicop:cat?.coicop||null, subs:{}};
        catUsage[catId].txCount++;
        if(sub) catUsage[catId].subs[sub] = (catUsage[catId].subs[sub]||0)+1;

        // Custom kategorie (mimo DEFAULT)
        if(!defIds.has(catId) && cat){
          if(!customCats[catName]) customCats[catName] = {userCount:0, txCount:0, coicop:cat.coicop||null, catId, icon:cat.icon||'📦'};
          customCats[catName].txCount++;
        }
      });

      // Počítej unikátní uživatele na kategorii
      const usedCatIds = new Set(
        (Array.isArray(udata?.data?.transactions) ? udata.data.transactions : Object.values(udata?.data?.transactions||{}))
          .filter(t=>t.type==='expense').map(t=>t.catId||t.category||'')
      );
      usedCatIds.forEach(id=>{ if(catUsage[id]) catUsage[id].userCount++; });
      Object.values(udata?.data?.categories||{}).forEach(c=>{ if(!defIds.has(c.id)&&c.name&&customCats[c.name]) customCats[c.name].userCount++; });
    });

    // Seřaď dle txCount desc
    ...
    el.innerHTML = `<div class="card-body">
      ...
      ${unusedDefs.length?`<div style="padding:8px 12px;background:rgba(251,191,36,.08);border:1px solid rgba(251,191,36,.3);border-radius:8px;margin-bottom:14px;font-size:.76rem">
        ⚠️ <strong>${unusedDefs.length} výchozích kategorií</strong> bez jediné transakce: ${unusedDefs.map(d=>`${d.icon} ${d.name}`).join(', ')}
      </div>`:''}

      <!-- Tabulka využití -->
      <div style="font-size:.72rem;font-weight:700;color:var(--text3);text-transform:uppercase;margin-bottom:8px">Využití kategorií (seřazeno dle počtu transakcí)</div>
      ${sorted.map(([id,c])=>{
        const barW = Math.round(c.txCount/maxTx*100);
        const coicopBadge = c.coicop ? `<span style="display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;border-radius:50%;background:${(COICOP_GROUPS_DEF.find(g=>g.id===c.coicop)||{}).color||'#aaa'};color:#000;font-size:.55rem;font-weight:800;flex-shrink:0">${c.coicop}</span>` : `<span style="font-size:.6rem;color:var(--expense);padding:1px 4px;border:1px solid var(--expense);border-radius:4px">bez COICOP</span>`;
        const customBadge = !c.isDefault ? `<span style="font-size:.6rem;color:var(--debt);padding:1px 5px;border:1px solid var(--debt);border-radius:6px">custom</span>` : '';
        const topSubs = Object.entries(c.subs).sort((a,b)=>b[1]-a[1]).slice(0,3);
        return `<div style="margin-bottom:10px;padding:8px 10px;background:var(--surface2);border-radius:8px">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
            <span style="font-size:1rem">${c.icon}</span>
            <span style="font-size:.8rem;font-weight:600;flex:1">${c.name}</span>
            ${coicopBadge}${customBadge}
            <span style="font-size:.72rem;color:var(--text2);margin-left:auto">${c.txCount} tx · ${c.userCount}/${userCount} už.</span>
          </div>
          <div style="height:6px;background:var(--surface3);border-radius:3px;overflow:hidden;margin-bottom:4px">
            <div style="height:100%;width:${barW}%;background:${c.color};border-radius:3px;transition:width .3s"></div>
          </div>
          ${topSubs.length?`<div style="font-size:.68rem;color:var(--text3)">Top podkat.: ${topSubs.map(([s,n])=>`${s} (${n})`).join(' · ')}</div>`:''}
        </div>`;
      }).join('')}
    </div>`;
  } catch(e) {
...
async function loadCustomCatsNoCoicop() {
  ...
  try {
    ...
    const defIds = new Set(DEFAULT_CATEGORIES.map(d=>d.id));
    const customMap = {}; // {catId: {name, icon, color, users: [], coicop}}

    Object.entries(data).forEach(([uid, udata]) => {
      const cats = Array.isArray(udata?.data?.categories) ? udata.data.categories : Object.values(udata?.data?.categories||{});
      cats.forEach(c => {
        if(defIds.has(c.id)) return; // přeskočit výchozí
    ...
    if(!Object.keys(customMap).length){
      el.innerHTML = '<div class="card-body" style="color:var(--text2);font-size:.8rem">✅ Žádné vlastní kategorie – všichni uživatelé používají pouze výchozí sadu.</div>';
      return;
    }
...
async function assignCoicop(catId, catName) {
  ...
  // Zapíše do Firebase admin_coicop_overrides/{catId} = číslo
  // Uživatelé si ho stáhnou při načtení (budoucí integrace)
  try {
    const idToken = await window._currentUser?.getIdToken?.();
    const res = await fetch(
    ...
    if(!res.ok) throw new Error('HTTP '+res.status);
    alert(`✅ Kategorie „${catName}" přiřazena do COICOP ${coicopNum}`);
