  ...
  return sorted.map(([name,v])=>{
    ...
    return `<div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--border)">
      ...
      <div style="text-align:right;flex-shrink:0;margin-left:10px">
        <div style="font-size:.95rem;font-weight:800;color:var(--text)">ø ${fmt(avgPrice)} Kč</div>
        ${v.prices.length>=2?`<div style="font-size:.7rem;color:var(--text3)">${fmt(minP)}–${fmt(maxP)} Kč</div>`:''}
      </div>
    </div>`;
  }).join('');
}
...
function buildStoresTab(storeStats, totalSpent) {
  ...
  if(!Object.keys(storeStats).length) {
    ...
    Object.entries(storeStats).sort((a,b)=>b[1].total-a[1].total).forEach(([store,stats])=>{
      ...
      html += `<div class="card" style="margin-bottom:10px"><div class="card-body" style="padding:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <div>
            <div style="font-weight:700;font-size:.9rem">${store}</div>
            <div style="font-size:.75rem;color:var(--text2)">${stats.visits} návštěv · ${stats.count} položek</div>
          </div>
          <div style="text-align:right">
            ...
            ${stats.visits > 1 ? `<div style="font-size:.72rem;color:var(--text2)">ø ${fmt(Math.round(stats.total/stats.visits))} Kč/nákup</div>` : ''}
          </div>
        </div>
        <div class="trap-bar"><div class="trap-bar-fill" style="width:${pct}%;background:var(--bank)"></div></div>
        <div style="font-size:.7rem;color:var(--text2);margin-top:3px">${pct}% z celkových výdajů</div>
      </div></div>`;
    });
  }
  return html+'</div>';
