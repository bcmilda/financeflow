...
function renderCatPage(){
  ...
  // Pole coicop/shared se neukládají do Firebase – doplníme je za runtime z DEFAULT_CATEGORIES
  ...
  cats.forEach(c => {
    ...
    if(!def) return;
    if(c.coicop === undefined && def.coicop !== undefined) c.coicop = def.coicop;
    if(!c.shared && def.shared) c.shared = def.shared;
    if(!c.coicopOverrides && def.coicopOverrides) c.coicopOverrides = def.coicopOverrides;
  });
