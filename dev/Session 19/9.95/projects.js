// FinanceFlow · v9.95 · projects.js · 2026-08-19
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderProjectDetail(id) {
  ...
  el.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
      ...
      ${(()=>{const ce=projectCurLabel(txs,D,'expense'),ci=projectCurLabel(txs,D,'income');
        if(!ce&&!ci) return '';
        return `<div style="font-size:.76rem;color:#a8aec8;margin:-4px 0 12px;line-height:1.6">
          💱 Projekt obsahuje jinou měnu než základní — ${ce?`výdaje <strong style="color:#c9cede">${ce}</strong>`:''}${ce&&ci?' · ':''}${ci?`příjmy <strong style="color:#c9cede">${ci}</strong>`:''}.
          Součty výše jsou přepočtené na ${(typeof curSym==='function')?curSym():'Kč'} kurzem z okamžiku zápisu.
        </div>`;})()}
