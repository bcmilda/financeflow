...
function buildFinanceContext(){
  ...
  const projDetails = projects.map(p=>{
    ...
    const pspent = ptxs.filter(t=>t.type==='expense').reduce((a,t)=>a+txCZK(t,D),0);  // FIX-252/A
...
async function aiCategorizeTx(){
  ...
  // TODO-014: Nejdřív zkontroluj lokální mapování
  await loadCategoryMappings();
  const cached = lookupCategoryMapping(name);
  if(cached){
    const cat=S.categories.find(c=>c.id===cached.catId);
    if(cat){
      el.innerHTML=`<div style="background:var(--surface2);border-radius:10px;padding:12px;border:1px solid var(--income)33">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
          <span style="font-size:1.1rem">${cat.icon}</span><strong>${cat.name}</strong>
          ${cached.subcat?`<span style="color:var(--text3);font-size:.78rem">→ ${cached.subcat}</span>`:''}
          <span style="margin-left:auto;font-size:.72rem;color:var(--income)">🧠 Z paměti (${cached.count}×)</span>
        </div>
        <div style="font-size:.76rem;color:var(--text3)">Dříve přiřazeno do této kategorie</div>
        <div style="display:flex;gap:6px;margin-top:8px">
          <button class="btn btn-accent btn-sm" onclick="applyAiCat('${cached.catId}','${(cached.subcat||'').replace(/'/g,"\\'")}','${name.replace(/'/g,"\\'")}')">✓ Použít</button>
          <button class="btn btn-ghost btn-sm" onclick="document.getElementById('aiCatResult').innerHTML='';aiCategorizeTxForce()">🔄 Dotázat AI</button>
        </div>
      </div>`;
      return;
    }
  ...
  await aiCategorizeTxForce(name);
}

async function aiCategorizeTxForce(nameOverride){
  const name=nameOverride||document.getElementById('aiCatInput')?.value?.trim();if(!name)return;
  const catNames=S.categories.map(c=>`${c.id}:${c.icon}${c.name}(${c.type})`).join(', ');
  const el=document.getElementById('aiCatResult');if(!el)return;
  el.innerHTML='<div style="color:var(--text3);font-size:.8rem;padding:8px">⏳ Dotazuji AI…</div>';
  const res=await callClaude(`Mám tyto kategorie: ${catNames}\n\nZařaď transakci "${name}" do nejlepší kategorie. Odpověz POUZE JSON: {"catId":"...", "catName":"...", "subcat":"...", "coicop": <číslo oddílu COICOP 2024 (1-13), kam výdaj patří>, "confidence":"high/mid/low", "reason":"..."}`, 'Odpovídej pouze validním JSON bez markdown bloků.');
  if(!res){el.innerHTML='';return;}
  try{
    const raw=JSON.parse(res.replace(/```json|```/g,'').trim());
    const j = typeof validateAiCatJSON === 'function' ? validateAiCatJSON(raw) : raw; // TODO-008
    ...
    el.innerHTML=`<div style="background:var(--surface2);border-radius:10px;padding:12px;border:1px solid var(--border)">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
        <span style="font-size:1.1rem">${cat?.icon||'📋'}</span><strong>${j.catName}</strong>
        ${j.subcat?`<span style="color:var(--text3);font-size:.78rem">→ ${j.subcat}</span>`:''}
        <span style="margin-left:auto;font-size:.72rem;color:${confColor}">${j.confidence==='high'?'✓ Jistý':j.confidence==='mid'?'~ Pravděpodobný':'? Nejistý'}</span>
      </div>
      <div style="font-size:.76rem;color:var(--text3);margin-bottom:10px">${j.reason}</div>
      ${j.coicop?`<div style="display:flex;align-items:center;gap:7px;font-size:.74rem;color:#a8aec8;margin-bottom:10px">
        <span style="display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;border-radius:50%;background:${(typeof COICOP_GROUPS_DEF!=='undefined'&&COICOP_GROUPS_DEF.find(g=>g.id===j.coicop))?.color||'#aaa'};color:#000;font-size:.66rem;font-weight:800;flex-shrink:0">${j.coicop}</span>
        🧭 COICOP ${j.coicop} – ${(typeof COICOP_GROUPS_DEF!=='undefined'&&COICOP_GROUPS_DEF.find(g=>g.id===j.coicop))?.name||''}
      </div>`:''}
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        <button class="btn btn-accent btn-sm" onclick="applyAiCatMemory('${j.catId}','${(j.subcat||'').replace(/'/g,"\\'")}','${name.replace(/'/g,"\\'")}')">🧠 Zapamatovat</button>
        ${j.coicop&&cat&&!cat.coicop?`<button class="btn btn-ghost btn-sm" onclick="applyAiCoicop('${j.catId}',${j.coicop})">🧭 Přiřadit COICOP ${j.coicop} kategorii</button>`:''}
        <button class="btn btn-ghost btn-sm" onclick="applyAiCatAndAdd('${j.catId}','${(j.subcat||'').replace(/'/g,"\\'")}','${name.replace(/'/g,"\\'")}')">➕ Zapamatovat & přidat transakci</button>
      </div>
    </div>`;
  }catch(e){el.innerHTML=`<div style="color:var(--text3);font-size:.8rem">Nepodařilo se zpracovat odpověď. Zkus znovu.</div>`;}
}

// S12.1: AI navrhlo COICOP – ulož ho kategorii do Firebase (vlastní kategorie bez čísla)
function applyAiCoicop(catId, num){
  if(viewingUid) return;
  num = parseInt(num); if(!(num>=1&&num<=13)) return;
  const c = S.categories.find(x=>x.id===catId); if(!c) return;
  c.coicop = num; save();
  if(typeof showToast==='function') showToast(`🧭 Kategorie ${c.name} → COICOP ${num}`);
  else alert(`🧭 Kategorie ${c.name} → COICOP ${num}`);
}

// Jen uloží mapování – NEotevírá modal
function applyAiCatMemory(catId, subcat, txName){
  if(txName) saveCategoryMapping(txName, catId, subcat||'');
  const el=document.getElementById('aiCatResult');
  if(el){
    const cat=S.categories.find(c=>c.id===catId);
    el.innerHTML+=`<div style="margin-top:8px;font-size:.76rem;color:var(--income)">✅ Zapamatováno → ${cat?.name||catId}</div>`;
  }
}

// Uloží mapování A otevře modal přidat transakci
function applyAiCatAndAdd(catId, subcat, txName){
  if(txName) saveCategoryMapping(txName, catId, subcat||'');
  selCatId=catId; selSub=subcat||'';
  openAddTx();
  setTimeout(()=>{ renderCatPicker(); },100);
}

// Legacy alias (pro případ volání z cache UI)
function applyAiCat(catId, subcat, txName){
  applyAiCatAndAdd(catId, subcat, txName);
}
