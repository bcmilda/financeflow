// FinanceFlow · v9.94 · projects.js · 2026-08-19
//  PROJEKTY
// ══════════════════════════════════════════════════════
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;align-items:end;min-height:${chartH+52}px">
              ${bars.map(b=>{
                ...
                return `<div style="display:flex;flex-direction:column;align-items:center;gap:5px">
                  <div style="font-family:Syne;font-size:.82rem;font-weight:800;color:${b.color}">${b.val>=0?'':'−'}${fmtB(Math.abs(b.val))}</div>
  ...
  if(typeof fxLossSummary==='function'){
    try{
      ...
      if(fx.n>=3 && fx.loss>200){
        const best=fx.pays.filter(x=>x.n>=2).sort((a,b)=>a.pct-b.pct)[0];
        const worst=fx.pays.filter(x=>x.n>=2).sort((a,b)=>b.pct-a.pct)[0];
        const rozpad=fx.pays.filter(x=>x.n>=1).slice(0,4)
          .map(x=>`${x.label} ${x.pct>=0?'+':''}${x.pct.toFixed(1)} % (${x.n}×)`).join(' · ');
        let tip=`Za rok jsi na kurzu a poplatcích nechal ${fmtB(Math.round(fx.loss))} z ${fmtB(Math.round(fx.spentCZK))} zaplacených v cizí měně. `;
        tip+=rozpad?`Podle způsobu platby: ${rozpad}. `:'';
        if(best&&worst&&worst.key!==best.key&&(worst.pct-best.pct)>1.5){
          tip+=`Rozdíl mezi „${worst.label}" a „${best.label}" je ${(worst.pct-best.pct).toFixed(1)} procentního bodu — kdybys platil jako u toho levnějšího, zůstalo by ti víc.`;
        } else {
          tip+='Přirážku srovnej s podmínkami své banky – u karet bývá 0–2 %, u výběrů v bankomatu a směny na přepážce výrazně víc.';
        }
        if(fx.missing) tip+=` (${fx.missing} starších plateb v cizí měně nemá uložený referenční kurz, do součtu nevstupují.)`;
        suggestions.push({
          category:'💱 Kurzy a poplatky',
          item:'Platby v cizí měně',
          current:`${fmtB(Math.round(fx.loss))}/rok · ${fx.pct>=0?'+':''}${fx.pct.toFixed(1)} % nad kurz ČNB`,
