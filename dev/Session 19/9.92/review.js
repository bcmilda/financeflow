// FinanceFlow · v9.92 · review.js · 2026-08-19
// ══════════════════════════════════════════════════════
      ...
      <div class="card-body">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:9px;margin-bottom:12px">
          ...
          <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:10px;min-width:0">
            ...
            <div style="font-size:.63rem;color:#8b93ad;margin-top:2px">${avgRating === null ? 'zatím nic neohodnoceno' : 'z 5 · nízko hodnoceno ' + fmt(Math.round(lowSum)) + ' Kč'}</div>
          </div>
        </div>
...
//
//  ⚠️ DENNÍ DOBA CHYBÍ – a to záměrně neobcházím.
//  Plán slibuje „nákupy po 22:00 hodnotíš 2,1". Jenže transakce nesou pouze
//  datum ve tvaru YYYY-MM-DD, žádný čas. Neukládá ho ani ruční zápis, ani import
//  z banky, ani parser účtenek. Prohledáno celé úložiště – čas nikde není.
//  Dopočítat ho nelze a použít čas ZÁPISU by bylo zavádějící: kdo zadává dávkově
//  večer, vyrobil by si vzorec „večer utrácím špatně", který v realitě neexistuje.
//  Sběr času proto začíná od v9.92 (t.enteredAt) a vzorec se zapne, až bude
//  dost záznamů zadaných v týž den, kdy se nakupovalo – viz revTimePatternReady().
//
...
// Je už dost transakcí zadaných v den nákupu, aby dávala smysl denní doba?
function revTimePatternReady(D) {
  D = D || getData();
  let ok = 0;
  (D.transactions || []).forEach(t => {
    if (!t || !t.enteredAt || !t.date || !parseInt(t.priority)) return;
    const zapsano = new Date(t.enteredAt);
    if (isNaN(zapsano)) return;
    const den = `${zapsano.getFullYear()}-${String(zapsano.getMonth() + 1).padStart(2, '0')}-${String(zapsano.getDate()).padStart(2, '0')}`;
    if (den === String(t.date).slice(0, 10)) ok++;
  });
  return { ready: ok >= 20, n: ok };
}

function revPatternsHTML(m, y) {
  let p, tp;
  try { p = revPatterns(m, y, 6); tp = revTimePatternReady(); }
  catch (e) { return ''; }
  ...
  return `<div class="denik-book" style="margin-top:14px"><div style="padding:16px 18px">
    ...
    <div style="font-size:.72rem;color:#b09f82;line-height:1.6;margin-top:11px;padding-top:9px;border-top:1px solid rgba(138,106,62,.35)">
      <b>Denní doba zatím chybí.</b> Transakce nesly jen datum, ne čas — od v9.92 se čas zápisu
      ukládá, ale použije se, jen když zapisuješ v den nákupu (jinak by vznikl vzorec
      „večer utrácím špatně\u201c u každého, kdo zadává dávkově).
      ${tp.ready ? 'Podmínka je splněna, vzorec přibude.' : `Zatím ${tp.n} z 20 potřebných záznamů.`}
      <br>Vzorec se ukazuje jen při ${REV_PAT_MIN_N}+ útratách v koši a rozdílu ${REV_PAT_MIN_DIFF}+ bodu — jinak jde o šum.
    </div>
  </div></div>`;
}
