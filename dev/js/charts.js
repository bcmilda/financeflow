...
async function importWishFromUrl() {
  ...
  try {
    ...
    if(parsed.name) {
      ...
      if(parsed.price && Number(parsed.price)>0) moneyInFill('wishPrice', parsed.price);   // TODO-216
...
function wishSetType(type){
  ...
  if(type==='goal'){
    ...
    document.getElementById('wishPriceLabel').textContent='Cílová částka ('+((typeof curSym==='function')?curSym():'Kč')+')';
  } else {
    document.getElementById('wishNameLabel').textContent='Název předmětu';
    document.getElementById('wishPriceLabel').textContent='Odhadovaná cena ('+((typeof curSym==='function')?curSym():'Kč')+')';
  }
...
function wishUpdateEstimate(){
  ...
  const target=moneyInRead('wishPrice');   // TODO-216
  const monthly=moneyInRead('wishMonthly');   // TODO-216
...
function editWish(id){
  ...
  moneyInFill('wishPrice', w.isGoal ? w.targetAmount : w.price);   // TODO-216
  moneyInFill('wishMonthly', w.monthlyTarget);   // TODO-216
...
function saveWish(){
  ...
  const amount=moneyInRead('wishPrice');   // TODO-216
  const monthlyTarget=moneyInRead('wishMonthly');   // TODO-216
...
//   Inspirace referenčním Excelem: mřížka mini-grafů, jeden na kategorii, den 1–31.
