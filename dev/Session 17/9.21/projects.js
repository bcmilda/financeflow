// FinanceFlow · v9.21 · projects.js · 2026-07-25
//  PROJEKTY
// ══════════════════════════════════════════════════════
      ...
      <div class="card-body">
        ...
        ${(() => {
          ...
          <div style="position:relative">
            ...
            <!-- S17.2 (Milan): rámečky nekazí graf – desktop VEDLE sloupce (první dva vpravo od bodu,
                 poslední dva vlevo, ať nepřetečou); mobil uvnitř grafu: nad bodem, u bodů u stropu pod bodem -->
            <div style="position:absolute;top:18px;left:0;width:100%;height:${chartH+8}px;pointer-events:none">
              ${pts.map((p,i)=>{
                ...
                const mob = (typeof window!=='undefined' && window.matchMedia) ? window.matchMedia('(max-width:640px)').matches : false;
                let posStyle;
                if(mob){
                  posStyle = p.cy<28
                    ? `left:${p.cx}%;top:${p.cy}px;transform:translate(-50%,35%)`
                    : `left:${p.cx}%;top:${p.cy}px;transform:translate(-50%,-140%)`;
                } else {
                  posStyle = i<2
                    ? `left:calc(${p.cx}% + 12px);top:${p.cy}px;transform:translateY(-50%)`
                    : `left:calc(${p.cx}% - 12px);top:${p.cy}px;transform:translate(-100%,-50%)`;
                }
                return `<div style="position:absolute;${posStyle};white-space:nowrap;text-align:center">
