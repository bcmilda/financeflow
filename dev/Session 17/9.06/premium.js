// FinanceFlow · v9.06 · premium.js · 2026-07-21
//  PREMIUM SYSTEM
// ══════════════════════════════════════════════════════
...
// S17.7 (FIX-210, Milan): DŘÍV se transakce vytvořila JEN když se uživatel trefil na přesný
// den splatnosti (today.getDate()===den). Kdo appku ten den neotevřel, výskyt propadl a už se
// nikdy nedohnal – šablona na 24. přidaná 21. se v červenci nevytvořila vůbec. Nově catch-up:
// projdeme všechny výskyty za posledních LOOKBACK dní a doplníme ty, které chybí (idempotentně).
function processAutoSablony() {
  if(!S.sablony)return;
  if(typeof viewingUid!=='undefined' && viewingUid) return;   // ne při prohlížení partnera
  const LOOKBACK=35;                                           // dohnat max ~měsíc zpět
  const today=new Date(); today.setHours(0,0,0,0);
  const from=new Date(today.getTime()-LOOKBACK*86400000);
  const iso=d=>d.toISOString().slice(0,10);
  const todayStr=iso(today);
  S.transactions=S.transactions||[];
  let added=0;

  // Vrátí seznam SPLATNÝCH dat (od 'from' do 'today' včetně) pro danou frekvenci+den.
  const dueDates=(freq,den)=>{
    const out=[];
    if(freq==='weekly'||freq==='biweekly'){
      const step=freq==='weekly'?7:14;
      // zarovnat na krok vůči dnešku směrem do minulosti
      let cur=new Date(today);
      while(cur>from){ if(cur<=today) out.push(iso(cur)); cur=new Date(cur.getTime()-step*86400000); }
    } else if(freq==='quarterly'||freq==='yearly'){
      const back=freq==='yearly'?[0,-1]:[0,-1];  // stačí projít pár období zpět v rámci 35 dní
      const monthsBack=freq==='yearly'?12:3;
      for(let k=0;k<=1;k++){
        const base=new Date(today.getFullYear(), today.getMonth()-monthsBack*k, den); base.setHours(0,0,0,0);
        if(base<=today && base>=from) out.push(iso(base));
      }
    } else { // monthly (default)
      // tento a minulý měsíc – pokrývá celé okno 35 dní
      for(let k=0;k<=1;k++){
        const d=new Date(today.getFullYear(), today.getMonth()-k, den); d.setHours(0,0,0,0);
        if(d<=today && d>=from) out.push(iso(d));
      }
    }
    return out;
  };
  ...
  S.sablony.filter(s=>s.auto).forEach(s=>{
    const den=s.den||1;
    dueDates(s.freq||'monthly', den).forEach(dateStr=>{
      if(s.endDate && s.endDate < dateStr) return;
      // idempotence: existuje už auto-transakce téže šablony k tomuto datu?
      const exists=(S.transactions).some(t=>t.date===dateStr && t.name===s.name && t.note && t.note.includes('Auto-šablona'));
      if(exists) return;
      if(s.type==='transfer'){
        const transferId=uid();
        S.transactions.push(
        ...
        if(s.debtId) tx.debtId=s.debtId;   // S17.7: splátka dluhu ze šablony se sváže s dluhem
  ...
  if(added>0){ save(); if(typeof renderPage==='function') renderPage();
    if(typeof showToast==='function') showToast(`🔄 Doplněno ${added} plateb ze šablon`); }
}
    ...
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
      ...
      <div class="fscore-ring" style="flex-shrink:0">
        ...
        <div class="fscore-number">
          ...
          <div style="font-size:.64rem;color:#a8aec8">/ ${sc.rawMax}</div>
        </div>
      </div>
      <!-- Info -->
