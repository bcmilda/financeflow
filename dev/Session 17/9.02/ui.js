// FinanceFlow · v9.02 · ui.js · 2026-07-19
//  RENDER ROUTER
// ══════════════════════════════════════════════════════
...
function renderSouhrn(tableOnly){
  ...
  let totalPred=0;
  let html=`<table class="stat-table" style="width:100%"><thead><tr><th>Kategorie</th><th>Minulý měsíc</th><th>Tento měsíc</th><th>🔮 Predikce</th><th>Změna</th><th>Podíl</th></tr></thead><tbody>`;  // S16.14 (Milan): minulý PRVNÍ (čtení zleva doprava v čase)
  expCats.forEach(cat=>{
    ...
    html+=`<tr><td><span style="margin-right:5px">${cat.icon}</span>${cat.name}</td><td style="color:#a8aec8">${prev?fmt(prev):'–'}</td><td>${cur?fmt(cur):'–'}</td>${predCell(pc,cur)}<td>${pct!==null?`<span class="pct-pill ${pct>5?'pct-up':pct<-5?'pct-dn':'pct-neu'}">${pct>0?'+':''}${pct}%</span>`:'–'}</td><td>${pctCelku}%</td></tr>`;
    (cat.subs||[]).forEach(sub=>{
      ...
      html+=`<tr style="font-size:.78rem;color:var(--text2)"><td style="padding-left:20px">↳ ${sub}</td><td style="color:#a8aec8">${sp?fmt(sp):'–'}</td><td>${sc?fmt(sc):'–'}</td>${predCell(sPc,sc)}<td>${spct!==null?`<span class="pct-pill ${spct>5?'pct-up':spct<-5?'pct-dn':'pct-neu'}">${spct>0?'+':''}${spct}%</span>`:'–'}</td><td></td></tr>`;
    });
  });
...
function renderSuhrnReport(expCats,totalCur,totalPrev,pm,py,D,targetId){
  ...
  const tile=(x,goodTile)=>{
    ...
    const pctTxt=goodTile?`−${Math.abs(x.pct)}%`:`+${x.pct}%`;
    const amtTxt=goodTile?`ušetřeno <strong style="color:${accCol}">${fmt(x.saved)}</strong>`:`navíc <strong style="color:${accCol}">${fmt(x.over)}</strong>`;
