// FinanceFlow · v9.39 · premium.js · 2026-07-31
//  PREMIUM SYSTEM
// ══════════════════════════════════════════════════════
...
function updatePaywallCtas() {
  ...
  if (canTrial) {
    // trial k dispozici → hlavní akce trial, ale ať jde zaplatit i rovnou
    cta.textContent = 'Vyzkoušet 30 dní zdarma';
    cta.onclick = () => startTrial();
    if (sub) sub.textContent = '✓ Bez karty · bez závazků';
    if (buy) { buy.style.display = 'block'; buy.textContent = 'Předplatit hned (bez čekání)'; }
  } else {
    // trial běží nebo je vyčerpaný → jediná smysluplná akce je platba
    cta.textContent = isTrial ? 'Předplatit Premium' : 'Získat Premium';
    cta.onclick = () => goPremium();
    if (sub) sub.textContent = isTrial
      ? `✓ Trial běží ještě ${st.daysLeft} dní – předplatné na něj naváže`
      : '✓ Zrušíš kdykoli';
    if (buy) buy.style.display = 'none';
  }
}
