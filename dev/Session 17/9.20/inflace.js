// FinanceFlow · v9.20 · inflace.js · 2026-07-25
// ══════════════════════════════════════════════════════
        ...
        <div class="card-body" style="overflow-x:auto">
          <table class="stat-table" style="width:100%;min-width:620px;font-size:.76rem">
            ...
            <tbody>${srows.map(r=>`<tr>
              ...
              <td style="text-align:right;color:#a8aec8">${fmt(Math.round(r.first))}</td>
              <td style="text-align:right;font-weight:700;color:${r.last===cheapest?'var(--income)':'#e8eaf2'}">${fmt(Math.round(r.last))}</td>
              <td style="text-align:right;color:#c9cede">${r.normAvg!=null?fmt(Math.round(r.normAvg)):'–'}</td>
              <td style="text-align:right;color:${r.discAvg!=null?'var(--debt)':'#5a6078'}">${r.discAvg!=null?fmt(Math.round(r.discAvg)):'–'}</td>
              <td style="text-align:right;color:#a8aec8">${r.perKgLast!=null?fmt(Math.round(r.perKgLast))+' Kč/'+(r.perKgUnit||'kg'):'–'}</td>
        ...
        <tbody>${sorted.map(r => `<tr>
          ...
          <td style="text-align:right">${(()=>{
            ...
            return `<div style="font-weight:700;color:${col(p)};white-space:nowrap">${p>0?'+':''}${p} %</div>
              <div style="font-size:.63rem;color:#a8aec8;white-space:nowrap">${fmt(Math.round(l))} Kč/${u}</div>
