// FinanceFlow · v8.83 · projects.js · 2026-07-08
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function _obrazProjChart(proj){
  ...
  const W=640,H=210,pad={l:56,r:12,t:16,b:30};
  const cW=W-pad.l-pad.r,cH=H-pad.t-pad.b;
  const vMax=Math.max(proj.avgInc,...rows.map(r=>Math.max(r.exp,r.bud||0)),1);
  const vMin=Math.min(0,...rows.map(r=>r.cash));
  const span=Math.max(1,vMax-vMin);
  const y=v=>pad.t+cH*(1-(v-vMin)/span);
  const slot=cW/rows.length, bw=Math.min(16,slot/4.6);
  let g='';
  // grid + osa Y
  ...
  // sloupce: příjem | výdaje | budoucí platby
  rows.forEach((r,i)=>{
    const cx=pad.l+i*slot+slot/2;
    const bars=[[proj.avgInc,'#4ade80'],[r.exp,'#f87171'],[r.bud||0,'#a78bfa']];
    bars.forEach(([v,col],bi)=>{
      if(!(v>0)) return;
      const x=cx+(bi-1)*(bw+2)-bw/2;
      const rY=y(v), rH=Math.max(y(0)-rY,1.5);
      g+=`<rect x="${x.toFixed(1)}" y="${rY.toFixed(1)}" width="${bw.toFixed(1)}" height="${rH.toFixed(1)}" rx="2.5" fill="${col}" opacity=".82"></rect>`;
    });
    // neviditelný hover pás přes celý měsíc
    const tip=`<b>${r.label} ${r.y}</b><br>Příjem Ø ${fmtB(proj.avgInc)}<br>Výdaje (predikce) ${fmtB(r.exp)}<br>Budoucí platby ${r.bud?fmtB(r.bud):'–'}<br>Cashflow ${r.cash>=0?'+':''}${fmtB(r.cash)}<br>Rezerva ${fmtB(r.reserve)}`;
    g+=`<rect x="${(pad.l+i*slot).toFixed(1)}" y="${pad.t}" width="${slot.toFixed(1)}" height="${cH}" fill="transparent" style="cursor:pointer"
      onmouseenter="_obrazTip(event,'${tip}')" onmouseleave="_obrazTipHide()" ontouchstart="_obrazTip(event,'${tip}')"></rect>`;
    g+=`<text x="${cx.toFixed(1)}" y="${H-6}" text-anchor="middle" font-size="9.5" fill="#e8eaf2" font-weight="600">${r.label}</text>`;
  });
  // cashflow čára s body
  const pts=rows.map((r,i)=>`${(pad.l+i*slot+slot/2).toFixed(1)},${y(r.cash).toFixed(1)}`).join(' ');
  g+=`<polyline points="${pts}" fill="none" stroke="#60a5fa" stroke-width="2.2" stroke-linejoin="round" stroke-linecap="round"/>`;
  rows.forEach((r,i)=>{
    g+=`<circle cx="${(pad.l+i*slot+slot/2).toFixed(1)}" cy="${y(r.cash).toFixed(1)}" r="3.5" fill="#60a5fa"></circle>`;
    g+=`<text x="${(pad.l+i*slot+slot/2).toFixed(1)}" y="${(y(r.cash)-7).toFixed(1)}" text-anchor="middle" font-size="8.5" fill="#93b8f5" font-weight="700">${r.cash>=0?'+':''}${_obrazK(r.cash)}</text>`;
  });
  const leg=`<div style="display:flex;gap:14px;justify-content:center;margin-top:6px;flex-wrap:wrap;font-size:.74rem">
    <span style="display:flex;align-items:center;gap:5px;color:#4ade80"><span style="width:12px;height:9px;background:#4ade80;border-radius:2px;display:inline-block"></span>Příjem Ø</span>
    <span style="display:flex;align-items:center;gap:5px;color:#f87171"><span style="width:12px;height:9px;background:#f87171;border-radius:2px;display:inline-block"></span>Výdaje (predikce)</span>
    <span style="display:flex;align-items:center;gap:5px;color:#a78bfa"><span style="width:12px;height:9px;background:#a78bfa;border-radius:2px;display:inline-block"></span>Budoucí platby</span>
    <span style="display:flex;align-items:center;gap:5px;color:#93b8f5"><span style="width:16px;height:2.5px;background:#60a5fa;display:inline-block"></span>Cashflow</span>
  </div>`;
  return `<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:${W}px;height:auto;display:block;margin-top:10px">${g}</svg>${leg}`;
}
...
function renderObraz() {
  ...
  // ── S16 (TODO-171): TREND = SMĚRNICE lineární regrese přes všech 6 měsíců (Kč/měs). ──
  //   Milanův nápad „sečíst meziměsíční rozdíly" se teleskopicky zkrátí na (poslední − první),
  //   tj. totéž co dřívější výpočet – prostřední měsíce ignoruje. Regrese využívá všechny body.
  //   Procentní first/last trendy (incTrend…) zůstávají jen pro interní skóre nahoře.
  const _slope=vals=>{ const pts=vals.map((v,i)=>[i,v]).filter(pt=>pt[1]!==null&&!isNaN(pt[1])); const n=pts.length; if(n<2)return 0;
    const sx=pts.reduce((a,pt)=>a+pt[0],0), sy=pts.reduce((a,pt)=>a+pt[1],0);
    const sxx=pts.reduce((a,pt)=>a+pt[0]*pt[0],0), sxy=pts.reduce((a,pt)=>a+pt[0]*pt[1],0);
    const dd=n*sxx-sx*sx; return dd? (n*sxy-sx*sy)/dd : 0; };
  const slInc=Math.round(_slope(series.map(x=>x.inc)));
  const slExp=Math.round(_slope(series.map(x=>x.exp)));
  const slMom=Math.round(_slope(series.map(x=>x.savings)));
  const slDebt=Math.round(_slope(series.map(x=>x.debt)));
  // S16.2: |směrnice| pod prahem (5 % průměrné úrovně, min 500 Kč) = „stabilní".
  //   Řeší Milanův zmatek 11 994 vs −467: Ø = ÚROVEŇ (kolik měsíčně přibývá),
  //   směrnice = TREND té řady (zrychluje/zpomaluje). Obojí platí zároveň.
  const _mean=vals=>{const v=vals.filter(x=>x!==null&&!isNaN(x));return v.length?v.reduce((a,b)=>a+Math.abs(b),0)/v.length:0;};
  const _tt=(sl,base)=>{ const thr=Math.max(500, base*0.05);
    return Math.abs(sl)<thr ? '↔ stabilní' : `${sl>0?'↑':'↓'} ${fmtB(Math.abs(sl))}/měs`; };
  // S16 (TODO-171): Úspory (saldo aktuálního měsíce) nahrazeny Wealth Momentum (Ø saldo 6 měs.)
  const metrics = [
    {label:'💰 Příjmy',   val:fmtB(last.inc),  trendTxt:_tt(slInc,_mean(series.map(x=>x.inc))),  good:slInc>=0},
    {label:'💸 Výdaje',   val:fmtB(last.exp),  trendTxt:_tt(slExp,_mean(series.map(x=>x.exp))),  good:slExp<=0},
    {label:'🚀 Momentum', val:`${momentum.perMonth>=0?'+':''}${fmtB(momentum.perMonth)}/měs`, sub:`tento měsíc ${last.savings>=0?'+':''}${fmtB(last.savings)}`, trendTxt:_tt(slMom,_mean(series.map(x=>x.savings))), good:slMom>=0},
    {label:'🏦 Dluhy',    val:fmtB(last.debt), trendTxt:_tt(slDebt,_mean(series.map(x=>x.debt))), good:slDebt<=0},
  ];
  ...
  const smerujCard = proj.hasData ? `
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:14px">
        ...
        <div style="font-size:.68rem;color:var(--text3);margin-top:8px;padding:7px 9px;background:var(--surface3);border-radius:7px">ℹ️ Orientační predikce: příjem = 12M klouzavý průměr; výdaje = engine karty Predikce (historie kategorií + sezónnost + narozeniny); cashflow = příjem − predikce výdajů; rezerva = dnešní hotovost + kumulovaný cashflow (v tooltipu). Budoucí platby = ZNÁMÉ šablony a splátky (informativní – opakované už jsou v predikci, proto se neodečítají dvakrát). Dluh = rovnoměrné umořování dle splátek. Najeď na měsíc pro všechny hodnoty.</div>
      </div>
    </div>` : '';
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:14px">
        ...
        ${(()=>{ // S16 (TODO-172): tabulka 1.–5. týden od výplaty + trend mezi cykly
          ...
          const cell=(v,prev)=>{
            if(v===undefined) return '<td style="padding:5px 6px;text-align:right;color:var(--text3)">–</td>';
            let col='#c9cede', arr='';
        ...
        <div class="card">
          <div class="card-body" style="padding:12px">
            ...
            <div style="font-size:.76rem;margin-top:4px;color:${m.good?'var(--income)':'var(--expense)'}">
              ${m.trendTxt} <span style="font-size:.66rem;color:#a8aec8">(regrese 6 měs.)</span>
              ${m.good?'✅':'⚠️'}
            </div>
          </div>
        </div>
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:0">
        <div style="display:grid;grid-template-columns:48px 1fr 1fr 1fr 1fr;font-size:.66rem;font-weight:700;color:var(--text3);text-transform:uppercase;padding:8px 10px;background:var(--surface3)">
          <span>Měsíc</span><span style="text-align:right">Příjmy</span><span style="text-align:right">Výdaje</span><span style="text-align:right">Momentum</span><span style="text-align:right">Dluh</span>
        </div>
        ${series.map((s,i)=>`
        ...
        ${(()=>{ // v8.74: sumář období – součty + průměry
          ...
          <div style="display:grid;grid-template-columns:48px 1fr 1fr 1fr 1fr;padding:7px 10px;font-size:.7rem;font-weight:700;border-top:1px solid var(--border)" title="Směrnice lineární regrese přes 6 měsíců – skutečný trend, ne jen první vs poslední měsíc">
            <span style="color:#a8aec8">Trend/měs</span>
            <span style="text-align:right;color:${slInc>=0?'var(--income)':'var(--expense)'}">${slInc>=0?'↑+':'↓'}${fmt(Math.abs(slInc))}</span>
            <span style="text-align:right;color:${slExp<=0?'var(--income)':'var(--expense)'}">${slExp>=0?'↑+':'↓'}${fmt(Math.abs(slExp))}</span>
            <span style="text-align:right;color:${slMom>=0?'var(--income)':'var(--expense)'}">${slMom>=0?'↑+':'↓'}${fmt(Math.abs(slMom))}</span>
            <span style="text-align:right;color:${slDebt<=0?'var(--income)':'var(--expense)'}">${slDebt>=0?'↑+':'↓'}${fmt(Math.abs(slDebt))}</span>
          </div>`;
        })()}
      </div>
    </div>`;
...
function denikNav(d){
  if(window._denikM===undefined){ window._denikM=S.curMonth; window._denikY=S.curYear; }
  window._denikM+=d;
  if(window._denikM<0){ window._denikM=11; window._denikY--; }
  if(window._denikM>11){ window._denikM=0; window._denikY++; }
  renderDenik();
}
...
function _denikPredCurve(D, m, y, predExp){
  ...
  if(!used){ // fallback: lineární
    return Array.from({length:days},(_,i)=>Math.round(predExp*(i+1)/days));
  }
...
function denikSnapshot(){
  const D=getData();
  const m=S.curMonth, y=S.curYear; // snímek vždy pro AKTUÁLNÍ reálný měsíc (skóre/predikce jsou k „teď")
  const key=_denikKey(y,m);
  if(!S.diary) S.diary={};
  if(S.diary[key] && !confirm('Snímek pro tento měsíc už existuje. Přepsat novým?')) return;
  const avgInc=(typeof computeEffectiveIncome==='function')?computeEffectiveIncome(D,12):0;
  // predikce výdajů = engine karty Predikce (predictCat)
  let predExp=0,hit=0;
  ...
  // známé budoucí platby do konce měsíce
  let predBud=0;
  if(typeof budouciGetAll==='function'){
  ...
  save();
  renderDenik();
  if(typeof showToast==='function') showToast('📸 Snímek predikce uložen');
}
...
// Graf den po dni: skutečnost (zelená, plocha) vs predikce (fialová přerušovaná)
function _denikDayChart(days, actual, pred, todayD){
  const W=640,H=200,pad={l:56,r:12,t:16,b:28};
  const cW=W-pad.l-pad.r,cH=H-pad.t-pad.b;
  const vMax=Math.max(...actual,...(pred||[0]),1);
  const xf=i=>pad.l+(i+0.5)*(cW/days);
  const yf=v=>pad.t+cH*(1-v/vMax);
  let g='';
  [vMax,vMax/2,0].forEach(v=>{
    g+=`<line x1="${pad.l}" y1="${yf(v)}" x2="${W-pad.r}" y2="${yf(v)}" stroke="rgba(168,174,200,${v===0?'.5':'.18'})" stroke-width="1" ${v!==0?'stroke-dasharray="3,3"':''}/>`;
    g+=`<text x="${pad.l-6}" y="${yf(v)+3.5}" text-anchor="end" font-size="9.5" fill="#a8aec8">${_obrazK(v)}</text>`;
  });
  // predikce – fialová přerušovaná
  if(pred&&pred.length){
    const pts=pred.slice(0,days).map((v,i)=>`${xf(i).toFixed(1)},${yf(v).toFixed(1)}`).join(' ');
    g+=`<polyline points="${pts}" fill="none" stroke="#a78bfa" stroke-width="2" stroke-dasharray="6,4"/>`;
  }
  // skutečnost – zelená s plochou (jen do dneška, pokud aktuální měsíc)
  const lastIdx=todayD?Math.min(todayD,days):days;
  const act=actual.slice(0,lastIdx);
  if(act.length){
    let area=`M ${xf(0).toFixed(1)} ${yf(0).toFixed(1)} `;
    act.forEach((v,i)=>{ area+=`L ${xf(i).toFixed(1)} ${yf(v).toFixed(1)} `; });
    area+=`L ${xf(act.length-1).toFixed(1)} ${yf(0).toFixed(1)} Z`;
    g+=`<path d="${area}" fill="rgba(74,222,128,.18)"></path>`;
    const pts=act.map((v,i)=>`${xf(i).toFixed(1)},${yf(v).toFixed(1)}`).join(' ');
    g+=`<polyline points="${pts}" fill="none" stroke="#4ade80" stroke-width="2.5" stroke-linejoin="round"/>`;
  }
  // osa X po 2 dnech + tooltipy přes pásy
  for(let d2=1;d2<=days;d2+=2){ g+=`<text x="${xf(d2-1).toFixed(1)}" y="${H-6}" text-anchor="middle" font-size="10" fill="#a8aec8">${d2}</text>`; }
  if(days%2===0) g+=`<text x="${xf(days-1).toFixed(1)}" y="${H-6}" text-anchor="middle" font-size="10" fill="#a8aec8">${days}</text>`;
  for(let i=0;i<days;i++){
    const a=i<lastIdx?actual[i]:null, p=pred&&i<pred.length?pred[i]:null;
    const diff=(a!==null&&p)?Math.round((a-p)/p*100):null;
    const tip=`<b>${i+1}. den</b>`+(a!==null?`<br>Skutečnost ${fmtB(a)}`:'')+(p!==null?`<br>Predikce ${fmtB(p)}`:'')+(diff!==null?`<br>${diff>=0?'+':''}${diff} % vs predikce`:'');
...
function renderDenik(){
  ...
  const D=getData();
  if(window._denikM===undefined){ window._denikM=S.curMonth; window._denikY=S.curYear; }
  const m=window._denikM, y=window._denikY;
  const key=_denikKey(y,m);
  ...
  txs.forEach(t=>{ if(t.type!=='expense'||t.isBalancing||t.splitParent||isTransferTx(t))return; const di=new Date(t.date).getDate()-1; if(di>=0&&di<days) arr[di]+=txCZK(t,D); });
  let sm=0; const actCumul=arr.map(v=>{sm+=v;return Math.round(sm);});

  const dRow=(label,val,color)=>`<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px dashed rgba(168,174,200,.18);font-size:.8rem">
    <span style="color:#a8aec8">${label}</span><span style="font-weight:700;color:${color||'var(--text)'}">${val}</span></div>`;
  const diffTag=(act,pr,lowerBetter)=>{
    if(pr===null||pr===undefined||!pr) return '';
    const d=Math.round((act-pr)/Math.abs(pr)*100);
    const good=lowerBetter?d<=0:d>=0;
    return ` <span style="font-size:.68rem;font-weight:700;color:${good?'var(--income)':'var(--expense)'}">(${d>=0?'+':''}${d} %)</span>`;
  };

  // Levá stránka – predikce (snímek)
  const leftBody = snap ? `
    ${dRow('Zapsáno', new Date(snap.createdAt).toLocaleDateString('cs-CZ'))}
    ${dRow('Očekávaný příjem', fmtB(snap.predInc), 'var(--income)')}
    ${dRow('Predikce výdajů', fmtB(snap.predExp), 'var(--expense)')}
    ${dRow('Známé budoucí platby', snap.predBud?fmtB(snap.predBud):'–', '#a78bfa')}
    ${dRow('Očekávané saldo', `${snap.predInc-snap.predExp>=0?'+':''}${fmtB(snap.predInc-snap.predExp)}`, snap.predInc-snap.predExp>=0?'var(--income)':'var(--expense)')}
    ${dRow('Dluh (k datu snímku)', fmtB(snap.debtNow), 'var(--debt)')}
    ${dRow('Hotovost (k datu snímku)', fmtB(snap.wallets))}
    ${snap.scoreRaw!==null&&snap.scoreRaw!==undefined?dRow('Finanční skóre', `${snap.scoreRaw}/${snap.scoreMax}`):''}
    <div style="text-align:right;margin-top:8px"><button onclick="denikDeleteSnap('${key}')" style="background:none;border:1px solid var(--border);border-radius:8px;color:#f87171;font-size:.68rem;padding:4px 9px;cursor:pointer">🗑 Smazat snímek</button></div>
  ` : `
    <div style="color:#a8aec8;font-size:.8rem;line-height:1.6;padding:8px 0">Pro tento měsíc není zaznamenaná predikce.${isCurM?'<br><br>Zaznamenej ji hned – snímek je neměnný fakt „co jsme čekali", zatímco skutečnost se dopočítává živě z transakcí (zpětné úpravy se propíšou samy).':''}</div>
    ${isCurM?`<button onclick="denikSnapshot()" style="width:100%;padding:10px;border-radius:9px;border:none;background:#8b7cf6;color:#fff;font-weight:700;font-size:.84rem;cursor:pointer">📸 Zaznamenat predikci (${CZ_M[m]})</button>`:''}
  `;

  // Pravá stránka – skutečnost (živě)
  const rightBody = `
    ${dRow('Příjmy', fmtB(inc)+(snap?diffTag(inc,snap.predInc,false):''), 'var(--income)')}
    ${dRow('Výdaje', fmtB(exp)+(snap?diffTag(exp,snap.predExp,true):''), 'var(--expense)')}
    ${dRow('Saldo', `${saldo>=0?'+':''}${fmtB(saldo)}`+(snap?diffTag(saldo,snap.predInc-snap.predExp,false):''), saldo>=0?'var(--income)':'var(--expense)')}
    ${dRow('Transakcí', txs.filter(t=>!t.splitParent).length)}
    ${todayD?dRow('Den v měsíci', `${todayD}. / ${days}`):''}
    ${snap&&todayD?dRow('Tempo vs predikce', (()=>{ const pi=Math.min(todayD,snap.predCurve.length)-1; const pv=snap.predCurve[pi]||0; const av=actCumul[Math.min(todayD,days)-1]||0; if(!pv) return '–'; const d2=Math.round((av-pv)/pv*100); return `<span style="color:${d2<=0?'var(--income)':'var(--expense)'}">${d2>=0?'+':''}${d2} %</span>`; })()):''}
  `;

  const page=(title,body,side)=>`
    <div style="background:linear-gradient(${side==='l'?'105deg':'255deg'},var(--surface2),var(--surface));border:1px solid var(--border);border-radius:${side==='l'?'14px 4px 4px 14px':'4px 14px 14px 4px'};padding:16px 18px;box-shadow:inset ${side==='l'?'-8px':'8px'} 0 14px -12px rgba(0,0,0,.55)">
      <div style="font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:#a8aec8;margin-bottom:10px;text-align:center">${title}</div>
      ${body}
    </div>`;

  el.innerHTML=`
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px">
      <div style="font-size:.74rem;color:#a8aec8">📖 Predikce vs. skutečnost · ukládá se do cloudu · zatím jen admin</div>
      <div style="display:flex;align-items:center;gap:6px">
        <button class="btn btn-ghost btn-sm" onclick="denikNav(-1)">‹</button>
        <span style="font-size:.86rem;font-weight:700;min-width:120px;text-align:center">${CZ_M[m]} ${y}</span>
        <button class="btn btn-ghost btn-sm" onclick="denikNav(1)">›</button>
      </div>
    </div>
    <div class="grid2" style="gap:6px;align-items:stretch;margin-bottom:16px">
      ${page(`📜 Predikce – ${CZ_M[m]}`, leftBody, 'l')}
      ${page('✅ Skutečnost (živě)', rightBody, 'r')}
    </div>
    <div class="card">
      <div class="card-header"><span class="card-title">📈 Den po dni – kumulativní výdaje vs predikce</span></div>
      <div class="card-body">
        ${_denikDayChart(days, actCumul, snap?snap.predCurve:null, todayD)}
        <div style="display:flex;gap:14px;justify-content:center;margin-top:6px;flex-wrap:wrap;font-size:.74rem">
          <span style="display:flex;align-items:center;gap:5px;color:#4ade80"><span style="width:16px;height:2.5px;background:#4ade80;display:inline-block"></span>Skutečnost</span>
          ${snap?'<span style="display:flex;align-items:center;gap:5px;color:#a78bfa"><span style="width:16px;height:0;border-top:2px dashed #a78bfa;display:inline-block"></span>Predikce (snímek)</span>':''}
        </div>
        ${!snap?'<div style="font-size:.7rem;color:var(--text3);text-align:center;margin-top:6px">Predikční křivka se zobrazí po zaznamenání snímku.</div>':''}
      </div>
    </div>`;
}
