// FinanceFlow · v8.96 · stats.js · 2026-07-15
//  STATISTIKY
// ══════════════════════════════════════════════════════
    ...
    if(_statCatMode==='year'){
      ...
      if(rows.length){
        ...
        let html=`<div style="overflow-x:auto;margin-bottom:14px"><table style="border-collapse:collapse;font-size:.7rem;min-width:100%;white-space:nowrap">
          <thead><tr style="color:var(--text3)">
            <th style="text-align:left;padding:5px 8px;position:sticky;left:0;background:var(--surface);z-index:1">Kategorie</th>
        ...
        rows.forEach(r=>{
          ...
          html+=`<tr style="border-top:1px solid var(--border)${hasSubs?';cursor:pointer':''}" ${hasSubs?`onclick="statToggleCat('${r.c.id}')"`:''}>
            ...
            ${cols.map(({m,y})=>{const v=getActual(r.c.id,null,m,y,D);return `<td style="text-align:right;padding:5px 6px;color:${v?'var(--text)':'var(--text3)'}">${v?fmt(v):'·'}</td>`;}).join('')}
            <td style="text-align:right;padding:5px 8px;font-weight:700;color:${r.c.color}">${fmt(r.total)}</td>
          </tr>`;
          ...
          if(expanded){
            subs.sort((a,b)=>b.total-a.total).forEach(s=>{
              html+=`<tr style="background:var(--surface2);font-size:.66rem">
                ...
                ${cols.map(({m,y})=>{const v=getActual(r.c.id,s.name,m,y,D);return `<td style="text-align:right;padding:4px 6px;color:${v?'var(--text2)':'var(--text3)'}">${v?fmt(v):'·'}</td>`;}).join('')}
                <td style="text-align:right;padding:4px 8px;color:var(--text2)">${fmt(s.total)}</td>
              </tr>`;
            });
          }
        });
        html+=`<tr style="border-top:2px solid var(--border2);font-weight:700">
          ...
          ${colTotals.map(v=>`<td style="text-align:right;padding:6px 6px">${v?fmt(v):'·'}</td>`).join('')}
          <td style="text-align:right;padding:6px 8px">${fmt(colTotals.reduce((a,b)=>a+b,0))}</td>
        </tr></tbody></table></div>`;
      ...
      if(rows.length&&yearsAll.length){
        ...
        rows.forEach(r=>{
          ...
          html+=`<tr style="border-top:1px solid var(--border)${hasSubs?';cursor:pointer':''}" ${hasSubs?`onclick="statToggleCat('${r.c.id}')"`:''}>
            <td style="text-align:left;padding:5px 8px;position:sticky;left:0;background:var(--surface);color:${r.c.color};font-weight:600">${hasSubs?`<span style="display:inline-block;width:12px;color:var(--text3)">${expanded?'▾':'▸'}</span>`:'<span style="display:inline-block;width:12px"></span>'}${r.c.icon||''} ${r.c.name}</td>
            ${yearsAll.map(y=>{const v=cell(y,r.c.id,null);return `<td style="text-align:right;padding:5px 6px;color:${v?'var(--text)':'var(--text3)'}">${v?fmt(v):'·'}</td>`;}).join('')}
            <td style="text-align:right;padding:5px 8px;font-weight:700;color:${r.c.color}">${fmt(r.total)}</td>
          </tr>`;
          if(expanded){
            subs.sort((a,b)=>b.total-a.total).forEach(s=>{
              html+=`<tr style="background:var(--surface2);font-size:.66rem">
                <td style="text-align:left;padding:4px 8px 4px 24px;position:sticky;left:0;background:var(--surface2);color:var(--text2)">${s.name}</td>
                ${yearsAll.map(y=>{const v=cell(y,r.c.id,s.name);return `<td style="text-align:right;padding:4px 6px;color:${v?'var(--text2)':'var(--text3)'}">${v?fmt(v):'·'}</td>`;}).join('')}
                <td style="text-align:right;padding:4px 8px;color:var(--text2)">${fmt(s.total)}</td>
              </tr>`;
            });
          }
...
function moveCatUp(id, ev){
  ...
  // S16.14 (FIX-205, Milan): guard byl ČASOVÝ (900 ms) → při pomalejším klikání vypršel
  //   a klik trefil jinou kategorii. Nyní POHYBOVÝ: platí, dokud uživatel nepohne myší.
  if(_catMoveGuard && _catMoveGuard.dir==='up' && _catMoveGuard.id!==id){
    id=_catMoveGuard.id;
  }
...
function moveCatDown(id, ev){
  ...
  // S16.14 (FIX-205, Milan): guard byl ČASOVÝ (900 ms) → při pomalejším klikání vypršel
  //   a klik trefil jinou kategorii. Nyní POHYBOVÝ: platí, dokud uživatel nepohne myší.
  if(_catMoveGuard && _catMoveGuard.dir==='down' && _catMoveGuard.id!==id){
    id=_catMoveGuard.id;
  }
  const cats=S.categories||[];
  const i=cats.findIndex(c=>c.id===id);
...
let _catMoveGuard=null; // v8.71 (FIX-183): po clampnutém scrollu PŘESMĚRUJ klik na původně přesouvanou kartu
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    requestAnimationFrame(()=>{
      ...
      if(Math.abs((r2.top + r2.height/2) - targetY) > 8){
        ...
        _catMoveGuard={ id, dir, x: ev.clientX, y: targetY };
        if(!window._catGuardMove){
          window._catGuardMove=true;
          document.addEventListener('mousemove',(me)=>{
            if(!_catMoveGuard) return;
            if(Math.abs(me.clientX-_catMoveGuard.x)>14 || Math.abs(me.clientY-_catMoveGuard.y)>14) _catMoveGuard=null;
          },{passive:true});
          document.addEventListener('wheel',()=>{ _catMoveGuard=null; },{passive:true});
        }
