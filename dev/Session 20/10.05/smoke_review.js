...
// FIX-272 (S20): odstraněny testy „denní doby" (revTimePatternReady, t.enteredAt).
// Funkce byla podle ADR-104 ZÁMĚRNĚ zrušena o verzi po zavedení (SKILL 28) –
// Milan sám zapisuje transakce i druhý den, čas zápisu tedy neodpovídá času
// nákupu a vzorec by byl vymyšlený. Test testoval smazanou funkci, ne chybu.
...
vm.runInContext([groups,pick('_revGroupOf'),'const REV_MIN_COVERAGE=0.15;',
  ...
  pick('_revBuckets'),pick('revPatterns'),pick('revPatternsHTML')].join('\n'),sb);
...
check('HTML vzorců nikdy nehlásí NaN/undefined',()=>{
  DATA.transactions=[];
  for(let i=0;i<12;i++) DATA.transactions.push(mk(i,`2026-08-${String((i%28)+1).padStart(2,'0')}`,i%2?'p1':'p2',3));
