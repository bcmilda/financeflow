...
async function submitEmailAuth(){
  ...
  try {
    if(_emailAuthMode === 'register'){
      ...
      // Pošli ověřovací e-mail a odhlas – uživatel se musí nejdřív ověřit
      ...
      if(okEl){
        ...
        okEl.innerHTML = '\u2705 Účet vytvořen! Poslali jsme ověřovací odkaz na <b>'+email+'</b>.<br>Otevři e-mail, klikni na odkaz a pak se přihlas.';
      }
      ...
      if(cred.user && !cred.user.emailVerified){
        // Neověřený e-mail – nepouštěj dovnitř
        try { await sendEmailVerification(cred.user); } catch(_) {}
        await fbSignOut(auth);
        const el = document.getElementById('emailAuthError');
        if(el){
          ...
          el.innerHTML = '\u26a0\ufe0f E-mail není ověřený. Poslali jsme nový odkaz na <b>'+email+'</b> – klikni na něj a zkus to znovu.';
        }
        if(btn){ btn.disabled = false; btn.textContent = 'Přihlásit se'; }
        return;
      }
...
import { getAuth, GoogleAuthProvider, signInWithPopup, onAuthStateChanged, signOut as fbSignOut, createUserWithEmailAndPassword, signInWithEmailAndPassword, sendPasswordResetEmail, sendEmailVerification } from 'https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js';
