...
// Dokup AI kreditů – placeholder dokud není Stripe (ADR-053)
const CREDIT_PACKS = {
  start:  { credits: 500,  price: 149 },
  growth: { credits: 1500, price: 349 },
  power:  { credits: 5000, price: 600 },
};
function buyCredits(pack) {
  const p = CREDIT_PACKS[pack];
  if (!p) return;
  alert('⚡ Dokup AI kreditů (' + p.credits + ' za ' + p.price + ' Kč) bude brzy dostupný.\n\nPlatební brána Stripe se připravuje. Kredity se po nákupu připíšou jen na tvůj účet a nepropadají s měsíční obnovou.');
}
...
function renderTransferDropdowns(wallets) {
  ...
  if(sel0) sel0.innerHTML = wallets.map(w => `<option value="${w.id}">${WALLET_TYPES[w.type]?.split(' ')[0]||'👛'} ${w.name} (${w.currency||'CZK'})</option>`).join('');
  // transferTo: peněženky + virtuální cíle (🎯 goal:ID)
  const selTo = document.getElementById('transferTo');
  if(selTo) {
    const D = getData();
    const goals = (D.wishes||[]).filter(w=>w.isGoal);
    let html = wallets.map(w => `<option value="${w.id}">${WALLET_TYPES[w.type]?.split(' ')[0]||'👛'} ${w.name} (${w.currency||'CZK'})</option>`).join('');
    if(goals.length) {
      html += `<optgroup label="🎯 Virtuální peněženka – cíle">`
...
function doTransfer() {
  ...
  if(to.startsWith('goal:')) {
    ...
    const txOut={id:uid(),name:`Vklad do cíle → ${goal.name}`,amount:amt,type:'expense',date,wallet:from,note:note||'Převod do virtuální peněženky',transferId,category:'transfer'};
    if(!S.transactions)S.transactions=[];
    S.transactions.push(txOut);
    // 2) Vklad do cíle (goal_deposits)
    const dep={id:uid(),goalId,amount:parseFloat(amt.toFixed(2)),date,note:note||`Převod z ${wFrom?.name||'peněženky'}`,createdAt:Date.now()};
...
function computeFinancialScore(D) {
  ...
  // ── S1: Cash Flow (0–25 bodů) ──────────────────────────────
  let score1 = 12; // neutral (no data)
  let expRatio = null;
  if (totalInc > 0) {
    ...
    score1 = 0;
    for (const [threshold, pts] of s1table) {
      if (expRatio <= threshold) { score1 = pts; break; }
    }
  }
  const s1label = score1>=20?'🟢 Cash flow OK':score1>=12?'🟡 Výdaje '+Math.round((expRatio||0)*100)+'% příjmu':'🔴 Výdaje překračují příjmy';

  // ── S2: Zadluženost DTI (0–13) + DSTI (0–12) ───────────────
  const dti  = annualIncome > 0 ? totalDebt / annualIncome * 100 : 0;
  const dsti = (baseIncome||totalInc) > 0 ? monthlyPayments / (baseIncome||totalInc||1) * 100 : 0;

  let scoreDTI = 13;
  if (debts.length > 0) {
    const dtiTable = [
      [50,13],[100,12],[150,11],[200,10],[250,9],[300,8],
      [350,7],[400,6],[500,5],[600,4],[700,3],[800,2],[900,1],
    ];
    scoreDTI = 0;
    for (const [threshold, pts] of dtiTable) {
      if (dti <= threshold) { scoreDTI = pts; break; }
    }
  }
  let scoreDSTI = 12;
  if (debts.length > 0) {
    const dstiTable = [
      [5,12],[10,11],[15,10],[20,9],[25,8],[30,7],
      [35,6],[40,5],[45,4],[50,3],[55,2],[60,1],
    ];
    scoreDSTI = 0;
    for (const [threshold, pts] of dstiTable) {
      if (dsti <= threshold) { scoreDSTI = pts; break; }
    }
  }
  const score2 = scoreDTI + scoreDSTI;
  const s2label = score2>=20?'🟢 Nízké zadlužení':score2>=12?`🟡 DTI ${Math.round(dti)}% / DSTI ${Math.round(dsti)}%`:`🔴 Vysoké zadlužení – DSTI ${Math.round(dsti)}%`;

  // ── S3: Rezerva (0–25 bodů) — POUZE monthsReserve ──────────
  // FIX-078: Nezávislá na aktuálním saldu → eliminuje dvojitý postih.
  ...
  let score3 = 12; // neutral
  if (monthsReserve !== null) {
    const s3table = [
      [6.0,25],[5.5,24],[5.0,23],[4.5,22],[4.0,21],
      [3.5,19],[3.0,17],[2.5,15],[2.0,13],[1.5,10],
      [1.0,8],[0.75,6],[0.5,4],[0.25,2],[0.1,1],
    ];
    score3 = 0;
    for (const [threshold, pts] of s3table) {
      if (monthsReserve >= threshold) { score3 = pts; break; }
    }
  }
  const s3label = score3>=20?`🟢 Rezerva ${monthsReserve?monthsReserve.toFixed(1):'?'} měs.`:score3>=12?`🟡 Rezerva ${monthsReserve?monthsReserve.toFixed(1):'?'} měs.`:`🔴 Nízká rezerva`;

  // ── S4: Aktivní spoření (0–25 bodů) ─────────────────────────
  // FIX-078: isSaving kategorie / baseIncome — měří CHOVÁNÍ, ne výsledek.
  const savCats = (D.categories||[]).filter(c=>c.isSaving);
  let score4 = 12; // neutral
  let activeSavingRate = null;
  if (savCats.length > 0 && (baseIncome||0) > 0) {
    ...
    score4 = 0;
    for (const [threshold, pts] of s4table) {
      if (activeSavingRate >= threshold) { score4 = pts; break; }
    }
  }
  const s4label = score4>=20?`🟢 Spoříš ${activeSavingRate?Math.round(activeSavingRate):'?'}% příjmu`:score4>=12?`🟡 Spoříš ${activeSavingRate?Math.round(activeSavingRate):'?'}%`:`🔴 Spoření nízké / nenastaveno`;
  ...
  const consistencyBonus = [0,0,2,5,9,13,15][Math.min(6,cm)]||0;
  ...
  // ── CELKOVÝ VÝSLEDEK ─────────────────────────────────────────
  const baseTotal = score1+score2+score3+score4;
  const total = Math.min(100, baseTotal+consistencyBonus);
  ...
  return {
    total, baseTotal, consistencyBonus, grade,
    components: [
      {label:'💰 Cash flow',   score:score1, max:25, detail:s1label},
      {label:'🏦 Zadluženost', score:score2, max:25, detail:s2label,
       sub:[{label:'DTI',score:scoreDTI,max:13},{label:'DSTI',score:scoreDSTI,max:12}]},
      {label:'🐷 Rezerva',     score:score3, max:25, detail:s3label},
      {label:'💎 Spoření',     score:score4, max:25, detail:s4label},
    ],
    ...
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
      ...
      <div style="flex:2;min-width:200px">
        ${components.map(c => `
          ...
          ${c.sub?c.sub.map(s=>`
            <div class="fscore-row" style="padding-left:16px;opacity:.75">
              <div style="font-size:.66rem;color:var(--text3);min-width:130px">↳ ${s.label}</div>
              ...
              <div style="font-size:.66rem;font-weight:600;min-width:36px;text-align:right;color:${barColor(s.score,s.max)}">${s.score}/${s.max}</div>
            </div>`).join(''):''}
        `).join('')}
      </div>
    </div>
