...
//  Pozn.: per-zařízení (osobní finance = osobní zařízení). Nesynchronizuje
//  se mezi zařízeními – to by vyžadovalo zápis do Firebase (TODO).
// ══════════════════════════════════════════════════════
...
function getLocalNotifications() {
  try {
    ...
    return Array.isArray(arr) ? arr.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)) : [];
  } catch (e) { return []; }
}
...
function addLocalNotification(opts = {}) {
  try {
    ...
    localStorage.setItem(LS_LOCAL_NOTIFS, JSON.stringify(list.slice(0, 30))); // cap 30
    updateAnnounceBadge();
    // Pokud je panel právě otevřený, hned překresli
    const panel = document.getElementById('announcementsPanel');
    if (panel && panel.style.display !== 'none' && typeof renderAnnouncements === 'function') renderAnnouncements();
  } catch (e) { console.log('addLocalNotification error:', e); }
}

function dismissLocalNotification(id) {
  try {
    const list = getLocalNotifications().filter(n => n.id !== id);
    localStorage.setItem(LS_LOCAL_NOTIFS, JSON.stringify(list));
  } catch (e) {}
...
function openNotifLink(kind) {
  if (kind === 'receiptHistory') {
    ...
    // Po přepnutí stránky otevři záložku Historie
    setTimeout(() => {
...
// ══════════════════════════════════════════════════════
//  BADGE – počet nepřečtených (admin + osobní, přes localStorage)
// ══════════════════════════════════════════════════════
...
async function initAnnouncementsBadge() {
  if (_isLocalMode || !window._currentUser) {
    // I bez přihlášení můžou existovat osobní oznámení (offline sync)
    _announcements = [];
    updateAnnounceBadge();
    return;
  }
  try {
    await loadAnnouncements();
    updateAnnounceBadge(); // spočítá z admin + osobních vs ff_announce_seen
  } catch (e) { updateAnnounceBadge(); }
}
