...
function renderBubbleChart(D) {
  ...
  (D.transactions||[]).forEach(t=>{
    ...
    (t.tags||[]).forEach(tag=>addShared('#'+tag,c.id,c.name,c.color,amt));
  });
...
function buildTxRow(t, D, ro, dupMap={}) {
  ...
  const hasReceiptItems = t.receiptItems && t.receiptItems.length > 0;
  ...
  return `<div class="${rowClass}" ${isSplitParent?`onclick="toggleSplitChildren('${t.splitId}')" style="cursor:pointer"`:rcptExpandAttr}>
