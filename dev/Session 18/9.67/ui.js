// FinanceFlow · v9.67 · ui.js · 2026-08-03
//  RENDER ROUTER
// ══════════════════════════════════════════════════════
...
function renderSouhrn(tableOnly){
  ...
  _list.forEach(cat=>{
    ...
    html+=`<tr style="${_bg}"><td><span style="margin-right:5px">${cat.icon}</span>${cat.name}</td><td style="color:#a8aec8">${prev?fmt(prev):'–'}</td><td>${cur?fmt(cur):'–'}</td>${predCell(pc,cur)}<td>${_pctCell(pct,cur,prev)}</td><td>${pctCelku}%</td></tr>`;
