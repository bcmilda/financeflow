// FinanceFlow · v9.62 · projects.js · 2026-08-03
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderReport() {
  ...
  const catRows = expCats.map(cat => {
    ...
    const trend = (() => {
      ...
      return d > 5 ? `<span style="color:#fbbf24;font-size:.64rem"${tip}>↑${d}%</span>` :
             ...
             `<span style="color:#a8aec8;font-size:.64rem"${tip}>↔</span>`;
    })();
    ...
    return `<div class="cat-tile" title="${cat.name}: ${fmt(spent)}${noLimit?'':' / plán '+planAmt+' ('+limitPct+')'}">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px;font-size:.64rem;color:#a8aec8;gap:6px">
  ...
  el.innerHTML = tabBar + `
    ...
    <div class="card" style="margin-bottom:14px;border-color:rgba(96,165,250,.3)">
      <div class="card-body" style="padding:11px 14px;display:flex;gap:16px;flex-wrap:wrap;align-items:center">
        <div style="font-size:.7rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:#93c5fd">2 · Výdajové zdraví</div>
        ...
        <div style="font-size:.68rem;color:#a8aec8;margin-left:auto">
          ${_s1pts!==null?'Stejný poměr, dvě škály: Dashboard ukazuje '+_s1pts+'/75, report '+scores.expScore+'/100.':'Bez příjmu nelze poměr spočítat.'}
        </div>
      </div>
    </div>

    ${(()=>{
      const wl = reportWatchlist(D, months);
      if (!wl.length) return `
        <div class="card" style="margin-bottom:14px;border-color:rgba(74,222,128,.25)">
          <div class="card-body" style="padding:12px 14px;font-size:.8rem;color:#c9cede">
            ...
            <span style="color:#a8aec8">Prověřeno ${months===1?'proti minulému měsíci':'napříč '+months+' měsíci'}:
      ...
      return `
      <div class="report-section-title">⚠️ 2b · Na co si dát pozor</div>
    ...
    <div class="report-section-title">💚 3 · Rozpočtové zdraví dle kategorií</div>
    ...
    <div class="report-section-title">💎 4 · Úsporové zdraví</div>
    <div class="card" style="margin-bottom:14px">
      <div class="card-body" style="padding:13px 15px">
        <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
          <div>
            <div style="font-family:Syne,sans-serif;font-size:1.7rem;font-weight:800;color:${healthColor(scores.savingScore)}">${scores.savingScore}<span style="font-size:.85rem;color:#a8aec8">/100</span></div>
            <div style="font-size:.7rem;color:#a8aec8">aktivní spoření (složka S4)</div>
          </div>
          <div style="flex:1;min-width:190px">
            <div style="height:9px;background:var(--surface3);border-radius:99px;overflow:hidden">
              <div style="height:100%;width:${Math.max(2,scores.savingScore)}%;background:${healthColor(scores.savingScore)};border-radius:99px"></div>
            </div>
            <div style="font-size:.72rem;color:#a8aec8;margin-top:5px">
              Kolik ze Základu příjmu (${fmt(scores.baseIncome)}) jsi tento měsíc odložil do 📈 investic a spořicích kategorií.
              ${scores.savingScore===0?'Tento měsíc zatím nic – jakmile zadáš převod do spoření nebo investice, číslo se zvedne.':''}
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="report-section-title">🏆 5 · Celkové finanční zdraví</div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div style="display:grid;grid-template-columns:auto repeat(3,1fr);gap:6px 8px;align-items:center;margin-bottom:8px">
            <div></div>
            <div style="text-align:center;font-size:.64rem;color:#a8aec8;font-weight:700;text-transform:uppercase;letter-spacing:.03em">Výdajové</div>
            <div style="text-align:center;font-size:.64rem;color:#a8aec8;font-weight:700;text-transform:uppercase;letter-spacing:.03em">Rozpočtové</div>
            <div style="text-align:center;font-size:.64rem;color:#a8aec8;font-weight:700;text-transform:uppercase;letter-spacing:.03em">Úsporové</div>
          </div>
    ...
    <div class="report-section-title">📈 6 · Vývoj finančního skóre</div>
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:12px 8px">
        ...
        <div style="font-size:.68rem;color:#a8aec8;margin-top:6px;text-align:center">Finanční skóre 0–310 (jako na Dashboardu) · vyšší = lepší · osa: měsíce</div>
      </div>
    </div>
    ...
    ${(()=>{
      ...
      try{
        ...
        if(nw) nwHtml = `
          <div class="report-section-title">💎 8 · Stav bohatství</div>
          <div class="report-stat-grid" style="margin-bottom:14px">
            <div class="report-stat"><div class="report-stat-label">Čisté jmění</div>
              <div class="report-stat-value" style="color:${nw.netWorth>=0?'var(--income)':'var(--expense)'}">${fmt(Math.round(nw.netWorth))}</div>
              <div style="font-size:.68rem;color:#a8aec8">majetek ${fmt(Math.round(nw.totalAssets+nw.totalWallets))} − dluhy ${fmt(Math.round(nw.totalDebts))}</div></div>
            <div class="report-stat"><div class="report-stat-label">Dluhy</div>
              <div class="report-stat-value" style="color:${nw.totalDebts>0?'var(--debt)':'var(--income)'}">${fmt(Math.round(nw.totalDebts))}</div>
              <div style="font-size:.68rem;color:#a8aec8">${nw.totalDebts>0?'celkem k doplacení':'bez dluhů'}</div></div>
            <div class="report-stat"><div class="report-stat-label">Rezerva vydrží</div>
              <div class="report-stat-value" style="color:${mExp>0&&liqNow/mExp>=3?'var(--income)':'var(--debt)'}">${mExp>0?(liqNow/mExp).toFixed(1).replace('.',',')+' měs.':'–'}</div>
              <div style="font-size:.68rem;color:#a8aec8">${fmt(Math.round(liqNow))} při výdajích ${fmt(Math.round(mExp))}</div></div>
          </div>`;
      }catch(e){}
      ...
      let recHtml = '';
      try{
        const ym = `${S.curYear}-${String(S.curMonth+1).padStart(2,'0')}`;
        const recs = (S.receipts||[]).filter(r=>String(r.date||'').slice(0,7)===ym);
        if(recs.length){
          ...
          recHtml = `
          <div class="report-section-title">🧾 9 · Z účtenek tohoto měsíce</div>
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:12px 14px;display:flex;gap:22px;flex-wrap:wrap">
            <div><div style="font-size:.66rem;color:#a8aec8;text-transform:uppercase;letter-spacing:.05em">Účtenek</div>
              <div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800">${recs.length}</div></div>
            <div><div style="font-size:.66rem;color:#a8aec8;text-transform:uppercase;letter-spacing:.05em">Položek</div>
              <div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800">${nItems}</div></div>
            <div><div style="font-size:.66rem;color:#a8aec8;text-transform:uppercase;letter-spacing:.05em">Celkem</div>
              <div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800">${fmt(Math.round(sum))}</div></div>
            ${top?`<div><div style="font-size:.66rem;color:#a8aec8;text-transform:uppercase;letter-spacing:.05em">Nejvíc utraceno</div>
              <div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800">${top[0]}</div>
              <div style="font-size:.68rem;color:#a8aec8">${fmt(Math.round(top[1]))}</div></div>`:''}
            <div style="flex:1;min-width:180px;font-size:.7rem;color:#a8aec8;align-self:center">
              Detailní vývoj cen a shrinkflaci najdeš v <b>Účtenkách → Zdražování</b>.</div>
          </div></div>`;
        }
      }catch(e){}
      ...
      try{
        const ym = `${S.curYear}-${String(S.curMonth+1).padStart(2,'0')}`;
        const ms = ((S.milestones)||[]).filter(x=>!x.hidden && String(x.date||'').slice(0,7)===ym);
        if(ms.length) msHtml = `
          <div class="report-section-title">🗺️ 10 · Milníky období</div>
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:12px 14px">
            ...
            <div style="font-size:.68rem;color:#8b93ad;margin-top:4px">Události slouží jako kontext k číslům výše. Skóre neovlivňují.</div>
          </div></div>`;
      }catch(e){}
      ...
      try{
        ...
        if(plan!==null || pred!==null) fwHtml = `
          <div class="report-section-title">🔮 11 · Výhled na ${CZ_M[nm]}</div>
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:12px 14px;display:flex;gap:22px;flex-wrap:wrap">
            ${plan!==null?`<div><div style="font-size:.66rem;color:#a8aec8;text-transform:uppercase">Známé budoucí platby</div>
              <div style="font-family:Syne,sans-serif;font-size:1.25rem;font-weight:800">${fmt(Math.round(plan))}</div></div>`:''}
            ${pred!==null?`<div><div style="font-size:.66rem;color:#a8aec8;text-transform:uppercase">Predikce výdajů</div>
              <div style="font-family:Syne,sans-serif;font-size:1.25rem;font-weight:800">${fmt(Math.round(pred))}</div></div>`:''}
            <div style="flex:1;min-width:170px;font-size:.7rem;color:#a8aec8;align-self:center">
      ...
      let rtHtml = '';
      try{
        const r = reportRatingSummary(D, S.curMonth, S.curYear);
        if(r) rtHtml = `
          <div class="report-section-title">🎯 12 · Stálo to za to? – výsledky</div>
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:12px 14px">
            <div style="display:flex;gap:22px;flex-wrap:wrap;align-items:center">
              ...
              <div style="flex:1;min-width:200px">
                <div style="display:flex;justify-content:space-between;font-size:.76rem;padding:3px 0"><span>Hodnoceno 4–5 ⭐</span><b style="color:var(--income)">${fmt(Math.round(r.good))}</b></div>
                <div style="display:flex;justify-content:space-between;font-size:.76rem;padding:3px 0"><span>Hodnoceno 1–2 ⭐</span><b style="color:${r.bad>0?'var(--debt)':'#a8aec8'}">${fmt(Math.round(r.bad))}</b></div>
              </div>
            </div>
            ${r.worst.length?`<div style="margin-top:10px;padding-top:9px;border-top:1px solid var(--border)">
              <div style="font-size:.7rem;color:#a8aec8;margin-bottom:5px">Nejdražší z nízko hodnocených:</div>
              ${r.worst.map(w=>`<div style="display:flex;justify-content:space-between;font-size:.76rem;padding:2px 0"><span>${String(w.n).replace(/</g,'&lt;')}</span><b>${fmt(Math.round(w.a))}</b></div>`).join('')}
              <div style="font-size:.7rem;color:#c9cede;margin-top:7px">Kdybys polovinu příště nasměroval jinam, je to <b style="color:var(--income)">${fmt(Math.round(r.bad*6))} Kč za rok</b>.</div>
            </div>`:''}
          </div></div>`;
      }catch(e){}
  ...
  setTimeout(() => {
    ...
    if (n === 1) {
      ...
      cont.innerHTML = `<div style="text-align:center;padding:8px">
        <div style="font-size:2rem;font-weight:800;font-family:Syne,sans-serif;color:${col}">${mo.score}<span style="font-size:.9rem;color:#a8aec8">/${mo.max}</span></div>
        <div style="font-size:.8rem;font-weight:700;color:${col};margin-top:2px">${mo.grade ? mo.grade.emoji + ' ' + mo.grade.label : ''}</div>
        <div style="font-size:.72rem;color:#a8aec8;margin-top:2px">${mo.label} ${mo.y} · stejné číslo jako na Dashboardu · pro graf zvol 3M / 6M / 12M</div>
      </div>`;
  ...
  const sEl = document.getElementById('reportSouhrn');
  if(sEl){
    if(curN===1 && _reportPeriod!=='advisor'){  // S16.15 (Milan): pod Poradcem se souhrn zobrazoval omylem
      ...
      sEl.innerHTML = `
        ...
        <div class="card">
          <div class="card-header">
            <span class="card-title">💰 Souhrn výdajů – ${CZ_M[S.curMonth]} ${S.curYear}</span>
      ...
      if(typeof renderSouhrnInto==='function') renderSouhrnInto('reportSuhrnTable');
    } else {
              ...
              ${bars.map(b=>{
                ...
                return `<div style="display:flex;flex-direction:column;align-items:center;gap:5px">
                  ...
                  <div style="display:inline-flex;align-items:baseline;gap:4px;padding:2px 7px;border-radius:6px;background:rgba(15,17,28,.85);border:1px solid ${b.color}44;white-space:nowrap">
                    ${b.isRemaining?`<span style="font-size:.58rem;color:#a8aec8">zbývá</span>`:''}
    ...
    <div class="card" style="margin-bottom:12px">
      <div class="card-body" style="padding:14px">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-bottom:10px">
          <span style="font-size:.82rem;font-weight:700">🧭 Cesta finančního zdraví</span>
        ...
        ${_prev.hasData ? `
          <div style="display:flex;align-items:center;justify-content:center;gap:14px;flex-wrap:wrap;margin-bottom:12px">
            <div style="text-align:center"><div style="font-family:Syne,sans-serif;font-size:1.35rem;font-weight:800;color:var(--text3)">${_prev.score}</div>
              <div style="font-size:.64rem;color:#a8aec8">před ${_back} měs.</div></div>
            ...
            <div style="text-align:center"><div style="font-family:Syne,sans-serif;font-size:1.35rem;font-weight:800;color:${trendColor}">${score}</div>
              <div style="font-size:.64rem;color:#a8aec8">dnes</div></div>
            <div style="background:${_dScore>=0?'rgba(74,222,128,.13)':'rgba(248,113,113,.13)'};border:1px solid ${_dScore>=0?'rgba(74,222,128,.3)':'rgba(248,113,113,.3)'};border-radius:11px;padding:6px 13px;text-align:center">
              ...
              <div style="font-size:.62rem;color:#a8aec8">za ${_back} měsíců</div></div>
          </div>` : `
        ...
        ${_sc.parts.map(p=>`
          <div style="display:grid;grid-template-columns:110px 1fr 46px;gap:8px;align-items:center;padding:3px 0">
            <span style="font-size:.73rem;color:#c9cede">${p.label}</span>
            <div style="height:14px;background:var(--surface3);border-radius:5px;position:relative;overflow:hidden">
              <div style="position:absolute;left:50%;top:0;bottom:0;width:1px;background:var(--border)"></div>
              ${p.d!==0?`<div style="position:absolute;top:0;bottom:0;border-radius:4px;${p.d>0?`left:50%;width:${Math.abs(p.d)/_wfMax*50}%;background:var(--income)`:`right:50%;width:${Math.abs(p.d)/_wfMax*50}%;background:var(--expense)`}"></div>`:''}
            </div>
            <span style="font-family:Syne,sans-serif;font-size:.8rem;font-weight:800;text-align:right;color:${p.d>0?'var(--income)':p.d<0?'var(--expense)':'var(--text3)'}">${p.d>0?'+':''}${p.d||'0'}</span>
          </div>
          <div style="font-size:.63rem;color:#8b93ad;margin:-2px 0 4px 118px">${p.note}</div>`).join('')}
      </div>
    </div>`;
  ...
  const ffrCard = `
    <div class="card" style="margin-bottom:12px">
      <div class="card-body" style="padding:14px">
        ...
        ${_ffrD!==null?`
          <div style="margin-top:8px;padding:7px 9px;background:var(--surface2);border-left:2px solid ${_ffrD>=0?'var(--income)':'var(--expense)'};border-radius:0 8px 8px 0">
            <div style="font-size:.62rem;font-weight:800;color:${_ffrD>=0?'var(--income)':'var(--expense)'};text-transform:uppercase;letter-spacing:.05em">FFR Momentum</div>
            <div style="font-family:Syne,sans-serif;font-size:.95rem;font-weight:800;margin:1px 0">${_ffrD>=0?'+':''}${_ffrD} b. za ${series.length} měs.</div>
            <div style="font-size:.66rem;color:#a8aec8;line-height:1.45">Porovnáno se začátkem okna, ne půlením – FFR je pomalu se měnící zásoba, ne měsíční tok.</div>
          </div>`:''}
        ${_liq?`
          <div style="margin-top:7px;padding:7px 9px;background:var(--surface2);border-left:2px solid #22d3ee;border-radius:0 8px 8px 0">
            <div style="font-size:.62rem;font-weight:800;color:#22d3ee;text-transform:uppercase;letter-spacing:.05em">Likvidita</div>
            <div style="font-family:Syne,sans-serif;font-size:.95rem;font-weight:800;margin:1px 0">${fmtB(Math.round(_liqNow))}</div>
            <div style="font-size:.66rem;color:#a8aec8;line-height:1.45">Peněženky + rezerva${(last.exp>0)?` · vydrží ${(_liqNow/last.exp).toFixed(1).replace('.',',')} měsíce při současných výdajích`:''}.</div>
          </div>`:''}
      </div>
    </div>`;
    ...
    <div class="card" style="margin-bottom:12px">
      <div class="card-body" style="padding:14px">
        ...
        <div style="margin-top:8px;padding:7px 9px;background:var(--surface2);border-left:2px solid ${_savedInWin>=0?'var(--income)':'var(--expense)'};border-radius:0 8px 8px 0">
          <div style="font-size:.62rem;font-weight:800;color:${_savedInWin>=0?'var(--income)':'var(--expense)'};text-transform:uppercase;letter-spacing:.05em">Net Worth Momentum</div>
      ...
      const row = (key, icon, title, sub, body) => {
        ...
        <div style="margin-bottom:10px">
          <div onclick="obrazToggleRow('${key}')" style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:7px 10px;background:var(--surface2);border:1px solid var(--border);border-radius:${open?'10px 10px 0 0':'10px'}">
            ...
            <span style="font-size:.65rem;color:#a8aec8">${sub}</span>
            <span style="margin-left:auto;color:var(--text3);font-size:.8rem;transform:rotate(${open?'0':'-90'}deg);transition:transform .15s">▾</span>
          </div>
          ${open?`<div style="border:1px solid var(--border);border-top:0;border-radius:0 0 10px 10px;padding:10px 10px 2px">${body}</div>`:''}
        </div>`;
      };
      return row('lifestyle','🏠','Lifestyle','životní styl a jeho vývoj', lifestyleCard)
           + row('indep','🏖️','Nezávislost a stabilita','FFR, likvidita, diverzifikace', ffrCard + diversCard)
           + row('wealth','💎','Majetek','čisté jmění a jeho růst', nwCard + momentumCard);
    })()}
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:0">
        ...
        ${(()=>{ // v8.74: sumář období – součty + průměry
          ...
          <div style="display:grid;grid-template-columns:48px 1fr 1fr 1fr 1fr;padding:7px 10px;font-size:.7rem;font-weight:700;border-top:1px solid var(--border)" title="Ø posledních 3 měsíců vs Ø předchozích 3 měsíců">
...
function renderIdleYieldCard(D){
  ...
  return `
    <div class="card" style="margin-bottom:14px">
      <div class="card-header">
        <span class="card-title">💤 Ušlý zisk – peníze, které nepracují</span>
      ...
      <div class="card-body">
        <div style="display:flex;gap:14px;flex-wrap:wrap;align-items:end;margin-bottom:12px">
          ...
          <div style="flex:1;min-width:180px;text-align:right">
            ...
            <div style="font-family:Syne;font-size:1.35rem;font-weight:800;color:${sumCol}">${lostYear>0?'−':''}${fmt(lostYear)} Kč</div>
...
function msSave(){
  ...
  if(isEra && dateTo && dateTo < date){ if(typeof toast==='function') toast('Konec etapy je před začátkem'); return; }
      ...
      <div style="padding:16px 18px">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:10px;flex-wrap:wrap">
          ...
          ${f?'':`<span style="display:flex;gap:6px">
            ...
            <button onclick="msOpen(null,'era')" style="${BTN};background:transparent;color:#8a6a3e">+ Etapa</button>
          </span>`}
        </div>
        ${form}
        ${body}
