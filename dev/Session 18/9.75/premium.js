// FinanceFlow · v9.58 · premium.js · 2026-08-03
//  PREMIUM SYSTEM
// ══════════════════════════════════════════════════════
...
function rateApp() {
  ...
  if (!uid) { if (typeof toast === 'function') toast('Pro hodnocení se musíš přihlásit'); return; }
  _revStars = 0;
  _openReviewModal();
  _loadReviewStats();
    ...
    m.innerHTML = `
      <div class="modal-content" style="max-width:460px">
        <div class="modal-header">
          <span id="revTitle">Ohodnotit aplikaci</span>
          <button onclick="closeReviewModal()" class="modal-close">&times;</button>
        </div>
        <div class="modal-body" style="padding:16px">
          <div id="revStatsBox" style="font-size:.76rem;color:#a8aec8;margin-bottom:14px"></div>
          <div style="font-size:.84rem;margin-bottom:8px">Jak jsi s FinanceFlow spokojený?</div>
          <div id="revStars" style="display:flex;gap:8px;margin-bottom:14px"></div>
          <textarea id="revText" maxlength="600" rows="4" placeholder="Co ti vyhovuje a co bys zlepšil? (nepovinné)"
            style="width:100%;padding:10px;border-radius:9px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-family:inherit;font-size:.84rem;resize:vertical"></textarea>
          <div style="font-size:.7rem;color:#a8aec8;margin-top:8px;line-height:1.5">
            Hodnocení je veřejné pouze v souhrnu (průměr a počet). Text čte jen autor aplikace
            a slouží ke zlepšování – nikde se nezveřejňuje.
          </div>
          <div style="display:flex;gap:8px;margin-top:14px">
            <button onclick="submitReview()" class="btn btn-primary" style="flex:1">Odeslat hodnocení</button>
            <button onclick="closeReviewModal()" class="btn btn-ghost">Zrušit</button>
          </div>
        </div>
      </div>`;
...
async function submitReview() {
  const uid = window._currentUser && window._currentUser.uid;
  if (!uid) return;
  if (!_revStars) { if (typeof toast === 'function') toast('Vyber prosím počet hvězdiček'); return; }
  ...
  try {
    ...
    closeReviewModal();
    if (typeof toast === 'function') toast('Díky za hodnocení! 💚');
    _revStats = null;
    _loadReviewStats();
    if (typeof renderSettingsPage === 'function') renderSettingsPage();
  } catch (e) {
    console.warn('[review] uložení selhalo:', e);
    if (typeof toast === 'function') toast('Hodnocení se nepodařilo uložit');
  }
}
