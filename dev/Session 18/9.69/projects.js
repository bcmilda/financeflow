// FinanceFlow · v9.69 · projects.js · 2026-08-03
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderObraz() {
  ...
  const journeyCard = `
    <div class="card" style="margin-bottom:12px">
      <div class="card-body" style="padding:14px">
        ...
        ${(()=>{
          //  v9.69: Monthly Score a 6M Momentum Score – v modelu byly, v appce chyběly.
          //  Monthly = jak dopadl TENHLE měsíc proti průměru okna (kolísá).
          //  Momentum = SMĚR a STÁLOST pohybu, ne úroveň: vysoké momentum při nízkém
          //  skóre znamená rychlé zlepšování, nízké při vysokém = držíš si dobrou pozici.
          const hist = [];
          ...
          const stable = Math.round(ups/(hist.length-1)*100);
          const mx = Math.max(...hist, 1);
          return `
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
            <div style="padding:9px 11px;background:var(--surface2);border-left:2px solid #60a5fa;border-radius:0 9px 9px 0">
              <div style="font-size:.62rem;font-weight:800;color:#60a5fa;text-transform:uppercase;letter-spacing:.05em">Monthly Score</div>
              <div style="font-family:Syne,sans-serif;font-size:1.15rem;font-weight:800;margin:1px 0">${monthly}
                <span style="font-size:.72rem;color:${dAvg>=0?'var(--income)':'var(--expense)'}">${dAvg>=0?'+':''}${dAvg} vs Ø</span></div>
              <div style="font-size:.66rem;color:#a8aec8;line-height:1.45">Jak dopadlo poslední období proti průměru okna. Jeden měsíc nic neznamená.</div>
            </div>
            <div style="padding:9px 11px;background:var(--surface2);border-left:2px solid ${stable>=60?'var(--income)':'var(--debt)'};border-radius:0 9px 9px 0">
              <div style="font-size:.62rem;font-weight:800;color:${stable>=60?'var(--income)':'var(--debt)'};text-transform:uppercase;letter-spacing:.05em">${series.length}M Momentum Score</div>
              <div style="font-family:Syne,sans-serif;font-size:1.15rem;font-weight:800;margin:1px 0">${_dScore!==null&&_dScore>=0?'+':''}${_dScore!==null?_dScore:'–'} ${_dScore!==null?'bodů':''}</div>
              <div style="font-size:.66rem;color:#a8aec8;line-height:1.45">${ups} z ${hist.length-1} kroků nahoru (${stable} %). Měří směr a stálost, ne úroveň.</div>
            </div>
          </div>
          <div style="display:flex;align-items:flex-end;gap:4px;height:46px;margin-bottom:12px">
            ${hist.map((v,i)=>`<div title="${v}/100" style="flex:1;height:${Math.max(6,v/mx*100)}%;border-radius:3px 3px 0 0;background:${i===hist.length-1?'#60a5fa':'rgba(96,165,250,.4)'}"></div>`).join('')}
          </div>
          <div style="font-size:.66rem;color:#8b93ad;text-align:center;margin:-8px 0 12px">Vývoj skóre v okně · poslední sloupec = aktuální stav</div>`;
        })()}
  ...
  const smerujCard = proj.hasData ? `
    <div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text3);margin-bottom:10px">🧭 3 · Kam směřuju – příštích 6 měsíců</div>
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:14px">
  ...
  const cyklyCard = cycles.length>=2 ? `
    <div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text3);margin-bottom:10px">💶 8 · Od výplaty k výplatě – historie cyklů</div>
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:14px">
  ...
  el.innerHTML=tabIntro('obraz','🖼️','Finanční obraz',
    ...
    <div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text3);margin:18px 0 10px">📈 2 · Hlavní metriky a jejich podmetriky</div>
    ...
    <div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text3);margin-bottom:10px">📐 4 · Pokročilé metriky</div>
    ${(()=>{
      ...
      return row('lifestyle','🏠','Lifestyle','životní styl, kam růst přistál, reálný růst', lifestyleCard)
           + row('indep','🏖️','5 · Nezávislost a stabilita','FFR, likvidita, diverzifikace',
                 `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">${ffrCard}${diversCard}</div>`)
           + row('wealth','💎','6 · Majetek','čisté jmění a jeho růst',
                 `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">${nwCard}${momentumCard}</div>`);
    })()}
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:0">
        <div style="display:grid;grid-template-columns:44px repeat(6,1fr);font-size:.62rem;font-weight:700;color:var(--text3);text-transform:uppercase;padding:8px 10px;background:var(--surface3)">
        ...
        ${series.map((s,i)=>`
          <div style="display:grid;grid-template-columns:44px repeat(6,1fr);padding:8px 10px;border-bottom:1px solid var(--border);font-size:.73rem;${i===series.length-1?'font-weight:600':''}">
