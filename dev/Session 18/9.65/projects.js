// FinanceFlow · v9.65 · projects.js · 2026-08-03
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderReport() {
  ...
  el.innerHTML = tabBar + `
    ...
    ${(()=>{
      ...
      try{
        ...
        if(!recs.length){
          recHtml = `
          <div class="report-section-title">🧾 11 · Z účtenek tohoto měsíce</div>
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:14px">
            <div style="font-size:.82rem;color:#c9cede">V ${CZ_M[S.curMonth].toLowerCase()} nemáš naskenovanou žádnou účtenku.</div>
      ...
      try{
        ...
        if(!ms.length) msHtml = `
          ...
          <div class="card" style="margin-bottom:14px"><div class="card-body" style="padding:14px">
            <div style="font-size:.82rem;color:#c9cede">V ${CZ_M[S.curMonth].toLowerCase()} nemáš označenou žádnou událost.</div>
            <div style="font-size:.74rem;color:#a8aec8;line-height:1.6;margin-top:5px">
  ...
  setTimeout(() => {
    ...
    if (n === 1) {
      ...
      //  v9.65: jeden měsíc = jeden bod, čára nedává smysl. Místo osamoceného
      //  čísla ukážeme skóre s pruhem, známkou a kolik chybí do lepší známky.
      const _f = mo.max>0 ? Math.max(0,Math.min(1,mo.score/mo.max)) : 0;
      const _next = (()=>{ try{
      ...
      cont.innerHTML = `<div style="padding:6px 4px">
        <div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap">
          <span style="font-size:2rem;font-weight:800;font-family:Syne,sans-serif;color:${col}">${mo.score}</span>
          <span style="font-size:.9rem;color:#a8aec8">z ${mo.max}</span>
          <span style="font-size:.9rem;font-weight:700;color:${col}">${mo.grade ? mo.grade.emoji + ' ' + mo.grade.label : ''}</span>
          <span style="margin-left:auto;font-size:.72rem;color:#a8aec8">${mo.label} ${mo.y}</span>
        </div>
        <div style="height:10px;background:var(--surface3);border-radius:99px;overflow:hidden;margin:9px 0 7px">
          <div style="height:100%;width:${(_f*100).toFixed(1)}%;background:${col};border-radius:99px"></div>
        </div>
        <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;font-size:.74rem;color:#c9cede">
          <span>${_next?`Do známky <b>${_next.l}</b> chybí <b>${_next.need}</b> ${_next.need===1?'bod':_next.need<5?'body':'bodů'}`:'🏆 Jsi v nejvyšším pásmu hodnocení'}</span>
          <span style="color:#8b93ad">Stejné číslo jako na Dashboardu · pro graf v čase zvol 3M / 6M / 12M</span>
        </div>
      </div>`;
