// FinanceFlow · v9.66 · projects.js · 2026-08-03
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderReport() {
  ...
  setTimeout(() => {
    ...
    if (n === 1) {
      ...
      const _comps = (fs && fs.components) ? fs.components : [];
      ...
      cont.innerHTML = `<div style="padding:4px 2px">
        <div style="display:flex;align-items:flex-start;gap:16px;flex-wrap:wrap;margin-bottom:14px">
          <div>
            <div style="font-family:Syne,sans-serif;font-size:2.1rem;font-weight:800;color:${col};line-height:1">${mo.score}</div>
            <div style="font-size:.74rem;color:#a8aec8;margin-top:2px">z ${mo.max} · <b style="color:${col}">${mo.grade?mo.grade.emoji+' '+mo.grade.label:''}</b></div>
          </div>
          <div style="flex:1;min-width:190px;font-size:.8rem;color:#c9cede;line-height:1.7">
            ${_next?`Do známky <b>${_next.l}</b> chybí <b>${_next.need}</b> ${_next.need===1?'bod':_next.need<5?'body':'bodů'}.`:'🏆 Jsi v nejvyšším pásmu hodnocení.'}
            ${_dTotal!==null?`<br>Za ${CZ_M[S.curMonth].toLowerCase()} <b style="color:${_dTotal>=0?'var(--income)':'var(--expense)'}">${_dTotal>=0?'+':''}${_dTotal} ${Math.abs(_dTotal)===1?'bod':Math.abs(_dTotal)<5?'body':'bodů'}</b> vs. ${CZ_M[_pm2].toLowerCase()}.`:''}
          </div>
        </div>
        ${_comps.map(c=>{
          ...
          return `<div style="margin-bottom:9px">
            <div style="display:flex;align-items:baseline;gap:8px">
              <span style="font-size:.82rem">${c.label}</span>
              <span style="margin-left:auto;font-family:Syne,sans-serif;font-weight:800;font-size:.88rem;color:${cc}">${c.score} / ${c.max}</span>
              ${d!==null&&d!==0?`<span style="font-size:.66rem;font-weight:700;padding:1px 7px;border-radius:99px;background:${d>0?'rgba(74,222,128,.15)':'rgba(248,113,113,.15)'};color:${d>0?'var(--income)':'var(--expense)'}">${d>0?'+':''}${d}</span>`:''}
            </div>
        ...
        <div style="font-size:.7rem;color:#8b93ad;margin-top:10px;padding-top:8px;border-top:1px solid var(--border)">
          Stejné číslo jako na Dashboardu · odznaky ukazují změnu proti minulému měsíci · pro graf v čase zvol 3M / 6M / 12M
        </div>
      </div>`;
