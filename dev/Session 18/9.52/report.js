// FinanceFlow · v9.52 · report.js · 2026-08-03
// ══════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════
//  v9.52 (TODO-208, Milan): VLASTNÍ EDITOVATELNÉ SEKTORY
//
//  PROČ: sektory se dosud odvozovaly z cat.coicop. Kategorie ale ten údaj
//  většinou nemají, takže VŠECHNO spadlo do „Ostatní" a rozdělení fakticky
//  nefungovalo. Nyní si uživatel přiřadí kategorie do bloků sám.
//
//  Pořadí rozhodování: vlastní přiřazení → COICOP → Ostatní.
//  Uloženo v S.reportSectors (uzel pod users/{uid}/data, kaskáda .write ho kryje).
// ══════════════════════════════════════════════════════
const REPORT_SECTOR_PRESETS = [
  {id:'dum',      name:'Dům a bydlení',   icon:'🏠', color:'#f59e0b'},
  {id:'splatky',  name:'Splátky a úvěry', icon:'💳', color:'#3b82f6'},
  {id:'jidlo',    name:'Jídlo a nákupy',  icon:'🍽️', color:'#10b981'},
  {id:'doprava',  name:'Auto a doprava',  icon:'🚗', color:'#6366f1'},
  {id:'deti',     name:'Děti a rodina',   icon:'👨‍👩‍👧', color:'#ec4899'},
  {id:'zdravi',   name:'Zdraví',          icon:'🩺', color:'#ef4444'},
  {id:'zabava',   name:'Zábava a volno',  icon:'🎬', color:'#a78bfa'},
  {id:'sluzby',   name:'Služby a členství', icon:'📱', color:'#22d3ee'},
  {id:'invest',   name:'Investice a spoření', icon:'📈', color:'#4ade80'},
];
function reportSectorsDef(){
  const saved = (S.reportSectors && S.reportSectors.list) || null;
  return saved && saved.length ? saved : REPORT_SECTOR_PRESETS;
}
function reportCatSector(catId){
  const map = (S.reportSectors && S.reportSectors.map) || {};
  return map[String(catId)] || null;
}
function reportSetCatSector(catId, secId){
  S.reportSectors = S.reportSectors || {};
  S.reportSectors.map = S.reportSectors.map || {};
  if(secId) S.reportSectors.map[String(catId)] = secId;
  else delete S.reportSectors.map[String(catId)];
  if(typeof save==='function') save();
  if(typeof renderReport2==='function') renderReport2(); else if(typeof renderReportTab==='function') renderReportTab();
}
let _reportSecEdit = false;
function reportToggleSecEdit(){
  _reportSecEdit = !_reportSecEdit;
  if(typeof renderReport2==='function') renderReport2(); else if(typeof renderReportTab==='function') renderReportTab();
}
...
function reportMatice() {
  ...
  // ── řádky per kategorie ──
  //  v9.52: „Rozdíl vs. loni" se počítá ke STEJNÉMU MĚSÍCI loňského roku (YTD vs YTD).
  //  Srovnávat letošních 8 měsíců s celým loňským rokem by bylo zavádějící.
  const _ytd = (catId, y) => {
    let sum = 0;
    for (let m = 0; m <= curM; m++) sum += _reportCatMonthExp(catId, y, m, D);
    return sum;
  };
  const mkRow = c => {
    const yearVals = years.map(y => _reportCatYearExp(c.id, y, D));
    const ytdNow = _ytd(c.id, curY), ytdPrev = _ytd(c.id, curY - 1);
    return {
  ...
  // ── seskupení do sektorů (v9.52: vlastní přiřazení má PŘEDNOST před COICOP) ──
  const sectors = [];
  const SEC = reportSectorsDef();
  SEC.forEach(g => {
    const rows = allRows.filter(r => reportCatSector(r.id) === g.id);
    if (rows.length) sectors.push({ id: 's_' + g.id, name: g.name, icon: g.icon, color: g.color, rows: rows.sort((a, b) => b.total - a.total) });
  });
  const assigned = new Set(sectors.flatMap(s => s.rows.map(r => r.id)));
  GRP.forEach(g => {
    const rows = allRows.filter(r => !assigned.has(r.id) && (cats.find(c => c.id === r.id) || {}).coicop === g.id);
  ...
  ordered.forEach(sec => {
    ...
    sec.rows.forEach(r => {
      body += `<tr>
        <td style="position:sticky;left:0;background:var(--surface2);text-align:left;z-index:1;white-space:nowrap;padding-left:16px;border-left:4px solid ${sec.color}55">${r.icon || ''} ${r.name}${_reportSecEdit && String(r.id).indexOf('debt_') !== 0 ? `
          <select onchange="reportSetCatSector('${r.id}', this.value)" onclick="event.stopPropagation()" style="margin-left:8px;padding:2px 5px;border-radius:6px;border:1px solid var(--border);background:var(--surface3);color:var(--text);font-size:.68rem;font-family:inherit">
            <option value="">— nezařazeno —</option>
            ${reportSectorsDef().map(g => `<option value="${g.id}"${reportCatSector(r.id) === g.id ? ' selected' : ''}>${g.icon} ${g.name}</option>`).join('')}
          </select>` : ''}</td>
    ...
    <div class="card">
      <div class="card-header">
        ...
        <span style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
          ...
          <button onclick="reportToggleSecEdit()" style="padding:4px 10px;border-radius:8px;cursor:pointer;font-size:.7rem;font-weight:700;font-family:inherit;border:1px solid ${_reportSecEdit?'rgba(96,165,250,.55)':'var(--border)'};background:${_reportSecEdit?'rgba(96,165,250,.16)':'transparent'};color:${_reportSecEdit?'#93c5fd':'#a8aec8'}">${_reportSecEdit?'✓ Hotovo':'⚙ Upravit sektory'}</button>
        </span>
      </div>
      ${_reportSecEdit?`<div style="margin:0 14px 8px;padding:9px 11px;background:rgba(96,165,250,.08);border-left:2px solid #60a5fa;border-radius:0 8px 8px 0;font-size:.72rem;color:#bcc3d8;line-height:1.5">
        U každé kategorie vyber sektor. Nezařazené se řadí podle klasifikace COICOP, a když ji nemají, spadnou do „Ostatní" &mdash; proto tam dosud byly skoro všechny.</div>`:''}
