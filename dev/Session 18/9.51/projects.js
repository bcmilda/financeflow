// FinanceFlow · v9.51 · projects.js · 2026-08-03
//  PROJEKTY
// ══════════════════════════════════════════════════════
  ...
  el.innerHTML = tabBar + `
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:12px 8px">
        ...
        <div style="font-size:.68rem;color:#a8aec8;margin-top:6px;text-align:center">Kruhy se skóre propojené čarou · vyšší = lepší · osa: měsíce</div>
      </div>
    </div>`;
...
function _zivotniMapaHTML(m,y){
      ...
      ${all.map(x=>{
        ...
        return `<div style="position:relative;margin-bottom:11px">
          ...
          <div style="display:flex;align-items:baseline;gap:7px;flex-wrap:wrap">
            ...
            <span style="font-size:.7rem;color:#8a7a5e">${isNaN(d)?'':d.toLocaleDateString('cs-CZ')}${x.kind==='era'?(' – '+(x.dateTo?new Date(x.dateTo).toLocaleDateString('cs-CZ'):'dosud')):''}</span>
            ${x.kind==='era'?'<span style="font-size:.62rem;padding:1px 6px;border-radius:99px;border:1px solid #8a6a3e;color:#8a6a3e">etapa</span>':''}
  ...
  const eraCmp = eras.length < 2 ? '' : `
    <div style="margin-top:14px;padding-top:12px;border-top:1px solid rgba(138,106,62,.4)">
        ...
        return eras.map(e=>`
          <div style="margin-bottom:7px">
            <div style="display:flex;justify-content:space-between;gap:8px;font-size:.76rem">
              <span>${esc(e.label)} <span style="color:#8a7a5e;font-size:.68rem">${esc(e.from)} – ${esc(e.to)} · ${e.months} měs.</span></span>
      ...
      <div style="font-size:.7rem;color:#6b5741;margin-top:4px">Počítáno jen z měsíců, ve kterých máš data. Etapy nijak neovlivňují tvoje skóre.</div>
    </div>`;
...
function _denikCestaHTML(){
  try{
    ...
    const col = dir==='lepší' ? '#7fb069' : dir==='horší' ? '#b06b6b' : '#8a7a5e';
    return `
      <div class="denik-book" style="margin-top:14px">
        <div style="padding:13px 18px;display:flex;align-items:center;gap:14px;flex-wrap:wrap">
          <div>
            <div style="font-size:.68rem;color:#8a7a5e;text-transform:uppercase;letter-spacing:.08em">Kde jsi na cestě</div>
            <div class="denik-h" style="font-size:1.05rem;margin-top:2px">Životní styl spotřebuje <b style="color:${col}">${Math.round(er*100)} %</b> příjmu</div>
            <div style="font-size:.74rem;color:#6b5741;margin-top:2px">
          ...
          <a href="#" onclick="if(typeof showPage==='function')showPage('obraz');return false"
             style="margin-left:auto;font-family:Georgia,serif;font-size:.76rem;color:#8a6a3e;text-decoration:none;border:1px solid #8a6a3e;border-radius:7px;padding:5px 11px">Finanční obraz →</a>
        </div>
      </div>`;
  }catch(e){ return ''; }
}
