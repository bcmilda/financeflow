// FinanceFlow · v9.55 · app.js · 2026-08-03
  ...
  try {
    ...
    try {
      const s = {transactions:S.transactions||[],debts:S.debts||[],categories:S.categories||[],
                 ...
                 assets:S.assets||[],shareSettings:S.shareSettings||{},calNotes:S.calNotes||{},workCal:S.workCal||{},diary:S.diary||{},idleCfg:S.idleCfg||{},milestones:S.milestones||[],reportSectors:S.reportSectors||{},_savedAt:Date.now()};
...
// ══════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════
//  v9.55 (TODO-177 fáze 2b): OKNO PŘI NAČÍTÁNÍ
//
//  Fáze 2 (v9.46) vyřešila PRŮBĚŽNÝ sync – změna jedné transakce už netahá
//  celou databázi. Zbývalo úvodní načtení: při startu se pořád stahovala
//  celá historie. Tady se omezuje na posledních N měsíců.
//
//  ⚠️ NEJVĚTŠÍ RIZIKO CELÉ ZMĚNY (a proč je tolik pojistek):
//  saveToFirebase() má větev `_set(dataRef, full)`, která přepíše celý uzel
//  tím, co je v paměti. Kdyby se spustila při částečně načtených datech,
//  SMAZALA BY starší transakce ze serveru. To je jediná nevratná cesta v celé
//  aplikaci, proto ji `_dwPartial` tvrdě blokuje.
//
//  VÝCHOZÍ STAV: VYPNUTO. Zapíná se `localStorage.ff_read_window = '12'`
//  (počet měsíců). Důvod: dokud si to neověříš na svých datech, nemá smysl
//  měnit způsob načítání všem – přínos je výkonový, riziko datové.
// ══════════════════════════════════════════════════════
let _dwPartial   = false;   // true = v paměti NENÍ celá historie
let _dwCutoffISO = null;    // od kterého data jsou transakce načtené
let _dwFullPromise = null;  // rozpracované dotahování historie

function _dwWindowMonths(){
  try{
    const v = parseInt(localStorage.getItem('ff_read_window')||'0', 10);
    return (v >= 1 && v <= 120) ? v : 0;     // 0 = vypnuto, načíst vše
  }catch(e){ return 0; }
}
function _dwCutoffFor(months){
  const d = new Date(); d.setMonth(d.getMonth() - months); d.setDate(1);
  return d.toISOString().slice(0,10);
}

// Dotažení CELÉ historie na vyžádání. Volají moduly, které potřebují všechny roky
// (matice v Reportu, Grafy → Všechny roky, sezónnost). Idempotentní.
async function ensureFullHistory(){
  if(!_dwPartial) return true;
  if(_dwFullPromise) return _dwFullPromise;
  _dwFullPromise = (async ()=>{
    try{
      setSyncStatus('syncing');
      const uid = window._currentUser && window._currentUser.uid;
      if(!uid) return false;
      const snap = await _get(_ref(_db, `users/${uid}/data/transactions`));
      const val = snap && snap.exists && snap.exists() ? snap.val() : null;
      if(val){
        const arr = Array.isArray(val) ? val : Object.values(val);
        arr.forEach(t => { if(t && t.id) _txUpsert(t); });
      }
      _dwPartial = false; _dwCutoffISO = null;
      console.log('[diff-read] historie dotažena, v paměti je vše');
      setSyncStatus('ok');
      if(typeof renderPageDebounced==='function') renderPageDebounced();
      return true;
    }catch(e){
      console.warn('[diff-read] dotažení historie selhalo:', e);
      setSyncStatus('ok');
      return false;
    }finally{ _dwFullPromise = null; }
  })();
  return _dwFullPromise;
}
window.ensureFullHistory = ensureFullHistory;
// Kolik historie je v paměti – pro upozornění v UI
window.dwHistoryInfo = () => ({ partial:_dwPartial, since:_dwCutoffISO });
...
function _attachOwnListeners(userRef, uid, initialVal){
  ...
  if(!useSplit){ _dwPartial = false; _dwCutoffISO = null; _attachFullListener(userRef); return; }
  ...
  try {
    ...
    //  v9.55: volitelné okno – načte se jen posledních N měsíců.
    //  Query vyžaduje .indexOn: "date" v database_rules.json, jinak Firebase
    //  řadí na klientovi a stáhne stejně všechno (funkčně OK, výkonově ne).
    const txPath = _ref(_db, `users/${uid}/data/transactions`);
    const _win = _dwWindowMonths();
    let txRef = txPath;
    if(_win){
      try{
        _dwCutoffISO = _dwCutoffFor(_win);
        txRef = _query(txPath, _orderByChild('date'), _startAt(_dwCutoffISO));
        _dwPartial = true;
        console.log('[diff-read] okno '+_win+' měsíců · transakce od '+_dwCutoffISO);
      }catch(e){
        console.warn('[diff-read] okno selhalo, načítám vše:', e);
        txRef = txPath; _dwPartial = false; _dwCutoffISO = null;
      }
    } else { _dwPartial = false; _dwCutoffISO = null; }
    ...
    _dwPartial = false; _dwCutoffISO = null;     // fallback načte vše
    _detachOwnListeners(userRef);
...
// ══════════════════════════════════════════════════════
const _DW_META = ['debts','categories','bank','birthdays','wishes','wallets','payTypes','sablony','projects','receipts','nakupList','assets','noSyncKeys','importHistory','shareSettings','calNotes','workCal','diary','idleCfg','milestones','reportSectors'];
...
// ══════════════════════════════════════════════════════
//  v9.55: ROČNÍ AGREGÁTY users/{uid}/stats/{YYYY}
//  Umožní zobrazit starší roky (matice, Všechny roky) bez stahování transakcí.
//  Počítá se JEN z toho, co je v paměti – při částečném načtení by starší roky
//  vyšly nulové, proto se v tom případě přepisují pouze roky, které máme celé.
// ══════════════════════════════════════════════════════
async function _dwWriteYearStats(uid){
  if(!uid) return;
  const D = getData();
  const cutYear = _dwCutoffISO ? parseInt(_dwCutoffISO.slice(0,4),10) : null;
  const by = {};
  (D.transactions||[]).forEach(t=>{
    ...
    //  Při částečném načtení nezapisovat roky, které máme jen zčásti –
    //  přepsali bychom správný agregát nulami.
    if(_dwPartial && cutYear !== null && y <= cutYear) return;
    const a = (typeof txCZK==='function') ? txCZK(t, D) : (t.amount||0);
    by[y] = by[y] || { inc:0, exp:0, m:Array.from({length:12},()=>0) };
    if(a > 0) by[y].inc += a; else { by[y].exp += Math.abs(a); by[y].m[m] += Math.abs(a); }
  });
  const upd = {};
  Object.keys(by).forEach(y=>{
    upd[y] = { inc:Math.round(by[y].inc), exp:Math.round(by[y].exp),
               m:by[y].m.map(v=>Math.round(v)), at:Date.now() };
  });
  if(Object.keys(upd).length) await _update(_ref(_db, `users/${uid}/stats`), upd);
}
...
async function saveToFirebase() {
  ...
  try {
    ...
    //  ⚠️ v9.55 KRITICKÁ POJISTKA: tahle větev přepisuje CELÝ uzel tím, co je
    //  v paměti. Při částečně načtené historii by NEVRATNĚ smazala starší
    //  transakce. Proto nejdřív dotáhneme vše a teprve pak uložíme.
    if((S.schemaV!==2 || !_dw.ready) && _dwPartial){
      console.warn('[diff-write] plný zápis při částečných datech – nejprve dotahuji historii');
      const ok = await ensureFullHistory();
      if(!ok || _dwPartial){
        setSyncStatus('error');
        console.error('[diff-write] ZÁPIS ZRUŠEN: nelze plně uložit bez celé historie (chránili bychom data před smazáním)');
        return;
      }
    }
    ...
    //  v9.55: roční agregáty – aby šlo ukázat starší roky bez stahování transakcí.
    //  Postranní zápis: vlastní try/catch, nikdy nesmí shodit hlavní uložení.
    try { await _dwWriteYearStats(uid); } catch(e){ console.warn('[stats] přeskočeno:', e); }
    setSyncStatus('ok');
    publishCommunityStats(getData());
  } catch(e) {
    setSyncStatus('error');
