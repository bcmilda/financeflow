// FinanceFlow · v9.56 · report.js · 2026-08-03
// ══════════════════════════════════════════════════════
...
function renderReport2() {
  ...
  if (!el) return;
  //  v9.55: matice pracuje přes VŠECHNY roky – při okně 12M by starší roky
  //  vyšly nulové. Dotáhneme historii a překreslíme.
  if (typeof window.ensureFullHistory === 'function' && (window.dwHistoryInfo||(()=>({})))().partial) {
    el.innerHTML = '<div style="padding:22px;text-align:center;color:#a8aec8;font-size:.82rem">Načítám celou historii…</div>';
    window.ensureFullHistory().then(() => renderReport2());
    return;
  }
    ...
    <div class="card">
      <div class="card-header">
        ...
        <span style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
          <span style="font-size:.7rem;color:#a8aec8">Měsíční = ${CZ_M[curM]} ${curY} · Roční = leden–${CZ_M[curM]} ${curY}</span>
        </span>
      </div>
      <div class="card-body" style="overflow-x:auto;padding-top:6px">
        <table class="report-matrix" style="min-width:${440 + years.length * 76}px;width:100%;border-collapse:collapse;font-size:.74rem">
          <thead><tr>
            ...
            <th style="text-align:right;color:#60a5fa;padding:8px 8px" title="Kumulace leden–${CZ_M[curM]} ${curY}">Roční (k ${CZ_M[curM]})</th>
