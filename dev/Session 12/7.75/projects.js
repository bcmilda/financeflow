...
function renderRadarPayday(el, D){
  ...
  let weSum=0,wdSum=0;
  cycSoFar.forEach(t=>{ if(t.type!=='expense'||t.isBalancing||t.splitParent) return; const wd=new Date(t.date).getDay(); const a=(t.amount||t.amt||0); if(wd===0||wd===6) weSum+=a; else wdSum+=a; });
  ...
  const flexSoFar=cycSoFar.filter(t=>t.type==='expense'&&!t.isBalancing&&!t.splitParent&&grpOf(t)!=='regular')
    .reduce((a,t)=>a+(t.amount||t.amt||0),0);
