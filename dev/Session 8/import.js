  ...
  try {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      if (data.stop_reason === 'max_tokens') {
        console.warn('[PDF] Dávka', i, '⚠️ Odpověď oříznuta max_tokens – některé transakce mohou chybět');
      }
...
function calcDupScore(r, existing) {
  ...
  // Optimalizováno (FIX-055): pre-cache rDate timestamp, predčasné ukončení při score>=90,
  // a očekává, že `existing` má pre-cached _ts a _nameLow (viz buildExistingIndex).
  ...
  for(let i = 0; i < existing.length; i++) {
    ...
    // Datum ±1 den (30 bodů) – pre-cached timestamp v t._ts
    const tTs = t._ts;
    if (tTs != null && !isNaN(rTs)) {
      const diffDays = Math.abs((rTs - tTs) / 86400000);
      if (diffDays <= 1) score += 30;
      else if (diffDays <= 3) score += 10;
      else if (diffDays > 14) continue; // mimo rozumný rozsah – přeskoč úplně
    }

    // Částka (30 bodů)
    const amt1 = r.amount, amt2 = t._amt;
    ...
    // Název – fuzzy match (30 bodů) – pre-cached t._nameLow
    const n2 = t._nameLow;
    if(rNameLow && n2) {
      if(rNameLow === n2) score += 30;
      else if(rNameLow.includes(n2) || n2.includes(rNameLow)) score += 20;
      else if (rWords.length && t._words && t._words.length) {
        let shared = 0;
        for (const w of rWords) {
          for (const w2 of t._words) {
            if (w2.includes(w) || w.includes(w2)) { shared++; break; }
          }
          if (shared >= 3) break;
        }
        if (shared > 0) score += Math.min(15, shared * 5);
      }
    }

    if(score > best) { best = score; bestTx = t; }
    if (best >= 90) break; // dost dobré – ukončíme
  }
