...
function bCluster(cats,totalAll,SHARED,W,body){
  const H=320, maxV=cats[0].total;
  const POS=[{x:.18,y:.26},{x:.52,y:.14},{x:.84,y:.26},{x:.13,y:.68},{x:.5,y:.75},{x:.84,y:.68},{x:.34,y:.50},{x:.67,y:.50}];
  ...
  body.innerHTML=`<svg viewBox="0 0 ${W} ${H}" style="display:block;width:100%">${defs}${lines}${circles}</svg>
...
function renderBarChart(D){
  ...
  data.forEach((d,i)=>{
    ...
    const x0=cx-bw-1;
    const x1=cx+1;
    const ih=Math.max(2,d.inc/maxV*cH);
    const eh=Math.max(2,d.exp/maxV*cH);
