...
const VERZE_LOG = [
  {
    ...
    datum: '2026-05-19',
    zmeny: [
      '🐛 FIX-059 (TODO-074): projects.js – Detektor úspor: nesprávné měsíční částky (sčítal 3 měsíce ale labeloval "Kč/měs" → 3× nadhodnocení)',
      '🐛 FIX-059: projects.js – Detektor úspor: duplicitní záznam jedné transakce (Patreon* Membership 2× kvůli klíčovým slovům patreon+membership)',
      '🐛 FIX-059: projects.js – Odstraněna příliš široká klíčová slova: membership, členství, předplatné, google (falešné nálezy)',
      '🐛 FIX-059: projects.js – Nová logika: per-transaction matching (každá tx má MAX 1 klíčové slovo, nejdelší match)',
      '🐛 FIX-059: projects.js – Skupiny podle (klíčové_slovo + normalizovaný_název): "Patreon* John" a "Patreon* Channel X" jsou 2 různé skupiny, ne jedna',
      '🐛 FIX-059: projects.js – Měsíční průměr = totalAmt / monthsHit (ne / 3) – jednorázová tx neukáže 1/3 reálné částky',
      '🐛 FIX-059: projects.js – Label označuje četnost: "pravidelně 3 měs.", "2× za 3 měs.", "1× za 3 měs. (nemusí být předplatné)"',
      '🐛 FIX-059: projects.js – Skip transakcí < 50 Kč/měs (nezajímavé pro úsporu)',
      '🐛 FIX-059: projects.js – Úspora snížena z 40 % na 25 % (realističtější odhad)',
    ]
  },
  {
    verze: 'v6.57',
...
function adminViewUserAs(uid) {
  if (!confirm('Přepnout pohled jako tento uživatel? (Read-only – nelze měnit jejich data)')) return;
  // Použij existující viewingUid mechanismus pokud existuje
