...
// Session 10: mřížka kruhů Celkového zdraví pro 6M (6 kruhů) / 12M (12 kruhů)
// dle náčrtu uživatele – každý měsíc jeden mini ring s číslem + průměr s popiskem.
function renderHealthRingGrid(D) {
  const cont = document.getElementById('healthGridContainer');
  if (!cont) return;
  D = D || getData();
  const n = _reportPeriod === '12M' ? 12 : 6;
  const months = [];
  for (let i = n - 1; i >= 0; i--) {
  ...
  const cells = months.map(mo => `
    <div style="text-align:center">
      <canvas id="hgrid_${mo.m}_${mo.y}" width="54" height="54" style="width:54px;height:54px"></canvas>
      <div style="font-size:.62rem;color:var(--text3);margin-top:2px">${mo.label}</div>
    </div>`).join('');

  cont.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(${cols},1fr);gap:6px;justify-items:center;margin-bottom:12px">${cells}</div>
    <div style="font-size:.7rem;color:var(--text3);text-transform:uppercase;letter-spacing:.05em">Průměr</div>
    <div class="health-score-label" style="color:${healthColor(avg)}">${avg}</div>
    <div class="health-score-sub">${healthLabel(avg)}</div>`;

  setTimeout(() => {
    months.forEach(mo => drawHealthRing(`hgrid_${mo.m}_${mo.y}`, mo.score, 54));
  }, 20);
}
...
// přes CELÉ období dle zvolené periody (7D/1M/3M/6M/12M), ne jen aktuální měsíc.
...
function getActualRange(catId, sub, period, D) {
  ...
  const n = period === '3M' ? 3 : period === '6M' ? 6 : period === '12M' ? 12 : 1;
  let sum = 0;
  for (let i = 0; i < n; i++) {
    let m = S.curMonth - i, y = S.curYear; while (m < 0) { m += 12; y--; }
...
function renderReport() {
  ...
  // Záložky period + Poradce
  const periods = ['7D','1M','3M','6M','12M'];
  const labels  = {'7D':'7 dní','1M':'Měsíc','3M':'3 měs.','6M':'6 měs.','12M':'12 měs.'};
  const tabBar = `<div style="display:flex;gap:3px;margin-bottom:16px;background:var(--surface2);border-radius:12px;padding:4px;border:1px solid var(--border);overflow-x:auto">
    ${periods.map(p=>`<button onclick="reportSetPeriod('${p}')"
  ...
  const periodMonths = _reportPeriod === '3M' ? 3 : _reportPeriod === '6M' ? 6 : _reportPeriod === '12M' ? 12 : 1;
  const expCats = (D.categories||[]).filter(c => c.type==='expense'||c.type==='both');
  ...
  el.innerHTML = tabBar + `
    ...
    <div class="grid2" style="margin-bottom:16px;align-items:start">
      <div class="card" style="text-align:center;padding:24px">
        ${(_reportPeriod==='6M'||_reportPeriod==='12M')
  ...
  // Draw ring after DOM update (jen pro 1M/3M/7D – velký ring)
  if (_reportPeriod!=='6M' && _reportPeriod!=='12M') {
    setTimeout(() => drawHealthRing('mainHealthRing', scores.overall, 160), 50);
  } else {
    // 6M/12M: mřížka malých kruhů (jeden per měsíc) + průměr s popiskem
    setTimeout(() => renderHealthRingGrid(D), 50);
  }

  // Session 10: Graf vývoje skóre dle zvolené periody (1M=1, 3M=3, 6M=6, 12M=12)
  setTimeout(() => {
    const cont = document.getElementById('reportScoreChartContainer');
    if (!cont) return;
    const periodMap = { '7D':1, '1M':1, '3M':3, '6M':6, '12M':12 };
    const n = periodMap[_reportPeriod] || 1;
    const months = [];
    for (let i = n - 1; i >= 0; i--) {
      let m = S.curMonth - i, y = S.curYear; while (m < 0) { m += 12; y--; }
      const t = getTx(m, y, D);
