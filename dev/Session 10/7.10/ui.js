...
function bCluster(cats,totalAll,SHARED,W,body){
  ...
  // Relativní pozice 0–1; SVG viewBox = 0 0 W H, width:100% → vždy se vejde
  ...
  cats.forEach((cat,i)=>{
    ...
    const catTip=`<b>${cat.icon||''} ${cat.name}</b><br>${fmt(cat.total)} Kč<br><span style='color:${cat.color}'>${Math.round(cat.total/totalAll*100)} % výdajů</span>${hasShared?'<br>📎 obsahuje sdílenou podkat.':''}`;
    ...
    cat.subs.forEach((sub,j)=>{
      ...
      const subTip=`<b>${sub.name}</b><br>${fmt(sub.val)} Kč<br><span style='color:${cat.color}'>${cat.name}</span>${isS?' · 🔗 sdílená':''}`;
      ...
      circles+=`<circle cx="${sx}" cy="${sy}" r="${sr}" fill="${bRgba(isS?'#7a8aaa':cat.color,.16)}" stroke="${isS?'#7a8aaa':cat.color}" stroke-width="${isS?1.5:1.2}" ${isS?'stroke-dasharray="4,2"':''} style="cursor:pointer" onmouseenter="bTip(this,'${bEsc(subTip)}')" onmouseleave="bTip(this,'')"/>`;
    ...
    // 📎 sponka uvnitř kategorie obsahující sdílenou subkat.
    if(hasShared) circles+=`<text x="${cx+cr-7}" y="${cy-cr+11}" text-anchor="middle" font-size="11" pointer-events="none">📎</text>`;
  });
  ...
  body.innerHTML=`<svg viewBox="0 0 ${W} ${H}" style="display:block;width:100%;height:auto;overflow:visible">${defs}${lines}${circles}</svg>
    <div style="margin-top:8px;display:flex;flex-wrap:wrap">${leg}</div>
    ${SHARED.size?'<div style="font-size:.65rem;color:var(--text3);margin-top:6px">Hover pro detail · klikni na kategorii pro detail · 📎 / šedá = sdílená podkategorie</div>':'<div style="font-size:.65rem;color:var(--text3);margin-top:6px">Hover pro detail · klikni na kategorii pro detail</div>'}`;
}
