...
function renderRadar() {
  ...
  // Session 10: vykresli denní cashflow graf (po vložení do DOM)
  if(isCurrentMonth){
    setTimeout(()=>renderRadarDailyChart(txs, totalInc, avgInc, avgExp, D), 30);
  }
}
...
function renderRadarDailyChart(txs, monthInc, avgInc, avgExp, D){
  ...
  // denní výdaje + den příjmu
  const dailyExp=Array(daysInMonth+1).fill(0);
  let incomeDay=0, incomeAmount=0;
  (txs||[]).forEach(t=>{
    const d=new Date(t.date).getDate();
    if(t.type==='expense' && !t.isBalancing) dailyExp[d]+=Math.abs(t.amount||t.amt||0);
    if(t.type==='income'){ incomeAmount+=(t.amount||t.amt||0); if(!incomeDay)incomeDay=d; }
  });
  // cíl příjmu: skutečný měsíční příjem nebo průměr
  const incomeTarget = Math.max(monthInc, avgInc, 1);
  // kumulativní výdaje den po dni (do dneška)
  ...
  drawRadarDaily(canvas, daysInMonth, todayDay, cumExp, dailyExp, incomeTarget, predEnd, incomeDay);
}
function drawRadarDaily(canvas, days, todayDay, cumExp, dailyExp, incomeTarget, predEnd, incomeDay){
  const W=canvas.parentElement.clientWidth||500; canvas.width=W;
  const H=canvas.height||200;
  const ctx=canvas.getContext('2d'); ctx.clearRect(0,0,W,H);
  const maxDailyExp=Math.max(...dailyExp,1);
  const maxV=Math.max(incomeTarget, cumExp[todayDay]||0, predEnd, 1)*1.05;
  const pad=46,right=14,top=12,bottom=22;
  ...
  for(let i=0;i<=4;i++){const y=top+H2*(1-i/4);ctx.strokeStyle='rgba(46,51,71,.5)';ctx.lineWidth=1;ctx.setLineDash([3,3]);ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(W-right,y);ctx.stroke();
    ctx.fillStyle='#7e84a0';ctx.font='8px Instrument Sans';ctx.textAlign='right';ctx.fillText(Math.round(maxV*i/4/1000)+'k',pad-4,y+3);}
  ctx.setLineDash([]);
  // modré sloupce – denní výdaj (škálované do spodní 1/3, aby nezakryly čáry)
  const barMaxH=H2*0.4;
  for(let d=1;d<=todayDay;d++){
    if(dailyExp[d]<=0) continue;
    const x=xOf(d), bh=dailyExp[d]/maxDailyExp*barMaxH;
    ctx.fillStyle='rgba(96,165,250,.45)';
    ctx.fillRect(x-2, H-bottom-bh, 4, bh);
  }
  // zelená linie příjmu (vodorovná na úrovni cíle)
  ...
  // bílá kumulativní výdaje (do dneška)
  ...
  ctx.stroke();
  // žlutá predikce (od dneška do konce, navazuje na bílou)
  ctx.strokeStyle='var(--debt)';ctx.strokeStyle='#fbbf24';ctx.lineWidth=2;ctx.setLineDash([5,4]);ctx.beginPath();
  ...
  if(incomeDay>0){
    ...
    ctx.fillStyle='#4ade80';ctx.font='8px Instrument Sans';ctx.textAlign='center';ctx.fillText('příjem',ix,incY-9);
  }
  // osa X – dny (po 5)
  ctx.fillStyle='#7e84a0';ctx.font='9px Instrument Sans';ctx.textAlign='center';
  for(let d=1;d<=days;d+=5){ctx.fillText(d,xOf(d),H-4);}
  // svislá čára "dnes"
  if(todayDay<days){const tx=xOf(todayDay);ctx.strokeStyle='rgba(232,234,242,.2)';ctx.lineWidth=1;ctx.setLineDash([2,3]);ctx.beginPath();ctx.moveTo(tx,top);ctx.lineTo(tx,H-bottom);ctx.stroke();ctx.setLineDash([]);}
}
