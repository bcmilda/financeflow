...
function renderRadar() {
  ...
  el.innerHTML = tabIntro('radar','🎯','Finanční radar',
      ...
      <div class="card-body">
        ...
        <div style="background:${freeToSpend>=0?'rgba(74,222,128,.08)':'var(--expense-bg)'};border:1px solid ${freeToSpend>=0?'rgba(74,222,128,.25)':'rgba(248,113,113,.3)'};border-radius:10px;padding:11px 14px;margin-bottom:12px">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <span style="font-size:.8rem;color:var(--text2)">💸 Můžeš ještě utratit do konce měsíce</span>
            <span style="font-family:Syne;font-size:1.25rem;font-weight:800;color:${freeToSpend>=0?'var(--income)':'var(--expense)'}">${freeToSpend>=0?'':'−'}${fmt(Math.abs(freeToSpend))} Kč</span>
          </div>
          <div style="font-size:.66rem;color:var(--text3);margin-top:3px">Příjem ${fmt(Math.round(incForFree))} − utraceno ${fmt(totalExp)} − rezerva na budoucí platby ${fmt(Math.round(budRestMonth))}</div>
        </div>` : ''}
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:8px;font-size:.68rem;color:var(--text3)">
          ...
          ${isCurrentMonth?`<span style="display:flex;align-items:center;gap:4px"><span style="width:14px;height:3px;background:var(--debt);display:inline-block;border-radius:2px"></span>Predikce</span>`:''}
          <span style="display:flex;align-items:center;gap:4px"><span style="width:14px;height:3px;background:#fbbf24;display:inline-block;border-radius:2px"></span>Trend (% z měs.)</span>
    ...
    ${bud90.length ? `
    <div class="card" style="margin-bottom:14px">
      <div class="card-header"><span class="card-title">🗓️ Nadcházející platby</span><span style="font-size:.7rem;color:var(--text3)">30 / 60 / 90 dní</span></div>
      <div class="card-body" style="padding:10px 14px">
        ${(() => {
          ...
          // agregace dle názvu: do kterého okna spadá nejbližší výskyt + sumy v oknech
          let sum30=0,sum60=0,sum90=0;
          bud90.forEach(b=>{ const dt=new Date(b.date); if(dt<=d30)sum30+=b.amount; if(dt<=d60)sum60+=b.amount; if(dt<=d90)sum90+=b.amount; });
          // seznam nejbližších plateb (do 90 dní), max 8
          const rows=bud90.filter(b=>new Date(b.date)<=d90).sort((a,b)=>new Date(a.date)-new Date(b.date)).slice(0,8);
          return `
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px">
            <div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center">
              <div style="font-size:.66rem;color:var(--text3)">30 dní</div>
              <div style="font-family:Syne;font-size:1rem;font-weight:800;color:var(--expense)">${fmt(Math.round(sum30))}</div>
            </div>
            <div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center">
              <div style="font-size:.66rem;color:var(--text3)">60 dní</div>
              <div style="font-family:Syne;font-size:1rem;font-weight:800;color:var(--expense)">${fmt(Math.round(sum60))}</div>
            </div>
            <div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center">
              <div style="font-size:.66rem;color:var(--text3)">90 dní</div>
              <div style="font-family:Syne;font-size:1rem;font-weight:800;color:var(--expense)">${fmt(Math.round(sum90))}</div>
            </div>
          </div>
          <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:.78rem">
            <thead><tr style="color:var(--text3);text-align:left">
              <th style="padding:5px 6px">Platba</th><th style="padding:5px 6px">Datum</th>
              <th style="padding:5px 6px;text-align:center">Okno</th><th style="padding:5px 6px;text-align:right">Částka</th>
            </tr></thead><tbody>
            ${rows.map(b=>{
              const dt=new Date(b.date);
              const win = dt<=d30?'30':dt<=d60?'60':'90';
              const winColor = win==='30'?'var(--expense)':win==='60'?'var(--debt)':'var(--text3)';
              return `<tr style="border-top:1px solid var(--border)">
                <td style="padding:5px 6px"><span style="margin-right:5px">${b.icon}</span>${b.name}</td>
                <td style="padding:5px 6px;color:var(--text3)">${dt.toLocaleDateString('cs-CZ',{day:'numeric',month:'short'})}</td>
                <td style="padding:5px 6px;text-align:center"><span style="font-size:.66rem;color:${winColor};border:1px solid ${winColor};border-radius:4px;padding:1px 5px">${win} dní</span></td>
                <td style="padding:5px 6px;text-align:right;font-weight:600;color:${b.color||'var(--expense)'}">${fmt(b.amount)} Kč</td>
              </tr>`;
            }).join('')}
            </tbody></table></div>
          ${bud90.filter(b=>new Date(b.date)<=d90).length>8?`<div style="font-size:.7rem;color:var(--text3);text-align:right;margin-top:6px">+${bud90.filter(b=>new Date(b.date)<=d90).length-8} dalších plateb</div>`:''}
          `;
        })()}
      </div>
    </div>` : ''}
...
function renderRadarDailyChart(txs, monthInc, avgInc, avgExp, D){
  ...
  // Session 10: žlutá křivka TRENDU UTRÁCENÍ – poměr kumul. útraty k CELKOVÉ útratě měsíce (0–100 %)
  const totalMonthExp = cumExp[daysInMonth]||0;
  const trendPct=[0];
  for(let d=1;d<=daysInMonth;d++){ trendPct[d] = totalMonthExp>0 ? cumExp[d]/totalMonthExp : 0; }
  const isCurrent = (S.curMonth===today.getMonth()&&S.curYear===today.getFullYear());
  // ulož data pro interaktivitu (hover)
  canvas._radarData = {daysInMonth, todayDay, cumolEnd, cumExp, dailyExp, incomeTarget, predEnd, incomeDay, trendPct, totalMonthExp, isCurrent};
...
function drawRadarDaily(canvas, hoverDay){
  ...
  const {daysInMonth:days, todayDay, cumolEnd, cumExp, dailyExp, incomeTarget, predEnd, incomeDay, trendPct, totalMonthExp, isCurrent}=d0;
  const W=Math.max(canvas.parentElement?.clientWidth||0, 320); canvas.width=W;
  const H=250; canvas.height=H;
  const ctx=canvas.getContext('2d'); ctx.clearRect(0,0,W,H);
  const pad=54,right=46,top=18,bottom=32; // větší pravý pad – konec neuteče
  ...
  const yPct=p=>top+H2*(1-p); // pravá osa 0–1 (trend %)
  // horizontální mřížka + osa Y (Kč) vlevo + % vpravo
  for(let i=0;i<=4;i++){const y=top+H2*(1-i/4);ctx.strokeStyle='rgba(46,51,71,.5)';ctx.lineWidth=1;ctx.setLineDash([3,3]);ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(W-right,y);ctx.stroke();
    ...
    ctx.fillStyle='#fbbf24';ctx.textAlign='left';ctx.fillText((i*25)+'%',W-right+4,y+3);}
  ...
  ctx.setLineDash([]);
  // popisek osy Y
  ctx.save();ctx.translate(13,top+H2/2);ctx.rotate(-Math.PI/2);ctx.fillStyle='#7e84a0';ctx.font='9px Instrument Sans';ctx.textAlign='center';ctx.fillText('Kč (kumulativně)',0,0);ctx.restore();
  ...
  // žlutá křivka TRENDU UTRÁCENÍ (% z celkové měsíční útraty, pravá osa)
  if(trendPct && totalMonthExp>0){
    ctx.strokeStyle='#fbbf24';ctx.lineWidth=2;ctx.beginPath();
    let f=true;
    for(let d=1;d<=cumolEnd;d++){const x=xOf(d),y=yPct(trendPct[d]);if(f){ctx.moveTo(x,y);f=false;}else ctx.lineTo(x,y);}
    ctx.stroke();
  }
  ...
  ctx.stroke();
  // oranžová predikce zbytku – jen pro AKTUÁLNÍ měsíc (u minulých nemá smysl)
  if(isCurrent){
  ...
  // DNEŠNÍ den – tečka na kumulativní + svislá čára
  ...
  // osa X – VŠECHNY dny po 2 (a poslední)
  ctx.fillStyle='#a8aec8';ctx.font='9px Instrument Sans';ctx.textAlign='center';
  for(let d=1;d<=days;d+=2){ ctx.fillText(d,xOf(d),H-12); }
  if(days%2===0) ctx.fillText(days,xOf(days),H-12);
  ...
  if(hoverDay){
    ...
    const hv = cumExp[Math.min(hoverDay,cumolEnd)]||0;
    ctx.fillStyle='#e8eaf2';ctx.beginPath();ctx.arc(x,yOf(hv),4,0,7);ctx.fill();
  }
}
function radarDailyHover(e, canvas){
  ...
  const {daysInMonth:days, cumolEnd, cumExp, dailyExp}=d0;
  const pad=54,right=46; const W2=canvas.width-pad-right;
  ...
  tt.innerHTML = `<b>${day}. den</b><br>výdaj: ${fmt(Math.round(dEx))} Kč<br>kumul.: ${fmt(Math.round(cumExp[Math.min(day,cumolEnd)]||0))} Kč`;
}
