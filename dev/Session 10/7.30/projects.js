  ...
  el.innerHTML = tabIntro('radar','🎯','Finanční radar',
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="card-body">
        ...
        <div style="font-size:.66rem;color:var(--text3);margin-top:6px;line-height:1.5">Bílá čára = kolik jsi celkem utratil (kumulativně). Zelená = úroveň příjmu měsíce. Modré sloupce = denní výdaj (ve stejném měřítku jako osa). Najeď myší pro detail dne.</div>
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="card-body" style="padding:10px 14px">
        ${(() => {
          ...
          const mkEnd=n=>{const d=new Date(now);d.setDate(d.getDate()+n);return d;};
          const windows=[{n:30,color:'var(--expense)'},{n:60,color:'var(--debt)'},{n:90,color:'#a78bfa'}];
          const fmtD=dt=>dt.toLocaleDateString('cs-CZ',{day:'numeric',month:'numeric'});
          const colHtml = windows.map(w=>{
            const end=mkEnd(w.n);
            const items=bud90.filter(b=>{const dt=new Date(b.date);return dt>=now && dt<=end;})
              .sort((a,b)=>new Date(a.date)-new Date(b.date));
            const sum=items.reduce((a,b)=>a+b.amount,0);
            const show=items.slice(0,7);
            return `<div style="flex:1;min-width:200px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;overflow:hidden">
              <div style="text-align:center;padding:8px;border-bottom:1px solid var(--border)">
                <div style="font-size:.68rem;color:var(--text3)">${w.n} dní</div>
                <div style="font-family:Syne;font-size:1.05rem;font-weight:800;color:${w.color}">${fmt(Math.round(sum))} Kč</div>
              </div>
              <table style="width:100%;border-collapse:collapse;font-size:.74rem">
                ...
                ${show.map(b=>`<tr style="border-top:1px solid var(--border)">
                  <td style="padding:4px 8px"><span style="margin-right:4px">${b.icon}</span>${b.name.length>16?b.name.slice(0,15)+'…':b.name}</td>
                  <td style="padding:4px 6px;color:var(--text3);white-space:nowrap">${fmtD(new Date(b.date))}</td>
                  <td style="padding:4px 8px;text-align:right;font-weight:600;color:${b.color||'var(--expense)'};white-space:nowrap">${fmt(b.amount)}</td>
                </tr>`).join('')}
                ${items.length>7?`<tr><td colspan="3" style="padding:4px 8px;text-align:center;color:var(--text3);font-size:.68rem">+${items.length-7} dalších</td></tr>`:''}
                ${!items.length?`<tr><td colspan="3" style="padding:8px;text-align:center;color:var(--text3)">Žádné platby</td></tr>`:''}
                </tbody>
              </table>
            </div>`;
          }).join('');
          return `<div style="display:flex;gap:10px;flex-wrap:wrap">${colHtml}</div>
            <div style="font-size:.66rem;color:var(--text3);margin-top:8px;line-height:1.5">Sloupce jsou kumulativní: 60 dní zahrnuje i platby z 30 dní, 90 dní zahrnuje vše. Opakující se platby (nájem, splátky) se objevují každý měsíc.</div>`;
        })()}
      </div>
    </div>` : ''}
...
function renderRadarDailyChart(txs, monthInc, avgInc, avgExp, D){
  ...
  let incomeDay=0, realMonthInc=0;
  (txs||[]).forEach(t=>{
    const d=new Date(t.date).getDate();
    if(t.type==='expense' && !t.isBalancing) dailyExp[d]+=Math.abs(t.amount||t.amt||0);
    if(t.type==='income'){ realMonthInc+=(t.amount||t.amt||0); if(!incomeDay) incomeDay=d; }
  });
  ...
  // TREND PO TÝDNECH OD VÝPLATY (Session 10) – referenční bod = den výplaty (incomeDay)
  // Týden 1 = dny 1–7 od výplaty, atd. Počítáme průměr Kč/den (poslední týden má méně dní).
  const payday = incomeDay || 1;
  ...
  canvas._radarData = {daysInMonth, todayDay, cumolEnd, cumExp, dailyExp, incomeTarget, incomeIsReal, predEnd, incomeDay, weeks, isCurrent};
...
function drawRadarDaily(canvas, hoverDay){
  ...
  const {daysInMonth:days, todayDay, cumolEnd, cumExp, dailyExp, incomeTarget, incomeIsReal, predEnd, incomeDay, isCurrent}=d0;
