// FinanceFlow · v8.61 · kalendar.js · 2026-07-02
// ══════════════════════════════════════════════════════
...
function renderKalendar() {
  ...
  txs.forEach(t => {
    ...
    const amt = txCZK(t, D); // v8.61 (TODO-151): denní sumy v základní měně
    if (t.type === 'income')  perDay[d].inc += amt;
    if (t.type === 'expense') perDay[d].exp += amt;
    perDay[d].txs.push(t);
  });

  const totalInc = txs.filter(t=>t.type==='income').reduce((a,t)=>a+txCZK(t,D),0); // v8.61
  ...
  // Pace indikátor: průměrný výdaj na "utrácecí" den (Session 11)
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    html += `
      ...
      >
        ...
        <!-- Hlavní číslo - saldo (uprostřed, plné, Session 11) -->
        ${hasTx ? `
          ...
          ${(() => {
            // Pace indikátor – jak rychle se daný den utrácí vs. průměrný utrácecí den
            if (!(data.exp > 0) || avgDailyExp <= 0) return '';
            const ratio = data.exp / avgDailyExp;
            const w = Math.min(100, Math.round(ratio / 2 * 100)); // 2× průměr = plná šířka
...
// Formátuj číslo kompaktně (1234 → 1,2k; funguje i pro záporná: -1492 → -1,5k)
function fmtK(n) {
  const neg = n < 0 ? '-' : '';
  const a = Math.abs(n);
...
function showKalendarDay(d, m, y) {
  ...
  const inc = txs.filter(t=>t.type==='income').reduce((a,t)=>a+txCZK(t),0); // v8.61
