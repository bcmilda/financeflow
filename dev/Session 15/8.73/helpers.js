// FinanceFlow · v8.73 · helpers.js · 2026-07-06
//  HELPERS
// ══════════════════════════════════════════════════════
...
const getActual=(catId,sub,m,y,data)=>{
  ...
  return txs.filter(t=>t.type==='expense'&&!t.isBalancing&&t.catId===catId&&(!sub||t.subcat===sub)).filter(t=>{const d=new Date(t.date);return d.getMonth()===m&&d.getFullYear()===y;}).filter(t=>!(t.splitId&&t.splitParent&&splitIdsWithChildren.has(t.splitId))).reduce((a,t)=>a+txCZK(t,D),0);};
// ══════════════════════════════════════════════════════
...
// Efektivní měsíční příjem pro DTI/DSTI: základ příjmu, fallback 3M průměr skutečných příjmů
function computeEffectiveIncome(D){
  let inc=(typeof computeBaseIncome==='function')?computeBaseIncome(D):0;
  if(!inc){
    let t=0,n=0;
    for(let i=0;i<3;i++){ let m=S.curMonth-i,y=S.curYear; if(m<0){m+=12;y--;}
      const mi=incSum(getTx(m,y,D)); if(mi>0){t+=mi;n++;} }
    inc=n?Math.round(t/n):0;
  }
  return inc;
}
