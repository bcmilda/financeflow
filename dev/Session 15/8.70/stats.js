// FinanceFlow · v8.70 · stats.js · 2026-07-05
//  STATISTIKY
// ══════════════════════════════════════════════════════
...
function moveCatUp(id, ev){
  if(_catMoveGuard && Date.now()<_catMoveGuard.until && _catMoveGuard.id!==id){
    // v8.70 (FIX-182): kurzor po clampu skončil nad jinou kartou – tento klik by přesunul špatnou kategorii
    const mc=document.querySelector(`.cat-item[data-cat-id="${_catMoveGuard.id}"]`);
    if(mc){ mc.style.outline='2px solid var(--bank)'; setTimeout(()=>{ mc.style.outline=''; }, 500); }
    return;
  }
...
let _catMoveGuard=null; // v8.70 (FIX-182): po clampnutém scrollu blokuj klik na JINOU kategorii (anti-bounce)
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    requestAnimationFrame(()=>{
      ...
      if(Math.abs((r2.top + r2.height/2) - targetY) > 8){
        ...
        _catMoveGuard={ id, until: Date.now()+500 };
...
function catTypeChanged(){
  ...
  if(savRow) savRow.style.display=isTransfer?'flex':'none';
  const invRow=document.getElementById('catIsInvestRow');
  if(invRow) invRow.style.display=isTransfer?'flex':'none'; // v8.70
  // Hint pro přesun
