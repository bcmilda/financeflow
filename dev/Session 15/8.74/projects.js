// FinanceFlow · v8.74 · projects.js · 2026-07-06
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function renderReport() {
  ...
  const catRows = expCats.map(cat => {
    ...
    const trend = (() => {
      ...
      // v8.74: trend = MEZIMĚSÍČNÍ ZMĚNA (ne limit). Když kategorie DRŽÍ LIMIT (zelená/bez limitu),
      // růst není červený poplach – obarvíme šedě, aby to nemátlo (436 % růst + 100 skóre = OK).
      const inLimit = noLimit || score===null || score>=71;
      const upColor = inLimit ? 'var(--text3)' : 'var(--expense)';
      const tip = inLimit ? ' title="Meziměsíční změna výdajů – v limitu, jen informativní"' : ' title="Meziměsíční růst výdajů + překročený limit"';
      return d > 5 ? `<span style="color:${upColor};font-size:.7rem"${tip}>↑${d}%</span>` :
