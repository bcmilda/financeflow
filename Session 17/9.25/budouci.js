...
// S17.25 (Milan): volitelný 3. parametr `fromDate` – umožňuje získat plán i ZPĚTNĚ od zvoleného
// data (např. od 1. dne měsíce), ne jen od dneška. Používá Finanční radar pro plný sloupec
// „Budoucí platby" za celý měsíc. Bez parametru se chová přesně jako dřív.
function budouciGetAll(D, horizonDays, fromDate) {
  D = D || getData();
  const today   = fromDate ? new Date(fromDate) : new Date(); today.setHours(0,0,0,0);
...
function budouciPayControl(item, today) {
  ...
  if (!sameMonth) return '';                 // jen aktuální měsíc, budoucí měsíce bez ovladače
  const due = new Date(item.date); due.setHours(0, 0, 0, 0);
  const badge = (bg, col, txt) => `<span style="flex-shrink:0;margin-left:8px;padding:2px 8px;border-radius:6px;background:${bg};color:${col};font-size:.7rem;font-weight:700;white-space:nowrap">${txt}</span>`;

  if (due > now) {
    // S17.8 (Milan): splatnost TENTO měsíc, ale ještě NENASTALA. Zaplaceno jen když je
    // opravdu zapsaná transakce přesně k tomuto dni (auto-vytváření), jinak Nezaplaceno.
    // Žádné tlačítko – nebudeme pobízet k zápisu platby, která ještě není splatná.
    return budouciIsPaid(item, getData(), true)
      ? badge('var(--income-bg)', 'var(--income)', '✓ Zaplaceno')
      : badge('var(--surface3)', 'var(--text3)', 'Nezaplaceno');
  }
