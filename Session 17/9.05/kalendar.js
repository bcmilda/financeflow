// FinanceFlow · v9.05 · kalendar.js · 2026-07-20
// ══════════════════════════════════════════════════════
...
function _workWageCard(cfg, y, m, daysInMonth) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    const wd = _workDay(y, m, d);
    if (!wd || wd.type !== 'smena') continue;
    shiftDays++;
    const h = wd.hours || 0;
    const p = Math.max(0, h - cfg.breakMin / 60);
    grossH += h; payH += p;
    otH += Math.max(0, h - cfg.hpd);
...
function showWorkDay(d, m, y) {
  ...
  document.getElementById('kalendarDayContent').innerHTML = `
    ...
    <div id="workHoursRow" style="${curType === 'smena' ? '' : 'display:none;'}margin-bottom:14px">
      <div style="font-size:.78rem;color:var(--text3);margin-bottom:4px">Odpracované hodiny <span style="color:#a8aec8">(směna ${cfg.hpd} h + přesčas = celkem)</span></div>
