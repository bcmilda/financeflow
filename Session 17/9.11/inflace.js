// FinanceFlow · v9.11 · inflace.js · 2026-07-23
// ══════════════════════════════════════════════════════
...
function _inflCollect() {
  ...
  receipts.forEach(r => {
    ...
    (r.items || []).forEach(it => {
      ...
      obs.push({
        key, name: (it.name || '').trim(), store, date, ts, unit,
  ...
  const items = [...new Set(obs.map(o => o.key))].sort();
...
function renderInflaceItemPicker() {
  ...
  box.innerHTML = keys.map(k => {
    ...
    return `<button onclick="inflToggleItem('${k.replace(/'/g, "\\'")}')" style="padding:4px 10px;border-radius:12px;font-size:.7rem;cursor:pointer;border:1px solid ${on ? 'var(--income)' : 'var(--border)'};background:${on ? 'rgba(74,222,128,.16)' : 'transparent'};color:${on ? 'var(--income)' : '#c9cede'};white-space:nowrap">${k}</button>`;
