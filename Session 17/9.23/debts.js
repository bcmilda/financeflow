//  FinanceFlow · v9.23 · debts.js · 2026-07-26
//  ADD / EDIT TX
// ══════════════════════════════════════════════════════
function openAddTx(){
...
function renderDebtStressWidget(D) {
  ...
  el.innerHTML=`<div class="stress-card" style="background:${stressBg};border-color:${stressBorder}">
    ...
    <!-- gauge ve stylu Finančního obrazu -->
    <div style="position:relative;height:9px;border-radius:5px;overflow:hidden;background:linear-gradient(90deg,#4ade80 0%,#4ade80 33%,#fbbf24 33%,#fbbf24 66%,#f87171 66%,#f87171 100%);margin-bottom:5px">
      <div style="position:absolute;top:-2px;left:${Math.min(99,Math.max(0,totalScore))}%;transform:translateX(-50%);width:4px;height:13px;background:#fff;border-radius:2px;box-shadow:0 0 5px rgba(0,0,0,.6)"></div>
    </div>
    <div style="display:flex;justify-content:space-between;font-size:.66rem;color:#a8aec8;margin-bottom:10px">
      <span>Stabilní</span><span>Rizikové</span><span>Spirála</span>
    </div>
    <div style="text-align:center;margin-bottom:14px">
      <span style="font-family:Syne,sans-serif;font-size:1.05rem;font-weight:800;color:${stressColor}">${stressLabel}</span>
      ${incomeTrend!==0?`<div style="font-size:.7rem;color:#a8aec8;margin-top:2px">Příjem ${incomeTrend>0?'↑ roste':'↓ klesá'} o ${Math.abs(Math.round(incomeTrend))} % vs. minulý měsíc</div>`:''}
    </div>
