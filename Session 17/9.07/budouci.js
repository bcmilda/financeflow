// FinanceFlow · v9.07 · budouci.js · 2026-07-21
// ══════════════════════════════════════════════════════
...
function budouciIsPaid(item, D) {
  D = D || getData();
  const dateStr = item.date.toISOString().slice(0, 10);
  const ym = dateStr.slice(0, 7);
  const nm = (item.name || '').trim().toLowerCase();
  // shoda: stejný měsíc, podobný název, a je to výdaj/přesun (ne příjem)
  return (D.transactions || []).some(t => {
    ...
    if (!tn || !nm) return false;
    return tn === nm || tn.includes(nm) || nm.includes(tn);
  });
}
function budouciPayControl(item, today) {
  if (!(item.amount > 0 && item.source !== 'goal' && item.source !== 'bday')) return '';
  if (typeof viewingUid !== 'undefined' && viewingUid) return '';
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sameMonth = item.date.getFullYear() === now.getFullYear() && item.date.getMonth() === now.getMonth();
  if (!sameMonth) return '';                 // jen aktuální měsíc
  const paid = budouciIsPaid(item, getData());
  if (paid) {
    return `<span style="flex-shrink:0;margin-left:8px;padding:3px 9px;border-radius:7px;background:var(--income-bg);color:var(--income);font-size:.72rem;font-weight:700;white-space:nowrap">✓ Zaplaceno</span>`;
  }
  const due = new Date(item.date); due.setHours(0, 0, 0, 0);
  if (due > now) {
    // splatnost tento měsíc, ale ještě nenastala → jen tichý štítek, ne akce
    return `<span style="flex-shrink:0;margin-left:8px;padding:3px 9px;border-radius:7px;background:var(--surface3);color:var(--text3);font-size:.72rem;white-space:nowrap">Nezaplaceno</span>`;
  }
  // po splatnosti a nezaplaceno → nabídni zapsání (předvyplní transakci)
  return `<button type="button" onclick="event.stopPropagation();budouciMarkPaid('${encodeURIComponent(item.name)}',${item.amount},${item.isTransfer ? 1 : 0},'${item.date.toISOString().slice(0, 10)}')"
      ...
      style="flex-shrink:0;margin-left:8px;padding:4px 10px;border-radius:7px;border:1px solid rgba(251,191,36,.45);background:rgba(251,191,36,.10);color:var(--debt);font-size:.72rem;font-weight:700;cursor:pointer;white-space:nowrap">Zaznamenat</button>`;
}
