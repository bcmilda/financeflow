...
## Nové úkoly Session 11 (TODO-100 až 101)

### TODO-100 · Server-side automatizace Body → Premium **(Session 11, 🟢 P3)**
- **Popis:** Aktuálně (v7.33) uplatnění bodů běží na klientovi: `redeemPointsForPremium()` odečte `N×500` z `users/{uid}/referral/earned` (vlastní zápis uživatele) a vytvoří požadavek do `/support`; admin pak aktivuje Premium ručně přes `activatePremiumManually()`. Při současném modelu důvěry je to konzistentní, ale odečet bodů i samotná aktivace Premium patří na **server (Cloudflare Worker)** se server-side validací – aby si klient nemohl Premium ani body upravit sám.
- **Návrh:** Worker endpoint `/redeem` ověří zůstatek bodů (Firebase Admin SDK / REST + JWT, viz ADR-041), atomicky odečte body a nastaví `premium.until`. Klient pouze volá endpoint.
- **Závislost:** Firebase Admin SDK přes JWT REST v Workeru (sdíleno s AI Rate Limiting ADR-041). Vhodné spustit současně s platbami (TODO-097 / ADR-053).
- **Priorita:** 🟢 P3 (čeká na odblokování plateb / Worker auth).
- **🔗 Cross-reference:** `decisions.md` ADR-055; `bugs.md` FIX-112/113; ADR-041, ADR-053, TODO-097.

### TODO-101 · Badge nepřečtených oznámení v navigaci **(Session 11, 🔵 P4)**
- **Popis:** `announcements.js` má hotovou logiku `unreadAnnouncementsCount()` + `updateAnnounceBadge()` (`localStorage ff_announce_seen`) a badge `#announceBadge` na rozbalovacím řádku v O aplikaci. Zbývá (volitelně) přidat badge i k položce „O aplikaci" v hlavní navigaci/sidebaru a aktualizovat ho při startu (např. v `updateNotificationBadge`).
- **Priorita:** 🔵 P4 (nice-to-have).

*Konsolidováno: 2026-04-23 | Doplněno S6: 2026-05-15 | Session 7.0+7.1 patch: 2026-05-15 | Sessions: 1 → 11 | Poslední update: Session 11, 2026-06-02 | Autor: Milan Migdal*
