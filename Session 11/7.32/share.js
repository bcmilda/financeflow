...
async function renderShareSection() {
  ...
  const monthsEarned = Math.floor(earned / POINTS_PER_PREMIUM_MONTH);
  const toNext       = POINTS_PER_PREMIUM_MONTH - (earned % POINTS_PER_PREMIUM_MONTH);
  const progressPct  = Math.min(100, Math.round((earned % POINTS_PER_PREMIUM_MONTH) / POINTS_PER_PREMIUM_MONTH * 100));
  ...
  el.innerHTML = `
    <!-- Referral stats (počítadlo – ponecháno) -->
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px">
      <div style="background:var(--surface2);border-radius:10px;padding:11px;text-align:center;border:1px solid var(--border)">
        ...
        <div style="font-size:.72rem;color:#a8aec8">Kliknutí</div>
      </div>
      <div style="background:var(--surface2);border-radius:10px;padding:11px;text-align:center;border:1px solid var(--border)">
        <div style="font-size:1.4rem;font-weight:700;font-family:'Syne',sans-serif;color:var(--bank)">${_refStats.conversions}</div>
        <div style="font-size:.72rem;color:#a8aec8">Registrací</div>
      </div>
      <div style="background:var(--surface2);border-radius:10px;padding:11px;text-align:center;border:1px solid var(--border)">
        <div style="font-size:1.4rem;font-weight:700;font-family:'Syne',sans-serif;color:var(--premium)">${earned}</div>
        <div style="font-size:.72rem;color:#a8aec8">Bodů</div>
      </div>
    </div>

    <!-- Tvůj odkaz (ponecháno) -->
    <div style="background:var(--surface2);border-radius:10px;padding:12px 14px;border:1px solid var(--border);margin-bottom:14px">
      <div style="font-size:.74rem;color:#a8aec8;margin-bottom:6px">Tvůj osobní odkaz</div>
      ...
      <div style="font-size:.7rem;color:#a8aec8;margin-top:6px">Kód: <strong style="color:var(--text);font-family:monospace">${_myRefCode}</strong> · Za každou registraci získáš ${SHARE_REWARDS.signup} bodů</div>
    </div>
    ...
    <div style="background:linear-gradient(135deg,var(--premium-bg,rgba(168,85,247,.08)) 0%,rgba(168,85,247,.03) 100%);border:1px solid var(--premium-border,rgba(168,85,247,.28));border-radius:12px;padding:14px;margin-bottom:14px">
      ...
      ${canRedeem ? `
        <div style="font-size:.82rem;color:var(--text);margin-bottom:10px">Máš nasbíráno na <strong style="color:var(--premium)">${monthsEarned}&nbsp;${monthsEarned===1?'měsíc':(monthsEarned<5?'měsíce':'měsíců')}</strong> Premium zdarma! 🎉</div>
        <button class="btn btn-accent" onclick="redeemPointsForPremium()" style="width:100%;padding:12px;border-radius:10px;font-weight:700">💎 Uplatnit ${monthsEarned*POINTS_PER_PREMIUM_MONTH} bodů za ${monthsEarned}&nbsp;${monthsEarned===1?'měsíc':(monthsEarned<5?'měsíce':'měsíců')} Premium</button>
      ` : `
        <div style="font-size:.82rem;color:var(--text2);margin-bottom:8px">Sbírej body sdílením a získej Premium zdarma. Chybí ti <strong style="color:var(--premium)">${toNext} bodů</strong> do prvního měsíce.</div>
        ...
        <div style="display:flex;justify-content:space-between;font-size:.68rem;color:#a8aec8;margin-top:5px"><span>${earned % POINTS_PER_PREMIUM_MONTH} b.</span><span>${POINTS_PER_PREMIUM_MONTH} b. = 1 měsíc</span></div>
      `}
    </div>
...
//  Bezpečně: vytvoří požadavek do /support, admin ručně aktivuje
...
async function redeemPointsForPremium() {
  ...
  const ok = confirm(`Uplatnit ${months*POINTS_PER_PREMIUM_MONTH} bodů za ${months} měsíc(ů) Premium zdarma?\n\nPožadavek odešleme ke zpracování a Premium ti aktivujeme.`);
  if (!ok) return;
  ...
  try {
    ...
    await _set(_ref(_db, `support/redeem_${uid}_${Date.now()}`), {
      ...
      message: `Žádost o uplatnění ${months*POINTS_PER_PREMIUM_MONTH} bodů za ${months} měsíc(ů) Premium`,
      points: months * POINTS_PER_PREMIUM_MONTH,
      months,
      date: new Date().toISOString(),
    });
    showShareToast(`✅ Požadavek odeslán – Premium na ${months} měs. brzy aktivujeme`);
  } catch (e) {
