...
function doTransfer() {
  ...
  if(from.startsWith('goal:')) {
    ...
    // Příjem na peněženku
    const txIn={id:uid(),name:`Výběr z cíle ← ${goal.name}`,amount:amtDest,type:'income',date,wallet:to,note:note||`Cíl: ${goal.name} · výběr na ${wDest?.name||'peněženku'}`,transferId:uid(),category:'transfer',catId:'virtual_transfer',subcat:'Výběr z cíle'};
  ...
  if(to.startsWith('goal:')) {
    ...
    const txOut={id:uid(),name:`Vklad do cíle → ${goal.name}`,amount:amt,type:'expense',date,wallet:from,note:note||`Cíl: ${goal.name} · převod z ${wFrom?.name||'peněženky'}`,transferId,category:'transfer',catId:'virtual_transfer',subcat:'Vklad do cíle'};
    if(!S.transactions)S.transactions=[];
...
function renderNetWorth() {
  ...
  el.innerHTML = `<div class="networth-card">
    ...
    <div class="networth-total" style="color:${totalColor}">${fmt(displayTotal)} Kč</div>
    <div style="display:flex;flex-direction:column;gap:0">
      ${nw.rows.map(r=>`<div class="networth-row">
        ...
        <span style="font-weight:600;color:${r.balance>=0?'var(--text)':'var(--expense)'}">${r.currency&&r.currency!=='CZK'?fmtP(r.balanceCZK)+' Kč':fmt(r.balance)+' Kč'}</span>
      </div>`).join('')}
      ${nw.totalDebt>0&&_networthShowDebt?`<div class="networth-row">
        ...
        <span style="font-weight:600;color:var(--expense)">−${fmt(nw.totalDebt)} Kč</span>
      </div>`:''}
    </div>
