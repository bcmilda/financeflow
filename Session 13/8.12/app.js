...
const DEFAULT_CATEGORIES = [
  ...
  {id:'cat9', name:'Spoření',          icon:'🐷', color:'#818cf8', type:'expense', coicop:13, isSaving:true,   healthPct:10,
   subs:['Spořicí účet','Stavební spoření']},
  {id:'cat10',name:'Investice',        icon:'📈', color:'#34d399', type:'both',    coicop:13, isSaving:true,   healthPct:5, stabilityWeight:0, stable:false,
   subs:['ETF','Akcie','Krypto','Podílové fondy','DIP','Penzijní připojištění','Dluhopisy']},
  ...
  {id:'cat37',name:'Trading',          icon:'📊', color:'#059669', type:'both',    coicop:13,
   subs:['Bybit','XTB','Binance','Revolut Invest','ETF','Forex','Krypto nákup','Krypto prodej']},
  ...
  {id:'virtual_transfer',name:'Virtuální přesun', icon:'🎯', color:'#f59e0b', type:'both', coicop:null, isSaving:true, stable:false, stabilityWeight:0,
   subs:['Vklad do cíle','Výběr z cíle']},
];
...
// Zajisti že existuje kategorie Virtuální přesun (migrace pro stávající uživatele)
function ensureVirtualTransferCat() {
  if(!S.categories) S.categories = [];
  if(!S.categories.find(x => x.id === 'virtual_transfer')) {
    S.categories.push({id:'virtual_transfer',name:'Virtuální přesun',icon:'🎯',color:'#f59e0b',type:'both',coicop:null,isSaving:true,stable:false,stabilityWeight:0,subs:['Vklad do cíle','Výběr z cíle']});
  }
}
