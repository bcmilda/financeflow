...
function drawSaldoBars(id,labels,data){
  ...
  ctx.textAlign='right'; ctx.font='9.5px Instrument Sans';
...
function wishSetType(type){
  ...
  if(type==='goal'){
    ...
    // Cíl: ukaž ikonu, měsíční vklad, deadline, estimate; skryj prioritu a URL import
    ...
    document.getElementById('wishPriorityWrap').style.display='none';
    document.getElementById('wishUrlImport').style.display='none';
    ...
    // Přání: skryj cíl-pole, ukaž prioritu a URL import
    document.getElementById('wishIconWrap').style.display='none';
    document.getElementById('wishMonthlyWrap').style.display='none';
    document.getElementById('wishDeadlineWrap').style.display='none';
    document.getElementById('wishEstimate').style.display='none';
...
function openWishModal(){
  ...
  document.getElementById('wishModalTitle').textContent='Přidat přání / cíl';
  wishSetType('wish');
...
function editWish(id){
  ...
  if(w.isGoal){
    wishSetType('goal');
    document.getElementById('wishPrice').value=w.targetAmount||'';
...
function saveWish(){
  ...
  if(type==='goal'){
    const targetAmount=parseFloat(document.getElementById('wishPrice').value)||0;
    ...
    const obj={name,desc,targetAmount,monthlyTarget,deadline,icon,isGoal:true,priority:'mid'};
    ...
    const url=document.getElementById('wishUrl')?.value.trim()||'';
    const obj={name,desc,price,priority,url:url||undefined,isGoal:false};
    if(eid){const w=S.wishes.find(x=>x.id===eid);if(w)Object.assign(w,obj);}
...
function renderKumulChart(days, cumul, medVal) {
  ...
  ctx.fillStyle='#7e84a0'; ctx.font='9px Instrument Sans';
