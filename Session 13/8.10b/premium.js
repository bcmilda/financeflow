...
function doTransfer() {
  ...
  if(from.startsWith('goal:')) {
    ...
    // Příjem na peněženku
    const txIn={id:uid(),name:`Výběr z cíle ← ${goal.name}`,amount:amtDest,type:'income',date,wallet:to,note:note||'Výběr z virtuální peněženky',transferId:uid(),category:'transfer'};
  ...
  el.innerHTML = `<div class="networth-card">
    ...
    <div style="display:flex;flex-direction:column;gap:0">
      ${nw.rows.map(r=>`<div class="networth-row">
        ...
        <span style="flex:1;color:var(--text2)">${typeIcons[r.type]||'👛'} ${r.name}${r.currency&&r.currency!=='CZK'?` <span style="font-size:.7rem;color:var(--text3)">(${r.currency})</span>`:''}</span>
        <span style="font-weight:600;color:${r.balance>=0?'var(--text)':'var(--expense)'}">${r.currency&&r.currency!=='CZK'?fmtP(r.balance)+' '+r.currency:fmt(r.balance)}</span>
      </div>`).join('')}
