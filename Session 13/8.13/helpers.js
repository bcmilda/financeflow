...
const _BUILTIN_CATS={
  virtual_transfer:{id:'virtual_transfer',name:'Virtuální přesun',icon:'🎯',color:'#f59e0b',subs:['Vklad do cíle','Výběr z cíle','Reverz']},
  transfer:{id:'transfer',name:'Převod',icon:'🔄',color:'#a78bfa',subs:[]}
};
const getCat=(id,cats)=>(cats||getData().categories||[]).find(c=>c.id===id)||_BUILTIN_CATS[id]||{name:'?',icon:'📋',color:'#666',subs:[]};
...
const isTransferTx=t=>!!(t&&(t.transferId||t.catId==='transfer'||t.category==='transfer'));
const incSum=txs=>txs.filter(t=>t.type==='income'&&!t.isBalancing&&!t.splitParent&&!isTransferTx(t)).reduce((a,t)=>a+(t.amount||t.amt||0),0);
const expSum=txs=>txs.filter(t=>t.type==='expense'&&!t.isBalancing&&!t.splitParent&&!isTransferTx(t)).reduce((a,t)=>a+(t.amount||t.amt||0),0);
...
const getActual=(catId,sub,m,y,data)=>{
  ...
  return txs.filter(t=>t.type==='expense'&&!t.isBalancing&&t.catId===catId&&(!sub||t.subcat===sub)).filter(t=>{const d=new Date(t.date);return d.getMonth()===m&&d.getFullYear()===y;}).filter(t=>!(t.splitId&&t.splitParent&&splitIdsWithChildren.has(t.splitId))).reduce((a,t)=>a+(t.amount||t.amt||0),0);};
