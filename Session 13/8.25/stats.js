// FinanceFlow · v8.25 · stats.js · 2026-06-20
//  STATISTIKY
// ══════════════════════════════════════════════════════
...
function renderStats(){
  ...
  if(catEl && _statCatMode==='year'){ catEl.innerHTML=''; }
  else if(catEl){
...
function renderCatPage(){
  ...
  const renderGroup=(groupCats, groupLabel)=>{
    ...
    ${groupCats.map((c)=>{
      ...
      return `<div class="cat-item" ${sharedTitle} style="flex-direction:column;align-items:stretch;padding:0;${_goldBorder}border-radius:10px;margin-bottom:6px;overflow:hidden">
        <div style="display:flex;align-items:center;gap:8px;padding:10px">
          ...
          <div style="display:flex;flex-direction:column;gap:2px;flex-shrink:0">
            ${!ro && !_catLocked?`<button class="btn btn-ghost btn-icon btn-sm" title="Posunout nahoru" onclick="moveCatUp('${c.id}')" style="font-size:.7rem;padding:2px 5px;opacity:${isFirst?.3:1}" ${isFirst?'disabled':''}>▲</button>
