...
function renderRadar() {
  ...
  const histMonths = [];
  for(let i=3;i>=1;i--){ let m=today.getMonth()-i,y=today.getFullYear(); while(m<0){m+=12;y--;} histMonths.push(getTx(m,y,D)); }
  ...
  const bud90 = typeof budouciGetAll==='function' ? budouciGetAll(D, 95) : [];
  const futureMonths = [];
  let runningBalance = eomLeft;
  for(let k=1;k<=3;k++){
    let m=today.getMonth()+k, y=today.getFullYear(); while(m>11){m-=12;y++;}
  ...
  qMonths.forEach(m => {
    if(m < today.getMonth() || (m===today.getMonth())){
  ...
  el.innerHTML = tabIntro('radar','🎯','Finanční radar',
    ...
    <!-- DENNÍ CASHFLOW AKTUÁLNÍHO MĚSÍCE (Session 10) -->
    ${isCurrentMonth ? `
    <div class="card" style="margin-bottom:14px">
      <div class="card-header">
        <span class="card-title">📊 Tento měsíc den po dni</span>
        <span style="font-size:.7rem;color:var(--text3)">výdaje vs příjem</span>
      </div>
      <div class="card-body">
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:8px;font-size:.68rem;color:var(--text3)">
          ...
          <span style="display:flex;align-items:center;gap:4px"><span style="width:14px;height:3px;background:var(--debt);display:inline-block;border-radius:2px"></span>Predikce</span>
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="card-body">
        ...
        ${isCurrentMonth ? `
        <div style="background:${eomLeft>=0?'rgba(74,222,128,.08)':'var(--expense-bg)'};border:1px solid ${eomLeft>=0?'rgba(74,222,128,.25)':'rgba(248,113,113,.3)'};border-radius:10px;padding:12px 14px;margin-bottom:12px">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <span style="font-size:.82rem;color:var(--text2)">Na konci měsíce ti ${eomLeft>=0?'zbude':'bude chybět'}</span>
            <span style="font-family:Syne;font-size:1.3rem;font-weight:800;color:${eomLeft>=0?'var(--income)':'var(--expense)'}">${eomLeft>=0?'+':''}${fmt(eomLeft)} Kč</span>
          </div>
          <div style="font-size:.68rem;color:var(--text3);margin-top:4px">
            Očekávaný příjem ${fmt(expectedIncMonth)} − dosud utraceno ${fmt(totalExp)}${budToEOM>0?` − známé platby ${fmt(Math.round(budToEOM))}`:''}
          </div>
        </div>` : ''}

        <!-- Predikce 3 měsíců -->
        <div style="font-size:.72rem;font-weight:600;color:var(--text3);text-transform:uppercase;margin-bottom:8px">Příští 3 měsíce (odhad)</div>
        ${(() => {
          const vals = futureMonths.map(f=>f.cumulative);
          const allV = [eomLeft, ...vals];
          const maxV = Math.max(...allV, 0), minV = Math.min(...allV, 0);
          const rng = (maxV-minV)||1;
          const W=320,H=110,padT=14,padB=22;
          const plotH=H-padT-padB;
          const yOf=v=>padT+(maxV-v)/rng*plotH;
          const pts=[{label:'teď',v:eomLeft},...futureMonths.map(f=>({label:f.label,v:f.cumulative}))];
          const n=pts.length, slotW=W/n;
          let line=[],dots='';
          pts.forEach((p,i)=>{
            const cx=i*slotW+slotW/2, cy=yOf(p.v);
            line.push(`${cx},${cy}`);
            dots+=`<circle cx="${cx}" cy="${cy}" r="4" fill="${p.v>=0?'var(--income)':'var(--expense)'}" stroke="#0e1018" stroke-width="1.5"/>`;
            dots+=`<text x="${cx}" y="${H-7}" text-anchor="middle" font-size="10" fill="var(--text3)">${p.label}</text>`;
            dots+=`<text x="${cx}" y="${cy-9}" text-anchor="middle" font-size="9" fill="${p.v>=0?'var(--income)':'var(--expense)'}" font-weight="700">${fmt(Math.round(p.v/1000))}k</text>`;
          });
          const zeroY=yOf(0);
          return `<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:480px;height:auto;display:block;margin:0 auto;overflow:visible">
            <line x1="0" y1="${zeroY}" x2="${W}" y2="${zeroY}" stroke="var(--border)" stroke-width="1" stroke-dasharray="3 3"/>
            <polyline points="${line.join(' ')}" fill="none" stroke="var(--bank)" stroke-width="2.5" stroke-linejoin="round"/>
            ${dots}
          </svg>`;
        })()}
        ...
        <div style="font-size:.66rem;color:var(--text3);margin-top:8px;line-height:1.5">
        ...
        <div style="margin-top:12px">
          <button class="tx-filt-btn" onclick="(typeof showPagePremium==='function'?showPagePremium:showPage)('predikce')" style="width:100%;padding:11px;font-size:.82rem;font-weight:600">
            🔮 Plná predikce roku po kategoriích →
          </button>
          <div style="font-size:.64rem;color:var(--text3);margin-top:6px;text-align:center;line-height:1.5">
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="card-body">
        ...
        ${(() => {
          ...
          const W=320, H=150, padB=24, padT=10;
          ...
          const yOf = v => padT + (maxPos - v)/range*plotH; // y pozice hodnoty
          const n = cfSeries.length;
          const slotW = W/n;
          const barW = slotW*0.16;
          let bars='', cfPts=[], cfDots='';
          cfSeries.forEach((s,i)=>{
            const cx = i*slotW + slotW/2;
            const y0 = yOf(0);
            const iy = yOf(s.inc), ey = yOf(s.exp);
            bars += `<rect x="${cx-barW-2}" y="${iy}" width="${barW}" height="${Math.max(0,y0-iy)}" fill="var(--income)" opacity=".75" rx="2"/>`;
            bars += `<rect x="${cx+2}" y="${ey}" width="${barW}" height="${Math.max(0,y0-ey)}" fill="var(--expense)" opacity=".75" rx="2"/>`;
            const cfy = yOf(s.cf);
            cfPts.push(`${cx},${cfy}`);
            const sel = s.m===S.curMonth&&s.y===S.curYear;
            cfDots += `<circle cx="${cx}" cy="${cfy}" r="${sel?5:4}" fill="var(--bank)" stroke="#0e1018" stroke-width="1.5"/>`;
            cfDots += `<text x="${cx}" y="${H-8}" text-anchor="middle" font-size="10" fill="${sel?'var(--income)':'var(--text3)'}" font-weight="${sel?'700':'400'}">${s.label}</text>`;
            cfDots += `<text x="${cx}" y="${cfy + (s.cf>=0?-9:16)}" text-anchor="middle" font-size="9" fill="${s.cf>=0?'var(--income)':'var(--expense)'}" font-weight="700">${s.cf>=0?'+':''}${fmt(Math.round(s.cf/1000))}k</text>`;
          });
          const zeroY = yOf(0);
  ...
  // Session 10: vykresli denní cashflow graf (po vložení do DOM, robustně)
  if(isCurrentMonth){
...
function renderRadarDailyChart(txs, monthInc, avgInc, avgExp, D){
  ...
  (txs||[]).forEach(t=>{
    ...
    if(t.type==='income' && !incomeDay) incomeDay=d;
  });
  const incomeTarget = Math.max(monthInc, avgInc, 1);
  const cumExp=[0]; for(let d=1;d<=daysInMonth;d++){ cumExp[d] = cumExp[d-1] + (d<=todayDay?dailyExp[d]:0); }
  const spentSoFar=cumExp[todayDay]||0;
  const dailyRate = todayDay>0 ? spentSoFar/todayDay : 0;
  const predEnd = spentSoFar + dailyRate*(daysInMonth-todayDay);
  // ulož data pro interaktivitu (hover)
  canvas._radarData = {daysInMonth, todayDay, cumExp, dailyExp, incomeTarget, predEnd, incomeDay};
...
function drawRadarDaily(canvas, hoverDay){
  ...
  const {daysInMonth:days, todayDay, cumExp, dailyExp, incomeTarget, predEnd, incomeDay}=d0;
  // robustní šířka: rodič, nebo fallback
  const W=Math.max(canvas.parentElement?.clientWidth||0, 320); canvas.width=W;
  const H=240; canvas.height=H;
  const ctx=canvas.getContext('2d'); ctx.clearRect(0,0,W,H);
  const pad=52,right=16,top=16,bottom=30;
  const W2=W-pad-right, H2=H-top-bottom;
  const maxDailyExp=Math.max(...dailyExp,1);
  const maxV=Math.max(incomeTarget, cumExp[todayDay]||0, predEnd, 1)*1.08;
  ...
  ctx.setLineDash([]);
  // popisek osy Y
  ctx.save();ctx.translate(12,top+H2/2);ctx.rotate(-Math.PI/2);ctx.fillStyle='#7e84a0';ctx.font='9px Instrument Sans';ctx.textAlign='center';ctx.fillText('Kč (kumulativně)',0,0);ctx.restore();
  // modré sloupce – denní výdaj (spodní 40 %)
  const barMaxH=H2*0.4;
  for(let d=1;d<=todayDay;d++){
    if(dailyExp[d]<=0) continue;
    const x=xOf(d), bh=Math.max(2,dailyExp[d]/maxDailyExp*barMaxH);
    ctx.fillStyle = (hoverDay===d)?'rgba(96,165,250,.95)':'rgba(96,165,250,.5)';
    ctx.fillRect(x-2.5, H-bottom-bh, 5, bh);
  }
  ...
  ctx.fillStyle='#4ade80';ctx.font='9px Instrument Sans';ctx.textAlign='left';ctx.fillText('příjem '+fmt(Math.round(incomeTarget)),pad+2,incY-4);
  // bílá kumulativní výdaje
  ctx.strokeStyle='#e8eaf2';ctx.lineWidth=2.5;ctx.beginPath();
  for(let d=1;d<=todayDay;d++){const x=xOf(d),y=yOf(cumExp[d]);d===1?ctx.moveTo(x,y):ctx.lineTo(x,y);}
  ctx.stroke();
  // žlutá predikce (navazuje na bílou)
  ctx.strokeStyle='#fbbf24';ctx.lineWidth=2;ctx.setLineDash([5,4]);ctx.beginPath();
  ctx.moveTo(xOf(todayDay), yOf(cumExp[todayDay]||0));
  ctx.lineTo(xOf(days), yOf(predEnd));ctx.stroke();ctx.setLineDash([]);
  // koncový bod predikce
  ctx.fillStyle='#fbbf24';ctx.beginPath();ctx.arc(xOf(days),yOf(predEnd),3,0,7);ctx.fill();
  ctx.textAlign='right';ctx.fillText('odhad '+fmt(Math.round(predEnd)),xOf(days)-2,yOf(predEnd)-6);
  // tečka dne příjmu
  if(incomeDay>0){
  ...
  // osa X – dny (1, 5, 10... a poslední), zvětšené
  ctx.fillStyle='#a8aec8';ctx.font='10px Instrument Sans';ctx.textAlign='center';
  const xt=[1,5,10,15,20,25,days];
  xt.forEach(d=>{if(d<=days)ctx.fillText(d,xOf(d),H-10);});
  ctx.fillStyle='#7e84a0';ctx.font='9px Instrument Sans';ctx.fillText('den v měsíci',pad+W2/2,H-1);
  // svislá čára "dnes"
  if(todayDay<days){const tx=xOf(todayDay);ctx.strokeStyle='rgba(232,234,242,.25)';ctx.lineWidth=1;ctx.setLineDash([2,3]);ctx.beginPath();ctx.moveTo(tx,top);ctx.lineTo(tx,H-bottom);ctx.stroke();ctx.setLineDash([]);}
  // hover zvýraznění
  if(hoverDay && hoverDay<=todayDay){
    const x=xOf(hoverDay);
    ctx.strokeStyle='rgba(255,255,255,.3)';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(x,top);ctx.lineTo(x,H-bottom);ctx.stroke();
    ctx.fillStyle='#e8eaf2';ctx.beginPath();ctx.arc(x,yOf(cumExp[hoverDay]),4,0,7);ctx.fill();
  }
}
function radarDailyHover(e, canvas){
  ...
  const {daysInMonth:days, todayDay, cumExp, dailyExp}=d0;
  const pad=52,right=16; const W2=canvas.width-pad-right;
  let day=Math.round((mx-pad)/W2*(days-1))+1;
  day=Math.max(1,Math.min(days,day));
  drawRadarDaily(canvas, day);
  let tt=document.getElementById('radarDailyTip');
  if(!tt){ tt=document.createElement('div'); tt.id='radarDailyTip'; tt.style.cssText='position:fixed;z-index:9999;background:#1a1d27;border:1px solid var(--border);border-radius:8px;padding:6px 10px;font-size:.72rem;color:#e8eaf2;pointer-events:none;box-shadow:0 4px 12px rgba(0,0,0,.4)'; document.body.appendChild(tt); }
  tt.style.display='block';
  tt.style.left=(e.clientX+12)+'px';
  tt.style.top=(e.clientY-10)+'px';
  const dEx = day<=todayDay ? (dailyExp[day]||0) : 0;
  tt.innerHTML = `<b>${day}. den</b><br>výdaj: ${fmt(Math.round(dEx))} Kč<br>kumul.: ${fmt(Math.round(cumExp[Math.min(day,todayDay)]||0))} Kč`;
}
