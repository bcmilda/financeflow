...
function drawSeasLines(canvas,real,model){
  ...
  // zaokrouhli na 5% násobky pro pěknou osu
  maxV=Math.ceil(maxV/0.05)*0.05; minV=Math.floor(minV/0.05)*0.05;
  const rng=(maxV-minV)||1;
  const legendH=26, pad=42,right=14,bottom=22;
  const top=legendH+8, n=12;
  const W2=W-pad-right,H2=H-bottom-top;
  const yOf=v=>top+H2*(1-(v-minV)/rng);
  // osa Y po 5 %
  for(let v=minV; v<=maxV+0.001; v+=0.05){
    const y=yOf(v);
    ...
    ctx.fillStyle=is100?'#60a5fa':'#7e84a0';ctx.font='8px Instrument Sans';ctx.textAlign='right';
    ctx.fillText(Math.round(v*100)+'%',pad-4,y+3);
  }
...
function drawPaceChart(canvas, days, todayDay, curCum, histAvg, histMonthsCount){
  ...
  const pad=52,right=16,top=26,bottom=30;
  ...
  for(let d=1;d<=days;d+=5){ctx.fillText(d,xOf(d),H-12);}
  ctx.fillStyle='#a8aec8';ctx.fillText('den v měsíci',pad+W2/2,H-1);
  // legenda
  ctx.textAlign='left';ctx.font='9px Instrument Sans';
  ...
  if(exp>0){
    ...
    ctx.fillStyle=col;ctx.font='bold 10px Instrument Sans';ctx.textAlign='right';ctx.fillText(txt,W-right,top-12);
  }
}
