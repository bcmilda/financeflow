  ...
  el.innerHTML = tabIntro('radar','🎯','Finanční radar',
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      <div class="card-body">
        ...
        <div style="font-size:.66rem;color:var(--text3);margin-top:6px;line-height:1.5">Bílá čára = kolik jsi celkem utratil (kumulativně). Zelená = úroveň příjmu. Modré sloupce ukazují, ve které dny jsi utrácel (orientačně, ne ve stejném měřítku jako čára). Najeď myší pro detail dne.</div>
      </div>
    </div>
...
function drawRadarDaily(canvas, hoverDay){
  ...
  const {daysInMonth:days, todayDay, cumolEnd, cumExp, dailyExp, incomeTarget, predEnd, incomeDay, isCurrent}=d0;
  ...
  // modré sloupce – denní výdaj. Vlastní škála: max sloupec = 22 % výšky grafu.
  // (sloupce jsou jen orientační „kde byl výdaj", ne ve stejném měřítku jako kumul. čára)
  const barMaxH=H2*0.22;
  for(let d=1;d<=days;d++){
    if(dailyExp[d]<=0) continue;
    const x=xOf(d), bh=Math.max(2,dailyExp[d]/maxDailyExp*barMaxH);
    ctx.fillStyle = (hoverDay===d)?'rgba(96,165,250,.95)':'rgba(96,165,250,.4)';
