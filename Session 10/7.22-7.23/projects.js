...
function renderRadar() {
  ...
  // Cíl: ukázat "kam směřuju" – kolik zbude na konci měsíce a predikce
  // dalších 3 měsíců na základě průměrů + známých budoucích plateb.
  ...
  // Predikce dalších 3 měsíců (M+1..M+3): průměrný příjem − (průměrný výdaj + budoucí platby toho měsíce)
  const bud90 = typeof budouciGetAll==='function' ? budouciGetAll(D, 95) : [];
  const futureMonths = [];
  let runningBalance = eomLeft; // navazujeme na konec aktuálního měsíce
  for(let k=1;k<=3;k++){
    ...
    // měsíční saldo = průměrný příjem − max(průměrný výdaj, samotné budoucí platby)
