...
function drawHealthScoreLineChart(canvasId, months) {
  ...
  const colorFor = s => s >= 75 ? '#4ade80' : s >= 50 ? '#fbbf24' : '#f87171';
  ...
  // Body + hodnoty + popisky měsíců
  ctx.textAlign = 'center'; ctx.textBaseline = 'alphabetic';
  months.forEach((mo, i) => {
    const x = xAt(i), y = yAt(mo.score);
    // bod
    ctx.fillStyle = colorFor(mo.score);
    ctx.beginPath(); ctx.arc(x, y, n > 8 ? 3 : 4, 0, Math.PI * 2); ctx.fill();
    // hodnota nad bodem (skryj pokud moc bodů a překrývaly by se)
    if (n <= 12) {
      ctx.fillStyle = colorFor(mo.score);
      ctx.font = (n > 8 ? '8px' : '9px') + ' Syne, sans-serif';
      ctx.fillText(mo.score, x, y - 8);
    }
    // popisek měsíce na ose X (každý druhý při 12M)
    if (n <= 6 || i % 2 === 0 || i === n - 1) {
      ctx.fillStyle = 'rgba(168,173,196,.7)';
      ctx.font = '8px Instrument Sans';
      ctx.fillText(mo.label, x, H - 8);
    }
  });
}
