...
function renderUctenky() {
  ...
  if(window._receiptEditorOpen) {
    const openSlot = document.querySelector('[id^="rcpt_hist_"]');
    let anyOpen = false;
    ...
    html += `<div class="card hist-row" data-store="${(r.store||'').toLowerCase()}" data-date="${r.date||''}" data-total="${r.total||0}" style="margin-bottom:6px;overflow:hidden">
      <div style="padding:10px 12px">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <div style="font-family:Syne,sans-serif;font-size:.88rem;font-weight:800;color:var(--expense);flex-shrink:0;white-space:nowrap">${(r.total||0).toLocaleString('cs-CZ',{minimumFractionDigits:2,maximumFractionDigits:2})} Kč</div>
...
function buildLearnTab(receipts, allItems, storeStats, totalSpent) {
  ...
  if(monthReceipts.length){
    ...
    html += '<div class="card" style="margin-bottom:14px"><div class="card-header"><span class="card-title">🏪 Obchody v měsíci</span>'
      ...
      + '<th style="padding:4px 2px;width:36%">Obchod</th>'
      + '<th style="padding:4px 2px;text-align:right;width:11%">×</th>'
      + '<th style="padding:4px 2px;text-align:right;width:20%">Celkem</th>'
      + '<th style="padding:4px 2px;text-align:right;width:16%">Ø</th>'
      + '<th style="padding:4px 2px;text-align:right;width:17%">Den</th>'
      + '</tr></thead><tbody>';
    storeRows.forEach(([s,v])=>{
      ...
      html += '<tr style="border-top:1px solid var(--border)">'
        + '<td style="padding:6px 2px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+storeBadgeHTML(s)+s+'</td>'
        + '<td style="padding:6px 2px;text-align:right">'+v.visits+'×</td>'
        + '<td style="padding:6px 2px;text-align:right;font-weight:700;white-space:nowrap">'+fmt(Math.round(v.total))+'</td>'
        + '<td style="padding:6px 2px;text-align:right;color:#a8aec8;white-space:nowrap">'+fmt(Math.round(v.total/v.visits))+'</td>'
        + '<td style="padding:6px 2px;text-align:right;color:#a8aec8;font-size:.66rem">'+dayName.slice(0,2)+'</td></tr>';
    });
    html += '</tbody></table><div style="font-size:.62rem;color:var(--text3);margin-top:4px;text-align:right">částky v Kč · den = nejčastější den nákupu v tomto měsíci</div></div></div>';
  }
  ...
  if(frequentItems.length) {
    ...
    frequentItems.forEach(it=>{
      ...
      const stores = storeList.slice(0,3).join(', ') + (storeList.length>3 ? ' +'+(storeList.length-3) : '');
...
function buildStoreTrendData(receipts){
  ...
  const top = Object.values(agg)
    .filter(a=>a.visits >= 2)                       // jednorázové platby (faktury) vynechat
