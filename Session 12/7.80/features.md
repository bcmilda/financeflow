...
### Nákupní DNA – obchody **(Session 12.1, v7.78)**
- Tabulka „🏪 Obchody v měsíci": návštěvy, celkem, Ø útrata, typický den nákupu (pro vybraný měsíc, top 8).
- Spojnicový graf „📈 Trend útrat dle obchodů": top 4 obchody za 6 měsíců; osy + mřížka + Kč popisky, tooltip myš i dotyk, legenda s barevnými badgi (`storeBrandColor` – 20 CZ řetězců s firemní barvou, jinak stabilní hash barva).

### Email + heslo přihlášení **(Session 12.1, v7.79)**
- Login: Google OAuth + Email/heslo (přepínač Přihlásit/Registrovat), validace, 22 českých chybových hlášek, reset hesla e-mailem, zobrazit/skrýt heslo. Vyžaduje povolení Email/Password v Firebase Console → Authentication.
- **v7.80:** „Pokračovat bez účtu" odstraněno z UI (signInLocal() v kódu ponechán neaktivní).

### Zabezpečení hostingu **(Session 12.1, v7.79)**
- firebase.json: bezpečnostní HTTP hlavičky (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy, X-XSS-Protection) + ignore rozšířen o database_rules.json, *.yml, staré dev HTML.
- Playwright starter kit (playwright-kit.zip): smoke testy landing/app, konzistence verzí title=sidebar=sw.js, existence hashovaných JS, integrita product-groups.json.
