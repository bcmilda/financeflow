...
- **v7.78–v7.80 (S12.1 d–f):** Nákupní DNA obchody (tabulka + trend graf), Playwright testy, Email+heslo přihlášení, bezpečnostní HTTP hlavičky, ignore rozšířen, „Pokračovat bez účtu" odstraněno z UI, database_rules validate COICOP ≥ 0.
- **Infra:** probíhá migrace DNS na Cloudflare (štít před Firebase Hosting; e-mail MX = DNS only).
