...
### FIX-136 · database_rules.json – validate blokoval COICOP 0 **(Session 12.1, v7.79)**
- **Příčina:** `admin_coicop_overrides` validate vyžadoval `coicop >= 1`, ale volba „0 – mimo COICOP" (v7.74) potřebuje zapsat 0 → Firebase zápis selhal.
- **Oprava:** validate `>= 0`; přidáno pravidlo pro `/subs` uzel (assignSubCoicop).
- **Soubor:** `database_rules.json`
