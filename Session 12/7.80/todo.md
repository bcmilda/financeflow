...
- **TODO-129** · Apple Sign In – vyžaduje Apple Developer Program ($99/rok); přidat až před iOS/TWA publikací (Firebase OAuthProvider('apple.com') + Service ID + doménové ověření).
- **TODO-130** · Cloudflare: po aktivaci ověřit SSL Full (strict), Bot Fight Mode ON, e-mail MX = DNS only (šedý mrak); přidat do Playwright testu kontrolu CF-Ray hlavičky.
- **TODO-131** · Nákupní DNA: rozšířit o COICOP skupiny z produktové DB (product-groups.json) – frequent items tagovat přes productGroupLookup, DNA graf i podle COICOP oddílů.
- **TODO-132** · Firebase App Check (reCAPTCHA Enterprise) – ochrana RTDB + workeru na úrovni aplikace; zvážit po Cloudflare a před Google Play.
