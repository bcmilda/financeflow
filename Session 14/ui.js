// FinanceFlow · v8.28 · ui.js · 2026-06-24
//  RENDER ROUTER
// ══════════════════════════════════════════════════════
...
function renderMonthlyChecklist(D){
  ...
  let hidden = false;
  try { hidden = localStorage.getItem('ff_monthlyHide_'+mKey) === '1'; } catch(e){}
  ...
  <div class="card" style="margin-bottom:14px;border:1px solid rgba(74,222,128,.3)">
    <div class="card-header">
      ...
      <button onclick="dismissMonthlyChecklist()" style="background:none;border:none;color:var(--text3);font-size:1rem;cursor:pointer;padding:2px 6px" title="Skrýt pro tento měsíc">✕</button>
    </div>
    <div class="card-body">
      <div style="height:7px;background:var(--surface3);border-radius:5px;overflow:hidden;margin-bottom:12px">
...
function dismissMonthlyChecklist(){
  const mKey = `${S.curYear}-${String(S.curMonth+1).padStart(2,'0')}`;
  try { localStorage.setItem('ff_monthlyHide_'+mKey,'1'); } catch(e){}
...
function renderTransferOverview(D){
  ...
  <div class="card" style="margin-bottom:14px">
    <div class="card-header">
      ...
      <span style="font-size:.64rem;color:var(--text3);cursor:pointer" onclick="showPage('aktiva')">Aktiva ›</span>
    </div>
    <div class="card-body">
      ...
      ${rows}
      <div style="font-size:.64rem;color:var(--text3);margin-top:10px;line-height:1.4">💡 Hodnoty jsou součtem tvých přesunů (peníze nasměrované do investic/rezervy). Nepočítají se jako výdaj. Záporná hodnota = víc vybráno než vloženo.</div>
    </div>
  </div>`;
}
...
function renderTx(){
  ...
  let txs;
  if (searchAllMonths && (searchFilter || tagFilter)) {
...
function editTx(id){
  ...
  // S14: transakce v transfer-kategorii (vklad do investic/spoření) → otevři rovnou v režimu Přesun → Do aktiv
  const _isAssetTx = t.type==='expense' && window._transferCatIds && window._transferCatIds.has(t.catId||t.category);
  if(_isAssetTx && typeof setTransferMode==='function'){
    setTxType('transfer');
