//  FinanceFlow · v8.55 · assets.js · 2026-06-29
// ══════════════════════════════════════════════════════
...
function resyncAssetsFromTransfers(){
  try {
    if (typeof window.S === 'undefined' || !window.S){ alert('⚠️ Data nejsou načtena.'); return; }
    ...
    if(isWallets){
      ...
      html += ws.map(w=>{
        ...
        return `<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:9px 12px;margin-bottom:6px;display:flex;align-items:center;gap:10px">
          ...
          <span style="font-size:.74rem;color:var(--text3);white-space:nowrap;text-align:right;min-width:88px">${isForeign?fmtP(bal)+' '+cur:''}</span>
          <span style="font-weight:600;font-variant-numeric:tabular-nums;color:${balCZK>=0?'#60a5fa':'var(--expense)'};white-space:nowrap;min-width:120px;text-align:right">${fmtP(balCZK)} Kč</span>
        </div>`;
