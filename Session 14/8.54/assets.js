//  FinanceFlow · v8.54 · assets.js · 2026-06-29
// ══════════════════════════════════════════════════════
...
function assetCatLiq(catId){
  const cc = (window.S && S.categories || []).find(x => x.id === catId);
  if (!cc) return null;
...
function syncInvestmentAssets(){
  ...
  if (!window.S || !Array.isArray(S.categories)) return;
  ...
  function _amtCZK(t){
    ...
    if (cur && cur !== 'CZK') amt = amt * (FX[cur] || 1);
    return amt;
  }
  ...
  const _blocked = Array.isArray(S.noSyncKeys) ? S.noSyncKeys : [];
  Object.keys(groups).forEach(key => {
    if (_blocked.includes(key)) return; // uživatel napojené aktivum smazal
    const g = groups[key];
...
// ADR-076: ruční přepojení aktiv z přesunových transakcí + diagnostika
function resyncAssetsFromTransfers(){
  ...
  S.noSyncKeys = []; // odblokuj vše dříve smazané
  const transferCats = (S.categories||[]).filter(c => c.type === 'transfer');
  const transferIds = new Set(transferCats.map(c => c.id));
  const txMatch = (S.transactions||[]).filter(t => !t.splitParent && (t.type==='expense'||t.type==='income') && transferIds.has(t.catId||t.category));
  // přehled podle podkategorie
  const bySub = {};
  txMatch.forEach(t => { const k=(S.categories.find(c=>c.id===(t.catId||t.category))||{}).name+' / '+((t.subcat||'(bez podkategorie)')); bySub[k]=(bySub[k]||0)+1; });
  ...
  msg += '• Přesunových kategorií (typ „Přesun"): ' + transferCats.length + '\n';
  if (transferCats.length) msg += '   ' + transferCats.map(c=>c.name).join(', ') + '\n';
  msg += '• Transakcí v nich (vklady/výběry): ' + txMatch.length + '\n';
  const subKeys = Object.keys(bySub);
  if (subKeys.length) msg += '   ' + subKeys.map(k=>k+' ×'+bySub[k]).join('\n   ') + '\n';
  msg += '• Napojených aktiv: ' + linked + '\n';
  if (hadBlocked) msg += '• Odblokováno dříve smazaných: ' + hadBlocked + '\n';

  if (transferCats.length === 0)
    msg += '\n⚠️ Žádná kategorie nemá typ „Přesun". Otevři kategorii (Investice, Spoření…) → Upravit → Typ = „🔄 Přesun (investice, rezerva)".';
  else if (txMatch.length === 0)
    msg += '\n⚠️ Žádná transakce nemíří do přesunové kategorie. Přidej transakci: + → Přesun → Investice/Spoření (NE převod mezi peněženkami).';
  else if (linked === 0)
    msg += '\n⚠️ Transakce existují, ale aktivum nevzniklo. Pošli mi tento výpis – prověřím.';
  else
    msg += '\n✅ Hotovo. Aktiva najdeš v sekci „Střednědobá a investiční aktiva".';
  alert(msg);
}
      ...
      ${(()=>{ // S14: 5 karet – Peněženky, Finanční rezerva, Střednědobá, Fyzická, Závazky
        ...
        return `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(92px,1fr));gap:8px">
          ${cards.map(cd=>`<div style="text-align:center;padding:9px 6px;background:var(--surface3);border-radius:10px;border-top:2px solid ${cd.color}">
            <div style="font-size:.84rem;font-weight:800;color:${cd.color};font-family:Syne,sans-serif;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${fmtP(cd.val)} Kč</div>
            <div style="font-size:.64rem;color:var(--text3);margin-top:2px">${cd.label}</div>
          </div>`).join('')}
        </div>`; })()}
    </div>
...
function assetBuildLiquiditySections(D){
  ...
  Object.entries(LIQ_GROUPS).forEach(([key,g])=>{
    ...
    html += `<div style="margin:16px 0 8px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:4px">
      ...
      <span style="font-size:.84rem;font-weight:800;color:${g.color}">${fmtP(lt[key]||0)} Kč</span>
    </div>
    <div style="font-size:.64rem;color:var(--text3);margin:-4px 0 8px">${g.desc}</div>`;
    if(!hasContent){
    ...
    if(isWallets){
      ...
      html += ws.map(w=>{
        ...
        return `<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:9px 12px;margin-bottom:6px;display:flex;align-items:center;gap:9px">
          ...
          ${isForeign ? `<span style="font-size:.74rem;color:var(--text3);white-space:nowrap;margin-right:8px">${fmtP(bal)} ${cur}</span>` : ''}
          <span style="font-weight:800;font-family:Syne,sans-serif;color:${balCZK>=0?'#60a5fa':'var(--expense)'};white-space:nowrap;min-width:80px;text-align:right">${fmtP(balCZK)} Kč</span>
        </div>`;
...
function saveAsset() {
  ...
  // ADR-076: ruční hodnota = baseline; všechny dosavadní vklady jsou v ní zahrnuté → další se přičtou
  ...
  const obj = {
    ...
    valueManual: true,
    valueBaseline: val, investedAtBaseline: _invNow,
    updatedAt: Date.now(),
  };
...
function assetDelete(id) {
  ...
  const _a = (S.assets || []).find(x => x.id === id); // ADR-076: smazané napojené aktivum už znovu nevytvářet
  if (_a && _a.linkedKey) { if (!Array.isArray(S.noSyncKeys)) S.noSyncKeys = []; if (!S.noSyncKeys.includes(_a.linkedKey)) S.noSyncKeys.push(_a.linkedKey); }
