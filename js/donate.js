...
function startPremiumSubscription(period) {
  ...
  // S17.41 (FIX-223, Milan): pojistka proti popup blockeru. Když prohlížeč nové okno
  // zablokuje (window.open vrátí null), nabídneme přechod v aktuální záložce – uživatel
  // jinak jen viděl varovnou lištu a nic se nestalo.
  const w = window.open(url, '_blank', 'noopener,noreferrer');
  if (!w || w.closed || typeof w.closed === 'undefined') {
    if (confirm('Prohlížeč zablokoval nové okno.\n\nOtevřít platební bránu v této záložce?')) {
      window.location.href = url;
    }
    return;
  }
  // S17.42 (Milan): Stripe checkout se otevírá v NOVÉ ZÁLOŽCE, která nemá historii –
  // tlačítko Zpět v prohlížeči je proto šedé a uživatel má pocit, že je v pasti.
  // Není to nastavení Stripu, ale důsledek `_blank`. Dokud nepřejdeme na Checkout Session
  // s cancel_url (TODO-199), aspoň jasně řekneme, jak se z platby dostat pryč.
  if (typeof showCheckoutHint === 'function') showCheckoutHint();
}

// Informační lišta po otevření platby – vysvětlí, že platbu lze bez následků opustit.
function showCheckoutHint() {
  const id = 'checkoutHintBar';
  document.getElementById(id)?.remove();
  const el = document.createElement('div');
  el.id = id;
  el.style.cssText = 'position:fixed;left:50%;bottom:18px;transform:translateX(-50%);z-index:9500;max-width:420px;width:calc(100% - 32px);background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:13px 15px;box-shadow:0 8px 28px rgba(0,0,0,.5)';
  el.innerHTML = `
    <div style="display:flex;gap:10px;align-items:flex-start">
      <span style="font-size:1.1rem">💳</span>
      <div style="flex:1;min-width:0">
        <div style="font-size:.82rem;font-weight:700;color:#e8eaf2;margin-bottom:3px">Platba se otevřela v nové záložce</div>
        <div style="font-size:.74rem;color:#a8aec8;line-height:1.5">
          Rozmyslel sis to? Stačí tu záložku <strong>zavřít</strong> – nic se nestrhne a vrátíš se sem.
        </div>
      </div>
      <button onclick="document.getElementById('${id}').remove()" style="background:none;border:none;color:#a8aec8;font-size:1.1rem;cursor:pointer;padding:0 2px;line-height:1">×</button>
    </div>`;
