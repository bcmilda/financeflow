...
// ──────────────────────────────────────────────────────────────────────
// FIX-058 (TODO-021): Pokud volající (receipts.js) už předkomprimoval fotku
// přes compressReceiptImageDual(), poznáme to podle:
//   - vstup je Blob (ne File s name) typu image/jpeg
//   - velikost je rozumná (<500 KB) → je už menší než MAX_PHOTO_WIDTH × HEIGHT
// V takovém případě komprese přeskočena (ušetří CPU + zabrání degradaci JPEG).
const PRECOMPRESSED_MAX_BYTES = 500 * 1024; // 500 KB práh

function compressPhoto(file) {
  // FIX-058: Skip re-compression pokud je vstup už zkomprimovaný Blob
  const isPrecompressed = (
    file instanceof Blob &&
    !(file instanceof File) &&  // File má .name, runtime Blob ne
    file.type === 'image/jpeg' &&
    file.size > 0 &&
    file.size <= PRECOMPRESSED_MAX_BYTES
  );
  if (isPrecompressed) {
    const sizeKB = Math.round(file.size / 1024);
    console.log(`📸 Komprese přeskočena – vstup už zkomprimován (${sizeKB} KB)`);
    return Promise.resolve({
      blob: file,
      width: null,   // neznámé – necheme načítat Image jen kvůli rozměrům
      height: null,
      originalKB: sizeKB,
      compressedKB: sizeKB,
    });
...
// ──────────────────────────────────────────────────────────────────────
// ──────────────────────────────────────────────────────────────────────
// FIX-057: Wait for Firebase auth (user může být obnovován delší dobu po reconnect)
// ──────────────────────────────────────────────────────────────────────
...
async function runSync() {
  ...
  // FIX-057: V lokálním módu (bez Firebase přihlášení) nemáme Worker token,
  // takže sync nemůže fungovat. Účtenky zůstanou ve frontě dokud se uživatel
  // nepřihlásí přes Google.
  if (window._isLocalMode) {
    console.log('[Sync] Lokální mód – sync přeskočen (vyžaduje Firebase login)');
    return;
  }
  ...
  // FIX-057: Počkej až je Firebase auth k dispozici (jinak Worker vrátí 401)
  const authReady = await waitForFirebaseAuth(10000);
  if (!authReady) {
    console.warn('[Sync] Firebase auth se neobnovil – přerušuji sync');
    if (typeof showToast === 'function') {
      showToast('⚠️ Sync přerušen – přihlaš se znovu');
    }
    return;
  }
  ...
  for (const item of pendingTx) {
    try {
      ...
      console.error('[Sync] Tx selhalo:', item.id, e.message);
  ...
  for (let ri = 0; ri < pending.length; ri++) {
    const item = pending[ri];
    try {
      showAnalyzingBadge(`🔄 Analyzuji účtenku ${ri + 1}/${pending.length}…`);
      ...
      console.error('[Sync] Účtenka selhala:', item.id, e.message);
  ...
  updateOfflineBadge();
  if (errorCount === 0) {
    if (typeof showToast === 'function') showToast(`✅ Synchronizováno (${successCount})`);
  } else if (successCount === 0) {
    if (typeof showToast === 'function') showToast(`❌ Sync selhal (${errorCount} chyb)`);
  } else {
    if (typeof showToast === 'function') showToast(`⚠️ Sync částečný: ${successCount} OK, ${errorCount} chyb`);
  }
}
...
// ──────────────────────────────────────────────────────────────────────
// FIX-057 (Session 8): Původní kód spoléhal na neexistující saveTxToFirebase()
// a fallback zapisoval do neexistujícího nodu `data/transactions_queue`.
// Nový kód přidá tx přímo do S.transactions a zavolá save() (z app.js).
async function syncOneTx(item) {
  if (typeof save !== 'function') {
    throw new Error('save() není definováno (app.js se nenačetl?)');
  }
  if (typeof S === 'undefined' || !S) {
    throw new Error('Globální state S není inicializovaný');
  }

  // Přidej tx do S.transactions, pokud tam ještě není (idempotence pro retry)
  if (!S.transactions) S.transactions = [];
  const txData = item.tx || {};
  const exists = txData.id && S.transactions.some(t => t.id === txData.id);
  if (!exists) {
    // Vygeneruj ID pokud chybí (kolizi-odolné)
    if (!txData.id && typeof genTxId === 'function') {
      txData.id = genTxId();
    }
    S.transactions.push(txData);
  }

  // save() je async, čeká na Firebase write
  await save();
}
...
// ──────────────────────────────────────────────────────────────────────
// FIX-057 (Session 8): Komplet přepsáno – původní kód volal neexistující
// endpoint `/analyze-receipt` na špatné URL (financeflow.bcmilda.workers.dev)
// s payloadem {image, mimeType, ...} který Worker neumí parsovat.
// Nový kód používá WORKER_URL z ai.js (misty-limit-0523.bc-milda.workers.dev),
// posílá {type:'receipt', payload:{imageData, mediaType}} (stejný formát jako online),
// a výsledek předává do addReceiptAsTx() který založí transakci v Firebase.
async function syncOneReceipt(item) {
  if (typeof WORKER_URL === 'undefined') {
    throw new Error('WORKER_URL není definován (ai.js se nenačetl?)');
  }
  if (typeof addReceiptAsTx !== 'function') {
    throw new Error('addReceiptAsTx není dostupná (receipts.js se nenačetl?)');
  }

  const idToken = await (window._currentUser?.getIdToken?.() || Promise.resolve(null));
  if (!idToken) throw new Error('Nejste přihlášeni (chybí Firebase token)');

  // Převedeme Blob na base64 (bez prefixu data:...)
  const base64 = await blobToBase64(item.blob);

  // Stejný formát jako analyzeReceipt() v receipts.js
  ...
  if (!receipt.store && !receipt.total) {
    throw new Error('Účtenka nebyla rozpoznána – foto nečitelné');
  }

  // FIX-057: Založ transakci přes existující funkci v receipts.js
  // (ta dělá kategorizaci, save() do Firebase, publish cen do komunity)
  // addReceiptAsTx vrací savePromise → awaitneme aby případná Firebase chyba šla nahoru
  try {
    await addReceiptAsTx(receipt);
  } catch (e) {
    console.error('[Offline Sync] addReceiptAsTx selhalo:', e);
    throw new Error('Selhalo přidání transakce: ' + e.message);
  }

  // Toast notifikace pro uživatele (prchavý)
  if (typeof showToast === 'function') {
    showToast(`✅ Účtenka synchronizována: ${receipt.store || 'Nákup'} (${receipt.total || 0} Kč)`);
  }

  // Trvalé osobní oznámení (Session 11) – přežije toast, s odkazem do Historie.
  // Řeší UX díru: offline účtenka se po reconnectu zanalyzuje „neviditelně"
  // a rovnou uloží – tohle dá uživateli viditelný výsledek + proklik.
  if (typeof addLocalNotification === 'function') {
    addLocalNotification({
      icon: '🧾',
      title: 'Účtenka zanalyzována',
      text: `${receipt.store || 'Nákup'} · ${receipt.total || 0} Kč byla rozpoznána a přidána do historie.`,
      link: { kind: 'receiptHistory', label: 'Zobrazit v historii účtenek' },
      color: 'var(--income)',
    });
  }
}
...
// Viditelný stav „probíhá analýza" během syncu (Session 11)
function showAnalyzingBadge(text) {
  showOfflineBadge(); // zajistí existenci elementu + display:flex
  const badge = document.getElementById('ff-offline-badge');
  if (!badge) return;
  badge.style.background = 'var(--bank, #60a5fa)';
  badge.style.color = '#fff';
  badge.innerHTML = text || '🔄 Analyzuji…';
  badge.style.display = 'flex';
}

async function updateOfflineBadge() {
  const badge = document.getElementById('ff-offline-badge');
  // Resetuj barvu textu (mohla být změněna na bílou při „Analyzuji")
