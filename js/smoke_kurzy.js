...
// ── FIX-323 · další dva klíče bez uid ────────────────────────────
{
  const telo=(file,fn)=>{
    const src=R(file); const i=src.indexOf('function '+fn+'()');
    let d=0,j=src.indexOf('{',i),end=0;
    for(let k=j;k<src.length;k++){if(src[k]==='{')d++;else if(src[k]==='}'){d--;if(!d){end=k+1;break}}}
    const c=vm.createContext({}); c.window=c; vm.runInContext(src.slice(i,end), c); return c;
  };
  [['push.js','_notifPrefsKey','nastavení oznámení'],
   ['announcements.js','_announceSeenKey','přečtená oznámení']].forEach(([f,fn,popis])=>{
    const c=telo(f,fn);
    c._currentUser={uid:'milan'}; const a=c[fn]();
    c._currentUser={uid:'jarda'}; const b=c[fn]();
    c._currentUser=null;          const l=c[fn]();
    ok(`FIX-323 · ${popis}: každý účet má vlastní klíč`, a!==b && a!==l && b!==l);
    ok(`FIX-323 · ${popis}: klíč obsahuje uid`, a.includes('milan') && b.includes('jarda'));
  });
  ok('FIX-323 · nikde nezbyl starý společný zápis',
     !/localStorage\.setItem\('ff_notif_prefs'/.test(R('push.js')) &&
     !/localStorage\.setItem\('ff_announce_seen'/.test(R('announcements.js')));
  ok('FIX-323 · hlavní kopie nastavení oznámení zůstává ve Firebase',
     /users\/\$\{window\._currentUser\.uid\}\/notifPrefs/.test(R('push.js')));
}
