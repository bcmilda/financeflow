...
async function openNotifSettings() {
  ...
  try { _analyticsOn = localStorage.getItem('ff_cookie_analytics') === '1'; } catch(e){}
  ...
  m.innerHTML = `
    ...
    <div style="padding:16px;max-width:640px;width:100%;margin:0 auto">
      ...
      <div onclick="toggleAnalyticsCookies()" style="display:flex;align-items:center;gap:12px;padding:14px 2px;cursor:pointer;border-bottom:1px solid var(--border)">
        <span style="font-size:1.3rem">🍪</span>
        <div style="flex:1"><div style="font-size:.9rem;font-weight:600;color:var(--text)">Analytické cookies (Google Analytics)</div><div style="font-size:.74rem;color:#a8aec8">Anonymní statistiky návštěvnosti s anonymizací IP – pomáhá nám app vylepšovat</div></div>
        ${_notifSwitch(_analyticsOn, false)}
      </div>
      <div style="display:flex;align-items:center;gap:12px;padding:14px 2px;opacity:.55">
        <span style="font-size:1.3rem">🔒</span>
        <div style="flex:1"><div style="font-size:.9rem;font-weight:600;color:var(--text)">Nezbytné cookies</div><div style="font-size:.74rem;color:#a8aec8">Přihlášení, zabezpečení, uložení tvých voleb – nelze vypnout</div></div>
        ${_notifSwitch(true, true)}
      </div>
...
function toggleAnalyticsCookies() {
  let cur = false;
  try { cur = localStorage.getItem('ff_cookie_analytics') === '1'; } catch(e){}
  const next = !cur;
  try {
    localStorage.setItem('ff_cookie_analytics', next ? '1' : '0');
    localStorage.setItem('ff_cookie_choice', '1');
  } catch(e){}
  if (typeof gtag === 'function') gtag('consent', 'update', { 'analytics_storage': next ? 'granted' : 'denied' });
  if (typeof showToast === 'function') showToast(next ? '🍪 Analytické cookies zapnuty' : '🍪 Analytické cookies vypnuty');
  openNotifSettings(); // překreslit
}
