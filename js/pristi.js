// FinanceFlow · v9.79 · pristi.js · 2026-08-17
// ══════════════════════════════════════════════════════
...
const PRISTI_STABLE_MIN   = 0.7;   // stabilityWeight ≥ 0,7 → „pravděpodobné"
...
// Okno, za které se počítá. Kalendářní = 1.–poslední. Od výplaty = anchor → den před příští výplatou.
// Den výplaty se NEPOČÍTÁ ZNOVU – přebírá se z radarPaydayInfo() (SKILL 17).
function pristiWindow(D) {
  const t = pristiTargetMonth();
  if (_pristiMode === 'pay' && typeof radarPaydayInfo === 'function') {
    let anchor = 1;
    try { const P = radarPaydayInfo(D); anchor = Math.max(1, Math.min(28, parseInt(P && P.anchor) || 1)); } catch (e) { anchor = 1; }
    const from = new Date(t.y, t.m, anchor); from.setHours(0, 0, 0, 0);
    const to = new Date(t.y, t.m + 1, anchor); to.setDate(to.getDate() - 1); to.setHours(0, 0, 0, 0);
    return { from, to, m: t.m, y: t.y, anchor };
  }
  const from = new Date(t.y, t.m, 1); from.setHours(0, 0, 0, 0);
  const to = new Date(t.y, t.m + 1, 0); to.setHours(0, 0, 0, 0);
  return { from, to, m: t.m, y: t.y, anchor: null };
}
...
function pristiCfgRead(ym, D) {
  ...
  const c = all[ym] || {};
  return { start: (typeof c.start === 'number') ? c.start : null, rows: c.rows || {} };
}
...
function pristiBuildData(D) {
  ...
  (D.sablony || []).forEach(s => {
    ...
    pristiOccurrences(s.freq || 'monthly', s.den || 1, W.from, W.to).forEach(d => {
      inc.push({
        ...
        note: (typeof FREQ_LABELS !== 'undefined' && FREQ_LABELS[s.freq || 'monthly']) || 'měsíčně',
        src: 'šablona',
      });
      const cid = s.catId || '';
  ...
  // ── 2) PŘÍJMY z historie (🟡 pravděpodobné / ⚪ nejisté) ───────
  const hist = pristiHistMonths(W.m, W.y, PRISTI_HIST_MONTHS);
  const byCat = {};   // catId → {sum, days:[]}
  hist.forEach(h => {
    getTx(h.m, h.y, D).forEach(t => {
      ...
      if (!byCat[cid]) byCat[cid] = { sum: 0, days: [] };
  ...
  Object.keys(byCat).forEach(cid => {
    ...
    const level = w >= PRISTI_STABLE_MIN ? 2 : 3;
    const medDay = _pMid(byCat[cid].days) || 15;
    // datum uvnitř okna (u payday režimu se den mapuje na správný kalendářní měsíc)
    let d = new Date(W.from.getFullYear(), W.from.getMonth(), Math.min(medDay, new Date(W.from.getFullYear(), W.from.getMonth() + 1, 0).getDate()));
    d.setHours(0, 0, 0, 0);
    if (d < W.from) { d = new Date(W.from.getFullYear(), W.from.getMonth() + 1, Math.min(medDay, new Date(W.from.getFullYear(), W.from.getMonth() + 2, 0).getDate())); d.setHours(0, 0, 0, 0); }
    if (d > W.to) d = new Date(W.to);
    inc.push({
      key: 'h:' + (cid || '_none'), level, icon: cat ? (cat.icon || '💵') : '❔',
      name: cat ? cat.name : 'Nezařazený příjem', amount: rest, date: d,
      note: `Ø ${nHist} měs.${tplByCat[cid] ? ' · nad rámec šablony' : ''}`,
...
function pristiResetMonth() {
  ...
  const ym = _pristiLast.ym;
  pristiCfgWrite(ym, c => { c.start = null; c.rows = {}; });
...
function pristiRow(r, sign) {
  ...
  return `<tr style="${dim}border-bottom:1px solid var(--border)">
    ...
    ${ro ? '' : `<td style="padding:9px 0 9px 8px;text-align:right;white-space:nowrap">
      ...
      <button type="button" title="${r.off ? 'Zapnout zpět' : 'Nepočítat tento řádek'}" onclick="pristiToggleRow('${k}')" style="background:none;border:1px solid var(--border2);border-radius:6px;color:#a8aec8;font-size:.72rem;padding:2px 7px;cursor:pointer;margin-left:3px">${r.off ? '↺' : '✕'}</button>
    </td>`}
  </tr>`;
}
...
function pristiRenderHTML(P) {
  ...
  const rangeLabel = _pristiMode === 'pay'
    ? `${_pDay(W.from)} – ${_pDay(W.to)} (od výplaty k výplatě)`
    : `1. ${W.m + 1}. – ${_pDay(W.to)}`;
