...
//   V matici Reportu proto zůstávají holá čísla – přepočtená do základní měny –
//   a symbol je jednou v hlavičce sloupce a v popisku pod tabulkou.
//   ⚠️ Nikdy nemíchat s jinými jednotkami (kg, l, ks) ve stejném sloupci.
const _rNum = v => fmt(Math.round(czkToBase(v)));
// ══════════════════════════════════════════════════════
...
//  Vzhled dle referenčního Excelu: barevná hlavička sektoru → řádky kategorií → zelený
...
function reportMatice() {
  ...
  const mkSubRow = (c, sub) => {
    ...
    return {
      ...
      //  v9.56 (FIX-227): „Roční" = KUMULACE měsíců leden–zvolený měsíc, ne celý rok.
      //  Dřív se bralo _reportSubExp(..., null, ...) = celý rok, takže sloupec
      //  ukazoval pořád stejné číslo jako sloupec roku a přepnutí měsíce s ním nehnulo.
      yearVal: ytdNow,
  ...
  const cell = v => {
    ...
    return `<td style="text-align:right;background:rgba(248,113,113,${(0.04 + int * 0.28).toFixed(3)});color:#e8eaf2;font-weight:${int > 0.45 ? '700' : '500'}">${_rNum(v)}</td>`;
  };
  ...
  const diffCell = (r) => {
    ...
    return `<td style="text-align:right;border-right:2px solid var(--border);color:${col};font-weight:600" title="Letos ${_rNum(r.ytdNow)} vs loni ${_rNum(r.ytdPrev)} za stejné období">
      ${up ? '▲' : '▼'} ${_rNum(Math.abs(r.diffPrev))}${r.diffPct !== null ? ` <span style="font-size:.9em;opacity:.75">${up ? '+' : ''}${r.diffPct} %</span>` : ''}</td>`;
  };
  ...
  ordered.forEach(sec => {
    ...
    body += `<tr><td colspan="${nCols}" style="background:linear-gradient(90deg,${sec.color}33,${sec.color}0d);border-left:4px solid ${sec.color};padding:7px 10px;font-weight:800;font-size:.76rem;color:#e8eaf2;letter-spacing:.02em"><span style="position:sticky;left:10px;display:inline-block">${sec.icon} ${sec.name.toUpperCase()}</span></td></tr>`;
    sec.rows.forEach(r => {
      body += `<tr>
        <td style="position:sticky;left:0;width:170px;min-width:170px;max-width:170px;overflow:hidden;text-overflow:ellipsis;background:var(--surface2);text-align:left;z-index:2;white-space:nowrap;padding-left:16px;border-left:4px solid ${sec.color}55">${r.icon || ''} ${r.name}</td>
        <td style="text-align:right;color:#c9cede">${r.monthVal ? _rNum(r.monthVal) : '–'}</td>
        <td style="text-align:right;color:#c9cede">${r.yearVal ? _rNum(r.yearVal) : '–'}</td>
        ${diffCell(r)}
        ${r.yearVals.map(cell).join('')}
        <td style="border-left:2px solid var(--border);text-align:right;font-weight:700;color:#f87171">${_rNum(r.total)}</td>
      </tr>`;
    });
    // mezisoučet sektoru – zelený řádek jako v Excelu
    body += `<tr style="background:rgba(74,222,128,.10)">
      <td style="position:sticky;left:0;width:170px;min-width:170px;max-width:170px;overflow:hidden;text-overflow:ellipsis;background:rgba(30,50,38,.98);text-align:left;font-weight:800;color:#4ade80;z-index:2;border-left:4px solid ${sec.color};white-space:nowrap">Σ ${sec.name.length > 22 ? sec.icon : sec.name}</td>
      <td style="text-align:right;font-weight:800;color:#4ade80">${sMonth ? _rNum(sMonth) : '–'}</td>
      <td style="text-align:right;font-weight:800;color:#4ade80">${sYear ? _rNum(sYear) : '–'}</td>
      ${(()=>{ const a=sec.rows.reduce((x,r)=>x+(r.ytdNow||0),0), b=sec.rows.reduce((x,r)=>x+(r.ytdPrev||0),0);
        ...
        const d=a-b, up=d>0;
        return `<td style="text-align:right;border-right:2px solid var(--border);font-weight:700;color:${up?'#f87171':'#4ade80'}">${b?`${up?'▲':'▼'} ${_rNum(Math.abs(d))}`:'nové'}</td>`; })()}
      ${sYears.map(v => `<td style="text-align:right;font-weight:700;color:#4ade80">${v ? _rNum(v) : '–'}</td>`).join('')}
      <td style="border-left:2px solid var(--border);text-align:right;font-weight:800;color:#4ade80">${_rNum(sTotal)}</td>
    </tr>`;
  });
  ...
  body += `<tr style="background:rgba(251,191,36,.14);border-top:2px solid var(--debt)">
    <td style="position:sticky;left:0;width:170px;min-width:170px;max-width:170px;overflow:hidden;text-overflow:ellipsis;background:rgba(58,48,20,.98);text-align:left;font-weight:800;color:#fbbf24;z-index:2;white-space:nowrap">💰 CELKEM VÝDAJE</td>
    <td style="text-align:right;font-weight:800;color:#fbbf24">${gMonth ? _rNum(gMonth) : '–'}</td>
    <td style="text-align:right;font-weight:800;color:#fbbf24">${gYear ? _rNum(gYear) : '–'}</td>
    ${(()=>{ const a=ordered.reduce((x,s)=>x+s.rows.reduce((y,r)=>y+(r.ytdNow||0),0),0),
      ...
      if(!a && !b) return `<td style="border-right:2px solid var(--border);color:#5a6078;text-align:right">–</td>`;
      const d=a-b, up=d>0;
      return `<td style="text-align:right;border-right:2px solid var(--border);font-weight:800;color:${up?'#f87171':'#4ade80'}">${b?`${up?'▲':'▼'} ${_rNum(Math.abs(d))}`:'nové'}</td>`; })()}
    ${gYears.map(v => `<td style="text-align:right;font-weight:800;color:#fbbf24">${v ? _rNum(v) : '–'}</td>`).join('')}
    <td style="border-left:2px solid var(--border);text-align:right;font-weight:800;color:#fbbf24">${_rNum(gTotal)}</td>
  </tr>`;
  ...
  const legend = ordered.map(s => {
    ...
    return `<div style="display:flex;align-items:center;gap:7px;padding:6px 9px;border-radius:8px;background:${s.color}14;border:1px solid ${s.color}40;min-width:0">
      ...
      <div style="min-width:0;flex:1">
        ...
        <div style="font-size:.78rem;font-weight:800;color:${s.color}">${_rNum(t)} <span style="font-weight:500;color:#a8aec8">· ${pctv} %</span></div>
      </div>
    </div>`;
  }).join('');
    ...
    <div class="card">
      <div class="card-header">
        ...
        <span style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
          <span style="font-size:.7rem;color:#a8aec8">Měsíční = ${CZ_M[curM]} ${curY} · Roční = leden–${CZ_M[curM]} ${curY} · <strong style="color:#c9cede">všechny částky v ${curSym()}</strong></span>
        </span>
      </div>
      <div class="card-body" style="overflow-x:auto;padding-top:6px">
        <table class="report-matrix" style="min-width:${440 + years.length * 76}px;width:100%;border-collapse:collapse;font-size:.74rem">
          <thead><tr>
            <th style="position:sticky;left:0;background:var(--surface3);text-align:left;z-index:5;width:170px;min-width:170px;max-width:170px;padding:8px 10px">Kategorie</th>
            <th style="text-align:right;color:#4ade80;padding:8px 8px" title="${CZ_M[curM]} ${curY} · hodnoty v ${curSym()}">Měsíční</th>
            <th style="text-align:right;color:#60a5fa;padding:8px 8px" title="Kumulace leden–${CZ_M[curM]} ${curY} · hodnoty v ${curSym()}">Roční (k ${CZ_M[curM]})</th>
