...
function drawHealthScoreLineChart(canvasId, months) {
  ...
  // (dle náčrtu uživatele). Vyšší skóre = kruh výš. Osa Y vlevo.
  //  v9.60 (FIX-232): rozsah se BERE Z DAT, ne natvrdo 0–100. Od v9.58 sem
  //  chodí finanční skóre 0–310, takže při pevné stovce všechny kruhy
  //  přetékaly nad horní hranu grafu a osa lhala.
  const SC_MAX = Math.max(100, ...months.map(m => m.max || 0));
  const STEP = SC_MAX > 200 ? Math.round(SC_MAX / 4 / 10) * 10 : 25;
  ...
  // Osa Y: gridlines + popisky podle skutečného rozsahu
  ctx.lineWidth = 1;
  ctx.font = '10px Instrument Sans';
  ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
  for (let v = 0; v <= SC_MAX; v += STEP) {
    const y = padT + plotH * (1 - v / SC_MAX);
  ...
  const yAt = s => padT + plotH * (1 - Math.max(0, Math.min(SC_MAX, s)) / SC_MAX);
  //  v9.60 (FIX-233): barva podle ZNÁMKY předané z dat (stejné odstíny jako
  //  na Dashboardu). healthColor() zná jen škálu 0–100, takže by skóre 130
  //  obarvil zeleně, přestože je to „Rizikové".
  const colorFor = (s, mo) => (mo && mo.grade && mo.grade.color) ? mo.grade.color
    : (typeof healthColor === 'function')
    ? healthColor(SC_MAX === 100 ? s : Math.round(s / SC_MAX * 100))
  ...
  months.forEach((mo, i) => {
    ...
    // oblouk dle skóre
    ctx.beginPath();
    ctx.arc(x, y, ringR, -Math.PI / 2, -Math.PI / 2 + Math.max(0, Math.min(1, mo.score / SC_MAX)) * Math.PI * 2);
