...
async function renderAdmin() {
  ...
  const _activeTab = document.querySelector('.tx-filt-btn.active[id^="atab-"]')?.id?.replace('atab-','') || 'zdravi';   // S20: rozcestník místo seznamu uživatelů

  el.innerHTML = `
    <!-- Záložky admin panelu -->
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap">
      <button class="tx-filt-btn active" id="atab-zdravi"  onclick="switchAdminTab('zdravi',this)">🩺 Zdraví</button>
      <button class="tx-filt-btn"        id="atab-users"   onclick="switchAdminTab('users',this)">👥 Uživatelé</button>
    ...
    <!-- S20 (Milan): ZDRAVÍ APLIKACE – rozcestník. Panel měl 15 záložek a žádný
         přehled, takže o problému se člověk dozvěděl až proklikáním. -->
    <div id="atab-zdravi-content">
      <div class="card" style="margin-bottom:14px">
        <div class="card-header">
          <span class="card-title">🩺 Zdraví aplikace</span>
          <button class="btn btn-ghost btn-sm" onclick="renderAdminZdravi()" style="font-size:.72rem">↻ Obnovit</button>
        </div>
        <div class="card-body" id="adminZdraviBody">
          <div style="color:#a8aec8;font-size:.82rem">Načítám…</div>
        </div>
      </div>

      <div class="card" style="margin-bottom:14px">
        <div class="card-header"><span class="card-title">🔍 Kontrola integrity dat</span></div>
        <div class="card-body">
          <div style="font-size:.78rem;color:#a8aec8;line-height:1.5;margin-bottom:10px">
            Projde tvoje data a vypíše nesrovnalosti, které se v UI neprojeví chybou,
            jen tiše špatným číslem. Nic nemění – jen čte a hlásí.
          </div>
          <button class="btn btn-accent btn-sm" onclick="runIntegrityCheck()">Spustit kontrolu</button>
...
const VERZE_LOG = [
  ...
  },
  {
    ...
    datum: '2026-09-04',
    zmeny: [
      '📦 TODO-241 (Milan): POUŽITOU PENĚŽENKU UŽ NEJDE SMAZAT, JEN ARCHIVOVAT. Peněženka nese MĚNU – když zmizela, transakce sice zůstaly, ale ztratily informaci, v čem byly, a txCZK je spočítal kurzem CZK. Nákup za 100 € se sečetl jako 100 Kč. Nebyla to schovaná částka, ale ŠPATNÁ, a projevila se všude: v součtech, grafech, rodinném souhrnu i ve skóre (Kontrola integrity hlásila 52 takových transakcí).',
      '⚠️ TODO-241: vrátit smazanou peněženku nejde, takže jediná spolehlivá cesta je nenechat ji smazat. Archivovaná zmizí z nabídek při zadávání, ale zůstane v datech – transakce si podrží měnu i historii. Prázdnou peněženku, na kterou nic neodkazuje, jde smazat úplně; tam není o co přijít. Dialog říká POČET dotčených transakcí a u cizí měny varuje konkrétně.',
      '🔎 TODO-241 – PAST, KTEROU BYLO NUTNO OŠETŘIT: osm míst v kódu hledalo peněženku přes getWallets().find(id) – po zavedení archivu by ji nenašla a měna by se ztratila znovu, jen jinou cestou. Všechna přepsána na nový findWallet(), který archiv NEfiltruje. Filtruje se výhradně tam, kde se peněženka VYBÍRÁ (transakce, výchozí peněženka, převody).',
      '🏷️ TODO-241: kategorie se chová stejně – použitá se archivuje. Tady se nic nepřepočítá špatně, jen výdaje spadnou do „nezařazeno" a přestane být vidět, ZA CO byly; dialog to říká rovnou. Dosavadní mazání bylo jednořádkové „Smazat kategorii?" bez jakéhokoli upozornění.',
      '↩ Archivované položky se zobrazují v samostatné sekci s tlačítkem Vrátit – kdyby zmizely úplně, uživatel by nevěděl, kam se poděly, a marně by je zakládal znovu.',
      '⚙ Nový test tools/smoke_archiv.js (24 kontrol), z toho pět hlídá právě to, že vyhledání podle id archiv nepřeskočí.',
    ]
  },
  {
    verze: 'v10.45',
  ...
  },
  {
    ...
    datum: '2026-09-02',
    zmeny: [
      '🔮 SIMULACE ŽIVOTA: PŘESTAVĚNÝ VÝPOČET (FIX-294 až FIX-302). Aritmetika seděla, model ne — čísla vycházela z peněz, které uživatel nemá, a ze splátek, které nikdy neskončí.',
      '💸 FIX-294: scénář B investoval % PŘÍJMU bez ohledu na to, jestli na to uživatel má. Při nulovém přebytku „investoval" 1 500 Kč měsíčně odnikud a postavil na tom 1,8 milionu. Nově je vklad omezený tím, co reálně zbude.',
      '💳 FIX-295: scénáře A a B splácely dluh po celou dobu simulace. Na dluh 5 000 Kč tak odešlo 3 000 Kč × 360 měsíců = 1 080 000 Kč. Splátka nyní končí spolu s dluhem a uvolněné peníze se přesunou do investic.',
      '📊 FIX-296: dluh neměl úrok, takže „splatit dřív" v modelu neušetřilo ani korunu — scénář C vyhrával jen tím, že jako jediný splácení ukončil. Nové pole „Úrok dluhu (% p.a.)", předvyplněné váženým průměrem ze skutečných dluhů.',
      '🧮 FIX-297: scénář A se znehodnocoval inflací, B a C ne — věta „o X Kč více než při stejném tempu" odčítala dnešní peníze od budoucích. Celá simulace nyní běží nominálně a na konci se přepočte jedním deflátorem. Všechna čísla jsou v dnešních penězích.',
      '⚖️ FIX-298: čisté jmění nově odečítá nesplacený dluh — dřív se ve výsledku neobjevil vůbec. Graf proto umí i zápornou osu s nulovou linkou.',
      '🕐 FIX-299: „splacení za 0.1r" u dvou měsíců nahrazeno čitelným „za 2 měsíce" / „za 3,5 roku".',
      '🏆 FIX-300: titulek nejlepšího scénáře se rozhodoval jen mezi B a C, zatímco číslo pod ním bylo maximum ze všech tří — mohly ukazovat na jiný scénář.',
      '📈 FIX-301: graf měl VLASTNÍ kopii modelu a po opravě karet by ukazoval jiná čísla než dlaždice nad ním. Karty i graf nyní čtou z jediné funkce simCompute().',
      '🏷 FIX-302: karta B hlásila „Investuji 15 %", i když reálně investovala jinou částku. Nově ukazuje konkrétní částku za měsíc.',
      '⚙ Model je nově symetrický: B i C odkládají stejnou korunu, liší se jen tím, kam ji pošlou. Ověřeno testem — když se úrok dluhu rovná výnosu investic, vyjdou oba scénáře nastejno; nad tím vyhrává splácení, pod tím investování.',
      '⚠️ Simulace nově upozorní, když splátka nepokrývá ani úrok — dřív tiše počítala scénáře nad dluhem, který donekonečna roste.',
      '⚙ Nový test tools/smoke_simulace.js (30 kontrol) včetně ekonomického testu zlomu a kontroly falešných poplachů.',
    ]
  },
  {
    verze: 'v10.28',
    datum: '2026-09-02',
    zmeny: [
      '☁️ OPRAVA: AUTOMATICKÉ ZÁLOHY VŮBEC NEŠLY OTEVŘÍT (FIX-293). Okno se ukázalo prázdné a konzole hlásila „too much recursion". Tři exporty v settings.js byly zapsané jako window.renderBackupBody = () => renderBackupBody() – jenže funkce deklarovaná v klasickém skriptu UŽ vlastností window je, takže tenhle řádek ji přepsal šipkou, jejíž tělo volá zpátky window, tedy sebe sama. Nekonečná smyčka. Nešlo otevřít seznam záloh, zálohovat ručně ani obnovit ze zálohy.',
      '⚙ Exporty nyní přiřazují REFERENCI na funkci, ne obalovací šipku. Prověřeno, že stejný vzor nikde jinde v kódu není.',
      '⚙ Nový test tools/smoke_zalohy.js (7 kontrol) – včetně kontroly, že STARÝ zápis skutečně padá na RangeError, aby test nebyl jen ozdoba.',
    ]
  },
  {
    verze: 'v10.27',
    datum: '2026-08-28',
    zmeny: [
      '🩺 ADMIN: NOVÁ ZÁLOŽKA „ZDRAVÍ" jako rozcestník (bod 1 z rešerše admin panelu). Panel měl 15 záložek a žádný přehled – o tom, že něco čeká na rozhodnutí, se člověk dozvěděl teprve proklikáním. Dlaždice ukazují Low confidence, Item Tagy ke schválení, leady, nesoulady plateb a stav komunitního agregátu; každá se dá prokliknout na příslušnou záložku. Je to nově VÝCHOZÍ záložka místo seznamu uživatelů.',
      '🌍 ADMIN: stav komunitní agregace (bod 4) – po zavedení workeru v v10.24 byl uzel aggregate černá skříňka. Dlaždice ukazuje počet přispěvatelů a stáří posledního přepočtu, nebo upozorní, že agregát vůbec nevznikl.',
      '🔍 ADMIN: KONTROLA INTEGRITY DAT (bod 3) – tlačítko, které projde data a vypíše nesrovnalosti, jež se v UI neprojeví chybou, jen tiše špatným číslem: transakce odkazující na smazanou kategorii nebo peněženku (riziko špatného přepočtu měny), rozdělené transakce bez položek, přesuny jen s jednou stranou, splátka nepokrývající úrok, limity kategorií nad 100 %, aktiva na smazané kategorii, transakce bez platného data. NIC NEMĚNÍ – jen čte a hlásí.',
      '⚙ Každé čtení v rozcestníku má vlastní catch – když jeden uzel selže nebo na něj chybí pravidlo, zbytek přehledu se stejně zobrazí. Nenačtený údaj se ukáže jako „—", ne jako nula (SKILL 31: absence dat není informace).',
      '⚙ Nový test tools/smoke_zdravi.js (13 kontrol) – z toho pět hlídá FALEŠNÉ POPLACHY (korektní split, úplný přesun, zdravá splátka, čistá i prázdná data). Kontrola, která křičí zbytečně, je horší než žádná.',
      '📋 Rešerše Admin panelu v RESERSE-funkcni-celky-8.md. Tím je rešerše kompletní: 33 celků v osmi dílech.',
    ]
  },
  {
    verze: 'v10.26',
    datum: '2026-08-28',
    zmeny: [
      '📋 FIX-292: přehled „Rozděleno napříč kategoriemi" v modalu kategorie PŘESKAKOVAL PŘESUNOVÉ KATEGORIE. Filtr propouštěl jen `expense` a `both`, takže kategorie typu Přesun se do součtu nedostaly – přestože mohou mít limit a přestože právě ony bývají označené jako spoření/investice (Milanovo „Penzijní spoření" typu Přesun s 20 %). Editovaná kategorie se přitom přičítala VŽDY, bez ohledu na typ, takže se součet choval jinak podle toho, co měl člověk otevřené.',
      '✗ ODVOLANÝ NÁLEZ z rešerše: tvrdil jsem, že editCat() nevolá updateCatPctInfo(). NEPLATÍ – obě funkce (openCatModal i editCat) přepočet volají na svém posledním řádku. Můj původní grep procházel jen výseč funkce a ten řádek minul. Milanův snímek vysvětluje první nález sám.',
      '⚙ Nový test tools/smoke_katpct.js (7 kontrol) – hlavně ověření, že součet nezávisí na tom, kterou kategorii má uživatel zrovna otevřenou.',
      '📋 Rešerše karet 30–32 (Kategorie · Tagy · Typy plateb) v RESERSE-funkcni-celky-7.md. Tím je pokryto všech 32 uživatelských karet; zbývá jen Admin panel jako interní nástroj.',
    ]
  },
  {
    verze: 'v10.25',
  ...
  },
  {
    ...
    datum: '2026-08-28',
    zmeny: [
      '💰 FIX-289 (Milan): VIRTUÁLNÍ PENĚŽENKA ZMIZELA Z ČISTÉHO MAJETKU. computeNetWorth() počítala jen cíle s `done !== true`, takže jakmile uživatel označil cíl za splněný, peníze v něm z majetku zmizely. Označit cíl za splněný ale NEZNAMENÁ vybrat z něj peníze – ty tam leží dál, dokud si je člověk nepřevede zpátky do peněženky.',
      '📊 FIX-289 – drobné výseče v koláči byly neviditelné: Milanova Finanční rezerva (0,3 % majetku) měla 1,1°, tedy vlasovou čárku, kterou nešlo trefit myší ani prstem. Každá výseč teď dostane minimálně 8°; chybějící stupně se strhnou z velkých výsečí poměrně (91,6 % → z 329,9° na 319,1°). Částky a procenta v legendě zůstávají PŘESNÉ – zkresluje se jen kresba, ne čísla.',
      '📐 FIX-289 – popisky v legendě přetékaly z rámečků („Střednědobá a inves…" + částka + % se nevešly na řádek). Částka a procento jsou nově pod sebou, buňky mřížky širší (240 px), procento doplněno o „% majetku“.',
    ]
  },
  {
    verze: 'v10.22',
    datum: '2026-08-28',
    zmeny: [
      '📊 FIX-288 (Milan): KOLÁČ „ROZLOŽENÍ MAJETKU" PŘEPRACOVÁN. (a) Členil podle a.type, takže nesouhlasil s kartou Čisté jmění ani se sekcemi pod ním – nyní STEJNÉ členění dle likvidity (LIQ_GROUPS). (b) Cíle byly schované mezi Investicemi, teď mají vlastní výseč. (c) PENĚŽENKY SE PŘI ZÁPORNÉM ZŮSTATKU NEZOBRAZILY VŮBEC – podmínka byla `if (totalWallets > 0)`, takže Milanův přečerpaný účet (−19 500) z grafu tiše zmizel.',
      '⚙ FIX-288 – koláč umí jen kladné výseče, takže záporné složky se ukazují v legendě a pod grafem se vysvětlí, že se od celku odečítají. Zamlčet je by znamenalo, že součty nesedí.',
      '🎨 FIX-288 – graf je větší (240 px), vycentrovaný, s mezerami mezi výsečemi. Střed ukazuje čistý součet; při najetí na výseč (nebo na řádek legendy) se přepíše na tu položku a její podíl. Každá výseč má i nativní tooltip.',
      '⚙ FIX-288 – SVG má max-width + preserveAspectRatio (bez toho se malý viewBox na desktopu roztáhne) a legenda je v auto-fit mřížce místo úzkého sloupce.',
      '⚙ Nový test tools/smoke_alokace.js (10 kontrol).',
    ]
  },
  {
    verze: 'v10.21',
    datum: '2026-08-28',
    zmeny: [
      '💰 FIX-287 (Milan): DASHBOARD A AKTIVA UKAZOVALY JINÉ ČISTÉ JMĚNÍ. computeNetWorth() počítal jen peněženky + cíle − dluhy, finanční aktiva (nemovitosti, investice, rezerva) do něj vůbec nevstupovala. Nově se přičítají – ale VYJMA tier \'virtual\' („Vklad do cíle"), protože to jsou tytéž peníze, které už nese řádek Virtuální peněženka. Bez toho by se počítaly dvakrát.',
      '👁️ FIX-287 – nové tlačítko „Skrýt fin. aktiva" vedle „Skrýt dluhy". Volba se pamatuje (localStorage). Ve skrytém stavu zůstane jeden souhrnný řádek s počtem a částkou, aby nebylo tajemství, o kolik jde.',
      '📋 FIX-286 (Milan): karta „Moje úspory a investice" na Dashboardu započítávala vklady do cílů mezi INVESTICE („tento měsíc +19 500"). computeTransferTotals() nemělo pro kategorii „Virtuální přesun" žádné pravidlo, takže spadla do fallbacku \'investment\'. Nová skupina \'virtual\', z karty úspor se vyřazuje – patří do Čistého majetku.',
      '⚙ FIX-285b (Milan): sekce „Virtuální přesuny" se v Aktivech přesunula hned pod Peněženky (dřív na konci) a dostala vizuální propojení – odsazení, svislou linku a šipku ↳. Ukazuje, že ty peníze přišly z peněženky, ne že jde o další nezávislý majetek.',
      '✏️ OPRAVA VLASTNÍHO NÁLEZU z v10.20: tvrdil jsem, že „Penzijní spoření" spadne do Finanční rezervy kvůli pořadí vzorů. Přehlédl jsem, že assetCatLiq() čte NEJDŘÍV ruční nastavení `cc.liq` – kdo má u kategorie vyplněnou Likviditu aktiva (jako Milan „Dlouhodobé / fyzické"), toho se to netýká vůbec. Platí to jen pro kategorie s nevyplněným polem.',
    ]
  },
  {
    verze: 'v10.20',
    datum: '2026-08-28',
    zmeny: [
      '📋 FIX-285 (Milan): „VKLAD DO CÍLE" UŽ NENÍ INVESTICE. Přesun do cíle vytvoří aktivum s kategorií „Virtuální přesun" – ta nespadala do žádného vzoru v assetCatLiq, takže dostala výchozí \'mid\' a peníze odložené na cíle se počítaly mezi Střednědobá a investiční aktiva. Je to ale jen přehození v rámci vlastních peněz, ne investice. Nová sekce „📋 Virtuální přesuny" v Aktivech.',
      '💎 FIX-285 – karta Čisté jmění má nový řádek „Z toho virtuální přesuny" s rozpadem po jednotlivých cílech. Vysvětluje záporné číslo u Peněženek: peníze odešly z peněženky, ale z majetku ne. Není to šestá karta – je to ROZPAD už započítané částky, ne přírůstek, proto pod čarou a ne v mřížce.',
      '⚙ FIX-285 – celkové čisté jmění se NEMĚNÍ, peníze se jen přeřadily mezi sekcemi. Ověřeno testem (tools/smoke_virtualni.js, 8 kontrol).',
      '🔍 NÁLEZ (neopraveno): assetCatLiq() testuje vzor pro „rezerva/spoření" DŘÍV než vzor pro „penzijní/dlouhodobé". Kategorie „Penzijní spoření" tak spadne do Finanční rezervy, protože obsahuje „spoř" – přestože penzijko není likvidní rezerva. Kategorie pojmenovaná „Penzijko" (bez slova spoření) skončí správně v dlouhodobých. Pre-existing, ověřeno na nedotčeném originále. Neopraveno záměrně: přehodilo by to už zařazená aktiva, což je věcné rozhodnutí.',
    ]
  },
  {
    verze: 'v10.19',
    datum: '2026-08-28',
    zmeny: [
      '🎨 FIX-284 (Milan): widget „Jak pracuješ pro banky" – dny pro banku se barvily podle ZATÍŽENÍ, takže při bezpečném pásmu byly ZELENÉ, stejně jako dny pro sebe. Den odpracovaný pro banku není dobrá zpráva, i když je jich málo. Nově: příslušnost dne (banka = červená, ty = zelená) a hodnocení zatížení (Bezpečné/Zvýšené/Kritické) jsou DVĚ NEZÁVISLÉ barvy.',
      '👁️ FIX-284 – čitelnost: legenda měla `opacity:.4` na celém prvku, což vybledlo i text. Čísla 1–31 v kalendáři měla bílý text na světlém podkladu s opacity. Nově plné barvy, tmavý text na barevných dlaždicích, legenda barevnou tečkou místo podbarveného štítku. Popisky přešly z var(--text3) na #a8aec8.',
      '📏 FIX-284 – bar měl pevné popisky „0 dní / 21 dní" (21 = pracovních dní), zatímco kalendář pod ním má 28–31 dní a výplň se počítala z % příjmu. Tři jednotky v jednom widgetu. Nově popisky podle skutečné délky měsíce a výplň podle dnů – konzistentní s kalendářem i s hlavním číslem.',
      '⚙ Skloňování v legendě (1 den · 3 dny · 28 dní).',
      '⚙ Nový test tools/smoke_dluh_widget.js (9 kontrol) – hlídá hlavně to, aby se barva příslušnosti a barva hodnocení znovu nesloučily.',
    ]
  },
  {
    verze: 'v10.18',
...
// ══════════════════════════════════════════════════════
//  S20: ZDRAVÍ APLIKACE – rozcestník + kontrola integrity
// ══════════════════════════════════════════════════════
// Panel měl 15 záložek a žádný přehled: o tom, že něco čeká na rozhodnutí,
// se člověk dozvěděl teprve proklikáním. Tady se čísla sesbírají na jedno místo.
// Vše se načítá paralelně a KAŽDÉ ČTENÍ MÁ VLASTNÍ catch – když jeden uzel selže
// (nebo na něj chybí pravidlo), zbytek přehledu se stejně zobrazí.

async function renderAdminZdravi() {
  const el = document.getElementById('adminZdraviBody'); if (!el) return;
  if (!isAdmin()) { el.innerHTML = ''; return; }
  el.innerHTML = '<div style="color:#a8aec8;font-size:.82rem">Načítám…</div>';

  const base = 'https://financeflow-a249c-default-rtdb.europe-west1.firebasedatabase.app';
  let idToken = '';
  try { idToken = await window._currentUser?.getIdToken?.() || ''; } catch (e) {}
  const q = (path, params) => `${base}/${path}.json?${params ? params + '&' : ''}auth=${idToken}`;
  const safe = async (fn, fallback) => { try { return await fn(); } catch (e) { return fallback; } };
  const countOf = (path) => safe(async () => {
    const r = await fetch(q(path, 'shallow=true'));
    const j = r.ok ? await r.json() : null;
    return j ? Object.keys(j).length : 0;
  }, null);

  const monthKey = (typeof COMMUNITY_MONTH_KEY === 'function') ? COMMUNITY_MONTH_KEY() : '';

  const [lowConf, itemTags, leads, agg] = await Promise.all([
    countOf('ai_low_confidence'),
    countOf('community/itemTags'),
    countOf('leads'),
    safe(async () => {
      const r = await fetch(q(`community/${monthKey}/aggregate`));
      return r.ok ? await r.json() : null;
    }, null)
  ]);
  // Audit plateb záměrně BEZ čísla: runPaymentAudit() výsledek vykresluje rovnou
  // do DOM a nevrací počet. Rozebírat ji jen kvůli dlaždici by znamenalo duplikovat
  // logiku shody premium↔premiumLog na druhém místě – a dvě kopie téhož pravidla se
  // drív nebo později rozejdou. Dlaždice proto jen odkazuje na záložku.

  const tile = (icon, label, value, color, hint, tab) => `
    <div ${tab ? `onclick="document.getElementById('atab-${tab}')?.click()" style="cursor:pointer"` : ''}
      class="stat-card" style="padding:11px 12px;border-left:3px solid ${color}">
      <div style="font-size:.72rem;color:#a8aec8">${icon} ${label}</div>
      <div style="font-size:1.15rem;font-weight:800;color:${color};font-family:Syne,sans-serif;margin-top:2px">${value}</div>
      ${hint ? `<div style="font-size:.68rem;color:#8b91a8;margin-top:2px">${hint}</div>` : ''}
    </div>`;

  const nn = v => (v === null ? '—' : v);
  const col = (v, warnFrom) => (v === null ? '#8b91a8' : (v >= warnFrom ? 'var(--debt)' : 'var(--income)'));

  // Stav komunitní agregace (TODO-235) – po zavedení workeru byl aggregate
  // černá skříňka; tohle je jediné místo, kde jde poznat, jestli běží.
  let aggHtml;
  if (!agg) {
    aggHtml = tile('🌍', 'Komunitní agregát', 'chybí', 'var(--debt)',
      monthKey ? `pro ${monthKey} zatím nevznikl` : '');
  } else if (agg.enough === false) {
    aggHtml = tile('🌍', 'Komunitní agregát', `${agg.k || 0} lidí`, 'var(--text3)',
      `pod prahem ${agg.minN || 1} – nezveřejněno`);
  } else {
    const mins = agg.updatedAt ? Math.round((Date.now() - agg.updatedAt) / 60000) : null;
    const age = mins === null ? '' : (mins < 60 ? `před ${mins} min` : `před ${Math.round(mins / 60)} h`);
    aggHtml = tile('🌍', 'Komunitní agregát', `${agg.k || 0} lidí`, 'var(--income)', age);
  }

  el.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(165px,1fr));gap:9px">
      ${tile('⚠️', 'Low confidence', nn(lowConf), col(lowConf, 1), lowConf ? 'čeká na rozhodnutí' : 'nic nečeká', 'lowconf')}
      ${tile('🔖', 'Item Tagy', nn(itemTags), col(itemTags, 1), itemTags ? 'ke schválení' : 'vyřízeno', 'itemtags')}
      ${tile('📋', 'Leady', nn(leads), leads ? 'var(--bank)' : '#8b91a8', '', 'leads')}
      ${tile('💳', 'Audit plateb', 'zkontrolovat', 'var(--bank)', 'shoda premium ↔ platby', 'audit')}
      ${aggHtml}
    </div>
    <div style="font-size:.7rem;color:#8b91a8;margin-top:10px">
      Dlaždice se dají prokliknout na příslušnou záložku. „—“ znamená, že se údaj
      nepodařilo načíst – ne že je nula.
    </div>`;
}
window.renderAdminZdravi = renderAdminZdravi;

// Kontrola integrity: hledá nesrovnalosti, které se v UI neprojeví chybou,
// jen tiše špatným číslem. Většina dnešních nálezů (S20) by se takhle dala
// najít dřív, než si jich někdo všimne. NIC NEMĚNÍ – jen čte a hlásí.
function runIntegrityCheck() {
  const el = document.getElementById('integrityResult'); if (!el) return;
  const D = getData();
  const issues = [];
  ...
  // 1) odkazy do prázdna
  const orphanCat = txs.filter(t => t.catId && !catIds.has(t.catId));
  if (orphanCat.length) add('warn', `${orphanCat.length}× transakce odkazuje na smazanou kategorii`,
    'V přehledech spadnou do „nezařazeno“ a v součtu podle kategorií chybí.');

  const orphanWallet = txs.filter(t => t.wallet && !walletIds.has(t.wallet));
  if (orphanWallet.length) add('bad', `${orphanWallet.length}× transakce odkazuje na smazanou peněženku`,
    'Cizí měna se u nich přepočítá kurzem CZK – částka může být řádově jinde.');

  // 2) split rodiče bez dětí a naopak
  const splitIds = new Set(txs.filter(t => t.splitParent && t.splitId).map(t => t.splitId));
  const childIds = new Set(txs.filter(t => t.splitId && !t.splitParent).map(t => t.splitId));
  const lonelyParents = [...splitIds].filter(id => !childIds.has(id));
  if (lonelyParents.length) add('warn', `${lonelyParents.length}× rozdělená transakce bez položek`,
    'Rodič se z výdajů vylučuje, ale žádné dítě ho nenahradí – částka se ztratí ze součtu.');

  // 3) přesuny bez protistrany
  const trIds = {};
  txs.forEach(t => { if (t.transferId) trIds[t.transferId] = (trIds[t.transferId] || 0) + 1; });
  const halfTransfers = Object.values(trIds).filter(n => n === 1).length;
  if (halfTransfers) add('warn', `${halfTransfers}× přesun jen s jednou stranou`,
    'Peníze z jedné peněženky odešly, ale do druhé nedorazily – zůstatky nesedí.');

  // 4) záporné cíle a částky
  const negGoals = (D.wishes || []).filter(w => w.isGoal && (w.targetAmount || 0) < 0);
  if (negGoals.length) add('warn', `${negGoals.length}× cíl se zápornou cílovou částkou`, '');

  // 5) dluhy: splátka nepokryje úrok → kalendář se utne bez vysvětlení
  (D.debts || []).forEach(d => {
    const r = (parseFloat(d.interest) || 0) / 100 / 12;
    const firstInterest = (d.remaining || 0) * r;
    if (r > 0 && (d.payment || 0) > 0 && d.payment <= firstInterest) {
      add('bad', `Dluh „${d.creditor || d.name || '?'}“: splátka nepokryje ani úrok`,
        `Splátka ${Math.round(d.payment)} Kč vs. měsíční úrok ${Math.round(firstInterest)} Kč – dluh takhle neklesá.`);
    }
  });

  // 6) kategorie: součet limitů přes 100 %
  let expA = 0, savA = 0;
  cats.forEach(c => {
    if (!(c.type === 'expense' || c.type === 'both' || c.type === 'transfer')) return;
    const p = parseFloat(c.healthPct) || 0; if (p <= 0) return;
    if (c.isSaving || c.isInvest) savA += p; else expA += p;
  });
  if (expA + savA > 100) add('warn', `Limity kategorií dávají ${Math.round(expA + savA)} % příjmu`,
    'Přes 100 % znamená, že plán nejde dodržet – něco se musí snížit.');

  // 7) aktiva napojená na neexistující kategorii
  const orphanAssets = (D.assets || []).filter(a => a.linkedCatId && !catIds.has(a.linkedCatId));
  if (orphanAssets.length) add('warn', `${orphanAssets.length}× aktivum napojené na smazanou kategorii`,
    'Přestane se aktualizovat z přesunů a zamrzne na poslední hodnotě.');

  // 8) transakce bez data nebo s nesmyslným datem
  const badDate = txs.filter(t => !t.date || isNaN(new Date(t.date).getTime()));
  if (badDate.length) add('bad', `${badDate.length}× transakce bez platného data`,
    'Nespadne do žádného měsíce, takže v součtech chybí úplně.');

  const sevColor = { bad: 'var(--expense)', warn: 'var(--debt)' };
  const sevIcon = { bad: '🔴', warn: '🟡' };

  el.innerHTML = !issues.length
    ? `<div class="insight-item good"><div class="insight-icon">✅</div><div class="insight-text">
         <strong>Nic k nahlášení.</strong><br>
         <span style="font-size:.76rem;color:#a8aec8">Zkontrolováno: odkazy na kategorie a peněženky,
         rozdělené transakce, přesuny, cíle, splátky dluhů, limity kategorií, napojení aktiv a data transakcí.</span>
       </div></div>`
    : issues.map(i => `<div style="padding:9px 11px;border-radius:9px;background:var(--surface2);border-left:3px solid ${sevColor[i.sev]};margin-bottom:7px">
         <div style="font-size:.82rem;font-weight:700;color:var(--text)">${sevIcon[i.sev]} ${i.title}</div>
         ${i.detail ? `<div style="font-size:.74rem;color:#a8aec8;margin-top:3px;line-height:1.45">${i.detail}</div>` : ''}
       </div>`).join('')
