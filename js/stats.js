...
function updateCatPctInfo(){
  ...
  (S.categories||[]).forEach(c=>{
    ...
    // FIX-292 (S20): původně `expense || both`, takže kategorie typu PŘESUN se do
    //   součtu NEZAPOČÍTÁVALY – přestože mohou mít healthPct a přestože právě ony
    //   bývají označené jako spoření/investice (např. „Penzijní spoření" typu
    //   Přesun s 20 %). Editovaná kategorie se přitom přičítala vždy, bez ohledu na
    //   typ – součet se tedy choval jinak podle toho, co měl člověk otevřené.
...
//   – peníze v součtech zůstanou, ale přestane být vidět ZA CO byly. Na rozdíl
//   od peněženky se nic nepřepočítá špatně, takže stačí říct, co se stane,
//   a nabídnout archivaci místo mazání.
function deleteCat(id){
  if(viewingUid) return;
  const c=(S.categories||[]).find(x=>x.id===id); if(!c) return;
  const D=getData();
  const pocet=(D.transactions||[]).filter(t=>t && (t.catId===id || t.category===id)).length;

  if(pocet===0){
    if(!confirm(`Smazat kategorii „${c.name}"?\n\nŽádná transakce ji nepoužívá.`)) return;
    S.categories=S.categories.filter(x=>x.id!==id); save(); renderPage();
    return;
  }
  const volba=confirm(
      `Kategorii „${c.name}" používá ${pocet} ${pocet===1?'transakce':(pocet<5?'transakce':'transakcí')}.\n\n`
    + `Když ji smažeš, peníze v celkových součtech zůstanou, ale v přehledu podle kategorií spadnou do „nezařazeno" – přestane být vidět, za co byly.\n\n`
    + `OK = archivovat (zmizí z nabídek, historie zůstane čitelná)\n`
    + `Zrušit = nechat být`);
  if(!volba) return;
  c.archived=true; save(); renderPage();
  if(typeof showToast==='function') showToast(`📦 „${c.name}" archivována`);
}
function unarchiveCat(id){
  if(viewingUid) return;
  const c=(S.categories||[]).find(x=>x.id===id); if(!c) return;
  delete c.archived; save(); renderPage();
}
window.unarchiveCat=unarchiveCat;
...
function renderFamilySummary(){
  ...
  // TODO-230 (S20): VŠICHNI členové domácnosti, ne jen první partner.
  // Původně `{...partners.map(...)[0]}` – ostatní partneři se tiše zahodili,
  // takže rodinné součty byly u 3+ členné domácnosti nižší, než měly být,
  // a nic na to neupozornilo. partnerData objekt N členů unese už dávno.
  const members=[
    {name:myName,photo:window._userProfile?.photoURL||me?.photoURL,data:S},
    ...partners.map(([uid,p])=>({
      name:p.profile?.displayName||'Člen domácnosti',
      photo:p.profile?.photoURL,
      data:p.data
    }))
  ];
  ...
  <div class="card gap"><div class="card-header">
    ...
    <div class="card-body"><canvas id="familySaldoChart" height="150"></canvas></div>
  </div>`;

  // TODO-230: KDO NA CO UTRATIL – kombinovaný seznam napříč všemi členy, ne jen
  // souhrnná čísla za sloupec. Doteď šlo poznat "kolik utratila domácnost" a
  // "kolik utratil každý celkem", ale ne "čí byla TATO konkrétní útrata" na
  // jednom společném žebříčku. Stejný filtr jako expSum/getActual (SKILL 20):
  // vyloučen splitParent, isBalancing, přesuny – jinak by se vyrovnávací
  ...
  members.forEach(m=>{
    ...
    txs.forEach(t=>{
      ...
      familyTxs.push({
        name:t.name||cat?.name||'Bez názvu', date:t.date,
        catIcon:cat?.icon||'💸', catName:cat?.name||'Nezařazeno',
        amt:txCZK(t,D), memberName:m.name, memberPhoto:m.photo
      });
    });
  });
  familyTxs.sort((a,b)=>b.amt-a.amt);
  // Filtr podle člena. Filtruje se AŽ TADY, po sestavení celého žebříčku –
  // souhrnné dlaždice i graf nahoře zůstávají za celou domácnost, filtr
  // ovlivňuje jen tenhle seznam.
  const shownTxs=_famMember ? familyTxs.filter(t=>t.memberName===_famMember) : familyTxs;
  const topFamilyTxs=shownTxs.slice(0,10);
      ...
      <div class="family-member-body">
        ...
        ${topFamilyTxs.map(t=>`
          <div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid var(--border)">
            <div style="width:22px;height:22px;border-radius:50%;background:var(--surface3);display:flex;align-items:center;justify-content:center;font-size:.62rem;overflow:hidden;flex-shrink:0" title="${t.memberName}">${t.memberPhoto?`<img src="${t.memberPhoto}" style="width:22px;height:22px;border-radius:50%;object-fit:cover">`:'👤'}</div>
            <div style="flex:1;min-width:0">
              <div style="font-size:.82rem;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${t.catIcon} ${escHtml(t.name)}</div>
              <div style="font-size:.68rem;color:var(--text3)">${t.memberName} · ${t.catName} · ${(()=>{const d=new Date(t.date);return isNaN(d)?t.date:d.getDate()+'.'+(d.getMonth()+1)+'.';})()}</div>
            </div>
            <strong style="font-size:.85rem;color:var(--expense);white-space:nowrap">${fmtB(t.amt)}</strong>
          </div>`).join('')}
      </div>
    </div>`;
  }

  // Sloupce členů: auto-fit místo fixních 2 sloupců (.grid2) – domácnost může mít
  // 3 i 5 členů a poslední osamocený sloupec by se jinak roztáhl přes celou šířku.
  html+=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px">`;
  
  members.forEach(m=>{
    const D=Object.assign({transactions:[],debts:[],categories:[],bank:{startBalance:0},birthdays:[],wishes:[],wallets:[],payTypes:[],sablony:[],projects:[]},m.data);
    const txs=getTx(S.curMonth,S.curYear,D);
    const inc=incSum(txs,D),exp=expSum(txs,D),bank=computeBank(D);
    const debts=(D.debts||[]).reduce((a,d)=>a+d.remaining,0);
    const expCats=(D.categories||[]).filter(c=>c.type==='expense'||c.type==='both');
  ...
  // TODO-230: RODINNÝ TREND – saldo za posledních 6 měsíců, napříč VŠEMI členy.
  // 100% reuse drawSaldoBars (charts.js, renderGrafy) – jen vstupní data se
  // počítají přes všechny D, ne jen S. Stejný dvojitý rAF + setTimeout vzor
  // (SKILL 2): canvas je v tomto okamžiku čerstvě vložený přes innerHTML,
  // prohlížeč ještě nedokončil layout a getBoundingClientRect by vrátil 0.
  if(typeof drawSaldoBars==='function'){
    const famLabels=[], famSaldo=[];
    for(let i=_famRange-1;i>=0;i--){
      ...
      famLabels.push(CZ_M[m].slice(0,3));
      famSaldo.push(mInc-mExp);
    }
    requestAnimationFrame(()=>{
      requestAnimationFrame(()=>{
        setTimeout(()=>{ drawSaldoBars('familySaldoChart', famLabels, famSaldo); }, 50);
      });
    });
  }
}
