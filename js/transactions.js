...
function renderBank(){
  ...
  if(bc){
    ...
    bc.innerHTML=`<div class="bank-label">Kumulované úspory</div>
      <div class="bank-balance">${fmtB(bal)}</div>
      <div class="bank-sub">Automaticky z příjmů a výdajů</div>
      <div style="margin-top:9px;font-size:.8rem;color:rgba(232,244,253,.7)">Saldo ${CZ_M[S.curMonth]}: <strong style="color:${last>=0?'#a7f3d0':'#fca5a5'}">${last>=0?'+':''}${fmtB(last)}</strong></div>
  ...
  if(hEl){
    ...
    hEl.innerHTML=ser.map(s=>`<div class="bhi"><span>${s.saldo>=0?'📈':'📉'}</span><div style="flex:1"><div style="font-size:.85rem;font-weight:500">${CZ_M[s.m]} ${s.y}</div><div style="font-size:.7rem;color:var(--text3)">příjmy − výdaje</div></div><div style="font-weight:600;color:${s.saldo>=0?'var(--income)':'var(--expense)'};font-size:.87rem">${s.saldo>=0?'+':''}${fmtB(s.saldo)}</div><div style="color:var(--text2);font-size:.8rem;min-width:75px;text-align:right">${fmtB(s.balance)}</div></div>`).join('');
  }
}
...
function renderSeasTable(D){
  ...
  rows.forEach(r=>{
    ...
    r.avg.forEach(v=>{
      ...
      h+=`<td style="background:${cellCol(p)};color:#e8eaf2;font-weight:600" title="Ø ${fmtB(Math.round(v))}">${p===0?'0':'+'+p}%</td>`;
    });
    h+=`<td style="color:#a8aec8;white-space:nowrap">${fmtB(Math.round(r.min))}</td></tr>`;
  });
  h+=`</tbody></table></div>
...
function renderPredAccuracy(D){
  ...
  keys.slice(-12).forEach(k=>{
    ...
    rowsH+=`<tr${running?' style="opacity:.75"':''}>
      ...
      <td style="color:#a8aec8">${fmtB(s.predInc)}</td>
      <td style="color:var(--income)">${actInc?fmtB(actInc):'–'}</td>
  ...
  el.innerHTML=(D.debts||[]).map(d=>{
    ...
    return `<div class="debt-card">
      ...
      <div class="debt-stats">
        <div class="dst"><div class="dst-val" style="color:${pc}">${fmtB(d.remaining)}</div><div class="dst-lbl">Zbývá</div></div>
