...
function renderImport() {
  ...
  el.innerHTML = `
    <!-- Záložky -->
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap">
      <button class="tx-filt-btn active" id="itab-csv" onclick="switchImportTab('csv',this)">📊 CSV / Excel</button>
    ...
    <div id="itab-csv-content">
      <div class="card" style="margin-bottom:14px">
        <div class="card-header"><span class="card-title">📊 Import z CSV / Excel</span></div>
        <div class="card-body">
          <div style="font-size:.8rem;color:var(--text2);margin-bottom:14px">
          ...
          <!-- Banky shortcut -->
          <div style="margin-bottom:14px">
            <div style="font-size:.76rem;color:var(--text2);margin-bottom:8px;font-weight:600">Rychlý import dle banky:</div>
            <div style="display:flex;gap:8px;flex-wrap:wrap">
              ${[['Fio','fio'],['Air Bank','airbank'],['ČSOB','csob'],['KB','kb'],['Raiffeisenbank','rb'],['Naše šablona','template']].map(([name,id])=>
                `<button class="btn btn-ghost btn-sm" onclick="setImportBank('${id}')" id="bank-${id}" style="font-size:.76rem">${name}</button>`
              ).join('')}
            </div>
          </div>
    ...
    <div id="itab-pdf-content" style="display:none">
      <div class="card" style="margin-bottom:14px">
        <div class="card-header"><span class="card-title">📄 Import z PDF výpisu</span></div>
        <div class="card-body">
          <div style="font-size:.8rem;color:var(--text2);margin-bottom:14px">
...
function mapImportRows(rows, bank) {
  ...
  const maps = {
    ...
    kb: {date:'datum splatnosti', amount:'částka', name:'popis příkazu', note:''},
  ...
  preview.innerHTML = `
    <div style="background:var(--surface2);border-radius:12px;padding:14px;border:1px solid var(--border)">
      ...
      <div style="display:flex;gap:8px">
        <button class="btn btn-accent" style="flex:1" onclick="confirmImport('${filename}')">✅ Importovat ${rows.length} transakcí</button>
...
function confirmImport(filename) {
  ...
  if(status) {
    status.style.display = 'block';
    status.innerHTML = `<div class="insight-item good"><div class="insight-icon">✅</div><div class="insight-text">
