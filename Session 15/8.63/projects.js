// FinanceFlow · v8.63 · projects.js · 2026-07-03
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function _autoLimitsSuggest(){
  ...
  rows.forEach(r=>{
    if(base>0 && totalAvg>0 && r.avg>0) r.suggest=r2(r.avg/base*100);          // podíl skutečné útraty na základu příjmu
    else if(base>0 && totalAvg>0)       r.suggest=0;                            // kategorie bez útraty → 0 (uživatel doplní)
    else                                 r.suggest=rows.length?r2(80/rows.length):0; // žádná historie → rovnoměrně 80 %
  });
  return {rows, base, hasHistory: base>0 && totalAvg>0};
}
    ...
    <div style="padding:12px 16px;overflow-y:auto;flex:1">
      <div style="font-size:.72rem;color:#a8aec8;line-height:1.5;margin-bottom:10px">
        ${hasHistory
          ...
          : 'Zatím nemáš historii výdajů – navrhuji rovnoměrné rozdělení 80 % základu příjmu. Po pár měsících spusť znovu pro přesnější návrh.'}
      </div>
      ${rows.map((r,i)=>`
      ...
      <div style="display:flex;gap:10px;align-items:flex-start">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="display:flex;justify-content:space-between;margin-top:3px">
            <span style="font-size:.7rem;color:var(--text3)">${pctOfInc} % základu příjmu · plán ${planAmt} (${limitPct})</span>
