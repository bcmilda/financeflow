// FinanceFlow · v8.72 · projects.js · 2026-07-06
//  PROJEKTY
// ══════════════════════════════════════════════════════
...
function computeHealthScores(D, m, y) {
  ...
  let expScore;
  if(totalInc > 0) {
    expScore = (typeof finScoreS1==='function' ? finScoreS1(totalExp/totalInc) : 12) * 4;
  ...
  if(savingCats.length > 0 && baseIncome > 0) {
    ...
    savingScore = (typeof finScoreS4==='function' ? finScoreS4(savedRate) : 12) * 4;
  }
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div style="font-size:.72rem;color:#a8aec8;line-height:1.6;margin-top:10px;border-top:1px solid var(--border);padding-top:8px">
            <div><strong style="color:var(--text)">Výdajové</strong> – Dashboard scoring 💰 Cash flow (výdaje/příjmy, tabulka S1) × 4.</div>
            <div><strong style="color:var(--text)">Rozpočtové</strong> – dodržování nastavených limitů kategorií (průměr skóre).</div>
            <div><strong style="color:var(--text)">Úsporové</strong> – Dashboard scoring 💎 Spoření (📈 investice + 🛟 rezerva, % základu, tabulka S4) × 4.</div>
          </div>
...
function renderDTISection(D, baseIncome) {
  ...
  const effectiveIncome = computeEffectiveIncome(D);
