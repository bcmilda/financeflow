// FinanceFlow · v8.72 · premium.js · 2026-07-06
//  PREMIUM SYSTEM
// ══════════════════════════════════════════════════════
...
function computeFinancialScore(D) {
  ...
  const effIncome = computeEffectiveIncome(D);
  const totalDebt = debts.reduce((a,d)=>a+(d.remaining||0),0);
  const annualIncome = (effIncome||totalInc) * 12;
  ...
  if (totalInc > 0) {
    ...
    const s1table = _FIN_S1_TABLE; // (tabulka nahoře: _FIN_S1_TABLE)
    score1 = 0;
  ...
  const dsti = (effIncome||totalInc) > 0 ? monthlyPayments / (effIncome||totalInc||1) * 100 : 0; // v8.72 (FIX-188)
  ...
  // v8.72 (TODO-155): + aktiva v sekci 🛟 Finanční rezerva (vklady z rezervních kategorií
  // tečou do napojených aktiv) → Rezerva na Dashboardu konečně vidí Spořicí účet/Fond.
  ...
  // v8.72 (TODO-155): S4 = AKTIVNÍ SPOŘENÍ → kategorie 📈 isInvest; fallback 🛟 isSaving
  // (uživatelé bez rozlišení), vždy bez Virtuálního přesunu (informativní).
  ...
  if (savCats.length > 0 && (baseIncome||0) > 0) {
    ...
    const s4table = _FIN_S4_TABLE; // v8.72: sdílená tabulka
    score4 = 0;
