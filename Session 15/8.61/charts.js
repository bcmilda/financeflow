// FinanceFlow · v8.61 · charts.js · 2026-07-02
//  GRAFY (simplified)
// ══════════════════════════════════════════════════════
...
function renderKumulChart(days, cumul, medVal) {
  ...
  if(medVal > 0) {
    ...
    ctx.fillText(fmtB(Math.round(medVal)), W-pad.r+3, medY+3);
  }
  ...
  // popisek osy Y
  ctx.save();ctx.translate(11,pad.t+cH/2);ctx.rotate(-Math.PI/2);ctx.fillStyle='#7e84a0';ctx.font='9px Instrument Sans';ctx.textAlign='center';ctx.fillText(curSym()+' (kumulativně)',0,0);ctx.restore();
  ...
  if(medVal>0){
    ...
    ctx.fillStyle='#f87171';ctx.fillText('Medián 6 měs.: '+fmtB(Math.round(medVal)),llx2+20,lly+4);
  }
}
