//  FinanceFlow · v8.61 · transactions.js · 2026-07-02
//  BANK
// ══════════════════════════════════════════════════════
function renderBank(){
  const D=getData();
  ...
  el.innerHTML=(D.debts||[]).map(d=>{
    ...
    return `<div class="debt-card">
      ...
      <div style="display:flex;justify-content:space-between;font-size:.68rem;color:#a8aec8;margin:-2px 0 6px;gap:8px;flex-wrap:wrap">
        <span>✅ Zaplaceno ${fmtB(paid.paidSum)}${_overdueAmt>0?` · <span style="color:#fca5a5">po splatnosti ${fmtB(_overdueAmt)}</span>`:''}</span>
