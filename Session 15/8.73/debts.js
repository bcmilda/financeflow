//  FinanceFlow · v8.73 · debts.js · 2026-07-06
//  ADD / EDIT TX
// ══════════════════════════════════════════════════════
function openAddTx(){
...
function populateTxPayTypeSelect() {
  ...
  // S14: vyplň i Typ platby u Přesunu (txTransferPayType)
...
function computeDebtPaid(d, D){
  ...
  const paidSum=txs.reduce((a,t)=>a+(t.amount||t.amt||0),0);
...
function renderDebtStressWidget(D) {
  ...
  // v8.72 (FIX-188): sjednoceno s Bankovním hodnocením – sdílené helpery
  // (dřív ignoroval d.installments → DSTI 732 % vs 753 %).
  const baseIncome = computeEffectiveIncome(D);
  ...
  // 1. DSTI (splátky/příjem) – v8.73 (TODO-158): Milanova tabulka DSTI (0–40 b, 41 řádků)
  //    invertovaná na STRES 0–25 (stres index = víc bodů → HORŠÍ). Jemné odstupňování
  //    místo 4 skoků: stres = (1 − body/40) × 25.
  const dsti = baseIncome>0 ? monthlyPayments/baseIncome*100 : 0;
  const dstiScore = (typeof msc_DSTI==='function') ? Math.round((1 - msc_DSTI(dsti)/_SCORING.max.DSTI)*25) : (dsti<20?0:dsti<35?10:dsti<50?18:25);

  // 2. DTI (dluh/roční příjem) – Milanova tabulka DTI (0–60 b, 60 řádků), inverze na 0–25
  const dti = annualIncome>0 ? totalDebt/annualIncome*100 : 0;
  const dtiScore = (typeof msc_DTI==='function') ? Math.round((1 - msc_DTI(dti)/_SCORING.max.DTI)*25) : (dti<300?0:dti<600?8:dti<900?16:25);
...
// ══════════════════════════════════════════════════════
function _payoffSim(strategy, extra){
  const debts=(S.debts||[]).filter(d=>d.remaining>0).map(d=>({
    name:d.name, bal:d.remaining, rate:(d.interest||0)/100/12,
    pay:d.payment||Math.max(500, Math.round(d.remaining*0.02))
  }));
  if(!debts.length) return null;
  let months=0, totalInterest=0; const curve=[debts.reduce((a,d)=>a+d.bal,0)];
  const intCurve=[0]; let firstPaidMonth=null, firstPaidName=null; // v8.72: viditelný rozdíl strategií
  while(debts.some(d=>d.bal>0.5) && months<600){
    months++;
    // úroky
    debts.forEach(d=>{ if(d.bal>0){ const ir=d.bal*d.rate; d.bal+=ir; totalInterest+=ir; } });
    // rozpočet = minimální splátky živých dluhů + extra + uvolněné splátky splacených
    let budget = debts.reduce((a,d)=>a+(d.bal>0?d.pay:0),0) + extra
               + debts.reduce((a,d)=>a+(d.bal<=0?d.pay:0),0);
    // minimální splátky
    ...
    while(budget>0.5){
      ...
      alive.sort(strategy==='avalanche' ? (a,b)=>b.rate-a.rate : (a,b)=>a.bal-b.bal);
      const t=alive[0]; const p=Math.min(budget,t.bal); t.bal-=p; budget-=p;
    }
  ...
  return { months, totalInterest:Math.round(totalInterest), curve, intCurve, firstPaidMonth, firstPaidName };
}
...
function renderAvalanche(){
  ...
  const lbl=document.getElementById('avaExtraLbl'); if(lbl) lbl.textContent='+'+fmt(extra)+' Kč';
  const el=document.getElementById('avaResult'); if(!el) return;
  const av=_payoffSim('avalanche',extra), sn=_payoffSim('snowball',extra);
  if(!av||!sn){ el.innerHTML=''; return; }
  const dur=m=>`${Math.floor(m/12)}r ${m%12}m`;
  const saved=sn.totalInterest-av.totalInterest;
  // v8.72: graf KUMULATIVNÍCH ÚROKŮ – celkový zůstatek je u obou strategií téměř shodný
  // (stejné peníze dovnitř), rozdíl dělají úroky → křivky se viditelně rozestoupí.
  const maxM=Math.max(av.intCurve.length,sn.intCurve.length), maxV=Math.max(sn.totalInterest,av.totalInterest,1);
  const W=520,H=180,pad={l:52,r:10,t:22,b:26};
  const cw=W-pad.l-pad.r, ch=H-pad.t-pad.b;
  const X=i=>pad.l+i/(maxM-1)*cw, Y=v=>pad.t+ch*(1-v/maxV);
  const path=c=>c.map((v,i)=>`${i?'L':'M'}${X(i).toFixed(1)},${Y(v).toFixed(1)}`).join(' ');
  let grid='';
  [0,0.5,1].forEach(f=>{ const v=maxV*f,yy=Y(v); const b=czkToBase(v);
  ...
  el.innerHTML=`
    ...
    <div style="background:rgba(74,222,128,.08);border:1px solid rgba(74,222,128,.3);border-radius:10px;padding:9px 12px;font-size:.78rem;margin-bottom:10px;line-height:1.55">
    ...
    <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:${W}px;height:auto;display:block">
      ...
      <text x="11" y="${pad.t+ch/2}" font-size="9" fill="#a8aec8" transform="rotate(-90 11 ${pad.t+ch/2})" text-anchor="middle">Zaplacené úroky (${curSym()})</text>
      ...
      ${grid}${xt}
      <path d="${path(sn.intCurve)}" fill="none" stroke="#f472b6" stroke-width="2.5"/>
      <path d="${path(av.intCurve)}" fill="none" stroke="#60a5fa" stroke-width="2.5"/>
      ${sn.firstPaidMonth?`<circle cx="${X(sn.firstPaidMonth)}" cy="${Y(sn.intCurve[sn.firstPaidMonth]||0)}" r="4.5" fill="#f472b6"><title>⚪ první splacený dluh (${sn.firstPaidName})</title></circle>`:''}
      ${av.firstPaidMonth?`<circle cx="${X(av.firstPaidMonth)}" cy="${Y(av.intCurve[av.firstPaidMonth]||0)}" r="4.5" fill="#60a5fa"><title>🏔️ první splacený dluh (${av.firstPaidName})</title></circle>`:''}
    </svg>`;
}
  ...
  rEl.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
      ...
      <div style="background:var(--income-bg);border-radius:10px;padding:12px;border:1px solid rgba(74,222,128,.3);text-align:center">
        <div style="font-size:.68rem;color:var(--text3);margin-bottom:4px;font-weight:600">S EXTRA +${fmtB(extra)}</div>
...
function importConsolidated() {
  ...
  if(!confirm(`Importovat konsolidovanou půjčku ${fmtB(d.remaining)} jako novou půjčku?\n\nPůvodní půjčky zůstanou – odstraňte je ručně po splacení.`)) return;
  if(!S.debts) S.debts = [];
  d.id = uid();
