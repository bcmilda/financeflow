// FinanceFlow · v8.71 · stats.js · 2026-07-05
//  STATISTIKY
// ══════════════════════════════════════════════════════
...
function moveCatUp(id, ev){
  ...
  if(_catMoveGuard && Date.now()<_catMoveGuard.until && _catMoveGuard.dir==='up' && _catMoveGuard.id!==id){
    id=_catMoveGuard.id;
  }
...
function moveCatDown(id, ev){
  // v8.71 (FIX-183): kurzor po clampnutém scrollu zůstal nad JINOU kartou → klik přesměruj
  // na původně přesouvanou kategorii (stejný směr) = plynulé opakované klikání i u okraje stránky.
  if(_catMoveGuard && Date.now()<_catMoveGuard.until && _catMoveGuard.dir==='down' && _catMoveGuard.id!==id){
    id=_catMoveGuard.id;
  }
  const cats=S.categories||[];
  const i=cats.findIndex(c=>c.id===id);
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    requestAnimationFrame(()=>{
      ...
      if(Math.abs((r2.top + r2.height/2) - targetY) > 8){
        ...
        _catMoveGuard={ id, dir, until: Date.now()+900 };
