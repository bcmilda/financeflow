//  FinanceFlow · v8.71 · debts.js · 2026-07-05
//  ADD / EDIT TX
// ══════════════════════════════════════════════════════
function openAddTx(){
...
function _payoffSim(strategy, extra){
  ...
  return { months, totalInterest:Math.round(totalInterest), curve };
}
...
function renderAvalanche(){
  ...
  // SVG graf: dvě křivky celkového zůstatku
  const maxM=Math.max(av.curve.length,sn.curve.length), maxV=Math.max(av.curve[0],1);
  ...
  el.innerHTML=`
    ...
    <div style="background:rgba(74,222,128,.08);border:1px solid rgba(74,222,128,.3);border-radius:10px;padding:9px 12px;text-align:center;font-size:.78rem;margin-bottom:10px">
      ${saved>100?`🏔️ Avalanche ušetří <strong style="color:var(--income)">${fmtB(saved)}</strong> na úrocích${av.months<sn.months?` a je o <strong>${dur(sn.months-av.months)}</strong> rychlejší`:''}. Koule dává dřív pocit vítězství (první splacený dluh).`
        :`Rozdíl je zanedbatelný (${fmtB(Math.abs(saved))}) – vyber strategii, která tě víc motivuje.`}
    </div>
    <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:${W}px;height:auto;display:block">
      ...
      <text x="11" y="${pad.t+ch/2}" font-size="9" fill="#a8aec8" transform="rotate(-90 11 ${pad.t+ch/2})" text-anchor="middle">Zbývá (${curSym()})</text>
      ...
      ${grid}${xt}
      <path d="${path(sn.curve)}" fill="none" stroke="#f472b6" stroke-width="2.5"/>
      <path d="${path(av.curve)}" fill="none" stroke="#60a5fa" stroke-width="2.5"/>
    </svg>`;
}
