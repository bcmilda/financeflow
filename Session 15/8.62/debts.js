...
function renderAssetCatPicker(){
  ...
  picker.innerHTML=cats.map(c=>`<div class="cat-chip ${_assetCatId===c.id?'sel':''}" style="${_assetCatId===c.id?`background:${c.color}`:'border-color:'+c.color}" onclick="selAssetCat('${c.id}')">${c.icon} ${c.name}</div>`).join('');
...
function generateSchedulePreview() {
  ...
  el.innerHTML = '<div style="background:var(--surface3);border-radius:10px;padding:12px;font-size:.78rem"><div style="font-weight:600;color:var(--text2);margin-bottom:8px">📊 Přehled splácení</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><div><span style="color:var(--text3)">Počet splátek:</span> <strong>'+schedule.length+' × '+(freq==='weekly'?'týdně':freq==='biweekly'?'2 týdny':'měsíčně')+'</strong></div><div><span style="color:var(--text3)">Doba:</span> <strong>'+(years>0?years+'r ':'')+rem+'m</strong></div><div><span style="color:var(--text3)">Celkem zaplatíte:</span> <strong style="color:var(--expense)">'+fmtB(Math.round(totalPaid))+'</strong></div><div><span style="color:var(--text3)">Z toho úroky:</span> <strong style="color:var(--debt)">'+fmtB(Math.round(totalInterest))+'</strong></div>'+(rpsn>0?'<div style="grid-column:1/-1"><span style="color:var(--text3)">RPSN:</span> <strong style="color:'+(rpsn>50?'var(--expense)':rpsn>20?'var(--debt)':'var(--income)')+'">'+rpsn+'%</strong> '+(rpsn>50?'⚠️ Velmi vysoké!':rpsn>20?'⚠️ Vysoké':'')+'</div>':'')+'</div></div>';
}
