// FinanceFlow · v8.66 · projects.js · 2026-07-03
//  PROJEKTY
// ══════════════════════════════════════════════════════
    ...
    return `<div class="health-bar-row" style="flex-direction:column;align-items:stretch">
      <div style="display:flex;gap:10px;align-items:flex-start">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div class="health-bar-bg"><div class="health-bar-fill" style="width:${barW}%;background:${scColor};${noLimit?'opacity:.45':''}"></div></div>
          <div style="display:flex;justify-content:space-between;margin-top:3px">
            <span style="font-size:.7rem;color:var(--text3)">${noLimit?`${pctOfInc} % základu příjmu · bez limitu – nehodnotí se`:`${pctOfInc} % základu příjmu · plán ${planAmt} (${limitPct})`}</span>
            <span style="display:flex;align-items:center;gap:6px">${trend}<span class="health-score-pill" style="color:${scColor}">${scPill}</span></span>
          </div>
        </div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div style="font-size:.72rem;color:var(--text3);margin-top:8px">
            Splátky ${fmtB(Math.round(monthlyPayments))} / příjem ${fmtB(Math.round(baseIncome))}
          </div>
        </div>
      </div>
    </div>
...
function _obrazDivergingChart(series){
  ...
  rows.forEach((s,i)=>{
    ...
    g+=`<rect x="${(CX-3-wi).toFixed(1)}" y="${y}" width="${wi.toFixed(1)}" height="${h}" rx="3" fill="#4ade80" opacity=".88"><title>${s.month} ${s.year} · příjmy ${fmtB(s.inc)}</title></rect>`;
    g+=`<rect x="${CX+3}" y="${y}" width="${we.toFixed(1)}" height="${h}" rx="3" fill="#f87171" opacity=".88"><title>${s.month} ${s.year} · výdaje ${fmtB(s.exp)}</title></rect>`;
...
function _obrazSaldoChart(series, avg){
  const rows=series.filter(s=>s.inc>0||s.exp>0); const n=rows.length;
  if(n<2) return '';
  const W=640, H=185, pad={l:52,r:10,t:18,b:24};
  ...
  rows.forEach((s,i)=>{
    ...
    g+=`<rect x="${x.toFixed(1)}" y="${rY.toFixed(1)}" width="${bw.toFixed(1)}" height="${rH.toFixed(1)}" rx="3" fill="${v>=0?'#4ade80':'#f87171'}" opacity=".88"><title>${s.month} ${s.year} · saldo ${v>=0?'+':''}${fmtB(v)}</title></rect>`;
    g+=`<text x="${(x+bw/2).toFixed(1)}" y="${(v>=0?rY-4:rY+rH+11).toFixed(1)}" text-anchor="middle" font-size="9" fill="#a8aec8">${v>=0?'+':''}${_obrazK(v)}</text>`;
    g+=`<text x="${(x+bw/2).toFixed(1)}" y="${H-8}" text-anchor="middle" font-size="9.5" fill="#e8eaf2" font-weight="600">${s.month}</text>`;
  });
  if(isFinite(avg)&&avg!==0){
    g+=`<line x1="${pad.l}" y1="${y(avg)}" x2="${W-pad.r}" y2="${y(avg)}" stroke="#60a5fa" stroke-width="1.6" stroke-dasharray="6,4"><title>Průměr ${avg>=0?'+':''}${fmtB(avg)}/měs</title></line>`;
...
function renderObraz() {
  ...
  const metrics = [
    {label:'💰 Příjmy', trend:incTrend, val:`${fmtB(last.inc)}`, good:incTrend>0},
    {label:'💸 Výdaje', trend:-expTrend, val:`${fmtB(last.exp)}`, good:expTrend<0},
    {label:'🐷 Úspory', trend:savTrend, val:`${fmtB(last.savings)}`, good:savTrend>0},
    {label:'🏦 Dluhy', trend:-debtTrend, val:`${fmtB(last.debt)}`, good:debtTrend<0},
  ];
