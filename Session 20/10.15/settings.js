// FinanceFlow · v10.15 · settings.js · 2026-08-28
// ══════════════════════════════════════════════════════
  ...
  el.innerHTML = `
    ...
    <div class="settings-section">
      ...
      <div class="settings-item" onclick="openBackupModal()">
        <span class="settings-icon">☁️</span>
        <div class="settings-item-body">
          <div class="settings-item-title">Automatické zálohy</div>
          <div class="settings-item-sub">Denní snímek v cloudu – obnova po omylu</div>
        </div>
        <span class="settings-chevron">›</span>
      </div>
...
// ══════════════════════════════════════════════════════
//  ZÁLOHY (TODO-208, v9.87) – UI. Engine je v app.js.
// ══════════════════════════════════════════════════════
async function openBackupModal() {
  // FIX-256 (S19): modal má být OVERLAY s vnořeným .modal – původní `class="modal"`
  //   s `.modal-content`/`.modal-header` neexistuje v styles.css, takže se obsah
  //   vykreslil jako bezprizorní rámeček dole na stránce místo překryvu.
  let modal = document.getElementById('modalBackup');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'modalBackup';
  ...
  modal.innerHTML = `
    <div class="modal" style="max-width:520px">
      <div class="modal-head">
        <h3>☁️ Automatické zálohy</h3>
        <button class="btn btn-ghost btn-icon" onclick="closeModal('modalBackup')">✕</button>
      </div>
...
async function renderBackupBody() {
  const el = document.getElementById('backupBody'); if (!el) return;
  el.innerHTML = '<div style="padding:18px;text-align:center;color:#a8aec8;font-size:.85rem">Načítám seznam záloh...</div>';
  const list = (typeof backupList === 'function') ? await backupList() : [];
  const fmtSize = b => b > 1048576 ? (b / 1048576).toFixed(1) + ' MB' : Math.max(1, Math.round(b / 1024)) + ' kB';
  const fmtWhen = k => {
    if (k === 'pred-obnovou') return 'před poslední obnovou';
    const [y, m, d] = k.split('-'); return `${parseInt(d)}. ${parseInt(m)}. ${y}`;
  };
  const rows = list.map(b => `<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border)">
      <div style="flex:1;min-width:0">
        <div style="font-size:.86rem;font-weight:600;color:#e8eaf2">${fmtWhen(b.key)}</div>
        <div style="font-size:.72rem;color:#a8aec8">${fmtSize(b.bytes)}${b.ver ? ' · ' + b.ver : ''}</div>
      </div>
      <button class="btn btn-ghost btn-sm" onclick="doBackupRestore('${b.key}')" style="white-space:nowrap">↩ Obnovit</button>
    </div>`).join('');

  el.innerHTML = `
    <div style="font-size:.8rem;color:#a8aec8;line-height:1.7;margin-bottom:14px">
      Jednou denně se při spuštění uloží snímek tvých dat do cloudu. Drží se
      <strong style="color:#c9cede">posledních 5</strong>, starší se automaticky mažou.
      Zálohy vidíš jen ty — partner, se kterým sdílíš data, k nim přístup nemá.
    </div>
    <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap">
      <button class="btn btn-primary btn-sm" onclick="doBackupNow()">☁️ Zálohovat teď</button>
      <button class="btn btn-ghost btn-sm" onclick="exportData()">📥 Stáhnout jako soubor</button>
    </div>
    ${list.length ? `<div style="font-size:.74rem;color:#a8aec8;text-transform:uppercase;letter-spacing:.05em;font-weight:600;margin-bottom:4px">Dostupné zálohy</div>${rows}`
      : `<div style="padding:14px;background:var(--surface2);border-radius:10px;font-size:.82rem;color:#a8aec8;line-height:1.65">
           Zatím žádná záloha. První se vytvoří sama při příštím spuštění aplikace,
           nebo si ji vynuť tlačítkem výše.
         </div>`}
    <div style="font-size:.74rem;color:#a8aec8;line-height:1.65;margin-top:16px;padding-top:12px;border-top:1px solid var(--border)">
      <strong style="color:#c9cede">Obnova přepíše všechna současná data</strong> stavem ze zvoleného dne.
      Než se to stane, aplikace uloží ještě jeden snímek toho, co máš teď — kdybys
      obnovil omylem, dá se to vrátit zpět.
    </div>`;
}
...
async function doBackupNow() {
  ...
  if (typeof showToast === 'function') showToast('☁️ Zálohuji...');
  const r = await backupRun(true);
  if (r && r.ok) { if (typeof showToast === 'function') showToast('✅ Záloha vytvořena'); renderBackupBody(); }
  else if (typeof showToast === 'function') showToast('⚠️ ' + ((r && r.err) || 'Zálohu se nepodařilo vytvořit'));
}

async function doBackupRestore(key) {
  if (typeof backupRestore !== 'function') return;
  const nice = key === 'pred-obnovou' ? 'stavu před poslední obnovou' : key;
  if (!confirm(`Obnovit data ze zálohy ${nice}?\n\nVšechna současná data budou přepsána. Snímek toho, co máš teď, se uloží zvlášť, aby šlo obnovu vrátit.`)) return;
  if (typeof showToast === 'function') showToast('↩ Obnovuji...');
  const r = await backupRestore(key);
  if (r && r.ok) {
    if (typeof showToast === 'function') showToast(`✅ Obnoveno (${r.n} transakcí)`);
