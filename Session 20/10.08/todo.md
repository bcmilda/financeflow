...
| **TODO-230 (částečně)** | 👀 **„Kdo na co utratil"** — nová sekce v Rodinných souhrnech, kombinovaný žebříček výdajů napříč členy (dřív jen odděleně souhrn/top za sloupec). Soubor `stats.js`. Test `tools/smoke_family.js` (5 kontrol). **Zbývá:** grafy/vývoj v čase, `cat.shared` reálně vstupující do výpočtu — Milan zvolil jen tuhle část, zbytek je stále otevřený. |
