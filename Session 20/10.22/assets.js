//  FinanceFlow · v10.22 · assets.js · 2026-08-28
// ══════════════════════════════════════════════════════
...
function renderAssetAllocation(D) {
  ...
  const slices = pos.map((x,i) => {
    const sweep = x.value / totalPos * 360;
  ...
  const legend = segs.map((x,i) => {
    ...
    return `<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 8px;border-radius:8px;background:var(--surface2);border:1px solid var(--border)${isNeg?';opacity:.85':''}"
      ...
      >
      <span style="display:flex;align-items:center;gap:7px;min-width:0;font-size:.78rem">
      ...
      <span style="white-space:nowrap;font-size:.78rem;color:#a8aec8">
        <strong style="color:${isNeg?'var(--expense)':'var(--text)'}">${fmtBP(x.value)}</strong>${pct!==null?` · ${pct} %`:''}
      </span>
    </div>`;
  }).join('');
  ...
  cont.innerHTML = `
    <div class="card" style="margin-bottom:16px">
      ...
      <div class="card-body">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:6px">${legend}</div>
