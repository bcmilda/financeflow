//  FinanceFlow · v10.19 · debts.js · 2026-08-28
//  ADD / EDIT TX
// ══════════════════════════════════════════════════════
function openAddTx(){
  ...
  _czkTouched=false; _czkUserTyped=false; { const cf=document.getElementById('txAmtCZK'); if(cf) cf.value=''; }
  ...
  _txCurOverride=null;              // v9.86 (TODO-214): nová transakce = měna peněženky
  updateTxConverter();
  updateTxCzkField();
  updateTransferConv();
  if(typeof renderTxCurSel==='function') renderTxCurSel();
  // Reset kalkulačka
...
function updateTxCurrency() {
  if(typeof renderTxCurSel==='function') renderTxCurSel();   // v9.86 (TODO-214)
  const sel = document.getElementById('txWalletId'); if(!sel) return;
...
// FIX-261 (S19, nahlásil Milan): _czkTouched se při EDITACI nastaví na true už tím,
//   že transakce má uloženou částku – nedá se z něj tedy poznat, jestli uživatel
//   v této editaci do pole opravdu sáhl. Na to je zvlášť _czkUserTyped.
let _czkUserTyped=false;
...
function updateTxCzkField(){
  ...
  if(h){
    ...
    // S19 (Milan): nápověda musí říct, PROČ to má cenu upravit – jinak uživatel
    //   nechá předvyplněný kurz ČNB, rozdíl vyjde nula a kurzová ztráta se nikde
    //   neukáže. Přesně na tohle narazil při prvním testu.
    h.textContent = wid
      ? `Předvyplněno kurzem ČNB (1 ${cur} = ${rate} Kč). Přepiš podle výpisu z banky – z rozdílu appka spočítá, kolik tě směna stála navíc. Necháš-li to tak, kurzová ztráta vyjde nula.`
      : `Částku zadáváš v ${cur} (základní měna). Uloží se přepočet v Kč kurzem ČNB (1 ${cur} = ${rate} Kč) – přepiš podle výpisu, ať je vidět skutečná cena směny.`;
  }
}
// FIX-259 (S19, nahlásil Milan): pole „Skutečně v Kč" se při EDITACI nepřepočítávalo.
//  Je to záměr – kurz se u uložené transakce fixuje a nemá se sám měnit (jinak by
//  se historická útrata přepisovala dnešním kurzem). Jenže když uživatel v editaci
//  změní částku nebo měnu, čeká, že se přepočet nabídne. Tlačítko „↻ Přepočítat"
//  to dělá na vyžádání – rozhodnutí zůstává na uživateli, ale cesta existuje.
function recalcTxCzk(){
  _czkTouched = false; _czkUserTyped = false; _fxRestamp = true;
  if(typeof updateTxCzkField==='function') updateTxCzkField();
  if(typeof showToast==='function'){
    const cur=(typeof _txEntryCur==='function')?_txEntryCur():'CZK';
    const rate=(typeof _FX_RATES!=='undefined'&&_FX_RATES[cur])||1;
    showToast(`↻ Přepočteno kurzem ČNB (1 ${cur} = ${rate} Kč)`);
  }
}
window.recalcTxCzk = recalcTxCzk;

// Přečte pole „Skutečně v Kč“ při uložení. CZK peněženka → null (klíč se smaže).
function _readTxCzk(walletId, amt){
  // v9.86: rozhoduje MĚNA ZADÁVÁNÍ (může být ručně přepnutá), ne měna peněženky –
  //   jinak by česká karta s nákupem v eurech uložila 20 místo 594.
  const cur=(typeof _txEntryCur==='function')?_txEntryCur():'CZK';
...
// v9.86 (TODO-214): RUČNÍ PŘEPÍNAČ MĚNY. Doteď se měna zadávání odvozovala výhradně
//  z peněženky, což nepokrylo nejběžnější zahraniční případ: ČESKÁ KARTA, NÁKUP V EURECH.
//  Uživatel zaplatí 20 €, banka mu strhne 594 Kč (horší kurz + poplatek). Nyní si měnu
//  přepne, zadá 20, a v poli „Skutečně v Kč" opraví předvyplněný kurz ČNB na skutečně
//  strženou částku z výpisu. Uloží se amount:20, currency:'EUR', amtCZK:594 – tedy
//  i to, KOLIK HO TA SMĚNA STÁLA NAVÍC.
//  null = žádný přepis, platí měna peněženky (výchozí chování).
let _txCurOverride = null;
// Když se přepočítává částka, musí se přerazit i referenční kurz – jinak by se
// nová částka porovnávala se starým kurzem ČNB a rozdíl by byl smyšlený.
let _fxRestamp = false;

// ══════════════════════════════════════════════════════
//  REFERENČNÍ KURZ ČNB (TODO-215 fáze 1, v9.89)
//  Sběr dat pro budoucí výpočet kurzových ztrát. Aplikace zná kurz, který dala
//  BANKA (amtCZK / amount = 594/20 = 29,70), ale nemá s čím ho porovnat.
//  Ukládáme proto k cizoměnové transakci i kurz ČNB platný v době zápisu:
//    fxRef     – Kč za 1 jednotku měny podle ČNB
//    fxRefDate – datum kurzovního lístku ('19.08.2026')
//
//  Proč se ukládá i datum lístku: uživatel často zapisuje transakci se zpožděním
//  nebo ji zpětně datuje. Rozdíl mezi t.date a fxRefDate pak řekne, jak moc je
//  srovnání spolehlivé — místo abychom předstírali přesnost, kterou nemáme.
//  (Denní pohyb kurzu bývá pod 1 %, bankovní přirážka 2–11 %, takže i pár dní
//  staré datum je pro měření marže použitelné. Ale ať je to vidět.)
//
//  ⚠️ Ukládá se JEN živý kurz z ČNB. Když Worker neodpoví, kurzy.js nechá
//  orientační průměry z _FX_RATES — zapsat je jako „referenční ČNB" by byla lež.
// Doplní referenční kurz k DATU TRANSAKCE (TODO-215, v9.97). Běží až po uložení,
// aby zápis nečekal na síť; když se kurz nepodaří získat, zůstane ten z doby zápisu.
async function _fxRefBackfill(txId, cur, isoDate){
  try{
    if(!txId || !cur || cur==='CZK' || typeof fxRatesForDate!=='function') return;
    const h = await fxRatesForDate(isoDate);
    if(!h || !h.rates) return;
    const r = h.rates[cur];
    if(!r || !isFinite(r) || r<=0) return;
    const t = (S.transactions||[]).find(x=>x.id==txId);
    if(!t || t.currency!==cur) return;
    if(t.fxRef===r && t.fxRefDate===h.date) return;   // beze změny → neukládej znovu
    t.fxRef = r; t.fxRefDate = h.date;
    if(typeof save==='function') save();
  }catch(e){ /* zpřesnění navíc, nesmí nic shodit */ }
}

function _fxRefNow(cur){
  if(!cur || cur==='CZK') return null;
  try{
    if(typeof _fxData==='undefined' || !_fxData) return null;
    if(_fxData.source==='fallback') return null;
    const r=(_fxData.rates||{})[cur];
    if(!r || !isFinite(r) || r<=0) return null;
    return { rate:r, date:_fxData.date||null };
  }catch(e){ return null; }
}

function _txCurBase(){   // měna podle peněženky – bez ohledu na ruční přepis
  const isTransfer=(typeof curTxType!=='undefined'&&curTxType==='transfer');
  const isAsset=isTransfer&&typeof _transferMode!=='undefined'&&_transferMode==='assets';
...
function _txEntryCur(){
  const isTransfer=(typeof curTxType!=='undefined'&&curTxType==='transfer');
  const isAsset=isTransfer&&typeof _transferMode!=='undefined'&&_transferMode==='assets';
  // U přesunu mezi peněženkami se měna řídí peněženkami, ruční přepis se neuplatní
  if(isTransfer&&!isAsset) return 'CZK';
  if(_txCurOverride) return _txCurOverride;
  return _txCurBase();
}

// Naplní rozbalovátko měn vedle popisku Částka.
function renderTxCurSel(){
  const sel=document.getElementById('txCurSel'); if(!sel) return;
  const field=document.getElementById('txCurField');
  const hint=document.getElementById('txCurHint');
  const isTransfer=(typeof curTxType!=='undefined'&&curTxType==='transfer');
  const isAsset=isTransfer&&typeof _transferMode!=='undefined'&&_transferMode==='assets';
  // U přesunu mezi peněženkami má měnu na starosti vlastní řádek přepočtu → pole schovej
  if(field) field.style.display=(isTransfer&&!isAsset)?'none':'';
  if(isTransfer&&!isAsset) return;
  const base=_txCurBase();
  const list=Object.keys(_FX_RATES||{}).filter(c=>c!=='CZK').sort();
  const all=['CZK'].concat(list);
  if(!all.includes(base)) all.unshift(base);
  const cur=_txEntryCur();
  sel.innerHTML=all.map(c=>`<option value="${c}"${c===cur?' selected':''}>${c===base?c+' · peněženka':c}</option>`).join('');
  // Popisek ČÁSTKA drží jednotku, pole TYP MĚNY vysvětluje, proč zrovna tuhle
  if(hint){
    const w=(S.wallets||[]).find(x=>x.id===(document.getElementById('txWalletId')?.value||''));
    hint.textContent = (cur===base)
      ? (w?`Podle peněženky ${w.name}`:'Podle základní měny')
      : `Platíš v ${cur}, účet je v ${base} – níže uprav skutečně strženou částku`;
  }
}

function setTxCurOverride(cur){
  _txCurOverride = (cur && cur!==_txCurBase()) ? cur : null;
  _txCzkTouched = false;   // předvyplň kurzem nové měny
  if(typeof updateTxCzkField==='function') updateTxCzkField();
...
function _entryAmtCZK(walletId, amt){
  // v9.86: při ručním přepnutí měny chceme zachovat PŮVODNÍ částku v amount
  //   a Kč dát do amtCZK → tuhle větev přeskoč.
  if(typeof _txCurOverride!=='undefined' && _txCurOverride) return null;
...
function saveTx(){
  ...
  if(_eCzk!=null){ txObj.amount=_eCzk; txObj.amt=_eCzk; txObj.amtCZK=null; txObj.currency=null; }
    ...
    txObj.amtCZK=_readTxCzk(walletId,amt); // v8.58 (TODO-144): zafixovaná částka v Kč
    // v9.86 (TODO-214): měna, ve které bylo placeno. Ukládá se JEN když není CZK –
    //   u korunových transakcí by to byl zbytečný klíč u každého záznamu.
    //   Chybí-li, odvodí se z peněženky jako dřív → žádná migrace starých dat.
    const _ec=(typeof _txEntryCur==='function')?_txEntryCur():'CZK';
    txObj.currency = (_ec && _ec!=='CZK') ? _ec : null;
    // TODO-215 fáze 1: referenční kurz ČNB. Zapisuje se JEN při prvním uložení.
    //   ⚠️ txObj je VŽDY nový objekt (do transakce se pak sloučí přes Object.assign),
    //   takže se původní hodnota musí dohledat v S.transactions – jinak by editace
    //   tři měsíce staré transakce přerazila kurz na dnešní a srovnání ztratilo smysl.
    if(txObj.currency){
      const _prev = eid ? (S.transactions||[]).find(x=>x.id==eid) : null;
      // FIX-261: amtCZK a fxRef tvoří PÁR – oba musí popisovat TÝŽ stav transakce.
      //   Milan právem namítl, že se dají rozejít dvěma cestami:
      //     a) uživatel v editaci změní částku z 20 € na 25 €, ale zapomene
      //        přepsat „Skutečně v Kč" → 25 € by mělo pořád cenu 594 Kč a appka
      //        by vypočetla kurz 23,76 místo 29,70, tedy VYMYŠLENOU výhodnou směnu;
      //     b) klikne na Přepočítat po dvou týdnech → částka se spočte dnešním
      //        kurzem, ale porovná se s referenčním kurzem z doby zápisu.
      //   Pravidlo: dokud se nemění částka ani měna, oba údaje se drží zmrazené.
      //   Jakmile se jedno z toho změní, jde o NOVÉ MĚŘENÍ – přerazí se oboje.
      const _zmena = !!_prev && (Math.abs((_prev.amount||_prev.amt||0) - amt) > 0.005
                                 || (_prev.currency||null) !== (txObj.currency||null));
      if(_prev && _prev.fxRef!=null && !_zmena && !_fxRestamp){
        txObj.fxRef=_prev.fxRef; txObj.fxRefDate=_prev.fxRefDate||null;   // beze změny → zmrazeno
      } else {
        const _fx=_fxRefNow(txObj.currency);
        if(_fx){ txObj.fxRef=_fx.rate; txObj.fxRefDate=_fx.date; }
        else { txObj.fxRef=null; txObj.fxRefDate=null; }
        // Změnila se částka, ale uživatel do pole „Skutečně v Kč" nesáhl?
        // Nechat tam staré číslo by byla tichá chyba – přepočti dnešním kurzem.
        //   Platí i pro ruční „↻ Přepočítat" (_fxRestamp): tam se sice pole v UI
        //   překreslí, ale spoléhat se na to nesmíme – kdyby uživatel stiskl tlačítko
        //   a hned uložil, zůstala by stará částka u nového referenčního kurzu.
        if((_zmena || _fxRestamp) && !_czkUserTyped && _fx){
          txObj.amtCZK = Math.round(amt * _fx.rate * 100) / 100;
        }
      }
    } else { txObj.fxRef=null; txObj.fxRefDate=null; }
  }
  ...
  let _savedId = eid;
  if(eid) { const t=S.transactions.find(x=>x.id==eid); if(t) Object.assign(t,txObj); }
  else { _savedId = genTxId(); S.transactions.push({id:_savedId,...txObj}); }
  // v9.97 (TODO-215): dohledej kurz ČNB k DATU TRANSAKCE. Bez await – uložení
  //   ani zavření modalu nesmí čekat na síť; když se nepovede, zůstane kurz
  //   z doby zápisu (fxRefDate pak prozradí, ke kterému dni patří).
  if(txObj.currency) _fxRefBackfill(_savedId, txObj.currency, txObj.date);
  save(); closeModal('modalAdd'); { const pid=selProjectId; selProjectId=''; if(pid&&curPage==='projektDetail'){renderProjectDetail(pid);}else{renderPage();} }
}
...
function generateSchedulePreview() {
  ...
  const total = moneyInRead('dTotal');   // TODO-216
  const interest = parseFloat(document.getElementById('dInterest')?.value) || 0;
  const payment = moneyInRead('dPayment');   // TODO-216
  ...
  const remainNow = moneyInRead('dRemaining') || total;   // TODO-216
...
function editDebt(id) {
  ...
  moneyInFill('dTotal', d.total);   // TODO-216
  moneyInFill('dRemaining', d.remaining);   // TODO-216
  document.getElementById('dInterest').value=d.interest||'0';
  moneyInFill('dPayment', d.payment);   // TODO-216
  ...
  moneyInFill('dPenaltyFixed', d.penaltyFixed);   // TODO-216
...
function saveDebt() {
  ...
  const total=moneyInRead('dTotal');   // TODO-216
  const remaining=moneyInRead('dRemaining')||total;   // TODO-216
  const payment=Math.max(0,moneyInRead('dPayment'));   // TODO-216
  ...
  const obj={name,total,remaining,
    ...
    penaltyFixed:moneyInRead('dPenaltyFixed'),   // TODO-216
...
function openDebtSim() {
  ...
  moneyInFill('simAmt', first.payment||Math.round(calcAnnuity(first.remaining,first.interest||0,12,24)));   // TODO-216
...
function runDebtSim() {
  ...
  const d=S.debts.find(x=>x.id===id); if(!d) return;
  const simPayment=Math.max(1,moneyInRead('simAmt'));   // TODO-216
  const lump=moneyInRead('simLump');   // TODO-216
...
function openDebtVsInvest() {
  ...
  moneyInFill('dviAmount', Math.round((S.debts[0].payment||2000)*0.3));   // TODO-216
  runDebtVsInvest();
...
function runDebtVsInvest() {
  ...
  const d = S.debts?.find(x=>x.id===id); if(!d) return;
  const monthly = moneyInRead('dviAmount');   // TODO-216
...
function renderDebtFreedomWidget(D) {
  ...
  // FIX-284 (S20): VYHODNOCENÍ (jak vážné zatížení je) a PŘÍSLUŠNOST (čí je ten den)
  //   musí být dvě různé barvy. Dřív se na obojí používal barColor, takže při
  //   bezpečném zatížení byly „dny pro banky" ZELENÉ – tedy stejnou barvou jako
  //   dny pro sebe, jen o odstín jinak. Den, který odpracuješ pro banku, není
  //   dobrá zpráva, i když je jich málo.
  const bankColor = 'var(--expense)';   // dny pro banku – vždy
  const selfColor = 'var(--income)';    // dny pro sebe – vždy
  ...
  el.innerHTML = `<div class="debt-freedom-card">
    ...
    <div style="display:flex;align-items:flex-start;gap:16px;flex-wrap:wrap;margin-bottom:14px">
      <div style="text-align:center;min-width:80px">
        <div class="debt-days-big" style="color:${bankColor}">${daysForDebt}</div>
        <div style="font-size:.72rem;color:#a8aec8;margin-top:2px">dní / měsíc<br>pro banky</div>
      </div>
      <div style="flex:1;min-width:160px">
        <div style="display:flex;justify-content:space-between;font-size:.72rem;color:#a8aec8;margin-bottom:3px">
          <span>0 dní</span>
          <span style="color:${barColor};font-weight:700">${pct}% příjmu</span>
          <span>${daysPerMonth} dní</span>
        </div>
        <div class="trap-bar" style="height:14px">
          <div class="trap-bar-fill" style="width:${Math.round(daysForDebt/daysPerMonth*100)}%;background:${bankColor}"></div>
        </div>
        ...
        <div style="font-size:.72rem;color:#a8aec8;margin-top:3px">
    ...
    <div style="display:flex;gap:2px;flex-wrap:wrap;margin-bottom:8px">
      ${Array.from({length:daysPerMonth},(_,i)=>`
        <div title="${i<daysForDebt?'Pro banky':'Pro tebe'}" style="width:24px;height:24px;border-radius:4px;background:${i<daysForDebt?bankColor:selfColor};display:flex;align-items:center;justify-content:center;font-size:.7rem;color:#0d0a1f;font-weight:700">${i+1}</div>
      `).join('')}
    </div>
    <div style="font-size:.72rem;color:#c3c9de;margin-bottom:14px;display:flex;gap:12px;flex-wrap:wrap;align-items:center">
      <span style="display:inline-flex;align-items:center;gap:5px">
        <span style="width:11px;height:11px;border-radius:3px;background:${bankColor};display:inline-block"></span>
        ${daysForDebt} ${daysForDebt===1?'den':(daysForDebt>=2&&daysForDebt<=4?'dny':'dní')} pro banky
      </span>
      <span style="display:inline-flex;align-items:center;gap:5px">
        <span style="width:11px;height:11px;border-radius:3px;background:${selfColor};display:inline-block"></span>
        ${daysForSelf} ${daysForSelf===1?'den':(daysForSelf>=2&&daysForSelf<=4?'dny':'dní')} pro tebe
      </span>
    </div>
...
function runConsolidate() {
  ...
  const customPayment = moneyInRead('consPayment');   // TODO-216
